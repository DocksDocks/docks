...
if (!fs.existsSync(specPath)) {
  console.log('scaffold/test: no docs/scaffold/spec.yaml — skipped');
  process.exit(0);
}
...
try {
  ...
  const leftovers = [];
  walk(tmp, (f) => {
    if (fs.readFileSync(f, 'utf8').includes('{{')) leftovers.push(path.relative(tmp, f));
  });
  if (leftovers.length > 0) errors.push(`leftover placeholders:\n${leftovers.join('\n')}`);

  for (const tf of spec.templated_files || []) {
    const dest = subst(tf.dest);
    if (dest.endsWith('.sh') && !(fs.statSync(path.join(tmp, dest)).mode & 0o111))
  ...
  for (const tf of spec.templated_files || []) {
    ...
    try {
      ...
      errors.push(`${dest}: invalid JSON: ${e.message}`);
      continue;
    }
  ...
  if (errors.length === 0) {
    ...
    if (!fs.existsSync(seededCi))
      errors.push('seed has no scripts/ci.mjs — the spec should render ci.mjs.template into the seed');
    else {
      const ci = childProcess.spawnSync('node', [seededCi, '-q'], { encoding: 'utf8' });
      if (ci.status !== 0)
        errors.push(
          `seeded scripts/ci.mjs exited ${ci.status} — a fresh seed is NOT green:\n${ci.stdout}${ci.stderr}`.trim(),
        );
    }
  }
} finally {
