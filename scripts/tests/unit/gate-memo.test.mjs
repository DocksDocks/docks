...
const FULL_RUN = {
  ...
  phases: [
    ...
    { name: 'plugin: docks', duration_ms: 320_500 },
    { name: 'plugin: plan-lifecycle', duration_ms: 4_200 },
    { name: 'javascript quality', duration_ms: 1_100 },
  ],
...
// The dearest phase is docks in every one of these runs. Naming it would be
...
test('the advice follows the changed paths, never the dearest phase', () => {
  ...
  assert.doesNotMatch(advise(CHANGED_ONE), /--plugin docks\b/, 'never hand over a command for the untouched plugin');
