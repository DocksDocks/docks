...
function passed(req, leg, findings, verdict, score) {
  ...
  return {
    ...
    reviewer_output: {
      ...
      structured_output_sha256: policy.sha256(policy.jcs(structured)),
    },
    findings,
...
function testRepairArtifacts() {
  ...
  try {
    ...
    assert.equal(
      JSON.parse(fs.readFileSync(path.join(bundle, 'repair-targets.json'), 'utf8')).repair_targets_sha256,
      transition.repair_targets_sha256,
    );
    assert.equal(
      policy.verifyBundle({ bundle, expectedSha256: sealed.bundle_sha256 }).bundle_sha256,
      sealed.bundle_sha256,
    );
    assert.doesNotThrow(() =>
      policy.buildReviewerArgv({
        tool: 'claude',
        bundle,
        model: 'fable',
        effort: 'high',
    ...
    assert.equal(
      ...
      alternate.sealed.bundle_sha256,
    );
    assert.throws(
      () =>
        policy.buildReviewerArgv({
...
function testRepairSeries() {
  ...
  incompleteReconciliation.repair_targets_sha256 = policy.sha256(
    policy.jcs({
      schema: 1,
...
function testReviewerLive() {
  assert.equal(
    process.env.DOCKS_LIVE_CODEX_REVIEW,
    '1',
    'set DOCKS_LIVE_CODEX_REVIEW=1 to run the credentialed live acceptance',
  );
  const fixture = initializeFixture();
  let prepared;
  try {
    ...
    assert.deepEqual(argumentValues(argv, '-c'), ['model_reasoning_effort=high', 'service_tier="default"']);
  ...
  console.log(
    'live Codex reviewer preserves explicit Standard tier, typed output, sealed bundle, and workspace cleanup',
  );
}
...
function testCliTransport() {
  ...
  try {
    prepared = JSON.parse(
      execFileSync('node', [HELPER, 'reviewer-workspace-prepare', requestId, 'S'], { encoding: 'utf8' }),
    );
    ...
    const cleaned = JSON.parse(
      execFileSync('node', [HELPER, 'reviewer-workspace-cleanup', requestId, 'S', preparedPath], { encoding: 'utf8' }),
    );
    ...
    assert.equal(sealed.manifest.schema, 2);
    assert.equal(
      policy.verifyBundle({ bundle, expectedSha256: sealed.bundle_sha256 }).manifest.repair.repair_targets_sha256,
      transition.repair_targets_sha256,
    );
...
function assertOrchestrationStateHash(state) {
  ...
  assert.equal(
    state.state_sha256,
    policy.sha256(policy.jcs(payload)),
    'orchestration state hash covers every other closed field',
  );
}

function beginSchema6State({ inputSha256, requestId = FULL_REQUEST_ID, seriesId = ORCHESTRATION_SERIES_ID } = {}) {
...
function testSchema6RepairSeries() {
  for (const [name, exported] of [
    ['beginReviewOrchestration', policy.beginReviewOrchestration],
    ['advanceReviewOrchestrationRepair', policy.advanceReviewOrchestrationRepair],
    ['settleReviewOrchestration', policy.settleReviewOrchestration],
    ['consumeReviewIntent', policy.consumeReviewIntent],
    ['prepareReviewRequest', policy.prepareReviewRequest],
    ['buildReviewDispatchCommitment', policy.buildReviewDispatchCommitment],
  ])
    assert.equal(typeof exported, 'function', `${name} must be exported`);
  ...
  for (const field of ['previous_orchestration_state_sha256', 'current_orchestration_state_sha256']) {
    ...
    assert.throws(
      () => policy.validateReviewSeries(changed),
...
function testSchema6RepairArtifacts() {
  ...
  try {
    ...
    assert.equal(
      fullBundle.manifest.files.some(({ path: entry }) => entry === 'reviewer-output.primary.v5.schema.json'),
      false,
    );
    const bundledV6Schema = JSON.parse(
      fs.readFileSync(path.join(fullBundlePath, 'reviewer-output.primary.v6.schema.json'), 'utf8'),
    );
    ...
    assert.equal(
      policy.jcs(explicitV5Transition),
      policy.jcs(implicitV5Transition),
      'explicit schema 5 retains transition bytes',
    );
...
function testCurrentReviewerArgv() {
  ...
  try {
    ...
    assert.deepEqual(argv.slice(modelAt + 2, modelAt + 6), [
      '-c',
    ...
    assert.equal(
      argv[argv.indexOf('--add-dir') + 1],
      bundle,
      'reviewer can read the sealed bundle outside its isolated workspace',
    );
    assert.equal(
      argv[argv.indexOf('--output-schema') + 1],
      path.join(bundle, 'reviewer-output.primary.v5.schema.json'),
    );
...
function reportCaseFailure(name, error) {
...
if (args.length === 1 && args[0] === '--selector-self-test') {
  try {
    await testSelectorContracts();
  } catch (error) {
