...
| `ciLane` | required pull-request shard ownership: `core` for ordinary plugins, `relay` for Session Relay. With `root`, this is the changed-path → shard mapping the PR matrix resolves against; `repo` is the always-on repo-wide shard and no plugin may claim it |
...
| `release` | Closed, data-only release policy. Ordinary plugins declare exactly `{ kind: 'generic', install }`; Session Relay declares exactly `{ kind: 'reviewed-session-relay', assets, prereleaseBody, install }`. No callbacks, commands, safety gates, or ordering belong in descriptors. |
...
2. **One descriptor** appended to `PLUGINS` in `lib/plugins.mjs` — assign required `ciLane` ownership (`core` or `relay`; a diff under `root` then selects that shard), declare only capabilities that exist (`agents`/`selftest`/`rust` take `null`, `extraJson`/`authorChecks`/`releaseContracts` use `[]` when absent), and declare the exact data-only `release` policy for its lane.
...
5. Verify: `node scripts/ci.mjs --list` shows the plugin and full `node scripts/ci.mjs` is green. Ordinary plugins use the generic positional release command; a reviewed prebuilt CLI uses its reviewed prepare/publication modes.
...
The three `build-binaries.yml` matrix legs are native, not cross-build evidence. x86_64-apple-darwin is no longer published as of Session Relay 0.16.0; macOS support is aarch64-apple-darwin. Retained 0.13-0.15 preflight receipts and captured fixtures keep their frozen four-leg shape and must keep validating byte-identically; only the current lane narrowed. Before unchanged attestation/upload, Linux legs must complete the exact cgroup/pidfd/Landlock custody test and both smokes against that leg's explicit fresh binary; the Apple-Silicon macOS leg must complete the frozen negative-admission test. `verify-session-relay-preflight.mjs` verifies all three successful job identities, exact runner labels, native target mapping, and ordered build → platform evidence → Linux smoke/skip → attestation → upload steps through the GitHub jobs API. Missing, reordered, skipped, failed, duplicated, cross-run/cross-target, or Intel Darwin evidence refuses.

This verifier-side check does not add receipt fields. `SourceCiReceiptV1`, `ProducerPreflightReceiptV1`, and `SessionRelayBinaryAttestationV1` retain their exact keys; verifier producer identity remains version 2. The producer yields three binary+attestation artifacts and one checksum artifact, and publication consumes exactly four release assets. macOS success means the refusal was proven, never that managed writing is supported. No generated binary or `SHA256SUMS` belongs in the source commit.

A successful preflight run is recorded on a `preflight/session-relay-<version>-<commit12>` head ref by `verify-session-relay-preflight.mjs`, which contains no ref creation or deletion path of its own. Every `preflight/*` ref is retained indefinitely as release evidence: release automation never deletes, rewrites, or ages out one, and no release run carries a deletion row. Deleting an exact ref in the future requires a separate plan that first proves no finished plan, receipt, workflow run, tag, or recovery path relies exclusively on that ref, and each such deletion must be its own separately authorized `push` effect naming the exact ref.
