...
# Usage: ./guard-skills.sh [path]   (default: plugins/docks/skills)
set -u
SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
DIR="${1:-$SCRIPT_DIR/../plugins/docks/skills}"
errors=0
...
for skill_dir in "$DIR"/*/*/; do
  ...
  # Strip surrounding YAML quotes (single or double) so prefix + length checks see the visible string
  ...
  if [ "$has_upstream" -eq 0 ] && ! echo "$desc_clean" | grep -qiE '^use when'; then
