...
// 2. model_reasoning_effort full set
for (const v of ['none', 'minimal', 'low', 'medium', 'high', 'xhigh']) {
  if (!doc.includes(`"${v}"`)) fail(`codex-agents-builder.md missing model_reasoning_effort value "${v}" (set: none/minimal/low/medium/high/xhigh)`);
}
...
if (errors > 0) { console.error(`Guard FAILED: ${errors} Codex-fact drift error(s) in skill-agent-pipeline reference docs`); process.exit(1); }
