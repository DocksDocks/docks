...
function fixtureReceipt(mode, fixture, outcome) {
  ...
  if (mode === 'materialize-tdd-red')
    return {
      ...
      pre_production_commit: fixture.source_commit,
      test_paths: [],
      command: { cwd: REPO, argv: ['node', 'fixture'] },
      exit_code: 1,
      stdout_sha256: '0'.repeat(64),
      stderr_sha256: '0'.repeat(64),
      captured_at: baseTime,
      producer: { path: 'scripts/capture-tdd-red.mjs', blob_id: fixture.source_commit, version: '1' },
    };
  if (mode === 'verify-source-ci')
    return {
      schema: 1,
  ...
  if (mode === 'bind-completion')
    return {
      ...
      version: VERSION,
      source_commit: fixture.source_commit,
      tag_commit: fixture.source_commit,
      promoted_commit: fixture.promoted_commit,
      finished_plan_sha256: '0'.repeat(64),
      candidate_sha256: 'e'.repeat(64),
      completion_review_sha256: 'f'.repeat(64),
      bound_at: baseTime,
    };
  if (mode === 'publish-reviewed' || mode === 'finalize-reviewed') {
    ...
    return {
      ...
      repository_id: fixture.repository_id,
      version: VERSION,
      source_proof_sha256: 'a'.repeat(64),
      tag: fixture.tag,
      tag_commit: fixture.source_commit,
      workflow: {
        file: '.github/workflows/build-binaries.yml',
        workflow_sha: fixture.source_commit,
        run_id: 1,
        attempt: 1,
      },
      release_database_id: 10,
      release_state: stable ? 'stable' : 'prerelease',
      body_sha256: sha256(Buffer.from(stable ? STABLE_BODY : PRERELEASE_BODY)),
      assets: fixture.assets.map((asset) => ({ ...asset, size: 1 })).sort((a, b) => a.name.localeCompare(b.name)),
      transition: stable ? 'finalized' : 'reconciled',
      created_at: baseTime,
    };
  }
  if (mode === 'promote-reviewed' || mode === 'resume-promotion')
    return {
      schema: 1,
      ...
      version: VERSION,
      source_proof_sha256: 'a'.repeat(64),
      publication_receipt_sha256: 'b'.repeat(64),
      tag_commit: fixture.source_commit,
      promoted_commit: fixture.promoted_commit,
      transaction_ref: TRANSACTION_REF,
      attempt: fixture.scenario.startsWith('retry-') ? 1 : 0,
      terminal_sequence: 5,
      terminal_journal_commit: fixture.source_commit,
      journal_chain_sha256: '9'.repeat(64),
      prior_attempt_receipt_sha256: fixture.scenario.startsWith('retry-') ? 'c'.repeat(64) : null,
      lock_ref: LOCK_REF,
      lock_nonce: 'fixture-lock',
      expected_origin_main: fixture.expected_origin_main,
      outcome,
      completed_at: baseTime,
    };
  return null;
...
function fixtureJournal(fixture, outcome) {
  if (!fixture.scenario || !['success', 'conflict', 'failure', 'manual_incident', 'restored_failure'].includes(outcome))
    return [];
  const attempt = fixture.scenario.startsWith('retry-') ? 1 : 0;
  let phases;
  if (outcome === 'success')
    phases = ['initialized', 'locked', 'prepush_passed', 'main_pushed', 'live_passed', 'terminal_success'];
  else if (outcome === 'restored_failure')
    phases = ['initialized', 'locked', 'prepush_passed', 'main_pushed', 'restore_pushed', 'terminal_failure'];
...
function fixtureMutations(mode, fixture, outcome) {
  if (outcome === 'conflict' || fixture.scenario.includes('-recover') || fixture.scenario.startsWith('recover-'))
    return [];
  ...
  if (mode === 'promote-reviewed' || mode === 'resume-promotion')
    return fixtureJournal(fixture, outcome).map(({ phase }) => phase);
  return [];
}
...
export function runFixture(argv, parsed, parseError) {
  const fixturePath = canonicalPath(process.env.SESSION_RELAY_RELEASE_FIXTURE, 'SESSION_RELAY_RELEASE_FIXTURE', {
    absolute: true,
  });
  ...
  if (!forcedConflict) {
    ...
    if (mode === 'legacy-release') {
      ...
      const bump = argv.filter(
        (token, index) => token !== '--plugin' && argv[index - 1] !== '--plugin' && token !== '--dry-run',
      )[0];
      const version = /^\d+\.\d+\.\d+$/.test(bump)
        ? bump
        : bump === 'major'
          ? '1.0.0'
          : bump === 'minor'
      ...
      state = {
        ...
        tag: TAG,
        dry_run: dry,
        message: dry ? 'The real release will gate the exact changed tree.' : 'prepared',
      };
      ...
      state =
        mode === 'publish-reviewed'
          ? { tag: fixture.tag, version: VERSION, release: { prerelease: true, body: PRERELEASE_BODY } }
          : mode === 'finalize-reviewed'
            ? { tag: fixture.tag, version: VERSION, release: { prerelease: false, body: STABLE_BODY } }
    ...
    if (mode === 'materialize-tdd-red') {
      for (const name of ['docks-red-out', 'public-red-out'])
        writeCanonicalExclusive(parsed.options.get(name), receipt);
    }
  }
  const report = {
    schema: 1,
    type: 'SessionRelayReleaseFixtureReportV1',
