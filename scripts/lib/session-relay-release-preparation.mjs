...
function pinnedLegacyCompletionState(plan) {
  ...
  if (matches.length !== 1)
    fail('pinned legacy completion plan must contain exactly one Review-orchestration-state');
  let state;
  try {
...
function validatePinnedLegacyCompletionReceiptClosed(plan, completion, candidate, waivers) {
  ...
  if (
    sha256(Buffer.from(PINNED_LEGACY_COMPLETION_POLICY_JCS)) !==
    PINNED_LEGACY_COMPLETION.policySha256
  )
