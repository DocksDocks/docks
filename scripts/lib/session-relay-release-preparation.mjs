...
const PLANRUN_DOCKS_REVIEW_BASE = '2b414888fad7a9295a2ba9ba5a3a90eefb6f9486';
...
export function validateTddRedReceipt(value, context = {}) {
  ...
  if (expected.repositoryId !== undefined && value.repository_id !== expected.repositoryId)
    fail('TDD-red repository identity mismatch');
  ...
  for (const [index, entry] of value.test_paths.entries()) {
    ...
    if (paths.has(entry.path) || (prior !== null && prior.localeCompare(entry.path) >= 0))
      fail('TDD-red test paths must be unique and sorted');
  ...
  if (
    ...
    fail('TDD-red exit_code must be a nonzero i32');
  ...
  if (value.producer.path !== 'scripts/capture-tdd-red.mjs' || value.producer.version !== '1')
    fail('TDD-red producer identity mismatch');
...
function validateFileIdentity(value, label) {
  ...
  if (!['elf', 'mach-o'].includes(value.format) || !['x86_64', 'aarch64'].includes(value.architecture))
...
export function validateProducerPreflightReceipt(value, context = {}) {
  ...
  exactKeys(
    ...
    'ProducerPreflightReceiptV1',
  );
  if (value.schema !== 1 || value.type !== 'ProducerPreflightReceiptV1' || value.repository_id !== REPOSITORY_ID)
  ...
  if (expected.sourceCommit !== undefined && value.source_commit !== expected.sourceCommit)
    fail('preflight source commit mismatch');
  ...
  if (
    value.workflow.file !== BUILD_WORKFLOW ||
    value.workflow.event !== 'workflow_dispatch' ||
    value.workflow.head_branch !== branch ||
    value.workflow.head_sha !== value.source_commit ||
    value.workflow.conclusion !== 'success'
  )
    fail('preflight workflow identity mismatch');
  ...
  if (
    value.workflow.inputs.mode !== 'validate-only' ||
    value.workflow.inputs.expected_commit !== value.source_commit ||
    value.workflow.inputs.expected_tag !== ''
  )
    fail('preflight workflow inputs mismatch');
  ...
  for (const artifact of value.artifacts) {
    ...
    if (
      ...
      artifact.artifact_name !== `session-relay-binary-${artifact.target}` ||
      artifact.asset_name !== `session-relay-${artifact.target}` ||
      artifact.mode !== '0755'
    )
      fail('preflight artifact identity mismatch');
    ...
    if (
      artifact.file_identity.format !== expectedFormat ||
      artifact.file_identity.architecture !== expectedArchitecture
    )
      fail('preflight artifact target/file identity mismatch');
  }
  for (const attestation of value.attestations) {
    ...
    if (
      !artifact ||
      attestation.artifact_name !== artifact.artifact_name ||
      attestation.database_id !== artifact.database_id ||
      attestation.archive_digest !== artifact.archive_digest ||
      attestation.file_name !== `attestation-${attestation.target}.json`
    )
      fail('preflight attestation/artifact binding mismatch');
    ...
    if (
      attestation.runner_os !== expectedOs ||
      attestation.runner_arch !== expectedArch ||
      attestation.version_stdout !== `session-relay ${VERSION}`
    )
      fail('preflight native attestation mismatch');
  }
  exactKeys(
    value.checksum,
    ['artifact_name', 'database_id', 'archive_size', 'archive_digest', 'file_name', 'sha256', 'size', 'entries'],
    'preflight checksum',
  );
  if (value.checksum.artifact_name !== 'session-relay-checksums' || value.checksum.file_name !== 'SHA256SUMS')
    fail('preflight checksum identity mismatch');
  ...
  for (const artifact of value.artifacts)
    if (entries.get(artifact.asset_name) !== artifact.sha256) fail('preflight checksum does not bind binary');
  exactKeys(value.producer, ['path', 'version'], 'preflight producer');
  if (value.producer.path !== 'scripts/verify-session-relay-preflight.mjs' || value.producer.version !== '2')
    fail('preflight producer identity mismatch');
...
export function validateSourceCiReceipt(value, context = {}) {
  ...
  if (value.schema !== 1 || value.type !== 'SourceCiReceiptV1' || value.repository_id !== REPOSITORY_ID)
    fail('source-CI receipt identity mismatch');
  commit(value.source_commit, 'source-CI source_commit');
  if (expected.sourceCommit !== undefined && value.source_commit !== expected.sourceCommit)
    fail('source-CI source commit mismatch');
  exactKeys(
    value.workflow,
    [
      'file',
  ...
  const identity = sha256(
    Buffer.from(
      canonicalize({
        ...
        file_blob_id: value.workflow.file_blob_id,
        file_sha256: value.workflow.file_sha256,
      }),
    ),
  );
  ...
  const branch = `preflight/session-relay-${VERSION}-${value.source_commit.slice(0, 12)}`;
  if (
    value.workflow.file !== SOURCE_CI_WORKFLOW ||
    value.workflow.event !== 'workflow_dispatch' ||
    value.workflow.head_branch !== branch ||
    value.workflow.head_sha !== value.source_commit ||
    value.workflow.conclusion !== 'success'
  )
    fail('source-CI workflow identity mismatch');
  ...
  if (
    value.bootstrap.node !== '24' ||
    value.bootstrap.pnpm !== '--frozen-lockfile' ||
    value.bootstrap.claude !== 'materialized-on-PATH' ||
    value.bootstrap.rust !== '1.85.0' ||
    value.bootstrap.musl !== true
  )
    fail('source-CI bootstrap mismatch');
  ...
  for (const [index, step] of value.required_steps.entries()) {
    ...
    if (
      step.name !== REQUIRED_CI_STEPS[index] ||
      step.number !== REQUIRED_CI_STEP_NUMBERS[index] ||
      step.conclusion !== 'success'
    )
      fail('source-CI required step outcome mismatch');
  }
  ...
  if (
    ...
    Date.parse(value.created_at) < Date.parse(value.completed_at)
  )
    fail('source-CI timestamps are inconsistent');
  return value;
}
...
function validateCheckEvidence(check, expectedId) {
  ...
  if (check.exit_code !== 0 || check.steps.some((step) => step.exit_code !== 0))
    fail(`candidate check ${expectedId} did not pass`);
...
export function validateSourcePreparationCandidate(value, context = {}) {
  ...
  for (const item of value.docks_red.test_blobs) {
    exactKeys(item, ['path', 'blob_id'], 'candidate docks red test blob');
    safeLogical(item.path, 'candidate docks red test path');
    commit(item.blob_id, 'candidate docks red test blob_id');
  }
  exactKeys(
    value.companion,
    [
  ...
  for (const [name, workflow] of [
    ...
    exactKeys(
      workflow,
      ['sha256', 'workflow_file', 'workflow_blob_id', 'run_database_id', 'run_attempt'],
      `candidate ${name}`,
    );
  ...
  if (value.preflight.workflow_file !== BUILD_WORKFLOW || value.source_ci.workflow_file !== SOURCE_CI_WORKFLOW)
    fail('candidate workflow file mismatch');
...
function pinnedLegacyCompletionState(plan) {
  const matches = [...plan.matchAll(/^Review-orchestration-state: (\{.*\})$/gm)];
  if (matches.length !== 1) fail('pinned legacy completion plan must contain exactly one Review-orchestration-state');
  let state;
  try {
    state = JSON.parse(matches[0][1]);
  } catch {
    fail('pinned legacy completion Review-orchestration-state is not JSON');
  }
  if (canonicalize(state) !== matches[0][1])
    fail('pinned legacy completion Review-orchestration-state is not canonical JCS');
  if (canonicalize(state) !== PINNED_LEGACY_COMPLETION_STATE_JCS)
    fail('pinned legacy completion orchestration state tuple mismatch');
  const { state_sha256: stateSha256, ...preimage } = state;
  if (
    stateSha256 !== PINNED_LEGACY_COMPLETION.settledStateSha256 ||
    sha256(Buffer.from(canonicalize(preimage))) !== stateSha256
  )
    fail('pinned legacy completion orchestration state self-hash mismatch');
  return state;
}

function currentSingletonPolicy(author) {
  const candidate = author.tool === 'codex' ? { ...author, service_tier: 'default' } : { ...author };
  return {
    schema: 6,
    role: 'primary',
    fallback: 'none',
...
function normalizePinnedLegacyPolicies(value, currentPolicy, currentPolicySha256, counts) {
  if (typeof value === 'string') {
    if (value === PINNED_LEGACY_COMPLETION.policySha256) {
      counts.hashes += 1;
      return currentPolicySha256;
    }
    return value;
  }
  if (Array.isArray(value))
    return value.map((item) => normalizePinnedLegacyPolicies(item, currentPolicy, currentPolicySha256, counts));
  if (value !== null && typeof value === 'object') {
    if (
      ...
      if (canonicalize(value) !== PINNED_LEGACY_COMPLETION_POLICY_JCS)
        fail('pinned legacy completion nested policy copy mismatch');
      counts.policies += 1;
    ...
    return Object.fromEntries(
      Object.entries(value).map(([key, item]) => [
        key,
        normalizePinnedLegacyPolicies(item, currentPolicy, currentPolicySha256, counts),
      ]),
    );
  }
  return value;
...
function validatePinnedLegacyCompletionReceiptClosed(plan, completion, candidate, waivers) {
  const value = completion.value;
  if (candidate.digest !== PINNED_LEGACY_COMPLETION.candidateSha256)
    fail('pinned legacy completion candidate digest mismatch');
  if (value.reviewed_head !== PINNED_LEGACY_COMPLETION.reviewedHead)
    fail('pinned legacy completion reviewed-head pin mismatch');
  if (value.plan_input_sha256 !== PINNED_LEGACY_COMPLETION.planInputSha256)
    fail('pinned legacy completion plan-input pin mismatch');
  if (value.policy_sha256 !== PINNED_LEGACY_COMPLETION.policySha256)
    fail('pinned legacy completion policy-digest pin mismatch');
  if (canonicalize(value.policy) !== PINNED_LEGACY_COMPLETION_POLICY_JCS)
    fail('pinned legacy completion policy tuple mismatch');
  if (sha256(Buffer.from(PINNED_LEGACY_COMPLETION_POLICY_JCS)) !== PINNED_LEGACY_COMPLETION.policySha256)
    fail('pinned legacy completion policy self-digest mismatch');
  if (value.series?.orchestration_series_id !== PINNED_LEGACY_COMPLETION.seriesId)
    fail('pinned legacy completion orchestration-series pin mismatch');
  if (value.settled_orchestration_state_sha256 !== PINNED_LEGACY_COMPLETION.settledStateSha256)
    fail('pinned legacy completion settled-state hash pin mismatch');
  if (completion.digest !== PINNED_LEGACY_COMPLETION.receiptSha256)
    fail('pinned legacy completion receipt raw-digest pin mismatch');
  if (waivers.length !== 0) fail('pinned legacy completion waivers must be empty');

  const state = pinnedLegacyCompletionState(plan);
  const policy = currentSingletonPolicy(value.request.author);
  const policySha256 = sha256(Buffer.from(canonicalize(policy)));
  const counts = { policies: 0, hashes: 0 };
  const normalizedReceipt = normalizePinnedLegacyPolicies(value, policy, policySha256, counts);
  if (counts.policies !== 7 || counts.hashes !== 8)
    fail('pinned legacy completion nested policy copy/hash count mismatch');
  const { state_sha256: _legacyStateSha256, ...normalizedStatePreimage } = {
    ...state,
    series_sha256: sha256(Buffer.from(canonicalize(normalizedReceipt.series))),
  };
  const normalizedState = {
    ...normalizedStatePreimage,
    state_sha256: sha256(Buffer.from(canonicalize(normalizedStatePreimage))),
  };
  normalizedReceipt.settled_orchestration_state_sha256 = normalizedState.state_sha256;
  try {
    validatePlanCompletionReceipt(
      normalizedReceipt,
      { reviewed_head: PINNED_LEGACY_COMPLETION.reviewedHead, review_status: 'passed' },
      { waivers, orchestration: normalizedState },
    );
  } catch (error) {
    fail(`pinned legacy completion receipt envelope invalid: ${error.message}`);
  }
  return value;
}
...
function validateCurrentRedReceipt(value) {
  return validateTddRedReceipt(value, { repositoryId: REPOSITORY_ID });
}
...
function validateCurrentSourcePreparationProof(value) {
  ...
  exactKeys(
    ...
    planRunProof ? 'SourcePreparationProofV3' : 'SourcePreparationProofV2',
  );
  if (
  ...
  if (!planRunProof) {
    ...
    if (
      value.tdd_red.pre_production_commit === value.source_commit ||
      value.tdd_red.pre_production_commit === value.implementation_commit
    ) {
      fail('current TDD-red commit must be distinct from and precede the source implementation');
    }
  }

  exactKeys(
    value.completion_review,
    ['schema', 'type', 'reviewed_commit', 'diff_sha256', 'result_sha256', 'verdict'],
  ...
  digest(value.completion_review.diff_sha256, 'current source proof completion review diff');
  ...
  try {
    ...
    fail(`current source proof acceptance manifest is invalid: ${error.message}`);
  }
  digest(value.acceptance.verification_sha256, 'current source proof acceptance verification');
  const changedPaths = exactStringArray(
    value.acceptance.changed_paths,
    'current source proof acceptance changed_paths',
    { nonempty: true },
  );
  for (const [label, paths] of [
    ...
    for (const [index, logical] of paths.entries()) {
      safeLogical(logical, `current source proof ${label} path ${index}`);
      if (prior !== null && prior.localeCompare(logical) >= 0) {
        fail(`current source proof ${label} paths must be unique and sorted`);
      }
      prior = logical;
    }
  }
...
export function validateSourcePreparationProof(value) {
  if (
    ...
    ['SourcePreparationProofV2', 'SourcePreparationProofV3'].includes(value?.type)
  ) {
  ...
  if (value.candidate_sha256 !== sha256(Buffer.from(canonicalize(value.candidate))))
    fail('source proof candidate digest mismatch');
  exactKeys(
    value.plans,
    ['source_path', 'source_sha256', 'evidence_path', 'evidence_sha256', 'finished_path', 'finished_sha256'],
    'source proof plans',
  );
  if (
  ...
  for (const key of ['source_sha256', 'evidence_sha256', 'finished_sha256'])
    digest(value.plans[key], `source proof plans.${key}`);
  ...
  exactKeys(
    ...
    'source proof ancestry',
  );
  if (
    value.source_ancestry.source_commit !== value.source_commit ||
    value.source_ancestry.evidence_commit !== value.evidence_commit ||
    value.source_ancestry.shipped_commit !== value.shipped_commit ||
    value.source_ancestry.verified !== true
  )
    fail('source proof ancestry mismatch');
  exactKeys(
    value.non_plan_tree_equivalence,
    ['source_commit', 'shipped_commit', 'excluded_paths', 'verified'],
    'source proof tree equivalence',
  );
  if (
    value.non_plan_tree_equivalence.source_commit !== value.source_commit ||
...
export function prepare(options, fixtureJournal) {
  ...
  const files = [
    replaceJsonVersion(path.join(REPO, 'plugins/session-relay/.claude-plugin/plugin.json'), (value) => {
      value.version = VERSION;
    }),
    replaceJsonVersion(path.join(REPO, 'plugins/session-relay/.codex-plugin/plugin.json'), (value) => {
      value.version = VERSION;
    }),
  ...
  if (dryRun) {
    for (const item of files)
      process.stdout.write(`[dry-run] would update ${path.relative(REPO, item.file)} to ${VERSION}\n`);
    process.stdout.write(
      '[dry-run] no files changed; the real release will gate the exact changed tree with scripts/ci.mjs before committing.\n',
    );
    return {
      outcome: 'success',
      calls: [],
      ...
      receipt: null,
      state: { version: VERSION, tag: TAG, dry_run: true },
    };
  }
  ensureCleanTree();
  ...
  return {
    ...
    journal: fixtureJournal,
    receipt: null,
    state: { version: VERSION, tag: TAG, dry_run: false },
  };
}
...
function sourceCiDependencies(injected) {
  const value = injected ?? {
    ...
    apiBytes: (endpoint) => {
      ...
      if (result.error || result.signal || result.status !== 0)
        fail(
          `source CI log query failed: ${result.stderr?.toString().trim() || result.error?.message || result.signal || result.status}`,
        );
  ...
  for (const key of ['apiJson', 'apiBytes', 'now'])
    if (typeof value[key] !== 'function') fail(`source-CI dependency ${key} must be a function`);
  return value;
}
...
function decodeWorkflowFile(file, expectedPath) {
  if (
    ...
    !COMMIT.test(file.sha ?? '') ||
    file.encoding !== 'base64' ||
    typeof file.content !== 'string'
  )
    fail('source CI workflow file response mismatch');
  let bytes;
  try {
    bytes = Buffer.from(file.content.replace(/\s/g, ''), 'base64');
  } catch {
    fail('source CI workflow file is not base64');
  }
...
function validateSourceCiRun(run, runId, expectedCommit) {
  ...
  if (
    ...
    run.head_branch !== `preflight/session-relay-${VERSION}-${expectedCommit.slice(0, 12)}` ||
    run.event !== 'workflow_dispatch' ||
    run.status !== 'completed' ||
    run.conclusion !== 'success' ||
    run.path !== SOURCE_CI_WORKFLOW ||
    !inputsAreEmpty ||
    run.repository?.full_name !== REPOSITORY_ID ||
    run.head_repository?.full_name !== REPOSITORY_ID
  )
    fail('source CI run identity, inputs, or conclusion mismatch');
...
export function verifySourceCi(options, injected) {
  ...
  const workflowResponse = deps.apiJson(
    `repos/${REPOSITORY_ID}/contents/${SOURCE_CI_WORKFLOW}?ref=${encodeURIComponent(expectedCommit)}`,
  );
  ...
  for (const definition of workflow.requiredDefinitions) {
    ...
    if (
      matches.length !== 1 ||
      matches[0].number !== definition.number ||
      matches[0].status !== 'completed' ||
      matches[0].conclusion !== 'success'
    )
      fail(`source CI required step did not complete successfully: ${definition.name}`);
  ...
  const finalRun = validateSourceCiRun(
    deps.apiJson(`repos/${REPOSITORY_ID}/actions/runs/${runId}`),
    runId,
    expectedCommit,
  );
  if (
    ...
    finalRun.status !== run.status ||
    finalRun.conclusion !== run.conclusion
  )
    fail('source CI run identity changed while evidence was collected');
  ...
  const workflowIdentity = sha256(
    Buffer.from(
      canonicalize({ source_commit: expectedCommit, file_blob_id: workflow.blobId, file_sha256: workflowSha }),
    ),
  );
  const receipt = {
    schema: 1,
    type: 'SourceCiReceiptV1',
    repository_id: REPOSITORY_ID,
    source_commit: expectedCommit,
    workflow: {
      file: SOURCE_CI_WORKFLOW,
      file_blob_id: workflow.blobId,
      file_sha256: workflowSha,
      identity_sha256: workflowIdentity,
      run_database_id: runId,
    ...
    jobs_sha256: sha256(Buffer.from(jobsText)),
    logs_sha256: sha256(Buffer.from(logsText)),
    started_at: run.run_started_at,
    completed_at: finalRun.updated_at,
    created_at: deps.now(),
  };
...
function inspectPublicRepository(input) {
  ...
  try {
    ...
    try {
      ...
      fail('companion verification root disappeared before cleanup');
    }
    if (
      ...
      fail('companion verification root identity changed before cleanup');
...
function evidenceDependencies(injected) {
  ...
  if (
    ...
    fs.realpathSync.native(value.repoRoot) !== value.repoRoot ||
    !fs.lstatSync(value.repoRoot).isDirectory()
  )
    fail('preparation dependency repoRoot must be a canonical directory');
  for (const key of ['git', 'inspectPublic', 'run', 'now', 'readFile'])
    if (typeof value[key] !== 'function') fail(`preparation dependency ${key} must be a function`);
  return value;
}
...
function runStep(deps, executable, args, runOptions) {
  ...
  if (!record(result) || !Number.isSafeInteger(result.status))
    fail('preparation run adapter returned an invalid result');
...
function verifyRedBlobs(deps, receipt, implementationCommit, label) {
  ...
  for (const item of receipt.test_paths) {
    ...
    if (atRed !== item.blob_id || atImplementation !== item.blob_id)
      fail(`${label} test blob changed after TDD-red capture`);
  }
  ...
  if (producerAtRed !== receipt.producer.blob_id || producerAtImplementation !== receipt.producer.blob_id)
    fail(`${label} TDD-red producer blob mismatch`);
}
...
function receiptSummary(preflight, sourceCi) {
  return {
    preflight: {
      sha256: preflight.digest,
      workflow_file: preflight.value.workflow.file,
      workflow_blob_id: preflight.value.workflow.file_blob_id,
      run_database_id: preflight.value.workflow.run_database_id,
      run_attempt: preflight.value.workflow.run_attempt,
    },
    source_ci: {
      sha256: sourceCi.digest,
      workflow_file: sourceCi.value.workflow.file,
      workflow_blob_id: sourceCi.value.workflow.file_blob_id,
      run_database_id: sourceCi.value.workflow.run_database_id,
      run_attempt: sourceCi.value.workflow.run_attempt,
    },
  };
}
...
function verifyPublicBinding(deps, docksPlan, publicReceipt) {
  ...
  if (
    notes.repository_id !== PUBLIC_REPOSITORY_ID ||
    noteValue(docksPlan, 'Companion TDD-red receipt SHA-256') !== publicReceipt.digest
  )
    fail('companion Notes identity mismatch');
  ...
  if (publicReceipt.value.pre_production_commit === notes.commit)
    fail('companion TDD-red commit must predate implementation');
  ...
  if (
    ...
    fail('companion blocked lifecycle mismatch');
  exactArray(observed.test_blobs, 'companion inspected test blobs');
  if (observed.test_blobs.length !== publicReceipt.value.test_paths.length)
    fail('companion inspected test blob count mismatch');
  for (const [index, expected] of publicReceipt.value.test_paths.entries()) {
    ...
    if (
      ...
      actual.red_blob_id !== expected.blob_id ||
      actual.implementation_blob_id !== expected.blob_id
    )
...
export function checkPrepared(options, injected) {
  ...
  const docks = readCanonical(
    options.get('docks-red'),
    options.get('docks-red-sha256'),
    'TddRedReceiptV1',
    '--docks-red',
  );
  const publicReceipt = readCanonical(
    options.get('public-red'),
    options.get('public-red-sha256'),
    'TddRedReceiptV1',
    '--public-red',
  );
  const preflight = readCanonical(
    options.get('preflight'),
    options.get('preflight-sha256'),
    'ProducerPreflightReceiptV1',
    '--preflight',
  );
  const sourceCi = readCanonical(
    options.get('source-ci'),
    options.get('source-ci-sha256'),
    'SourceCiReceiptV1',
    '--source-ci',
  );
  validateTddRedReceipt(docks.value, { repositoryId: REPOSITORY_ID });
  validateTddRedReceipt(publicReceipt.value, { repositoryId: PUBLIC_REPOSITORY_ID });
  if (canonicalize(docks.value.producer) !== canonicalize(publicReceipt.value.producer))
    fail('Docks and companion TDD-red producer identities differ');
  ...
  const receipt = {
    ...
    docks_red: {
      sha256: docks.digest,
      pre_production_commit: docks.value.pre_production_commit,
      test_blobs: docks.value.test_paths,
    },
    companion: notes,
    preflight: summaries.preflight,
    source_ci: summaries.source_ci,
    checks,
    created_at: deps.now(),
  };
...
export function verifyEmbedded(options, injected) {
  const deps = evidenceDependencies(injected);
  const planPath =
    injected === undefined ? canonicalPath(options.get('plan'), '--plan') : path.resolve(REPO, options.get('plan'));
  ...
  if (canonicalize(evidence.docks.value.producer) !== canonicalize(evidence.publicReceipt.value.producer))
    fail('embedded TDD-red producer identities differ');
  ...
  if (canonicalize(companion) !== canonicalize(evidence.candidate.value.companion))
    fail('embedded candidate companion binding mismatch');
  requireSourceTree(deps, source);
  const executionBase = parseExecutionBase(plan);
  ...
  if (
    canonicalize(evidence.candidate.value.docks_red) !== canonicalize(expectedDocksRed) ||
  ...
  if (evidence.candidate.value.plan.source_blob_sha256 !== sha256(gitBytes(deps, ['show', `${source}:${PLAN_PATH}`])))
    fail('embedded candidate source plan blob mismatch');
  verifyRedBlobs(deps, evidence.docks.value, source, 'Docks');
  if (
    evidence.preflight.value.workflow.file_blob_id !== gitValue(deps, ['rev-parse', `${source}:${BUILD_WORKFLOW}`]) ||
  ...
  if (
    ...
    fail('fresh acceptance command/outcome evidence disagrees with candidate');
  return {
    receipt: evidence.candidate.value,
    state: {
      source_commit: source,
      evidence_commit: gitValue(deps, ['rev-parse', 'HEAD^{commit}']),
      candidate_sha256: evidence.candidate.digest,
    },
  };
}
...
function completionRecord(plan) {
  ...
  try {
    ...
    fail('completion review receipt is not JSON');
  }
...
function currentCompletionReview(plan, run, diffSha256) {
  const candidates = [
  ...
  const bound = candidates.filter(({ digest: resultSha256 }) => resultSha256 === run.completion_review.result_sha256);
  if (bound.length !== 1) {
...
function verifyCurrentRedBlobs(deps, red, implementationCommit) {
  if (red.command.cwd !== deps.repoRoot) {
    fail('current TDD-red command repository root does not match the bound repository');
  }
  for (const item of [red.producer, ...red.test_paths]) {
    const atRed = gitValue(deps, ['rev-parse', `${red.pre_production_commit}:${item.path}`]);
    const atImplementation = gitValue(deps, ['rev-parse', `${implementationCommit}:${item.path}`]);
    if (atRed !== item.blob_id || atImplementation !== item.blob_id) {
      fail(`current TDD-red producer or test blob identity drifted after capture: ${item.path}`);
    }
  }
}
...
function currentCompletionEvidence(
  deps,
  planBytes,
  run,
  ...
  for (const [index, logical] of changedPaths.entries()) {
    ...
    if (prior !== null && prior.localeCompare(logical) >= 0) {
      fail('current reviewed diff paths must be unique and sorted');
    }
    if (!allowedPaths.has(logical)) {
      fail(`current reviewed diff contains out-of-scope affected-path drift: ${logical}`);
    }
    prior = logical;
  }
  return {
...
function bindCurrentCompletion(options, deps, finishedRelative, planBytes, plan) {
  ...
  const receipt = {
    ...
    completion_review: {
      ...
      diff_sha256: completionEvidence.diffSha256,
      result_sha256: reviewRecord.digest,
      verdict: reviewRecord.value.verdict,
    },
    acceptance: {
      manifest: completionEvidence.acceptanceManifest,
      verification_sha256: run.acceptance.verification_sha256,
...
export function bindCompletion(options, injected) {
  ...
  if (frontmatterValue(plan, 'status') !== 'finished' || frontmatterValue(plan, 'review_status') !== 'passed')
    fail('finished plan has not passed completion review');
  const candidate = embeddedReceipt(plan, 'Source preparation candidate', 'SourcePreparationCandidateV1');
  ...
  const waivers = parsePlan(planBytes).frontmatter.review_waivers ?? [];
  if (
    finishedRelative === PINNED_LEGACY_COMPLETION.finishedPlanPath &&
    candidate.digest === PINNED_LEGACY_COMPLETION.candidateSha256
  )
  ...
  const currentHead = commit(gitValue(deps, ['rev-parse', 'HEAD^{commit}']), 'current release HEAD');
  const shippedCommit = commit(
    gitValue(deps, ['log', '-n1', '--format=%H', '--', finishedRelative]),
    'finished plan archive commit',
  );
  ...
  if (
    sha256(sourcePlan) !== candidate.value.plan.source_blob_sha256 &&
    legacyTrimmedSha256 !== candidate.value.plan.source_blob_sha256
  )
    fail('source plan blob no longer matches candidate');
  const receipt = {
    schema: 1,
