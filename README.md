...
PlanRunV1 binds repository/path/run identity, a shared cross-repository
`goal_id`, canonical requested effects and risk, plan/source hashes, separate
two-permit draft/completion phases, execution/implementation identities,
acceptance hashes, and at most one typed blocker. Reviews reserve before every
fresh launch; stale output is ignored and cold reserved state blocks. Ordinary
local work has no completion reviewer; sensitive/external exact-diff review is
bounded to one review plus one blocker-fix re-review.
