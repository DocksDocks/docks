...
updated: "2026-08-03T20:23:43.365+00:00"
started_at: null
finished_at: null
assignee: null
...
| A14 | `node -e '(async () => { const cp = require("child_process"), crypto = require("crypto"); const PARENT_RUN = process.env.PARENT_RUN_ID; if (!PARENT_RUN) throw new Error("PARENT_RUN_ID must be set"); const OWNER = "DocksDocks/public", SUFFIX = "-session-relay-0.16.0-docks-kit-0.14.0-release.md"; const gh = (a, buf) => cp.execFileSync("gh", a, buf ? { maxBuffer: 1 << 28 } : { encoding: "utf8", maxBuffer: 1 << 28 }); const raw = (p, ref) => gh(["api", "-H", "Accept: application/vnd.github.raw", "repos/" + OWNER + "/contents/" + p + "?ref=" + ref], true); const sha = (b) => crypto.createHash("sha256").update(b).digest("hex"); const listed = JSON.parse(gh(["api", "repos/" + OWNER + "/contents/docs/plans/finished?ref=main"])); const hits = listed.map((e) => e.name).filter((n) => n.endsWith(SUFFIX)); if (hits.length !== 1) throw new Error("expected exactly one child archive on remote main, found " + hits.length); const archivePath = "docs/plans/finished/" + hits[0]; const body = raw(archivePath, "main").toString("utf8"); const run = JSON.parse(body.split("\n").find((l) => l.startsWith("Plan-run:")).slice(9)); const must = (c, m) => { if (!c) throw new Error(m); }; must(run.plan_path === "docs/plans/active/session-relay-0.16.0-docks-kit-0.14.0-release.md", "child plan_path is " + run.plan_path); must(run.goal_id === "cef66d21-5bd3-4e07-a0e8-e393822dcfb0", "child goal_id differs from the parent"); must(run.run_id !== PARENT_RUN, "child run_id must differ from the parent"); must(run.draft_review.state === "passed", "child draft review is " + run.draft_review.state); must(run.completion_review.state === "passed", "child completion review is " + run.completion_review.state); must(/^status: finished/m.test(body), "child plan is not finished"); must(/^finished_at: "[^"]+"/m.test(body), "child finished_at is unset"); must(/^[0-9a-f]{40}$/.test(run.implementation_commit || ""), "child implementation_commit is malformed"); for (const d of ["da8b114216c3f2301ad582df8e59b49e91953abcc1112b510466b31637fda825", "816b6b8bd2d2c2518ea359a5a21502213347b387a1cc576a0fb9cf541e5646ed", "b3ca082dc5ea51e8322be407cdb4bbcaaa05d80bd62c3553f82ab98c1a95498a"]) must(body.includes(d), "child does not pin " + d); must(body.includes("session-relay--v0.16.0"), "child does not pin the parent tag"); cp.execFileSync("git", ["ls-remote", "--exit-code", "--tags", "https://github.com/" + OWNER + ".git", "refs/tags/cli-v0.14.0"], { stdio: "ignore" }); cp.execFileSync("npm", ["view", "docks-kit@0.14.0", "version"], { stdio: "ignore" }); const fmBlock = body.match(/^affected_paths:\n((?:[ \t]+-[ \t]+.*\n)+)/m); must(fmBlock !== null, "child declares no affected_paths"); const paths = fmBlock[1].split("\n").map((l) => l.replace(/^\s*-\s*/, "").trim()).filter(Boolean); must(paths.length > 0, "child affected_paths is empty"); const rel = await import("/home/vagrant/projects/docks/scripts/lib/session-relay-release.mjs"); const observed = paths.map((p) => ({ path: p, sha256: sha(raw(p, run.implementation_commit)) })); const digest = sha(Buffer.from(rel.canonicalize({ schema: 1, source_base: run.implementation_commit, paths: observed }))); const inst = require("/home/vagrant/projects/docks/scripts/lib/session-relay-release-instances/0.16.0.json"); const child = inst.public_child; if (child.version === "0.14.0") { must(child.implementation_content_sha256 === digest, "instance pins " + child.implementation_content_sha256 + " but the child tree reproduces " + digest); must(child.tag === "cli-v0.14.0", "instance child tag is " + child.tag); } else { must(child.version === "0.13.0", "instance child must be the retained 0.13.0 before finalize, found " + child.version); } console.log("child " + archivePath + " run " + run.run_id + " reproduces " + digest + " over " + paths.length + " paths (instance child " + child.version + ")"); })();'` | Exit 0 only when the child is proven from the REMOTE, never from a local checkout. It lists `docs/plans/finished` on `DocksDocks/public@main` through `gh api`, requires exactly one entry ending `-session-relay-0.16.0-docks-kit-0.14.0-release.md`, and reads those bytes from the remote. It then asserts the embedded PlanRunV1 `plan_path`, the shared `goal_id`, a `run_id` distinct from the parent's, both reviews `passed`, `status: finished`, a non-null `finished_at`, a well-formed `implementation_commit`, the published tag `cli-v0.14.0`, the published npm coordinate `docks-kit@0.14.0`, all three verified Relay 0.16.0 digests, and the parent tag. Finally it INDEPENDENTLY REPRODUCES the child implementation-content digest the way `scripts/lib/session-relay-release-promotion.mjs:1059-1077` defines it - sha256 over the canonicalized `{schema, source_base, paths[{path, sha256}]}` built by reading every child `affected_paths` entry at the child's own implementation commit from the remote, using the shipped `canonicalize` rather than a reimplementation. Before `step:finalize_instance` the instance still names the retained `0.13.0` child and the row asserts exactly that; after the repoint the instance names `0.14.0` and the row asserts the pinned digest equals the reproduction, so the comparison is never vacuous across the run. `PARENT_RUN_ID` must be bound, so an unset variable fails closed. |
...
Plan-run: {"acceptance":null,"blocker":null,"completion_review":{"input_sha256":null,"invocations":0,"result_sha256":null,"state":"not_started"},"draft_review":{"input_sha256":null,"invocations":0,"result_sha256":null,"state":"not_started"},"execution_parent":null,"goal_id":"cef66d21-5bd3-4e07-a0e8-e393822dcfb0","implementation_commit":null,"plan_path":"docs/plans/active/session-relay-0.16.0-release.md","plan_sha256":"b25820a0be87c9ff35765c8ac061b9c52d54b108437699190d42e9ee93fc6334","repository_id":"docks:/home/vagrant/projects/docks","requested_effects":["local","probe","push","release"],"risk":"external","run_id":"1c5b6f71-c69d-4566-adcc-4c15a9f0c599","schema":1,"source_base":"0b038ff003264f81a8669b40fffb8585477fe5e1","source_sha256":"6215b78d41f040aa9903a914d42b8758e64628ac17b2521a4e30ed05ec94b3a5"}
