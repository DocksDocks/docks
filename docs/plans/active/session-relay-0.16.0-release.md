...
updated: "2026-08-04T05:15:30.304+00:00"
started_at: null
finished_at: null
assignee: null
...
| 19 | refresh_child_source_successor_identity | Move the current 0.16.0 release instance and both independent parent oracles from the blocked predecessor to this plan's fresh `run_id`, without changing the immutable tag source, goal, child, assets, or historical instances. | `scripts/lib/session-relay-release-instances/0.16.0.json`; `plugins/session-relay/test/distribution-contract.mjs`; `plugins/session-relay/test/companion-distribution-contract.mjs` | 18 | `local` | `planned` | The instance current attempt and PlanRun attempt plus both current-run oracles equal this plan's `run_id`; source `875661640c1c45894c66a9b2fdc437fd47307e1f`, docks-kit 0.14.0, three asset digests, and all retained identities remain unchanged. |
| 20 | public_source_base_regression | Make the current finished-child fixture carry distinct `source_base` and `execution_parent` identities exactly like the released docks-kit 0.14.0 PlanRun, and reproduce the live rejection before changing production validation. | `plugins/session-relay/test/release-promotion-contract.mjs` | 19 | `local` | `planned` | The current promotion contract fails before the repair with `current public finished PlanRunV1 lifecycle, identity, effect, or completion binding is invalid`; the fixture still has the exact bound execution parent, implementation commit, plan digest, completion digest, and released affected-path content. A malformed plan or ancestry failure is not the required red result. |
| 21 | separate_child_source_base | Treat the child `source_base` and explicit request `companion_base_commit` as separate reviewed identities: validate the former as a real PlanRun commit and keep exact equality only for `execution_parent`. | `scripts/lib/session-relay-release-promotion.mjs` | 20 | `local` | `planned` | The canonical released child passes with `source_base != execution_parent`; the explicit request still pins `execution_parent`, and every repository, goal, run, effect, completion, implementation-content, archive, and ancestry check remains fail-closed. Failure: STOP rather than dropping the execution-parent pin or accepting a malformed source commit. |
| 22 | public_source_base_reproof | Run the focused promotion contract, then the complete authoritative repository gate on the repaired successor bytes. | all affected paths in frontmatter | 21 | `local` | `planned` | A16, A18, and A7 pass. Reverting only the source-base separation makes A18 fail with the live rejection; restoring it returns green. |
| 23 | public_source_base_checkpoint | Commit the repair, bind acceptance at that exact HEAD, and pass a fresh completion review over the predecessor implementation through this fix. | all affected paths in frontmatter | 22 | `local` | `planned` | HEAD, implementation commit, acceptance manifest source base, and CompletionReviewV1 bind one exact successor commit; the reviewed diff includes the live-representative fixture and the narrow verifier repair. |
| 24 | republish_child_source_successor | Re-bind completion and publish this successor implementation to `origin/main`, then reconcile the existing staged prerelease to the new reviewed PlanRun receipt. | — | 23 | `push` | `planned` | With exact live push and release authority, `--bind-completion` and `--publish-reviewed --rebind-complete-publication` emit fresh no-clobber receipts for this run; remote `main` becomes exactly the new reviewed implementation, while the immutable tag and staged asset bytes remain unchanged. Failure: STOP with receipts; never force, retag, replace an asset, or reuse the predecessor source proof. |
| 25 | promote_stable | Promote the existing byte-identical prerelease to stable through the remaining current PlanRun release modes. | release receipts | 24 | `release` | `planned` | With exact live release authority, after step:publish_main run in order `--emit-public-request`, `--verify-public-release`, `--promote-reviewed`, `--finalize-reviewed`; every no-clobber receipt validates, the stable release is non-draft/non-prerelease, its body includes the Intel deprecation sentence, and the three binaries plus checksum are byte-identical to step:verify_assets. The promotion target is the GitHub release `session-relay--v0.16.0` only; this row claims no branch target. Failure: STOP with receipts; no recovery-only mode without its exact documented state. |
| 26 | probe_refs | Re-read the remote preflight refs after promotion and prove every recorded baseline ref individually survives. | — | 25 | `probe` | `planned` | With exact live probe authority for read-only `git ls-remote --heads origin 'preflight/*'`, A8 passes: every baseline ref named in the rationale section C fenced list is individually present on the remote; an unchanged total count is not evidence, because the added 0.16.0 validate-only ref can mask one deletion. Failure: STOP; name the first missing ref and treat its absence as an unauthorized deletion. |
| 27 | archive_push | Finish, archive, commit, push, and remotely read back the terminal plan. | — | 25, 26 | `push` | `planned` | Exact live push authority matches the archive/main target; PlanRunV1 is finished with bound acceptance and passed completion review; the active plan moves once to the dated archive; the archive checkpoint is pushed and remotely readable; no preflight ref was deleted, with every baseline ref proven present by step:probe_refs. This step’s targets are the lifecycle-managed plan record `docs/plans/active/session-relay-0.16.0-release.md`, its dated archive `the lifecycle-selected archive path `docs/plans/finished/<finish-date>-session-relay-0.16.0-release.md`, where `<finish-date>` is the UTC date on which the archive transaction actually runs and is never pinned in advance; the push and the remote read-back both use the path that transaction reports`, and DocksDocks/docks branch `main`; the archive transaction owns those paths, so they are not step-edited files. Failure before the archive transaction: leave the run ongoing and do not claim archive completion. Failure after the archive transaction or archive checkpoint: preserve the finished archive and checkpoint, stop with receipts, and resume only the idempotent push plus A15 remote read-back under fresh exact push authority; never repeat archival. A15 is the runnable remote read-back proving this row's remote claim. |
