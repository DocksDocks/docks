...
updated: "2026-08-04T05:34:08.293+00:00"
started_at: null
finished_at: null
assignee: null
...
> so its publication row would fail even after the child verifier repair. Rows 1-18 remain
...
> reviewed main publication (`push`) from read-only prerelease rebind (`probe`) before promotion.
>
## Steps
...
| 18 | publish_main | Bind completion and publish the reviewed implementation to the target branch through the first two current PlanRun release modes. | — | 17 | `push` | `done` | With exact live push authority matching the DocksDocks/docks branch `main` target, run in order `--bind-completion`, then `--publish-reviewed --rebind-complete-publication`; remote `main` becomes exactly the reviewed implementation and every no-clobber receipt validates. Failure: STOP with receipts; no recovery-only mode without its exact documented state. A15 is the runnable remote read-back proving this row's remote claim. |
| 19 | refresh_source_separated_identity | Move the current 0.16.0 instance and both independent parent oracles to this plan's fresh `run_id`, while preserving the immutable release source, goal, child, assets, and historical instances. | `scripts/lib/session-relay-release-instances/0.16.0.json`; `plugins/session-relay/test/distribution-contract.mjs`; `plugins/session-relay/test/companion-distribution-contract.mjs` | 18 | `local` | `planned` | The instance current attempt and PlanRun attempt plus both current-run oracles equal this plan's `run_id`; instance `docks_source_base` and tag commit remain `875661640c1c45894c66a9b2fdc437fd47307e1f`, independently of this run's later `source_base`. |
| 20 | post_tag_source_regression | Add a PlanRun completion fixture whose immutable release source/tag, older `execution_parent`, later successor `source_base`, and implementation commit are distinct commits with both parent identities preceding implementation, and reproduce the current release-identity rejection before the repair. | `plugins/session-relay/test/release-evidence-contract.mjs` | 19 | `local` | `planned` | Before production changes, the otherwise valid fixture fails with `current PlanRunV1 release identity mismatch`; the fixture proves release-source-to-tag, tag-to-implementation, run-source-to-implementation, execution-to-implementation, completion diff, and accepted manifest independently. A malformed record or missing tag is not the required red result. |
| 21 | separate_parent_sources | Stop requiring the live PlanRun `source_base` to equal the instance release source; use the instance source only for immutable source-to-tag proof and use the validated run source only for its independent source-to-implementation proof. | `scripts/lib/session-relay-release-preparation.mjs` | 20 | `local` | `planned` | A post-tag successor binds successfully; SourcePreparationProofV3 `source_commit` stays the immutable instance source, `plan_run.source_base` stays the reviewed successor source, and an unrelated run source is refused by ancestry. Existing execution-parent diff, tag, acceptance, and completion-review checks remain unchanged. |
| 22 | public_source_base_regression | Make the current finished-child fixture carry distinct `source_base` and `execution_parent` identities exactly like released docks-kit 0.14.0, and reproduce the live rejection before changing child validation. | `plugins/session-relay/test/release-promotion-contract.mjs` | 21 | `local` | `planned` | The current promotion contract fails before the child repair with `current public finished PlanRunV1 lifecycle, identity, effect, or completion binding is invalid`; every other PlanRun, review, content, and archive binding remains valid. |
| 23 | separate_child_source_base | Remove only the false child source-base equality, retain exact request equality for `execution_parent`, and require independently observed `source_base`-to-implementation ancestry. | `scripts/lib/session-relay-release-promotion.mjs`; `plugins/session-relay/test/release-promotion-contract.mjs` | 22 | `local` | `planned` | The canonical child passes with distinct identities; a forged unrelated 40-hex source base fails `source-base-to-implementation ancestry`; all repository, goal, run, effect, review, content, release, and archive checks remain fail-closed. |
| 24 | source_separation_reproof | Run A16-A18 and the complete authoritative repository gate on the source-separated successor bytes. | all affected paths in frontmatter | 23 | `local` | `planned` | A16, A17, A18, and A7 pass. Reverting either narrow production repair reproduces its exact red signature; restoring both returns green. |
| 25 | source_separation_checkpoint | Commit the complete repair, bind acceptance at that exact HEAD, and pass a fresh completion review over the predecessor implementation through both source separations. | all affected paths in frontmatter | 24 | `local` | `planned` | HEAD, implementation commit, acceptance manifest, and CompletionReviewV1 bind one exact successor commit; the reviewed diff includes both red fixtures, both narrow repairs, and fresh current-run identity. |
| 26 | publish_source_separated_main | Bind this successor completion and fast-forward the exact reviewed implementation to `origin/main`. | — | 25 | `push` | `planned` | With exact live push authority, `--bind-completion` emits a fresh SourcePreparationProofV3 for this run and remote `main` becomes exactly the reviewed implementation. The tag, release, and assets do not change. Failure: STOP; never force or reuse a predecessor source proof. |
| 27 | rebind_source_separated_publication | Read back the existing staged prerelease and emit a publication receipt bound to this successor source proof. | release receipts | 26 | `probe` | `planned` | With exact live read-only probe authority, `--publish-reviewed --rebind-complete-publication` validates the immutable tag, successful workflow, release metadata, and downloaded asset bytes, then emits a fresh local no-clobber receipt. It performs no GitHub mutation. |
| 28 | promote_stable | Promote the existing byte-identical prerelease to stable through the remaining current PlanRun release modes. | release receipts | 27 | `release` | `planned` | With exact live release authority, after step:rebind_source_separated_publication consume that step's fresh receipt and run in order `--emit-public-request`, `--verify-public-release`, `--promote-reviewed`, `--finalize-reviewed`; every no-clobber receipt validates, the release becomes stable, and the three binaries plus checksum remain byte-identical. |
| 29 | probe_refs | Re-read the remote preflight refs after promotion and prove every recorded baseline ref individually survives. | — | 28 | `probe` | `planned` | With exact live probe authority for read-only `git ls-remote --heads origin "preflight/*"`, A8 passes for every named baseline ref; an unchanged total count is not sufficient. |
| 30 | archive_push | Finish, archive, commit, push, and remotely read back the terminal plan. | — | 28, 29 | `push` | `planned` | Exact live push authority matches the archive/main target; PlanRunV1 is finished with bound acceptance and passed completion review; the active plan moves once to its dated archive, the archive checkpoint is pushed and remotely readable, and no preflight ref was deleted. |
...
| A17 | `node plugins/session-relay/test/release-evidence-contract.mjs` | Exit 0; a current post-tag successor fixture keeps immutable release `source_commit` separate from its later PlanRun `source_base`, binds completion to the execution-parent diff, proves release-source-to-tag, tag-to-implementation, run-source-to-implementation, and execution-parent-to-implementation ancestry, and refuses an unrelated run source. Reverting only the production source selection reproduces `current PlanRunV1 release identity mismatch`. |
