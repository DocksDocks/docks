...
updated: "2026-08-04T06:21:53.495+00:00"
...
| 20 | refresh_source_separated_identity | Move the current 0.16.0 instance and both independent parent oracles to this plan's fresh `run_id`, while preserving the immutable release source, goal, child, assets, and historical instances. | `scripts/lib/session-relay-release-instances/0.16.0.json`; `plugins/session-relay/test/distribution-contract.mjs`; `plugins/session-relay/test/companion-distribution-contract.mjs` | 19 | `local` | `planned` | The instance current attempt and PlanRun attempt plus both current-run oracles equal this plan's `run_id`; instance `docks_source_base` and tag commit remain `875661640c1c45894c66a9b2fdc437fd47307e1f`, independently of this run's later `source_base`. |
| 21 | post_tag_source_regression | Add a PlanRun completion fixture whose immutable release source/tag, older `execution_parent`, later successor `source_base`, and implementation commit are distinct commits with both parent identities preceding implementation; reproduce both stale equality failures before repair. | `plugins/session-relay/test/release-evidence-contract.mjs`; `plugins/session-relay/test/release-promotion-contract.mjs` | 20 | `local` | `planned` | Before production changes, the valid fixture fails preparation with `current PlanRunV1 release identity mismatch`; the downstream promotion proof fixture fails the stale `plan_run.source_base` equality. The fixture independently proves release-source-to-tag, tag-to-implementation, run-source-to-implementation, execution-to-implementation, completion diff, accepted manifest, and downstream proof consumption. |
| 22 | separate_parent_sources | Separate immutable release source from live PlanRun source in completion binding and every promotion proof consumer. | `scripts/lib/session-relay-release-preparation.mjs`; `scripts/lib/session-relay-release-promotion.mjs` | 21 | `local` | `planned` | `currentPlanRun` no longer requires run `source_base` to equal instance source. SourcePreparationProofV3 `source_commit` stays the instance source for source-to-tag; `plan_run.source_base` stays the reviewed successor source and must precede implementation; execution parent independently precedes implementation. `validatePromotionEvidenceFreshContext` preserves these separate identities instead of restoring the false equality. Unrelated run source is refused. |
| 23 | public_source_base_regression | Make the current finished-child fixture carry distinct `source_base` and `execution_parent` identities exactly like released docks-kit 0.14.0, and reproduce the live rejection before changing child validation. | `plugins/session-relay/test/release-promotion-contract.mjs` | 22 | `local` | `planned` | The current promotion contract fails before the child repair with `current public finished PlanRunV1 lifecycle, identity, effect, or completion binding is invalid`; every other PlanRun, review, content, and archive binding remains valid. |
| 24 | separate_child_source_base | Remove only the false child source-base equality, retain exact request equality for `execution_parent`, and require independently observed `source_base`-to-implementation ancestry. | `scripts/lib/session-relay-release-promotion.mjs`; `plugins/session-relay/test/release-promotion-contract.mjs` | 23 | `local` | `planned` | The canonical child passes with distinct identities; a forged unrelated 40-hex source base fails `source-base-to-implementation ancestry`; all repository, goal, run, effect, review, content, release, and archive checks remain fail-closed. |
| 25 | source_separation_reproof | Run A16-A19 and the complete authoritative repository gate on the source-separated successor bytes. | all affected paths in frontmatter | 24 | `local` | `planned` | A16, A17, A18, A19, and A7 pass. Reverting either narrow source repair or the rebind early-return assertion reproduces its exact failure; restoring all returns green. |
| 26 | source_separation_checkpoint | Commit the complete repair, bind acceptance at that exact HEAD, and pass a fresh completion review over the predecessor implementation through both source separations. | all affected paths in frontmatter | 25 | `local` | `planned` | HEAD, implementation commit, acceptance manifest, and CompletionReviewV1 bind one exact successor commit; the reviewed diff includes the identity refresh, post-tag and child fixtures, downstream consumer, read-only rebind assertion, and narrow repairs. |
| 27 | publish_source_separated_main | Bind this successor completion and fast-forward the exact reviewed implementation to `origin/main`. | — | 26 | `push` | `planned` | With exact live push authority, `--bind-completion` emits a fresh SourcePreparationProofV3, then explicit `git push origin ${IMPLEMENTATION_COMMIT}:refs/heads/main` fast-forwards remote `main` to exactly the reviewed implementation. The tag, release, and assets do not change. |
| 28 | rebind_source_separated_publication | Read back the existing staged prerelease and emit a publication receipt bound to this successor source proof. | release receipts | 27 | `probe` | `planned` | With exact live read-only probe authority, `--publish-reviewed --rebind-complete-publication` returns through `rebindCompletePublication` before all tag/release mutators, validates the immutable tag, workflow, release metadata, and downloaded assets, then emits a fresh local receipt. |
| 29 | promote_stable | Promote the existing byte-identical prerelease to stable through the remaining current PlanRun release modes. | release receipts | 28 | `release` | `planned` | With exact live release authority, after step:rebind_source_separated_publication consume that step's fresh receipt and run in order `--emit-public-request`, `--verify-public-release`, `--promote-reviewed`, `--finalize-reviewed`; every no-clobber receipt validates, the release becomes stable, and the three binaries plus checksum remain byte-identical. |
| 30 | probe_refs | Re-read the remote preflight refs after promotion and prove every recorded baseline ref individually survives. | — | 29 | `probe` | `planned` | With exact live probe authority for read-only `git ls-remote --heads origin preflight/*`, A8 passes for every named baseline ref; an unchanged total count is not sufficient. |
| 31 | archive_push | Finish, archive, commit, push, and remotely read back the terminal plan. | — | 29, 30 | `push` | `planned` | Exact live push authority matches the archive/main target; PlanRunV1 is finished with bound acceptance and passed completion review; the active plan moves once to its dated archive, the archive checkpoint is pushed and remotely readable, and no preflight ref was deleted. |
...
| A17 | `node plugins/session-relay/test/release-evidence-contract.mjs && node plugins/session-relay/test/release-promotion-contract.mjs` | Exit 0; a post-tag successor keeps immutable release `source_commit` separate from later PlanRun `source_base` in preparation and downstream promotion proof, binds completion to the execution-parent diff, proves all independent ancestry chains, and refuses an unrelated run source. Reverting only the source separation reproduces the preparation and downstream equality failures. |
| A18 | `node plugins/session-relay/test/release-promotion-contract.mjs` | Exit 0 with the current child fixture carrying `source_base` equal to its implementation commit and a distinct, exactly pinned `execution_parent`; reverting only the production equality repair reproduces the live `current public finished PlanRunV1 lifecycle, identity, effect, or completion binding is invalid` rejection. |
...
- Steps 20-24 completed on implementation commit `03f79233fd0d5275b0610aa2d20aa011e6417269`.
- A16 passed. `release-instance-contract.mjs`, `distribution-contract.mjs`, and `companion-distribution-contract.mjs` bind run `3c2a2253-3999-464f-b58c-055bf60604e1` while the immutable release source remains `875661640c1c45894c66a9b2fdc437fd47307e1f`.
- A17 passed. `release-evidence-contract.mjs` binds immutable `source_commit` separately from the later PlanRun `source_base`, observes PlanRun-source-to-implementation ancestry, and refuses an unrelated PlanRun source. `release-promotion-contract.mjs` consumes that distinct identity and independently observes the same ancestry.
- A18 passed. The child fixture mirrors released docks-kit 0.14.0: `source_base` equals the implementation commit, `execution_parent` is distinct, and both precede or equal the implementation as required. Reverting the child equality repair reproduced `current public finished PlanRunV1 lifecycle, identity, effect, or completion binding is invalid` before the production repair.
- A19 passed. `release-publication-contract.mjs` completed 85 production-handler cases; explicit complete-publication rebind cases assert no branch, tag, release, or asset mutation.
- A7 passed on the final implementation bytes. `node scripts/ci.mjs` completed every repository and four-plugin gate, including Rust format, clippy, source-built binary, Session Relay release contracts, self-tests, and JavaScript quality checks.
- The post-tag parent failure was reproduced before repair: the current completion path rejected the later PlanRun source under the stale release-source equality. The repaired binder now persists `source_commit` for immutable release-source-to-tag proof and `plan_run.source_base` for independent source-to-implementation proof.
- The source proof consumer no longer restores the false equality. Both current promotion and fresh-context rebind paths independently observe PlanRun-source-to-implementation ancestry.
- The explicit main publication remains a `push` boundary. `--publish-reviewed --rebind-complete-publication` remains a read-only `probe` boundary through its early return.
