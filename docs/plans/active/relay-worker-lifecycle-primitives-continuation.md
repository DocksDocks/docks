...
updated: "2026-07-14T13:52:31-03:00"
...
private signing keys survived. Their hash-bound snapshots and receipt chains
become closed historical evidence; product migration is instead re-proved from fresh,
...
2026-07-14 the owner selected bounded recovery: retain hash-bound snapshots and
receipt chains as historical evidence, state the lost liveness explicitly,
create distinct live proof generations, then authenticated-clean the dead
on-disk roots without weakening the product migration test.
...
The recovery reader is available only to the native bootstrap's
`--migration-recover-snapshots` mode when the exact fixture, fixture hash,
reviewed recovery plan blob, and three closed tuples match. It opens the old
roots and authority files read-only, performs no socket operation, and copies
only a byte-sorted payload manifest. One deterministic recovery id is
`sha256(JCS({schema:1,uid,restart_recovery_binding_sha256,
recovery_receipt_plan_blob,recovery_merge,implementation_head}))`. Its
pre-import root is the sole uid-owned mode-0700
...
`Prepared→CustodiansReady→PayloadsCopied→RootsVerified→Imported→
MigrationActive→PostParentVerified→HistoricalCleanupPrepared→
...
`SnapshotRecoveryInputV1 {schema:1,recovery_id,
restart_recovery_binding_sha256,recovery_generation_receipt_sha256s,
input_sha256}` closes the ordered three receipts. It changes only the frozen
`migration_id` preimage to schema 2:
`{schema:2,uid,plan_commit,plan_blob,implementation_head,protocol_sha256,
legacy_validator_identity_sha256,snapshot_recovery_input_sha256,
predecessors:[step-range/1,runner-receipt/1,runner-receipt/2]}`. The predecessor
rows contain the fresh exact root/authority/pin/source tuples. Existing
`RootMigrationAnchorV2`, phase receipts, generation records, ledger, Active,
schema-v3 authorities, transition semantics, negative matrices, runner layout,
A109 command, completion commands, and 82-event order remain byte-shaped as
frozen; their anchor/ledger chain transitively commits the recovery input through
the new migration id. Validators recompute and substitution-test that path.

Before the migration anchor, the process holds both coordinator locks, writes
and fsyncs migration-coordinator-relative `RecoveryImportIntentV1 {schema:1,
recovery_id,migration_id,snapshot_recovery_input_sha256,source_parent,
source_basename,source_identity,destination_basename:"snapshot-recovery",
intent_sha256}`, then no-replace-renames the entire pre-import root to
`/tmp/relay-root-migration.<migration_id>/snapshot-recovery` and fsyncs both
parents. It writes the `Imported` phase under that destination, then writes
`RecoveryImportReceiptV1 {schema:1,recovery_id,migration_id,
snapshot_recovery_input_sha256,import_intent_sha256,
roots_verified_phase_sha256,imported_phase_sha256,imported_identity,
source_absence_sha256,source_parent_fsynced:true,
destination_parent_fsynced:true,receipt_sha256}` as
`recovery-import.json`. The nested `.lock` becomes immutable evidence; all later
recovery writes hold the migration coordinator's unchanged `.lock`.

This continuation extends the frozen migration coordinator filename set only
with `recovery-import-intent.json`, `recovery-import.json`, and
`snapshot-recovery/`. Pre-anchor retry accepts only the exact intent-only,
intent-plus-imported-directory, or completed-receipt state and adopts the
rename-after-effect case by identity; both source and destination, neither after
intent, a mismatched directory, or an unjournaled child is STOP. The imported
tree and both import records are included in `CoordinatorSnapshotV1` and the
`A138RecoveryBundleV1`; frozen `CoordinatorCleanup` therefore removes them with
...
`runner_job_custodian --migration-clean-closed-historical --recovery-id <id>
--migration-id <id> --post-parent-receipt-sha256 <hex>` accepts no path, holds
the migration coordinator lock, and writes `HistoricalCleanupPrepared` binding
retained `/tmp` fd identities and three exact quarantine destinations. Each ordered
`ClosedHistoricalGenerationCleanupV1` reuses the frozen fd-relative A138
generation-cleanup shapes from `Bound` through `Complete` but omits only
`ShutdownIntent` and `CustodianClosed`; `Bound` requires the recorded
PID/start/boot generation absent, and no socket or signal operation is allowed.
A reused PID is never touched. Intent precedes each
no-replace quarantine, payload removal, sentinel removal, root removal, and
authority-file removal, with parent fsync after every rename/removal.
`ClosedHistoricalCleanupReceiptV1 {schema:1,recovery_id,
post_parent_verification_receipt_sha256,generation_completion_sha256s,
root_absence_sha256s,authority_absence_sha256s,receipt_sha256}` closes all three
in fixed order and produces `HistoricalCleanupComplete`. The old-root write STOP
permits only this authenticated cleanup. A137/A138 then see exactly the five
...
`FinalExecutionEvidenceV1` shape while binding recovery. Continuation completion
binds the envelope hash in both `ContinuationScopeV1` and
`LifecycleCompletionEvidenceV1`; drop/substitute/replay negatives cover every
new hash.
...
false historical-liveness guarantees, the recovery input/generation/import
receipts, post-parent restart receipt, authenticated old-root cleanup, its own
...
- The dead schema-v2 roots are read-only only through verified copy and
  post-parent proof, never current authorities. Authenticated historical cleanup
  then removes their exact paths; only committed hashes/manifests remain. Only
  distinct fresh generations enter the migration ledger, and copied payload
...
**Rejected recovery-candidate review (2026-07-14T13:13:56-03:00):** both sealed
legs rejected `C0=82074b5f7ff1282c44cefa41c31582facafd8fa3`. The orchestrator reproduced and
accepted all findings: a self-referential checkpoint field, underclosed restart
journaling, an unsupported migration-ledger binding claim, an undefined
post-parent receipt, missing disposition for the three dead roots, and an
abbreviated binary release handoff. The consolidated repair removes the cycle,
defines one phase-journaled coordinator with deterministic locators and
interruption behavior, imports it into the native migration coordinator so A138
...
**Final recovery review receipt (2026-07-14T13:52:31-03:00):** the fresh S leg
returned READY 100 with no findings after verifying the sealed bundle and all
five repairs. The cross-company X leg remained `platform_denied` by host policy
before process start; no alternate transport was attempted. The canonical
schema-v1 outcome is `single`, pre-execution eligible, and bound to
`C=0716592cae10bc030bb88a449f84d5f38c669c89` with input SHA-256
`2258c9e633d531d46214892b207cf67f06883016b01ff9e0a62bdb7f1f61e191`.
This receipt-only attribution ends draft review and does not trigger CI or
another review.
...
- **2026-07-14T13:13:56-03:00**: The first bounded-recovery candidate described
  fresh generations but did not close its own restart journal, old-root
  disposition, post-parent receipt, or binary release sequence → both sealed
...
them and every finding was reproduced. The consolidated repair
`C=0716592cae10bc030bb88a449f84d5f38c669c89` is READY under the fresh sealed
single-leg review below. This receipt-only child is `B2`; implementation resumes
from it without another draft review or full CI.

Cross-check (2026-07-14): [X: anthropic fable high; result=platform_denied] no process started because host workspace-export policy denied the launch; no alternate transport attempted; [S: openai gpt-5.6-sol xhigh; result=passed] READY 100, 0 findings; [orchestrator: openai codex gpt-5.6-sol xhigh] independently validated the reviewer JSON, sealed request, bundle hash, and findings-free single-leg eligibility.

Review-receipt: {"S":{"raw":{"attempts":[{"child_id":"/root/relay_recovery_final2_s","denial_source":null,"effort":"xhigh","exit_code":0,"model":"gpt-5.6-sol","output_started":true,"reason":"Fresh read-only reviewer returned a schema-valid READY result","result":"passed","retry_cause":null,"schema":1,"signal":null,"started":true,"stderr_sha256":"e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855","stdout_sha256":"331be332be0a5663988fa2f7fbb37789e94a18298c38d9cdee8b6319a1db740a","timeout_mode":"orchestrator_tool","timeout_seconds":600,"transport":"in_session"}],"decision_evidence":null,"findings":[],"findings_sha256":"4f53cda18c2baa0c0354bb5f9a3ecbe5ed12ab4d8e11ba873c2f11161202b945","leg":"S","reason":null,"request":{"acceptance_inventory_sha256":null,"author":{"company":"openai","effort":"xhigh","model":"gpt-5.6-sol","tool":"codex"},"bundle_sha256":"0849620a69da5d49566616c5bf531d802c5688667e0993d313155dbe6d495e81","diff_sha256":null,"execution_base_commit":null,"input_sha256":"2258c9e633d531d46214892b207cf67f06883016b01ff9e0a62bdb7f1f61e191","lifecycle_intent":"none","phase":"draft","planned_at_commit":null,"policy":{"anthropic_tiers":[{"effort":"high","model":"fable","transports":["in_session","cli"]},{"effort":"max","model":"opus","transports":["in_session","cli"]}],"cross_company_consent":"always","openai_tiers":[{"effort":"xhigh","model":"gpt-5.6-sol","transports":["in_session","cli"]}],"orchestrator_preference":"auto","provenance":{"anthropic_tiers":"skill_default","cross_company_consent":"current_user","openai_tiers":"skill_default","orchestrator_preference":"skill_default","zero_reviewer_policy":"skill_default"},"schema":1,"zero_reviewer_policy":"ask"},"policy_sha256":"3c95e5e16578dbf7b675ed6324de50e5cd749744239d2b2bea45603cf766bee1","request_id":"58013c57-a86c-4f08-adb3-d6cd17d50481","reviewed_commit_or_head":"0716592cae10bc030bb88a449f84d5f38c669c89","schema":1},"result":"passed","reviewer_output":{"confirmations":["manifest.json covers every bundled file other than itself; every listed file SHA-256 matches, and every requested absent path is absent.","Per-generation recovery is closed by deterministic fixed paths, append-only attempt phase chains, intent-before-effect and receipt-after-effect ordering, adopt-or-clean retry rules, authenticated cleanup branches, and an interruption matrix spanning coordinator, process, filesystem, import, and fsync boundaries.","The migration parent now writes and fsyncs its own launch and terminal evidence, omits unknowable wait status, exits after releasing the migration lock, and permits post-parent verification only from the exact terminal receipt plus PID/start/boot absence proof.","The Step-1b mutation authority is narrowly composed from the frozen K1 allowlist identities: it adds exactly the restart-recovery fixture, limits the already-authorized wip-snapshot.mjs surface to three named recovery operations, rejects all other deltas, and is consumable only by the superseding Step-1b range descending K2.","Step 6 integrates source and imports completion evidence before committing the in_review transition; A101-A138 and the single full repository CI run only afterward in the disposable completion checkout.","The release handoff is remote-first: it records the exact source commit and workflow blob, pushes without force, requires origin/main equality before dispatch, selects a workflow_dispatch run at that commit, binds run head and workflow blob into the artifact manifest, and verifies the later binary commit remotely before release.","The dependency order, single-writer lifecycle ownership, recovery coordinator handoff, historical-root cleanup, STOP conditions, criterion inventory, and cold-handoff instructions are internally consistent and executable from the sealed plan and frozen predecessor."],"score":100,"structured_output_sha256":"d5fad16bdc001b59d3edce162608a8d54be130af8cdbe6b67d46cf2c3610bbeb","verdict":"ready"},"schema":1,"selected":{"effort":"xhigh","model":"gpt-5.6-sol","transport":"in_session"},"severity_totals":{"high":0,"low":0,"medium":0},"waiver":null,"waiver_sha256":null},"reconciliation":{"accepted":[],"rejected":[]},"request":{"acceptance_inventory_sha256":null,"author":{"company":"openai","effort":"xhigh","model":"gpt-5.6-sol","tool":"codex"},"bundle_sha256":"0849620a69da5d49566616c5bf531d802c5688667e0993d313155dbe6d495e81","diff_sha256":null,"execution_base_commit":null,"input_sha256":"2258c9e633d531d46214892b207cf67f06883016b01ff9e0a62bdb7f1f61e191","lifecycle_intent":"none","phase":"draft","planned_at_commit":null,"policy":{"anthropic_tiers":[{"effort":"high","model":"fable","transports":["in_session","cli"]},{"effort":"max","model":"opus","transports":["in_session","cli"]}],"cross_company_consent":"always","openai_tiers":[{"effort":"xhigh","model":"gpt-5.6-sol","transports":["in_session","cli"]}],"orchestrator_preference":"auto","provenance":{"anthropic_tiers":"skill_default","cross_company_consent":"current_user","openai_tiers":"skill_default","orchestrator_preference":"skill_default","zero_reviewer_policy":"skill_default"},"schema":1,"zero_reviewer_policy":"ask"},"policy_sha256":"3c95e5e16578dbf7b675ed6324de50e5cd749744239d2b2bea45603cf766bee1","request_id":"58013c57-a86c-4f08-adb3-d6cd17d50481","reviewed_commit_or_head":"0716592cae10bc030bb88a449f84d5f38c669c89","schema":1}},"X":{"raw":{"attempts":[{"child_id":null,"denial_source":"managed_policy","effort":"high","exit_code":null,"model":"fable","output_started":false,"reason":"Host workspace-export policy denied the cross-company CLI launch before process start","result":"platform_denied","retry_cause":null,"schema":1,"signal":null,"started":false,"stderr_sha256":null,"stdout_sha256":null,"timeout_mode":null,"timeout_seconds":600,"transport":"cli"}],"decision_evidence":null,"findings":[],"findings_sha256":null,"leg":"X","reason":"Host workspace-export policy denied exporting the sealed private workspace bundle; no reviewer process was launched and no alternate transport was attempted.","request":{"acceptance_inventory_sha256":null,"author":{"company":"openai","effort":"xhigh","model":"gpt-5.6-sol","tool":"codex"},"bundle_sha256":"0849620a69da5d49566616c5bf531d802c5688667e0993d313155dbe6d495e81","diff_sha256":null,"execution_base_commit":null,"input_sha256":"2258c9e633d531d46214892b207cf67f06883016b01ff9e0a62bdb7f1f61e191","lifecycle_intent":"none","phase":"draft","planned_at_commit":null,"policy":{"anthropic_tiers":[{"effort":"high","model":"fable","transports":["in_session","cli"]},{"effort":"max","model":"opus","transports":["in_session","cli"]}],"cross_company_consent":"always","openai_tiers":[{"effort":"xhigh","model":"gpt-5.6-sol","transports":["in_session","cli"]}],"orchestrator_preference":"auto","provenance":{"anthropic_tiers":"skill_default","cross_company_consent":"current_user","openai_tiers":"skill_default","orchestrator_preference":"skill_default","zero_reviewer_policy":"skill_default"},"schema":1,"zero_reviewer_policy":"ask"},"policy_sha256":"3c95e5e16578dbf7b675ed6324de50e5cd749744239d2b2bea45603cf766bee1","request_id":"58013c57-a86c-4f08-adb3-d6cd17d50481","reviewed_commit_or_head":"0716592cae10bc030bb88a449f84d5f38c669c89","schema":1},"result":"platform_denied","reviewer_output":null,"schema":1,"selected":null,"severity_totals":{"high":0,"low":0,"medium":0},"waiver":null,"waiver_sha256":null},"reconciliation":{"accepted":[],"rejected":[]},"request":{"acceptance_inventory_sha256":null,"author":{"company":"openai","effort":"xhigh","model":"gpt-5.6-sol","tool":"codex"},"bundle_sha256":"0849620a69da5d49566616c5bf531d802c5688667e0993d313155dbe6d495e81","diff_sha256":null,"execution_base_commit":null,"input_sha256":"2258c9e633d531d46214892b207cf67f06883016b01ff9e0a62bdb7f1f61e191","lifecycle_intent":"none","phase":"draft","planned_at_commit":null,"policy":{"anthropic_tiers":[{"effort":"high","model":"fable","transports":["in_session","cli"]},{"effort":"max","model":"opus","transports":["in_session","cli"]}],"cross_company_consent":"always","openai_tiers":[{"effort":"xhigh","model":"gpt-5.6-sol","transports":["in_session","cli"]}],"orchestrator_preference":"auto","provenance":{"anthropic_tiers":"skill_default","cross_company_consent":"current_user","openai_tiers":"skill_default","orchestrator_preference":"skill_default","zero_reviewer_policy":"skill_default"},"schema":1,"zero_reviewer_policy":"ask"},"policy_sha256":"3c95e5e16578dbf7b675ed6324de50e5cd749744239d2b2bea45603cf766bee1","request_id":"58013c57-a86c-4f08-adb3-d6cd17d50481","reviewed_commit_or_head":"0716592cae10bc030bb88a449f84d5f38c669c89","schema":1}},"author":{"company":"openai","effort":"xhigh","model":"gpt-5.6-sol","tool":"codex"},"decision_evidence":null,"input_sha256":"2258c9e633d531d46214892b207cf67f06883016b01ff9e0a62bdb7f1f61e191","outcome":"single","phase":"draft","policy":{"anthropic_tiers":[{"effort":"high","model":"fable","transports":["in_session","cli"]},{"effort":"max","model":"opus","transports":["in_session","cli"]}],"cross_company_consent":"always","openai_tiers":[{"effort":"xhigh","model":"gpt-5.6-sol","transports":["in_session","cli"]}],"orchestrator_preference":"auto","provenance":{"anthropic_tiers":"skill_default","cross_company_consent":"current_user","openai_tiers":"skill_default","orchestrator_preference":"skill_default","zero_reviewer_policy":"skill_default"},"schema":1,"zero_reviewer_policy":"ask"},"policy_sha256":"3c95e5e16578dbf7b675ed6324de50e5cd749744239d2b2bea45603cf766bee1","pre_execution_eligible":true,"reproduced":[],"request":{"acceptance_inventory_sha256":null,"author":{"company":"openai","effort":"xhigh","model":"gpt-5.6-sol","tool":"codex"},"bundle_sha256":"0849620a69da5d49566616c5bf531d802c5688667e0993d313155dbe6d495e81","diff_sha256":null,"execution_base_commit":null,"input_sha256":"2258c9e633d531d46214892b207cf67f06883016b01ff9e0a62bdb7f1f61e191","lifecycle_intent":"none","phase":"draft","planned_at_commit":null,"policy":{"anthropic_tiers":[{"effort":"high","model":"fable","transports":["in_session","cli"]},{"effort":"max","model":"opus","transports":["in_session","cli"]}],"cross_company_consent":"always","openai_tiers":[{"effort":"xhigh","model":"gpt-5.6-sol","transports":["in_session","cli"]}],"orchestrator_preference":"auto","provenance":{"anthropic_tiers":"skill_default","cross_company_consent":"current_user","openai_tiers":"skill_default","orchestrator_preference":"skill_default","zero_reviewer_policy":"skill_default"},"schema":1,"zero_reviewer_policy":"ask"},"policy_sha256":"3c95e5e16578dbf7b675ed6324de50e5cd749744239d2b2bea45603cf766bee1","request_id":"58013c57-a86c-4f08-adb3-d6cd17d50481","reviewed_commit_or_head":"0716592cae10bc030bb88a449f84d5f38c669c89","schema":1},"reviewed_at":"2026-07-14T13:52:31-03:00","reviewed_commit":"0716592cae10bc030bb88a449f84d5f38c669c89","schema":1}
