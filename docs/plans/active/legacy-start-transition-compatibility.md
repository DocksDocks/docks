...
updated: "2026-07-13T16:08:09-03:00"
...
|---|---|---|---|---|---|
| 1 | Implement strict-first compatibility application/material/receipt/binding/prerequisite validation, exact `CompletionReviewBlockV1` plus completion-stable view, history scope validation, and the five public read-only helper commands. Preserve the existing strict schema-1 return and exact error order outside the closed legacy-shape predicate. | `plugins/docks/skills/productivity/plan-review/scripts/review-policy.mjs:1-6,594-628,925-957` (current imports, Git/execution-range helpers, and CLI dispatch; place the new compatibility constructors/validators beside the execution-range boundary) | — | done | Direct A3–A4 probes pass. The historical legacy shape reaches the typed missing-evidence error without E/R/B/Q/F; full-identity drift and every other ordinary strict case retain original bytes; the exported prerequisite constructor and closed production/fake dependency seam above are exact; prerequisite observations and Review rendering are closed; helper outputs are read-only. |
| 2 | Add full positive and regression coverage for creation ancestry, start/history, exact diff material, owner evidence, constructor-bound remote/release/plugin/cache observations, release/archive prerequisite receipt, E/R/B/Q/F adjacency and attribution, findings-free `dual|single` eligibility, special-character-safe `CompletionReviewBlockV1`, strict/compatibility H→C reuse, exact 23-case differential behavior, and per-commit scope. | `scripts/tests/fixtures/plan-review-policy/sample-plan.md:1-50`; `scripts/tests/plan-review-policy.mjs:1-129,382-424,517-529`; `scripts/tests/plan-review-policy-regressions.mjs:1-180` | 1 | done | A1, A2, A5, and A6 pass against the real exported helpers and commit chains; the positive fixture calls `buildDocksCompatibilityPrerequisiteApplication` with raw-result dependencies, while public-CLI negatives reject dependency/time/path/observation injection; no test fabricates a final boolean, hand-authors observations, invents Q/Review bytes, or accepts a missing/reordered corpus case. |
...
| A2 | `node scripts/tests/plan-review-policy-regressions.mjs` | Exit 0; every creation/partition/material/E-R-B-Q-F/attribution/review/reuse/scope regression family is detected; prerequisite coverage rejects missing/extra constructor input/dependency keys and swapped values while accepting reordered closed-object keys with canonical-identical results; negatives cover malformed child result/error/signal/status/Buffer fields, wrong/missing child cwd, nonempty unrecorded-child stderr, stale or partially recomputed recorded-child `stderr_sha256` substitutions, missing/extra observation fields while accepting reordered observation-object keys with canonical-identical results, ambient-`origin`, repository-local `url.*.insteadOf`, and inherited `GIT_CONFIG_COUNT` substitutions, main/tag two-pattern argv/exit/stdout/projection/time/self-hash changes, lightweight/annotated remote-tag positives plus wrong/missing/extra/reordered/unpeeled/foreign remote tag rows, duplicate/missing plugin selection, release draft/prerelease/tag/URL, cache path/symlink/realpath/file-kind/hash, field/hash/authorization/archive/plugin-isolation/E-R-B-binding/marker/fence/JCS/LF/P-row/Q-delta substitutions, caller-supplied observations, and every extra CLI attempt to select dependencies or supply time/path/raw/parsed observation state; operational dependency-call, argv, and remote-output row ordering remains exact and regressions are detected; an explicit fully rehashed opaque-digest case documents that trusted plan-manager constructor provenance plus immutable Q/F commits—not self-authentication—is the boundary; Review-renderer negatives cover every projected field/order, an exact rejected-order golden, reproduced filtering, X-result inclusion, special characters, heading/label/punctuation/prose/receipt/CRLF/separator/LF changes, EOF/following-section splice, idempotence, and unchanged receipt keys; strict near misses preserve bytes and semantic assertions cover exact errors/closed objects. |
