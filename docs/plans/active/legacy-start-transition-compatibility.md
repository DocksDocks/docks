...
updated: "2026-07-13T23:31:06-03:00"
...
- Host setup: enable Corepack before completion checkout preparation. Inside each
  freshly prepared detached checkout, run `pnpm install --frozen-lockfile`
  before A1, then run A1–A5 and `node scripts/ci.mjs`. Setup is preparation,
  not acceptance evidence; a setup failure stops completion before any receipt.
  Never copy or symlink `node_modules` from the source checkout.
