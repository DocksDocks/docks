...
updated: "2026-07-13T09:08:40-03:00"
started_at: null
assignee: null
...
| A2 | `node scripts/tests/plan-review-policy-mutations.mjs` | Exit 0; every creation/partition/material/E-R-B-Q-F/attribution/review/reuse/scope mutation family is rejected; prerequisite negatives cover missing/extra/reordered observation fields, ambient-`origin` substitution, main/tag argv/exit/stdout/stderr/projection/time/self-hash changes, lightweight/annotated remote-tag positives plus wrong/missing/extra/reordered/unpeeled/foreign remote tag rows, duplicate/missing plugin selection, release draft/prerelease/tag/URL, cache path/symlink/hash, field/hash/authorization/archive/plugin-isolation/E-R-B-binding/marker/fence/JCS/LF/P-row/Q-delta substitutions, and caller-supplied observations; Review-renderer negatives cover every projected field/order, an exact rejected-order golden, reproduced filtering, X-result inclusion, hostile strings, heading/label/punctuation/prose/receipt/CRLF/separator/LF mutation, EOF/following-section splice, idempotence, and unchanged receipt keys; strict near misses preserve bytes and semantic assertions cover exact errors/closed objects. |
...
| A5 | `node scripts/tests/plan-review-policy.mjs --case execution-compatibility` | Exit 0 and print `execution compatibility: strict-first evidence/review/binding/prerequisite/final-review and reuse passed`; the disposable positive invokes the real prerequisite constructor through deterministic direct-argv child suppliers, capturing typed canonical-remote main/tag, GitHub Release, Codex/Claude plugin, source, and cache observations; it proves the remote tag peels to the direct-child Docks patch release, equal policy hashes, contiguous E/R/B, exact fenced Q/P-row replacement, and findings-free F; `LegacyExecutionRangeValidationV1` names exact Q/receipt hash; ordinary strict output and both H→C modes remain exact. |
...
3. **Interface & data contracts:** present — policy JCS/hash, identity domains, application/material/receipt/binding/prerequisite-observation records, E/R/B/Q/F protocol, exact attribution/reuse preimages, unchanged strict schema, and five CLI arities are closed.
