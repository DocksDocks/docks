...
updated: "2026-07-13T21:22:04-03:00"
...
| A2 | `node scripts/tests/plan-review-policy-regressions.mjs` | Exit 0; every creation/partition/material/E-R-B-Q-F/attribution/review/reuse/scope regression family is detected; prerequisite coverage rejects missing/extra constructor input/dependency keys and swapped values while accepting reordered closed-object keys with canonical-identical results; negatives cover malformed child result/error/signal/status/Buffer fields, wrong/missing child cwd, nonempty unrecorded-child stderr, stale or partially recomputed recorded-child `stderr_sha256` substitutions, missing/extra observation fields while accepting reordered closed-object keys with canonical-identical results, ambient-`origin`, repository-local `url.*.insteadOf`, inherited `GIT_CONFIG_COUNT`, private copied-artifact isolation, the `GIT_ATTR_NOSYSTEM=1` child invariant, real repository-local/committed/global binary, `-diff`, or named-driver Git-attribute substitutions, independently dropped owner id, plan path, planned commit, execution-base commit, or stored scope digest, scoped-owner replay against another exact-shape plan/history, and execution-scope self-broadening, plus main/tag two-pattern argv/exit/stdout/projection/time/self-hash changes, lightweight/annotated remote-tag positives and wrong/missing/extra/reordered/unpeeled/foreign remote tag rows, duplicate/missing plugin selection, release draft/prerelease/tag/URL, cache path/symlink/realpath/file-kind/hash, field/hash/authorization/archive/plugin-isolation/E-R-B-binding/marker/fence/JCS/LF/P-row/Q-delta substitutions, caller-supplied observations, and every extra CLI attempt to select dependencies or supply time/path/raw/parsed observation state; operational dependency-call, argv, and remote-output row ordering remains exact and regressions are detected; an explicit fully rehashed opaque-digest case documents that trusted plan-manager constructor provenance plus immutable Q/F commits—not self-authentication—is the boundary; Review-renderer negatives cover every projected field/order, an exact rejected-order golden, reproduced filtering, X-result inclusion, special characters, heading/label/punctuation/prose/receipt/CRLF/separator/LF changes, EOF/following-section splice, idempotence, and unchanged receipt keys; strict near misses preserve bytes and semantic assertions cover exact errors/closed objects. The driver seals one source snapshot, runs one full baseline, defaults to `max(1,min(4,os.availableParallelism()))` child jobs with a validated `--jobs <N>` override where `1 <= N <= os.availableParallelism()`, buffers and reconciles results in declaration order, and runs each of the closed 54 mutations exactly once. Every invocation owns a validated random-UUID root namespace; SIGINT/SIGTERM stops admission, terminates and awaits active child groups with bounded escalation, cleans only that namespace, and re-raises the original signal. The fast scheduler test proves the cap/order/exactly-once rules, nested interruption cleanup, and namespace closure; two concurrent scheduler processes must both pass without observing or removing each other's roots. The final record is exactly `plan-review-policy regressions passed`. |
...
The regression-driver performance audit also found that interrupted mutation runs leaked private fixtures and serialized fifty independent child processes on one CPU. This revision gives each run one immutable source snapshot and per-invocation UUID-namespaced owned roots, routes every child fixture through a private temp directory, runs a bounded asynchronous pool sized from `os.availableParallelism()` and capped at four, retargets broad mutations to exact existing selectors, performs the full baseline only once, and buffers all outcomes for declaration-order reconciliation. A pre-follow-up default four-job run detected the then-50 mutations, printed the exact final record, left zero owned roots, and completed in 214.23 seconds; the final inventory is 54 after independent owner-field mutations. A fresh stop-path review then reproduced two roots left by SIGINT, and a parallel jobs-1/jobs-4 probe exposed broad-prefix cross-invocation cleanup. The final driver tracks children and roots, preserves SIGINT/SIGTERM after bounded group shutdown, and restricts enumeration/removal to one validated random namespace; two concurrent scheduler self-tests now pass with zero shared residue. No serial speedup ratio is claimed because a redundant full serial benchmark was deliberately skipped. Coverage, deterministic output, namespace isolation, and broad pre-commit CI remain mandatory.
