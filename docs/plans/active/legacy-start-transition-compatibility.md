...
updated: "2026-07-14T00:04:17-03:00"
...
- Host setup: enable Corepack before completion checkout preparation. Inside each
  freshly prepared detached checkout, run
  `corepack pnpm install --frozen-lockfile --store-dir .git/.docks-pnpm-store`
  before A1, then run A1–A5 and `node scripts/ci.mjs`. The clone-private store is
  writable under the completion sandbox, invisible to worktree status, and
  deleted with the helper-owned checkout. Setup is preparation, not acceptance
  evidence; a setup failure stops completion before any receipt. Never copy or
  symlink `node_modules` from the source checkout or use the ambient shared
  pnpm store.
...
`prepareCompletionCheckout` stores its compact-JCS cleanup marker only at
`<checkout>/.git/.docks-plan-verify-sentinel`, never in the worktree. The
checkout and Git directory must be canonical owned real directories, and the
marker must be a mode-`0600` regular non-symlink file. The existing request id,
random cleanup token, original repository snapshot, reviewed head/source tree,
plan path, and planned/execution identities remain exact. A newly prepared
checkout therefore has empty `git status --porcelain`; cleanup revalidates the
same marker bytes and all existing identities before deleting only the owned
request directory. No prepared-result or completion-receipt schema changes.
...
The first isolated completion attempt returned S 82/100 NOT READY with one high
finding, independently reproduced by the exact A5 run. The helper-created
worktree marker made `git status --porcelain` nonempty before acceptance, while
the detached clone also lacked the repository dependencies required by CI.
This repair moves the unchanged cleanup identity into canonical private Git
metadata, adds type/mode/symlink negatives, and makes plan-documented dependency
setup an explicit main-context pre-verification step. The failed request,
bundle, and primary ledger are stale and cannot authorize completion.

The checkout repair at `72ba713` passed independent syntax, lifecycle,
surface-parity, content-hash, context-tree, diff, complete mutation, and full
repository gates. A fresh read-only code review returned READY with no findings
and confirmed the canonical directory, marker type/mode/ownership, closed-schema,
bounded cleanup, clean-worktree, and package-manager-neutral setup invariants.
The formal completion run must still seal the current plan-only closure commit,
install dependencies inside its detached checkout, and produce new evidence.

The next completion setup stopped before A1 because unqualified pnpm attempted
to open its ambient shared-store database outside the writable checkout. The
pinned pnpm help confirms `--store-dir <dir>`, and a disposable remediation
probe installed the exact lockfile successfully when the store was redirected.
This revision binds setup to `.git/.docks-pnpm-store`, inside the owned clone and
its cleanup boundary. The stopped request and its no-conclusion reviewer output
are stale and cannot authorize completion.

A fresh worker verified the exact revised setup at `29cdb8e` in an owned
detached clone: installation exited 0 in 3.1 seconds, the clone-private store and
`node_modules` were initially absent then created inside the clone, all four
dependency symlinks resolved outside the source checkout, and porcelain stayed
empty. Exact A5 passed with the sealed allowed-path digest over 21 commits, and
the worker deleted only its owned clone. Formal completion still reruns the
current sealed input and full CI once.
...
2. **Environment & commands:** present — Node 24, prepared-checkout dependency setup, non-overlapping focused probes, one full CI gate, historical probe, exact diff argv, and source-scope verification are literal.
...
- **2026-07-13T23:31:06-03:00**: Writing the cleanup marker into the prepared worktree made the exact clean-scope acceptance impossible, and cloning committed bytes alone omitted Docks' installed packages → keep the marker in validated private Git metadata and run the plan-documented setup inside the clone before acceptance and CI.
- **2026-07-14T00:00:38-03:00**: Unqualified pnpm setup tried to write its ambient shared-store database outside the completion sandbox → pin `--store-dir .git/.docks-pnpm-store` so installation, status isolation, and cleanup remain inside the owned checkout.
