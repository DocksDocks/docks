...
updated: "2026-07-13T07:28:28-03:00"
started_at: null
assignee: null
...
The first two records and diff fence form one application block inserted immediately before `## Review`. None of the three compatibility record names is added to `MACHINE_RECORD`; only the ordinary `Review-receipt:` remains stripped. The diff fence uses the smallest backtick length `N >= 3` greater than every backtick run in the exact diff, opening with exactly `N` backticks plus `diff` and closing with exactly `N` backticks.

Historical transition bytes are exactly stdout bytes from:

```bash
git diff --binary --full-index --no-ext-diff --no-textconv --no-color --src-prefix=a/ --dst-prefix=b/ "$EXECUTION_PARENT" "$EXECUTION_BASE_COMMIT" -- "$PLAN_PATH"
```

It must exit 0, emit valid UTF-8 normalized to LF, end in one LF, and have SHA-256 `transition_diff_sha256`. No shell, pager, textconv, external diff, contextual relabeling, or synthetic section diff participates.
...
`validateExecutionRange` preserves the strict path's current error ordering and exact schema-1 return bytes. Only after the shared ancestry/single-parent/plan-only/status invariants pass may the two legacy-specific strict checks—canonical start equality and exact full planned-base value at start—fall through to compatibility. All other strict errors propagate unchanged.
...
| A2 | `node scripts/tests/plan-review-policy-mutations.mjs` | Exit 0; every creation/partition/material/E-R-B-Q-F/attribution/review/reuse/scope mutation family is rejected and semantic-output assertions cover exact errors and closed objects. |
...
Score: **99/100** · trajectory **84→93→98→99→exact-byte NOT READY→99** · stopped: **plateau (K=3)**. The first passes exposed opaque historical evidence, ambiguous preimages, incomplete partitioning, and a non-adjacent chain. Fresh reviewers then caught mandatory attribution deltas, completion Review-block replacement, missing binding construction, typed-outcome ambiguity, an underclosed strict corpus, endpoint-only scope, the unreviewed binding-bearing plan, a free-form attribution preimage, and a union-only scope hash. This draft closes E/R/B/Q/F, a production binding constructor, Review-partition-aware reuse, a digest-bound 23-case differential corpus, the literal receipt-only attribution renderer, and the per-commit scope ledger; one point remains for the inherent complexity of committed legacy-history validation.
