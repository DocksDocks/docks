...
updated: "2026-07-13T10:04:28-03:00"
...
```text
{
  ...
  remote_tag:{
    schema:1,
    argv:["git","ls-remote","--exit-code","--tags","https://github.com/DocksDocks/docks.git","refs/tags/${release_tag}"],
    exit_code:0,stdout_sha256:Sha256,stderr_sha256:Sha256,
...
|---|---|---|---|---|---|
| 1 | Implement strict-first compatibility application/material/receipt/binding/prerequisite validation, exact `CompletionReviewBlockV1` plus completion-stable view, history scope validation, and the five public read-only helper commands. Preserve the existing strict schema-1 return and exact error order outside the closed legacy-shape predicate. | `plugins/docks/skills/productivity/plan-review/scripts/review-policy.mjs:1-6,594-628,925-957` (current imports, Git/execution-range helpers, and CLI dispatch; place the new compatibility constructors/validators beside the execution-range boundary) | — | planned | Direct A3–A4 probes pass. The historical legacy shape reaches the typed missing-evidence error without E/R/B/Q/F; full-identity drift and every other ordinary strict case retain original bytes; the exported prerequisite constructor and closed production/fake dependency seam above are exact; prerequisite observations and Review rendering are closed; helper outputs are read-only. |
| 2 | Add full positive and mutation coverage for creation ancestry, start/history, exact diff material, owner evidence, constructor-bound remote/release/plugin/cache observations, release/archive prerequisite receipt, E/R/B/Q/F adjacency and attribution, findings-free `dual|single` eligibility, hostile-string-safe `CompletionReviewBlockV1`, strict/compatibility H→C reuse, exact 23-case differential behavior, and per-commit scope. | `scripts/tests/fixtures/plan-review-policy/sample-plan.md:1-50`; `scripts/tests/plan-review-policy.mjs:1-129,382-424,517-529`; `scripts/tests/plan-review-policy-mutations.mjs:1-180` | 1 | planned | A1, A2, A5, and A6 pass against the real exported helpers and commit chains; the positive fixture calls `buildDocksCompatibilityPrerequisiteApplication` with raw-result dependencies, while public-CLI negatives reject dependency/time/path/observation injection; no test fabricates a final boolean, hand-authors observations, invents Q/Review bytes, or accepts a missing/reordered corpus case. |
| 3 | Document the application/binding/prerequisite/final-review protocol, strict-first and completion-stable rules, unchanged closed schema-1 surfaces, exact typed review eligibility, source-ready boundary, and cross-tool ownership in source and shipped plan contracts. | `docs/plans/AGENTS.md:241-306,345-359`; `plugins/docks/skills/productivity/plan-init/references/plans-agents-md-template.md:219-269,293-306`; `plugins/docks/skills/productivity/plan-manager/SKILL.md:114-221`; `plugins/docks/skills/productivity/plan-review/SKILL.md:150-204` | 1-2 | planned | Source and shipped template are semantically aligned; plan-manager owns E/R/B/Q/F writes, plan-review remains evidence-only, only findings-free `dual|single` authorizes compatibility, no existing schema-1 receipt changes, and docks-kit remains refresh-only. |
| 4 | Run focused and full validation, verify committed plus worktree scope, and hand the source-ready plan to completion review. | Read-only: `docs/plans/AGENTS.md`; `plugins/docks/skills/productivity/plan-init/references/plans-agents-md-template.md`; `plugins/docks/skills/productivity/plan-manager/SKILL.md`; `plugins/docks/skills/productivity/plan-review/SKILL.md`; `plugins/docks/skills/productivity/plan-review/scripts/review-policy.mjs`; `scripts/tests/fixtures/plan-review-policy/sample-plan.md`; `scripts/tests/plan-review-policy.mjs`; `scripts/tests/plan-review-policy-mutations.mjs` | 1-3 | planned | A1–A9 pass, all Steps are `done`, and plan-manager moves this plan to `in_review` without changing plugin versions or claiming downstream activation. |
...
| A2 | `node scripts/tests/plan-review-policy-mutations.mjs` | Exit 0; every creation/partition/material/E-R-B-Q-F/attribution/review/reuse/scope mutation family is rejected; prerequisite negatives cover missing/extra/reordered constructor input/dependency keys, malformed child result/error/signal/status/Buffer fields, wrong/missing child cwd, nonempty unrecorded-child stderr, recorded-child stored `stderr_sha256` substitution, missing/extra/reordered observation fields, ambient-`origin` substitution, main/tag argv/exit/stdout/projection/time/self-hash changes, lightweight/annotated remote-tag positives plus wrong/missing/extra/reordered/unpeeled/foreign remote tag rows, duplicate/missing plugin selection, release draft/prerelease/tag/URL, cache path/symlink/realpath/file-kind/hash, field/hash/authorization/archive/plugin-isolation/E-R-B-binding/marker/fence/JCS/LF/P-row/Q-delta substitutions, caller-supplied observations, and every extra CLI attempt to select dependencies or supply time/path/raw/parsed observation state; Review-renderer negatives cover every projected field/order, an exact rejected-order golden, reproduced filtering, X-result inclusion, hostile strings, heading/label/punctuation/prose/receipt/CRLF/separator/LF mutation, EOF/following-section splice, idempotence, and unchanged receipt keys; strict near misses preserve bytes and semantic assertions cover exact errors/closed objects. |
...
| A5 | `node scripts/tests/plan-review-policy.mjs --case execution-compatibility` | Exit 0 and print `execution compatibility: strict-first evidence/review/binding/prerequisite/final-review and reuse passed`; the disposable positive imports the real `buildDocksCompatibilityPrerequisiteApplication` and supplies only the closed raw-result dependency object above. A table-driven five-run matrix places deterministic nonempty stderr in exactly one of remote main, remote tag, GitHub Release, Codex plugin, or Claude plugin per run, requires success, and requires that row's derived `stderr_sha256` while the other four retain their own hashes. The constructor captures the typed source/cache observations, performs every decode/projection/hash, and proves the remote tag peels to the direct-child Docks patch release, equal policy hashes, contiguous E/R/B, exact fenced Q/P-row replacement, and findings-free F; `LegacyExecutionRangeValidationV1` names exact Q/receipt hash; ordinary strict output and both H→C modes remain exact. |
...
Score: **99/100** · trajectory **84→93→98→99→exact-byte NOT READY→99→formal X 82 NOT READY→99→formal S 72 NOT READY→99→formal S 84 NOT READY→99→formal S 91 NOT READY→99** · stopped: **plateau (K=3)**. The first passes exposed opaque historical evidence, ambiguous preimages, incomplete partitioning, and a non-adjacent chain. Fresh reviewers then caught attribution deltas, Review replacement, missing binding, typed-outcome ambiguity, underclosed strict/scope behavior, unreviewed binding bytes, free-form preimages, ambient diff configuration, undeclared serialization, stale machine-record wording, an unsealed Q receipt, renderer-free exact bytes, unbound external observations, object-sort ambiguity, an undefined prerequisite dependency seam, and imprecise Step locators. This draft closes E/R/B/Q/F, production evidence/binding/prerequisite constructors, exact Review rendering/reuse, the legacy predicate and differential corpus, deterministic Git/observation bytes, receipt-only attribution, and per-commit scope; one point remains for inherent legacy-history complexity.

## Cold-handoff checklist

1. **File manifest:** present — edited files carry current line-range locators and Step 4 enumerates the exact eight read-only verification inputs.
