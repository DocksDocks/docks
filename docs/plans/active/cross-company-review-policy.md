...
updated: "2026-07-11T15:51:21-03:00"
started_at: null
assignee: null
...
- **Leg result enum + ordered classifier:** `passed | waived | not_authorized | unavailable_auth | unavailable_model | timed_out | platform_denied | failed_unparseable | unavailable_unknown`. Classify in order: explicit per-plan waiver → `waived`; product consent denies X → `not_authorized`; host execution layer explicitly denies before child start → `platform_denied`; auth preflight fails → `unavailable_auth`; every ordered model returns explicit unknown/entitlement failure → `unavailable_model`; 600-second bound fires → `timed_out`; exit 0 but findings cannot be parsed → `failed_unparseable`; success + parse → `passed`; every other nonzero exit, signal, retry exhaustion, or ambiguous failure → `unavailable_unknown`. Retry at most once only for a transient launch/transport failure; never switch transport after policy/auth/model/waiver/unparseable outcomes.
...
- **Per-plan waivers (new frontmatter):** `review_waivers: [{ phase: draft | completion, input_sha256: <64hex>, legs: [X | S], actor: <string>, reason: <non-empty string>, at: <ISO datetime> }]`. plan-manager writes an entry only from an explicit current-user decision and includes it as `waived`; `cross_company_consent=never` is `not_authorized`, never a waiver. A waiver cannot cross phase or input hash.
- **Attempt + leg evidence:** `AttemptEvidence = { model, effort, transport: cli | relay, started: bool, result, exit_code, signal, reason, stdout_sha256, stderr_sha256 }`. `LegEvidence = { result, model, effort, findings: u32, severity: { high: u32, medium: u32, low: u32 }, accepted: [<id>], rejected: [{ id, reason }], reason, child_id, attempts: [AttemptEvidence], findings_sha256 }`, with unavailable values explicitly `null`. Attempt order proves tier fallback, the one transient retry, transport choice, and no retry after platform denial. This object is the X/S value in both receipt types.
...
- **Dispatch ownership protocol:** plan-manager is the sole lifecycle orchestrator and writer. It calls one plan-review runner exactly once with `{ phase, reviewed_commit, input_sha256, plan_path, author, policy }`; plan-review alone spawns/collects reviewer processes and returns `DraftRunResult = { X, S, findings }` or `CompletionRunResult = { X, S, findings, acceptance_evidence, ci: { command, exit_code, first_failure }, regressions: [{ severity, path, line, evidence }], goal_met: yes | partial | no, review_status: passed | partial | regressed, receipt_inputs: { planned_at_commit, reviewed_head, diff_sha256, plan_input_sha256 } }`, without editing. plan-manager reproduces/reconciles and alone writes `## Self-review` or `## Review` + `review_status`. No other workflow may launch a pair for the same phase/hash.
...
| 3 | Update plan-review as the sole process launcher/collector with fixed draft/completion rubrics, exact read-only CLI shapes, 600-second enforcement, tier/result classification, immutable inputs, parsing, per-finding reproduction, and typed `DraftRunResult`/`CompletionRunResult`. It returns evidence/results and never writes when called by plan-manager. Refresh hash. | `plugins/docks/skills/productivity/plan-review/SKILL.md:40-134` (draft/completion/cross-tool runners), `plugins/docks/skills/productivity/plan-review/SKILL.md:241-266` (anti-hallucination/success) | 1 | planned | A3/A4 prove Claude uses `--permission-mode plan`, Codex uses `-s read-only`, ambiguous failures stay unknown, attempt order is evidenced, and draft/completion keys/results are deterministic. Revert on any write-capable reviewer path or duplicate launcher. |
| 4 | Update plan-manager as sole lifecycle orchestrator/writer: commit immutable draft, capture author identity, resolve policy input, invoke the plan-review runner once, iterate changed drafts, persist phase/hash-bound waivers and receipts, reuse only matching receipts at `start`, apply zero-review policy, and write the typed completion verdict. Refresh hash. | `plugins/docks/skills/productivity/plan-manager/SKILL.md:42-70` (review policy), `plugins/docks/skills/productivity/plan-manager/SKILL.md:71-185` (workflow/new-plan/start/completion dispatch), `plugins/docks/skills/productivity/plan-manager/SKILL.md:186-207` (anti-hallucination/success) | 1,3 | planned | A4 simulations cover dual/single/zero, consent, waiver, changed-input rerun, exactly one runner call, and completion evidence→Review/status writing. Revert if plan-manager can advance before terminal outcomes or write a verdict without typed evidence. |
...
| A4 | Receipt/ownership/degradation behavior is executable. | `node scripts/tests/plan-review-policy.mjs --case lifecycle` | Exit 0; canonical hash is stable across waiver/receipt/status metadata but changes for goal/step/interface edits; both legs bind one immutable commit/hash; plan-manager calls the plan-review runner exactly once and alone writes; attempt ledgers prove tier/retry/transport behavior; evidence-rich dual/single/zero/not-authorized/phase-waiver outcomes pass; typed completion evidence produces Review/status; clean plan+affected reviewed-head remains valid across the plan-only receipt commit and stales on any plan/affected change. |
...
Score: **97/100** (Draft-6 candidate) · trajectory **86 claimed→55 verified (Draft-1)→88 claimed (Draft-2)→54 verified (Draft-3)→94 claimed/68 verified (Draft-4)→96 claimed/72 verified (Draft-5)→97 candidate (Draft-6)** · fresh X-leg re-review required before `start`.
