...
updated: "2026-07-11T14:58:00-03:00"
started_at: null
assignee: null
...
- **Two-reviewer attribution grammar (extends `docs/plans/AGENTS.md`), leg-namespaced IDs, reasons preserved:**
  ...
  ```
- **Availability-degradation matrix (strong default, never a hard block):** per-leg probe → attempt (600 s bound, ≤1 retry). Outcomes: both legs ok → dual review; only cross-company ok → record `S leg unavailable (<reason>)`, proceed; only same-company ok → record `X leg unavailable`, proceed (single-subscription path); neither ok / timeout / unparseable / budget-exhausted → record `both legs degraded (<reason>)`, proceed on local scored self-review; a big/risky plan with zero independent legs surfaces a picker ("proceed on self-review only?") rather than silently degrading. No outcome blocks a single-subscription user.
...
| A3 | Degradation + portability are explicit. | `grep -c "single-subscription\|degrad\|codex exec -s read-only\|claude -p" docs/plans/AGENTS.md` | ≥1 each: CLI baseline is normative, relay optional, single-subscription path defined. |
