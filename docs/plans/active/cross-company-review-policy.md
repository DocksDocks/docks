...
updated: "2026-07-11T15:45:10-03:00"
started_at: null
assignee: null
...
- **Author identity (new base plan frontmatter, captured at draft creation):** `review_author_company: openai | anthropic | unknown`, `review_author_tool: <string>`, `review_author_model: <string>`, `review_author_effort: <string>`. A legacy/unknown company triggers one picker before first review and is then persisted; X = other company, S = a second instance of the persisted company. This plan records the originating Claude session as `anthropic/claude/unknown/unknown`; gpt-5.6-sol is the later orchestrator, not the author.
...
- **Leg evidence object:** `{ result, model, effort, findings, severity, accepted, rejected, reason, child_id, child_started, exit_code, signal, stdout_sha256, stderr_sha256, findings_sha256 }`, with unavailable values explicitly `null`. This object is the X/S value in both receipt types, so exact failure evidence is not discarded.
- **Canonical draft receipt:** canonicalize UTF-8/LF plan content by removing mutable frontmatter keys (`status`, `updated`, `started_at`, `assignee`, `review_status`, `in_review_since`, `ship_commit`) and the complete `## Self-review`/`## Review` sections, then retain exactly one terminal newline and SHA-256 the bytes. Store one single-line JSON object inside `## Self-review`: `Review-receipt: {"schema":1,"phase":"draft","input_sha256":"<64hex>","reviewed_commit":"<40hex>","author":{"company":"...","tool":"...","model":"...","effort":"..."},"X":<leg-evidence>,"S":<leg-evidence>,"outcome":"dual|single|zero-degraded|blocked","reviewed_at":"<ISO>"}`. Receipt/self-review and lifecycle timestamps are excluded, so ingestion and `start` do not self-invalidate; any goal/context/interface/step/acceptance change does.
- **Dispatch ownership protocol:** plan-manager is the sole lifecycle orchestrator and writer. It calls one plan-review leg runner exactly once with `{ phase, reviewed_commit, input_sha256, plan_path, author, policy }`; plan-review alone spawns/collects the two reviewer processes and returns `{ X, S, findings }` without editing. plan-manager reproduces/reconciles and alone edits `## Self-review`. No other workflow may launch a pair for the same phase/hash.
- **Canonical completion receipt:** before review, require `git status --porcelain -- <affected_paths>` empty and capture `reviewed_head=$(git rev-parse HEAD)`, `diff_sha256=sha256(git diff --binary <planned_at_commit>..<reviewed_head> -- <affected_paths>)`, and the canonical plan input hash. Store single-line JSON inside `## Review`: `Completion-review-receipt: {"schema":1,"phase":"completion","planned_at_commit":"<sha>","reviewed_head":"<sha>","diff_sha256":"<hex>","plan_input_sha256":"<hex>","X":<leg-evidence>,"S":<leg-evidence>,"outcome":"dual|single|zero-degraded|blocked","reviewed_at":"<ISO>","review_commit":"<sha-or-null>"}`. The later plan-only review commit may advance HEAD without staling the receipt; ship-time reuse requires every commit after `reviewed_head` to change only this plan file and leave its canonical non-Review input unchanged. Any affected-path/worktree change re-runs completion. Completion review always runs by default; there is no subjective risk shortcut.
...
| 3 | Update plan-manager as sole lifecycle orchestrator/writer: commit immutable draft, capture author identity, resolve policy input, invoke the plan-review leg runner once, iterate changed drafts, persist phase/hash-bound waivers and receipts, reuse only matching receipts at `start`, and apply zero-review policy. Refresh hash. | `plugins/docks/skills/productivity/plan-manager/SKILL.md:42-70` (review policy), `plugins/docks/skills/productivity/plan-manager/SKILL.md:71-185` (workflow/new-plan/start/completion dispatch), `plugins/docks/skills/productivity/plan-manager/SKILL.md:186-207` (anti-hallucination/success) | 1,4 | planned | A4 simulations cover dual/single/zero, consent, waiver, changed-input rerun, and exactly one leg-runner call. Revert if plan-manager can advance before both terminal outcomes arrive. |
| 4 | Update plan-review as the sole process launcher/collector with fixed draft/completion rubrics, exact read-only CLI shapes, 600-second enforcement, tier/result classification, immutable inputs, parsing, and per-finding reproduction. It returns evidence/results and never writes when called by plan-manager. Refresh hash. | `plugins/docks/skills/productivity/plan-review/SKILL.md:40-134` (draft/completion/cross-tool runners), `plugins/docks/skills/productivity/plan-review/SKILL.md:241-266` (anti-hallucination/success) | 1 | planned | A3/A4 prove Claude uses `--permission-mode plan`, Codex uses `-s read-only`, ambiguous failures stay unknown, and draft/completion keys are deterministic. Revert on any write-capable reviewer path or duplicate launcher. |
...
| A4 | Receipt/ownership/degradation behavior is executable. | `node scripts/tests/plan-review-policy.mjs --case lifecycle` | Exit 0; canonical hash is stable across receipt/status timestamps but changes for goal/step/interface edits; both legs bind one immutable commit/hash; plan-manager calls the plan-review runner exactly once and alone writes; evidence-rich dual/single/zero/not-authorized/phase-waiver outcomes pass; clean reviewed-head completion remains valid across the plan-only receipt commit and stales on any affected-path change. |
...
3. Interface/data contracts — persisted author fields, per-field-provenance `ReviewPolicyInput`, ordered classifier, evidence object, phase/hash waivers, canonical draft/completion JSON receipts, one launcher/one writer, attribution. ✅
...
Score: **96/100** (Draft-5 candidate) · trajectory **86 claimed→55 verified (Draft-1)→88 claimed (Draft-2)→54 verified (Draft-3)→94 claimed/68 verified (Draft-4)→96 candidate (Draft-5)** · fresh X-leg re-review required before `start`.
