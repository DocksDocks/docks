...
| 5 | Dispatch `build-binaries.yml`, download the 4 target binaries + launcher checksums, commit `bin/*` + `SHA256SUMS`; re-run ci green. Redo note: the codex-first fallback amendment (user, 2026-07-06 — bare-CLI fallback is codex when its CLI resolves on PATH/`RELAY_SPAWN_CMD_CODEX`, else claude) landed after the first binary run, so a second dispatch is required | todo |
