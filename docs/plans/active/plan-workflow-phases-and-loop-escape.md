...
updated: "2026-07-19T08:44:06-03:00"
...
| 7 | Finish red→green, regenerate skill hashes, run focused checks, execute the helper-owned no-progress smoke, then run targeted and one full repository gate. | Every affected path; helper-owned disposable plan under `/tmp/docks-plan-review/` only | 6 | in-flight | A pre-release advisory found an operation-loss regression: the five-phase cutover dropped guarded existing-plan GitHub issue publication, invalidating the prior Step 7 evidence once source restoration began. Restore the manager skill, wrappers/contracts, and mutation coverage; regenerate affected skill hashes, then rerun the affected focused ladder, helper smoke, targeted Docks CI, and full `node scripts/ci.mjs` gate. No release until every rerun is green. |
