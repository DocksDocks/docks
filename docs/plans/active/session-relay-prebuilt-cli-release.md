...
| 7 | Finalize stable Session Relay, verify every remote identity and live install, then complete this plan. | `$FINAL_PUBLICATION_RECEIPT` or one distinct canonical resume receipt; external Session Relay Release; this plan only for lifecycle receipt/archive | 6 | done | Terminal receipt validates; stable Release keeps the closed five-asset set/checksums; tag CI, docks-kit release, Docks `origin/main`, and fresh-home `docks-kit sync` all match; completion review passes before plan-only archive/push. |
...
| A4 | `node scripts/ci.mjs` | Exits 0 at the reviewed head as the full gate; the protocol-owned final project-CI run remains authoritative. |
...
| A12 | `node --input-type=module -e 'import fs from "node:fs"; import assert from "node:assert/strict"; import {execFileSync} from "node:child_process"; import {validatePromotionReceipt} from "./scripts/lib/session-relay-release-promotion.mjs"; const receipt=JSON.parse(fs.readFileSync("/home/vagrant/.local/state/docks-release/session-relay-0.12.0/run.UA6vob/promotion-repair-1.json","utf8")); validatePromotionReceipt(receipt); const run=(command,args)=>execFileSync(command,args,{encoding:"utf8"}).trim(); const remote=(ref)=>run("git",["ls-remote","--exit-code","origin",ref]).slice(0,40); assert.equal(remote(receipt.transaction_ref),receipt.terminal_journal_commit); const currentMain=remote("refs/heads/main"); const comparison=run("gh",["api",`repos/DocksDocks/docks/compare/${receipt.observed_origin_main}...${currentMain}`,"--jq",".status"]); assert.ok(["ahead","identical"].includes(comparison)); assert.equal(receipt.attempt,1); assert.equal(receipt.outcome,"success"); assert.equal(receipt.retryable,false); assert.equal(receipt.prior_attempt_receipt_sha256,"d273e350b948ca39b43d84f01801a8013e7343878d5435dcd01e4c102d1ac389"); for(const smoke of [receipt.exact_source_smoke,receipt.live_smoke]){assert.deepEqual(smoke.sync_argv,["sync"]); assert.equal(smoke.docks_kit_target_commit,receipt.public_release_commit); assert.equal(smoke.installed_version,"session-relay 0.12.0");}'` | Exits 0 without replaying repair; the authoritative transaction tip remains exact and GitHub reports the receipt's observed `origin/main` as an ancestor of current `origin/main`, while the canonical terminal attempt-1 success receipt binds the immutable attempt-0 digest, public release commit, exact-source smoke, and live smoke. |
...
| A11 | `git status --short && git -C /home/vagrant/projects/public status --short` | Produces no paths at the pre-receipt completion-review boundary; the separately blocked correlated-messaging plan remains unchanged. Post-lifecycle cleanliness is outside this command and remains a plan-manager lifecycle verification. |
...
Completion-repair closure (2026-07-19): P1's adjacent-date regression failed
before the production fix; the exact-date regex and negative fixture landed in
`1a8e027`, after which the focused promotion contract passed. P2's repaired
acceptance extraction independently parsed A6, A7, and A12 with literal
`.slice(0,40)` and no whitespace regex. Targeted Session Relay CI passed in
66.05s and full CI passed in 176.46s. This closes only the two reopened steps;
the fresh completion review and receipt remain pending.

Completion acceptance repair (2026-07-19): schema-5 round-one request
`7c72b081-ff60-45b7-9eb8-ee0e423f0616` returned blockers P1 and P2, both
independently reproduced and accepted. Step 7 was reopened and reclosed through
plan-only commits. A4 now observes the full gate at the reviewed head, while
...
Review-orchestration-state: {"apply_state":"none","current_input_sha256":"a38c970138fb3cf94de77139e7e68e15aa4a27941423bcf7e49d2d36bac8a1db","initial_input_sha256":"a38c970138fb3cf94de77139e7e68e15aa4a27941423bcf7e49d2d36bac8a1db","lifecycle_intent":"none","orchestration_attempt":1,"phase":"completion","plan_path":"docs/plans/active/session-relay-prebuilt-cli-release.md","request_ids":["528f9072-b51d-4c4b-ab73-839b19aae51f"],"retry_authorization":null,"round_index":1,"schema":1,"series_id":"4ea5a7f2-f269-421b-a2a2-4a662de276c0","series_sha256":null,"state_sha256":"aa5e8ee6c5763a4db6adfe4c3b3c05a45268113cfc0b4c8332432c35d18ed43f","status":"active","stop_reason":null,"transitioned_from_state_sha256":null}
