...
updated: "2026-07-12T16:04:46-03:00"
...
| A138 | A18a | Bind the immutable Step-8 rehash and A137 snapshot+prefix to the same runner/step/event chains, run substitution negatives, then crash-durably clean the exact runner root and emit terminal A138 plus the final 38-row summary. | `test -n "$RUNNER_RECEIPT_ROOT" && test "$STEP8_REHASH_RECEIPT" = "$RUNNER_RECEIPT_ROOT/step-8-live-evidence-rehash.json" && test -f "$STEP8_REHASH_RECEIPT" && test -f "$FINAL_SCOPE_ROOT/acceptance-prefix-snapshot.json" && test -f "$FINAL_SCOPE_ROOT/acceptance-prefix.json" && node plugins/session-relay/test/final-scope.mjs --verify-step8-rehash --tip "$IMPL_TIP" --plan-blob "$PLAN_BLOB" --runner-state "$RUNNER_STATE_RECEIPT" --runner-receipt-root "$RUNNER_RECEIPT_ROOT" --receipts "$STEP_RECEIPT_DIR" --acceptance-events "$RUNNER_RECEIPT_ROOT/acceptance-events" --acceptance-prefix-snapshot "$FINAL_SCOPE_ROOT/acceptance-prefix-snapshot.json" --acceptance-prefix "$FINAL_SCOPE_ROOT/acceptance-prefix.json" --step8-rehash "$STEP8_REHASH_RECEIPT" --step-order 1b,3c,3d,1c,1d,4,5,6,7,8,9 && node plugins/session-relay/test/final-scope.mjs --step8-rehash-negative-matrix --tip "$IMPL_TIP" --plan-blob "$PLAN_BLOB" --runner-state "$RUNNER_STATE_RECEIPT" --runner-receipt-root "$RUNNER_RECEIPT_ROOT" --receipts "$STEP_RECEIPT_DIR" --acceptance-events "$RUNNER_RECEIPT_ROOT/acceptance-events" --acceptance-prefix-snapshot "$FINAL_SCOPE_ROOT/acceptance-prefix-snapshot.json" --acceptance-prefix "$FINAL_SCOPE_ROOT/acceptance-prefix.json" --step8-rehash "$STEP8_REHASH_RECEIPT" --step-order 1b,3c,3d,1c,1d,4,5,6,7,8,9 && node plugins/session-relay/test/final-scope.mjs --source-ready-derivation-matrix --acceptance-prefix-snapshot "$FINAL_SCOPE_ROOT/acceptance-prefix-snapshot.json" --acceptance-prefix "$FINAL_SCOPE_ROOT/acceptance-prefix.json" && node plugins/session-relay/test/final-scope.mjs --finalize-runner-receipt-cleanup --runner-receipt-root "$RUNNER_RECEIPT_ROOT" --expected-step8-rehash "$STEP8_REHASH_RECEIPT" --acceptance-prefix-snapshot "$FINAL_SCOPE_ROOT/acceptance-prefix-snapshot.json" --acceptance-prefix "$FINAL_SCOPE_ROOT/acceptance-prefix.json" --terminal-events "$FINAL_SCOPE_ROOT/acceptance-events" --pending "$FINAL_SCOPE_ROOT/final-acceptance-pending.json" --emit-acceptance-summary "$FINAL_SCOPE_ROOT/acceptance-summary.json"` | Exit 0 with `STEP8_REHASH_SCOPE PASS receipt_sha256=<same-hex>`, `STEP8_REHASH_SCOPE_NEGATIVE_MATRIX PASS`, `SOURCE_READY_DERIVATION_MATRIX PASS`, then `RUNNER_RECEIPT_CLEANUP PASS cleanup_sha256=<hex>` and `ACCEPTANCE_SUMMARY PASS events=82 criteria=38 receipt_sha256=<hex>`. Before finalization the verifier recomputes the live attempt chain, exact six-gate order, terminal Complete/cleanup, bootstrap/prior receipt manifests, full step ledger, events 1–81, and byte-compares the surviving A137 snapshot+prefix. The finalizer fsyncs pending intent with both digests, performs only the sentinel-bound runner-root cleanup, records cleanup evidence in A138 event 82, and emits the final 38-summary receipt entirely from the surviving snapshot plus event 82. Crash-matrix barriers immediately after cleanup and after event 82/before summary prove byte-identical exact retry without reading the deleted root, rerunning a gate, or cleaning twice. Source-ready negatives replace the completion prefix/snapshot, use a wrong archive path/commit, drop/reorder/fork an event, or replace a repeated criterion with its latest result; every row fails before handoff. Missing/early/swapped/escaped cleanup, pending-intent mismatch, snapshot/prefix substitution, wrong terminal event, or summary emitted before cleanup evidence fails. |
...
- **A137/A138:** final scope and the literal Step-8 scope command validate the rehash receipt against the same immutable complete A110→A111→A119→A120→A121→A125 RunGate attempt chain, full step ledger, empty Step-8 A106 receipt, and events 1–80 they independently recompute. A137 appends terminal event 81 and seals a byte-complete surviving snapshot plus digest-bound prefix only after every check; A138's resumable finalizer binds both digests, proves post-cleanup/post-event crash recovery, and binds cleanup evidence into event 82 and the 38 criterion summaries before reporting PASS. No deleted event byte must be reconstructed from a hash, no completed capability is replayed, and no repeated criterion is collapsed to its latest result.
...
Score: **98/100 (Draft-30 final candidate; current schema-v1 review pending)** · trajectory **98 Draft-29 author → 84 X formal NOT READY on post-cleanup reconstruction → 98 Draft-30 surviving prefix snapshot** · stopped: **formal blocker repair only; implementation resumes immediately on eligible READY; owner-provisioned Linux runner remains the later canonical A111 gate**.
