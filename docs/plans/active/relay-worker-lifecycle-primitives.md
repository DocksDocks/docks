...
updated: "2026-07-13T00:52:59-03:00"
...
pub struct PreBindingTerminalReceipt {
    ...
    outcome: PreBindingTerminalOutcome, // only RiskAcceptedAbandoned
    reason: String,
    ...
    terminal_at: String,
    gc_eligible_at: String,
    not_quiescence_proven: bool, // exactly true
    receipt_sha256: String,
}

pub enum PreBindingTerminalOutcome { RiskAcceptedAbandoned }
...
The closed worker transitions are `Attaching→Active|Fencing`; exceptional `Attaching→FencingUnconfirmed` only through the exact lost-authority mapping below; `Active→Fencing`; `Fencing→Fenced|FencingUnconfirmed`; `FencingUnconfirmed→Fencing` only through reconcile; `Fenced→TerminalRetained` only through crash-resumable proof retention; `TerminalRetained→TerminalReleasable` only through explicit release; `FencingUnconfirmed→TerminalReleasable` only through audited bound or pre-binding abandonment; and a pre-binding `TerminalReleasable→PreBindingGcDeleting→WorkerGenerationFence` GC path. `Active→Fencing` and reconcile mint `fence_epoch=Some(new_uuid)` and the resulting worker `version` is the exact `fencing_version`; later states clear the live field only after copying both values into proof/audit records.

Bound release/abandon linearize as one authority-file CAS, never as a worker-only transition. The transaction revalidates exact worker/generation/version, runtime session, `BindingState::Managed` tuple and binding epoch, optional capacity charge/version, proof hash or abandonment acknowledgement, and no competing `ManagedGcDeleting`; it refuses while any `LossCleanupRecord` is Pending/Running. Proved release requires `TerminalRetained` plus the retained proof at the required scope. Risk-accepted bound abandonment requires `FencingUnconfirmed`, explicit reason and acknowledgement, emits no signal, and records `not_quiescence_proven=true`. In one `lifecycle-v1.json` generation it writes worker `TerminalReleasable`, binding epoch N→N+1 as `ManagedTerminal`, the closed `ManagedTerminalReceipt`, every matching live operation/controller/custody/supervisor/watchdog/handoff as terminal audit/tombstone bound to that receipt, and `CapacityCharge::Held→Released` bound to the same receipt. `capacity_charge_id=None` is allowed only for a worker durably born without a reservation; later fan-out-reserved workers must carry Some and cannot substitute None. Bound capacity excludes the worker only when released charge, terminal worker, terminal binding, and receipt hashes all agree; any missing/mismatch remains charged. Retry returns the same receipt; stale actors are audit-only.

Pre-binding abandonment is a distinct authority-file transaction and never invents a session or binding epoch. It accepts only `FencingUnconfirmed` with `runtime_session_id=None`, no claimed token/boundary, no `SessionBinding` owned by that worker/generation, the exact terminal worker-owned birth operation and consumed/refused pending record, no Pending/Running cleanup, and exact optional capacity charge/version. One CAS writes worker `TerminalReleasable`, a closed `PreBindingTerminalReceipt`, terminal tombstones for the birth operation/pending/supervisor/watchdog/controller authority bound to that receipt, and `CapacityCharge::Held→Released` when present; a foreign session binding is byte-identical. `capacity_charge_id=None` is valid only for an unreserved birth. Capacity excludes a pre-binding worker only when terminal worker, receipt, retired-authority hash, and released charge when present all agree. Claim versus pre-binding abandonment has one authority-generation winner: if Claiming or any owned binding exists, pre-binding abandonment emits zero mutation and the bound path is required. Exact retry returns the same receipt; stale operation, pending, worker, or charge versions are audit-only.

`ManagedTerminal|ManagedGcDeleting|TerminalFence` and `PreBindingGcDeleting|WorkerGenerationFence` always refuse the authority family they fence before registry, marker, mailbox, RPC, process, or new-pending bytes. After the ordinary cutoff and exact receipt/reference validation, bound managed GC CASes `ManagedTerminal→ManagedGcDeleting`, performs crash-resumable physical projection cleanup, then in one final authority transaction removes the heavy worker/released charge/heavy terminal records and replaces the binding with compact `TerminalFence`; it never recreates Unmanaged. Pre-binding GC exact-CASes the matching terminal worker `TerminalReleasable→PreBindingGcDeleting`, persists a crash-resumable manifest without touching any session binding, cleans only worker-owned projections, and in one final transaction removes heavy worker/released-charge/receipt/retired-authority records while installing permanent `WorkerGenerationFence`. Both compact fences are permanent under this plan, especially for risk-accepted abandonment where a process may still exist. A future explicit purge policy requires separate review.
...
relay lifecycle abandon-prebinding <worker> --generation <uuid> --expected-version <n> --birth-operation <uuid> --expected-birth-operation-version <n> --capacity-charge <id|none> --expected-capacity-version <n|none> --reason <text> --i-understand-processes-may-still-be-running
...
`status` reports authority-store id/generation, state/version/fence epoch, optional binding/terminal epoch, operation/handoff/supervisor authority, optional capacity charge/version, backend/scope, proof gap, last attempts, and one exact version-bound retry/recovery command selected by structural state. Worker `reconcile` behaves as before. `reconcile-session` acts only on exact `UnmanagedCanceling` session/binding/cancellation epoch, operation, current operation version, and the still-live matching supervisor; successful reap writes a terminal tombstone and advances binding epoch. `abandon-session` requires the same exact selectors, reason, and acknowledgement, advances the binding epoch with a risk-accepted tombstone, and prints **“NOT QUIESCENCE-PROVEN”**. `abandon-prebinding` is offered only for the exact unbound `FencingUnconfirmed` variant and requires every literal worker/birth-operation/charge selector shown; it never accepts `--session`, invents a binding, or touches a foreign binding. Neither session command nor pre-binding command raw-signals observation-only PIDs. Shared servers cannot reconcile to ProtocolTree. Bound worker release/abandon require every literal worker/binding/charge/proof selector shown above and never infer one from the registry projection. Any stale selector changes nothing.
...
For cold execution, the following ownership clauses are part of the corresponding table rows. Step 3c creates/migrates all five explicit live-authority collections plus the three structurally distinct terminal collections, seeds valid cross-referenced records in A113, and proves the exact 0.10.0 writer changes neither their bytes nor the authority generation; A116 proves durable `cancellation_handoffs` reload and rejects dangling/mismatched references. Step 5 owns controller, authorization, and cleanup record persistence/restart behavior plus the fixed-deadline Pending and healthy-stalled Running recovery barriers, including preservation of terminalizable pre-binding loss selectors. Step 7 owns immutable proof-map insertion and same-generation retirement of every matching live authority entry during bound or pre-binding terminalization. Step 8 runs A133 after its closed repeatable set and then writes the empty-range A106 receipt; Step 9 runs A134–A137 plus A138. These clauses narrow and complete the existing row contracts; they do not add files, change the mutation allowlist, or alter step order.
...
| A129 | A11 | GC cannot erase durable authority/custody, preserves legacy Unmanaged aging, and never reopens terminal managed sessions. | `node plugins/session-relay/test/rust-test-inventory.mjs --case lifecycle_managed && (cd plugins/session-relay/rust && cargo test --locked --test lifecycle_managed -- --nocapture)` | Exit 0 with nonzero required GC IDs. Aged lifecycle states plus `UnmanagedCanceling`, pending/Claiming/Managed binding, tombstone, lock, proof, unresolved custody, supervisor/watchdog/socket/heartbeat, handoff, and stale audit survive. Only plain old Unmanaged enters ordinary `GcDeleting`; matching bound terminal tuples enter `ManagedGcDeleting`, crash-resume projection cleanup, and finish as compact `TerminalFence` while capacity remains released exactly once. Matching pre-binding terminal workers enter `PreBindingGcDeleting`, never touch a session binding, and finish as permanent `WorkerGenerationFence` while optional capacity remains released exactly once. Registry Entry loss/recreation cannot change authority; deterministic crash/race barriers preserve one complete authority generation. |
| A130 | A12 | Operator transitions, joint terminalization, retention recovery, session-operation recovery, supervisor reports, custody CAS, and competing fencers are exact-version. | `node plugins/session-relay/test/lifecycle-smoke.mjs --case reconcile && (cd plugins/session-relay/rust && cargo test --locked --test lifecycle_terminal -- --nocapture)` | Exit 0 with nonzero terminal IDs. Crash after durable FenceProofRecord/Fenced resumes exactly once to TerminalRetained; release rejects a different proof hash. Release/abandon versus SessionStart/admission/GC/stale supervisor has exactly one winner; pre-rename crash exposes all preterminal state, post-rename all terminal state, and retry returns the same receipt. Bound capacity remains Held until the joint worker+binding+retired-authority+receipt CAS and becomes Released once only when every hash agrees. Pre-binding reserved and unreserved loss uses only the exact worker/birth-operation/pending/retired-authority/charge transaction, leaves every absent/foreign binding byte-identical, returns one idempotent `PreBindingTerminalReceipt`, and retains `WorkerGenerationFence`; claim versus abandonment and GC versus stale actor have one authority-generation winner across pre/post-rename crashes. Risk acceptance emits no signal, prints NOT QUIESCENCE-PROVEN, and retains the matching permanent fence. Session reconcile uses exact binding/operation/current custody version plus stable cancellation epoch; stale completions are audit-only. |
...
- **A129:** every live/referenced entry in all five explicit maps is non-GCable. GC rejects dangling references rather than erasing them and terminal GC retains the matching compact `TerminalFence` or `WorkerGenerationFence` after same-generation retirement.
...
- Release/abandon changes a bound worker without the exact Managed binding/retired-authority/terminal-receipt/optional-charge transaction; changes a pre-binding worker without the exact absent-owned-binding/birth-operation/retired-pending/terminal-receipt/optional-charge transaction; frees capacity on any mismatch; races Pending/Running loss cleanup; permits terminal re-entry; or GC removes the matching permanent fence/recreates an attachable generation.
...
Review-receipt: {"S":{"raw":{"attempts":[{"child_id":"exec-session-79813","denial_source":null,"effort":"high","exit_code":124,"model":"fable","output_started":false,"reason":"Claude reviewer exceeded the 600-second GNU timeout with no output","result":"deadline_exceeded","retry_cause":null,"schema":1,"signal":null,"started":true,"stderr_sha256":"e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855","stdout_sha256":"e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855","timeout_mode":"gnu_timeout","timeout_seconds":600,"transport":"cli"}],"decision_evidence":null,"findings":[],"findings_sha256":null,"leg":"S","reason":"Claude reviewer exceeded the 600-second GNU timeout with no output","request":{"acceptance_inventory_sha256":null,"author":{"company":"anthropic","effort":"max","model":"opus","tool":"claude-code"},"bundle_sha256":"c78e2400a824de78fe0b981cc7317f3b6b9177fce50808e2f2fddac15b791301","diff_sha256":null,"execution_base_commit":null,"input_sha256":"cdf37648496219ba3912d260bc9bf15263cbcb35f4284a63ff1b75dd6a34f2de","lifecycle_intent":"none","phase":"draft","planned_at_commit":null,"policy":{"anthropic_tiers":[{"effort":"high","model":"fable","transports":["in_session","cli"]},{"effort":"max","model":"opus","transports":["in_session","cli"]}],"cross_company_consent":"always","openai_tiers":[{"effort":"xhigh","model":"gpt-5.6-sol","transports":["in_session","cli"]}],"orchestrator_preference":"auto","provenance":{"anthropic_tiers":"skill_default","cross_company_consent":"runtime_global","openai_tiers":"skill_default","orchestrator_preference":"skill_default","zero_reviewer_policy":"skill_default"},"schema":1,"zero_reviewer_policy":"ask"},"policy_sha256":"5f788dfdaa23f145c734213ebcd8d11d0acc85721cac57961436ee54ed0b023a","request_id":"96ce1601-454a-4146-821b-db6b5ccf4a8f","reviewed_commit_or_head":"98a1369d9fb086e9d237777da41b434a3abcc387","schema":1},"result":"timed_out","reviewer_output":null,"schema":1,"selected":null,"severity_totals":{"high":0,"low":0,"medium":0},"waiver":null,"waiver_sha256":null},"reconciliation":{"accepted":[],"rejected":[]},"request":{"acceptance_inventory_sha256":null,"author":{"company":"anthropic","effort":"max","model":"opus","tool":"claude-code"},"bundle_sha256":"c78e2400a824de78fe0b981cc7317f3b6b9177fce50808e2f2fddac15b791301","diff_sha256":null,"execution_base_commit":null,"input_sha256":"cdf37648496219ba3912d260bc9bf15263cbcb35f4284a63ff1b75dd6a34f2de","lifecycle_intent":"none","phase":"draft","planned_at_commit":null,"policy":{"anthropic_tiers":[{"effort":"high","model":"fable","transports":["in_session","cli"]},{"effort":"max","model":"opus","transports":["in_session","cli"]}],"cross_company_consent":"always","openai_tiers":[{"effort":"xhigh","model":"gpt-5.6-sol","transports":["in_session","cli"]}],"orchestrator_preference":"auto","provenance":{"anthropic_tiers":"skill_default","cross_company_consent":"runtime_global","openai_tiers":"skill_default","orchestrator_preference":"skill_default","zero_reviewer_policy":"skill_default"},"schema":1,"zero_reviewer_policy":"ask"},"policy_sha256":"5f788dfdaa23f145c734213ebcd8d11d0acc85721cac57961436ee54ed0b023a","request_id":"96ce1601-454a-4146-821b-db6b5ccf4a8f","reviewed_commit_or_head":"98a1369d9fb086e9d237777da41b434a3abcc387","schema":1}},"X":{"raw":{"attempts":[{"child_id":"/root/draft36_formal_x","denial_source":null,"effort":"xhigh","exit_code":0,"model":"gpt-5.6-sol","output_started":true,"reason":"completed","result":"passed","retry_cause":null,"schema":1,"signal":null,"started":true,"stderr_sha256":"e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855","stdout_sha256":"4ad3eb959fe8976d9894f7c5d11a71a39d2bd1c3711606f599bd47aa11c7604e","timeout_mode":"orchestrator_tool","timeout_seconds":600,"transport":"in_session"}],"decision_evidence":null,"findings":[],"findings_sha256":"4f53cda18c2baa0c0354bb5f9a3ecbe5ed12ab4d8e11ba873c2f11161202b945","leg":"X","reason":null,"request":{"acceptance_inventory_sha256":null,"author":{"company":"anthropic","effort":"max","model":"opus","tool":"claude-code"},"bundle_sha256":"c78e2400a824de78fe0b981cc7317f3b6b9177fce50808e2f2fddac15b791301","diff_sha256":null,"execution_base_commit":null,"input_sha256":"cdf37648496219ba3912d260bc9bf15263cbcb35f4284a63ff1b75dd6a34f2de","lifecycle_intent":"none","phase":"draft","planned_at_commit":null,"policy":{"anthropic_tiers":[{"effort":"high","model":"fable","transports":["in_session","cli"]},{"effort":"max","model":"opus","transports":["in_session","cli"]}],"cross_company_consent":"always","openai_tiers":[{"effort":"xhigh","model":"gpt-5.6-sol","transports":["in_session","cli"]}],"orchestrator_preference":"auto","provenance":{"anthropic_tiers":"skill_default","cross_company_consent":"runtime_global","openai_tiers":"skill_default","orchestrator_preference":"skill_default","zero_reviewer_policy":"skill_default"},"schema":1,"zero_reviewer_policy":"ask"},"policy_sha256":"5f788dfdaa23f145c734213ebcd8d11d0acc85721cac57961436ee54ed0b023a","request_id":"96ce1601-454a-4146-821b-db6b5ccf4a8f","reviewed_commit_or_head":"98a1369d9fb086e9d237777da41b434a3abcc387","schema":1},"result":"passed","reviewer_output":{"confirmations":["Verified the sealed bundle with the canonical decimal-byte-length plus NUL framing over raw manifest.json followed by each manifest-ordered file body; the result is c78e2400a824de78fe0b981cc7317f3b6b9177fce50808e2f2fddac15b791301. All 22 bundled file hashes match, all 82 requested file/absent states match, plan.review.md hashes to the requested input SHA-256, and the JCS policy hash matches.","Reproduced Draft-35 X1 and confirmed its repair: Step 1b now names plugins/session-relay/rust/Cargo.toml and plugins/session-relay/rust/Cargo.lock as two separate literal repo-root-relative code spans, each with its own path-local read-only annotation.","Reproduced Draft-35 X2 and confirmed its repair: every future annotation begins with exactly new, modify, delete, or read-only; the Step-4 and Step-5 annotations begin with the punctuation-free token modify, and their targets were created in Step 1b before the projected M rows.","Reproduced Draft-35 X3 and confirmed its repair: the Steps header, separator, and every one of the 15 data rows contain exactly seven unescaped pipe delimiters, hence exactly six Markdown columns. Step 3d contains no pipe-delimited code span.","Parsed every future Files cell under the normative grammar and folded existence in exact order 1b,3c,3d,1c,1d,4,5,6,7,8,9. The closed projection has 42 A occurrences, 69 M occurrences, one D occurrence, and empty mutation arrays for 1c, 1d, and 8; all 42 explicit new paths are baseline-absent, all modify targets are projected-present, and the sole delete is the historically present stale branch-plan path.","Confirmed all future mutation items are literal repo-root-relative paths with no glob, directory shorthand, optional row, absolute path, or annotation carry-forward. Read-only exclusions are path-local, Step 8 expands all 23 inputs literally and marks each read-only, and Step 9 keeps both WIP fixtures read-only while deleting only the stale plan copy.","Confirmed every A/M row is constrained to final_mode 100644 and regular-file state, the sole D row to final_mode null and absence, and no executable mutation is authorized. Production Cargo.toml/Cargo.lock remain separately read-only after STEP3B_HEAD; in Steps 3c, 4, and 6 each compile-fixture Cargo.lock is read-only while its companion Cargo.toml mutates to declare targets without dependency-resolution drift.","Recounted AcceptanceEventScheduleV1 as exactly 82 ordered occurrences covering exactly 38 unique A101-A138 criteria, with ten A104 and eleven A106 occurrences and the six one-shot RunGates in their declared positions. The bounded Draft-36 changes remain confined to parser/path/table closure; the stated goal, deliverables, completed-history contract, step order, runtime architecture, release contract, and 82-event schedule remain unchanged, and no further execution blocker was found."],"score":99,"structured_output_sha256":"4ad3eb959fe8976d9894f7c5d11a71a39d2bd1c3711606f599bd47aa11c7604e","verdict":"ready"},"schema":1,"selected":{"effort":"xhigh","model":"gpt-5.6-sol","transport":"in_session"},"severity_totals":{"high":0,"low":0,"medium":0},"waiver":null,"waiver_sha256":null},"reconciliation":{"accepted":[],"rejected":[]},"request":{"acceptance_inventory_sha256":null,"author":{"company":"anthropic","effort":"max","model":"opus","tool":"claude-code"},"bundle_sha256":"c78e2400a824de78fe0b981cc7317f3b6b9177fce50808e2f2fddac15b791301","diff_sha256":null,"execution_base_commit":null,"input_sha256":"cdf37648496219ba3912d260bc9bf15263cbcb35f4284a63ff1b75dd6a34f2de","lifecycle_intent":"none","phase":"draft","planned_at_commit":null,"policy":{"anthropic_tiers":[{"effort":"high","model":"fable","transports":["in_session","cli"]},{"effort":"max","model":"opus","transports":["in_session","cli"]}],"cross_company_consent":"always","openai_tiers":[{"effort":"xhigh","model":"gpt-5.6-sol","transports":["in_session","cli"]}],"orchestrator_preference":"auto","provenance":{"anthropic_tiers":"skill_default","cross_company_consent":"runtime_global","openai_tiers":"skill_default","orchestrator_preference":"skill_default","zero_reviewer_policy":"skill_default"},"schema":1,"zero_reviewer_policy":"ask"},"policy_sha256":"5f788dfdaa23f145c734213ebcd8d11d0acc85721cac57961436ee54ed0b023a","request_id":"96ce1601-454a-4146-821b-db6b5ccf4a8f","reviewed_commit_or_head":"98a1369d9fb086e9d237777da41b434a3abcc387","schema":1}},"author":{"company":"anthropic","effort":"max","model":"opus","tool":"claude-code"},"decision_evidence":null,"input_sha256":"cdf37648496219ba3912d260bc9bf15263cbcb35f4284a63ff1b75dd6a34f2de","outcome":"single","phase":"draft","policy":{"anthropic_tiers":[{"effort":"high","model":"fable","transports":["in_session","cli"]},{"effort":"max","model":"opus","transports":["in_session","cli"]}],"cross_company_consent":"always","openai_tiers":[{"effort":"xhigh","model":"gpt-5.6-sol","transports":["in_session","cli"]}],"orchestrator_preference":"auto","provenance":{"anthropic_tiers":"skill_default","cross_company_consent":"runtime_global","openai_tiers":"skill_default","orchestrator_preference":"skill_default","zero_reviewer_policy":"skill_default"},"schema":1,"zero_reviewer_policy":"ask"},"policy_sha256":"5f788dfdaa23f145c734213ebcd8d11d0acc85721cac57961436ee54ed0b023a","pre_execution_eligible":true,"reproduced":[],"request":{"acceptance_inventory_sha256":null,"author":{"company":"anthropic","effort":"max","model":"opus","tool":"claude-code"},"bundle_sha256":"c78e2400a824de78fe0b981cc7317f3b6b9177fce50808e2f2fddac15b791301","diff_sha256":null,"execution_base_commit":null,"input_sha256":"cdf37648496219ba3912d260bc9bf15263cbcb35f4284a63ff1b75dd6a34f2de","lifecycle_intent":"none","phase":"draft","planned_at_commit":null,"policy":{"anthropic_tiers":[{"effort":"high","model":"fable","transports":["in_session","cli"]},{"effort":"max","model":"opus","transports":["in_session","cli"]}],"cross_company_consent":"always","openai_tiers":[{"effort":"xhigh","model":"gpt-5.6-sol","transports":["in_session","cli"]}],"orchestrator_preference":"auto","provenance":{"anthropic_tiers":"skill_default","cross_company_consent":"runtime_global","openai_tiers":"skill_default","orchestrator_preference":"skill_default","zero_reviewer_policy":"skill_default"},"schema":1,"zero_reviewer_policy":"ask"},"policy_sha256":"5f788dfdaa23f145c734213ebcd8d11d0acc85721cac57961436ee54ed0b023a","request_id":"96ce1601-454a-4146-821b-db6b5ccf4a8f","reviewed_commit_or_head":"98a1369d9fb086e9d237777da41b434a3abcc387","schema":1},"reviewed_at":"2026-07-12T18:43:46-03:00","reviewed_commit":"98a1369d9fb086e9d237777da41b434a3abcc387","schema":1}

Score: **99/100 (Draft-38 pre-binding terminal closure)** · trajectory **99 Draft-37 author → 88 X NOT READY on one terminal-variant gap → 99 structurally separate bound/pre-binding repair** · execution eligibility is controlled solely by the current schema-v1 `Review-receipt:`.
