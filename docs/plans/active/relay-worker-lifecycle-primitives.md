...
updated: "2026-07-12T04:03:59-03:00"
...
- **Exact A0c-step1b-bootstrap command (run once, before any Step-1b edit):** only after Step 3b is committed and the orchestrator has independently recorded its full tip, dispatch `PLAN_COMMIT`, `PLAN_BLOB`, and `STEP3B_HEAD`; from `/tmp/docks-primitives-collab`, run:
  ...
  git diff --check HEAD
  ```
  This one-time bridge authenticates the clean post-3b tip before `wip-snapshot.mjs` exists. Step 1b must commit the helper, `test/fixtures/wip-historical-baseline.json`, and `test/fixtures/wip-step-allowlist.json`. The recursively closed historical fixture records the exact observed `12cf2ea..STEP3B_HEAD` range before any future allowance: `{schema,base,tip,entries:[{path,status,old_mode,new_mode,old_oid,new_oid,owner_step,owner_commits,final_disposition,entry_sha256}],manifest_sha256}`. Because the independently observed baseline contains only additions/modifications, `status` is exactly `A|M`; any rename/copy/delete/type status is a mismatch, `A` requires zero old mode/oid, and modes/oids are canonical git octal/40-hex. Entries are UTF-8 byte-sorted and unique; `owner_commits` are full SHAs that actually touch the entry; `final_disposition` is `present|must_be_absent_at_tip`, with the stale implementation-branch plan exactly `must_be_absent_at_tip`; each `entry_sha256` covers that entry excluding itself. Step 1b derives the entries from raw diff/history, independently byte-compares them to the named Step-1/2/3a/3b commits, and rejects an unowned or broadly absorbed path. The separate future allowlist enumerates every remaining step and exact `{path,mode,historical_entry_sha256?}`. A future step may intentionally edit a historical-present path only by carrying that exact historical entry hash; it never changes the stored baseline. Conflicting modes, a `must_be_absent_at_tip` extension, an unbound historical overlap, duplicate row within one step, or missing/extra/duplicate step id fails. A0c is never reused after Step 1b.
- **Runner live job capability:** on the owner-provisioned Linux job, set nonempty `RELAY_RUNNER_ID` to the owner-recorded diagnostic job/runner identity, `RELAY_RUNNER_RECEIPT` to one new harness-owned absolute temporary JSON path, and `RELAY_CGROUP_ROOT` to the validated delegated root; none is authority. Before A1c, start one foreground `RELAY_RUNNER_JOB=1 node plugins/session-relay/test/runner-job-custodian.mjs --serve --runner-id "$RELAY_RUNNER_ID" --runner-receipt "$RELAY_RUNNER_RECEIPT" --cgroup-root "$RELAY_CGROUP_ROOT"` process and retain that exact process/session through A7. The driver resolves/hashes one absolute Node executable and compiles/hashes one committed host-native helper with distinct `--custodian` and `--gate` modes. It creates exactly one `AF_UNIX|SOCK_SEQPACKET|SOCK_CLOEXEC` socketpair, spawns the helper's long-lived custodian mode with only the custodian endpoint, retains the private gate endpoint, and generates a public random `capability_id`; both processes must stay live. Its stdin accepts only the six literal recursively closed JCS `RunGate` frames below in fixed A1c→A1d→A5→A5b→A6→A7 order. `node_script`, `args`, and sorted `env` must byte-equal the named row; there is no caller-supplied executable, cwd, raw environment, shell text, assignment prefix, or extra field. The driver resolves each `env` source only from its validated start inputs; `literal_one` is exactly `"1"`. The Node-child base-name allowlist is literally `HOME|LANG|LC_ALL|PATH|TMPDIR`; authentication contributes either no environment name when the validated credential-artifact mechanism is used or only the exact runtime-specific secret name sealed by Step 1's auth preflight into the live tuple. The six frame rows add only their listed names, and `RELAY_RUNNER_CAP_FD` exists only in the native gate environment. Duplicate names, an auth name not sealed in that tuple, or any other ambient name fails before native spawn—there is never bulk inheritance.

  For every positive row the driver directly spawns the native helper in `--gate` mode as the credential-bearing gate; it never spawns `/bin/sh`, a launcher script, or Node as the credential principal. Immediately before that one spawn it duplicates the retained gate endpoint to one fixed child fd, clears `FD_CLOEXEC` only on that duplicate so the native gate inherits it, and injects only its decimal number as `RELAY_RUNNER_CAP_FD` into the native gate's environment. The native gate's first fd action is `fcntl(F_SETFD, FD_CLOEXEC)` on that duplicate; failure poisons and exits. Only after immediate restoration may it validate the fd as the expected connected `SOCK_SEQPACKET`, close every other non-allowlisted fd, and send OPEN itself. Only after OPEN acknowledgement does it spawn the pinned absolute Node with the exact frame script/args/env; the capability fd is CLOEXEC and absent from Node plus every runtime/helper descendant. The native gate exact-waits the Node child, hashes its result evidence, sends COMMIT itself, closes its duplicate after COMMIT acknowledgement, emits no application output except its closed result pipe, and exits. Thus the driver-spawn PID is the credential-bearing native gate for its full lifetime; Node is a non-capability payload child.
...
| 1b | After preserved Step 3b is clean/committed, run the one-time A0c handoff, create the reusable WIP snapshot helper, immutable exact historical-range manifest, and separate closed future per-step allowlist; migrate feasibility evidence to independent `cooperative_cgroup` and Claude-only `filtered_hardening`; prototype raw Codex `thread/start→controller claim candidate→turn/start` ordering (not production Rust claim), inventory experimental capability, define semantic hook-health snapshots, prove stable-runtime generation/pointer feasibility, and build the host-native live runner-job custodian harness. | `plugins/session-relay/test/feasibility-probe.mjs`, `plugins/session-relay/test/fixtures/lifecycle-capability-schema.json`, `plugins/session-relay/test/fixtures/wip-historical-baseline.json` (new), `plugins/session-relay/test/fixtures/wip-step-allowlist.json` (new), `plugins/session-relay/test/runtime-hook-abort.mjs` (new), `plugins/session-relay/test/runtime-hook-upgrade.mjs` (new), `plugins/session-relay/test/runner-job-custodian.mjs` (new), `plugins/session-relay/test/wip-snapshot.mjs` (new) | 1, 3b | planned | A0c/A0b/A1/A1b pass. Helper reproduces empty state and rejects every dirty class. Historical fixture exactly binds `12cf2ea..STEP3B_HEAD` path/status/modes/oids/owning commits and marks only the stale implementation plan `must_be_absent_at_tip`; future fixture closes every remaining step/path/mode mapping without absorbing baseline facts. Raw trace proves SessionStart-before-thread-response and zero turn before a prototype claim decision without pretending JS is shipped lifecycle logic. Stable generation/pointer survives pruning/crash/races. Custodian positives exact-compare all six literal frames and `spawn PID=OPEN pid=COMMIT pid=wait pid=result pid`; negatives cover copy/no-fd, fd leak, shell assignment/argv injection, non-exec wrapper, sibling borrower, duplicate/stale/out-of-order/concurrent OPEN, authoritative-before-OPEN, missing/wrong/duplicate COMMIT, result/evidence mismatch, EOF/crash/poison without running a real runtime. No implementation-branch plan/doc edit is permitted. |
...
| 5 | Implement managed first-prompt admission and durable controller. Claude uses Claiming-first SessionStart plus UserPromptSubmit. Codex creates `Attaching` plus a sealed one-use `ManagedBirthPermit`, then the supervisor alone consumes it to launch, hold the raw token, create pending state, fd-pin/schema-probe the app-server, retain Child/stdio/request map/server-request policy/notification pump, persist ControllerReady, and claim the non-claiming `thread/start` identity before first turn. Add the three exact controller `OperationKind` variants and one-time command authorizations. On custody loss, atomically publish durable logical loss plus one post-loss `LossCleanupRecord/Permit`, then immediately consume it for close/kill/reap without permitting proof or capacity release. Add ordinary-prune-safe standalone hook bootstraps, post-start dirfd/fd-pinned locked runtime installer, optional docks-kit verified-native proactive interface, deterministic schema fixture, doctor/install contracts, and unmanaged compatibility. Extend the Rust fixture before A6/A6a and rerun complete A9/A10 after controller variants exist. | `plugins/session-relay/rust/src/appserver.rs`, `plugins/session-relay/rust/src/hook.rs`, `plugins/session-relay/rust/src/lifecycle.rs`, `plugins/session-relay/rust/src/runtime_install.rs` (new), `plugins/session-relay/rust/src/main.rs`, `plugins/session-relay/rust/src/spawn.rs`, `plugins/session-relay/rust/src/store.rs`, `plugins/session-relay/rust/src/supervisor.rs`, `plugins/session-relay/rust/tests/lifecycle_admission.rs`, `plugins/session-relay/rust/tests/lifecycle_managed.rs`, `plugins/session-relay/rust/tests/lifecycle_controller.rs` (new), `plugins/session-relay/hooks/hooks.json`, `plugins/session-relay/hooks/codex-hooks.json`, `plugins/session-relay/test/appserver-schema-contract.mjs` (new), `plugins/session-relay/test/fixtures/appserver-server-requests.json` (new), `plugins/session-relay/test/rust-test-inventory.mjs`, `plugins/session-relay/test/fixtures/rust-test-inventory.json`, `plugins/session-relay/test/reentry-inventory.mjs`, `plugins/session-relay/test/fixtures/reentry-inventory.json`, `plugins/session-relay/test/lifecycle-smoke.mjs`, `plugins/session-relay/test/fixtures/runtime-doctor-schema.json` (new), `plugins/session-relay/test/fixtures/runtime-doctor-goldens/` (new), `plugins/session-relay/test/runtime-hook-abort.mjs` (new), `plugins/session-relay/test/runtime-hook-upgrade.mjs` (new), `plugins/session-relay/test/selftest.mjs` | 1b-4 | planned | Fixture extensions precede gates. A6/A6a/A6b/A6c plus post-Step-5 A9/A10 prove birth-permit single use, token secrecy, production claim ordering, schema-bound controller startup, every controller command/loss row, atomic permit mint, post-publication cleanup authority, one-use/stale/replay refusal, crash no-remint, cleanup success without capacity release, later-process wake, stable hook events, ordinary prune fallback at every shell window, known-at-check corruption refusal, native-installer source-fd races, complete same-version identity, optional direct verified-native install, deterministic schema bytes, and Claude barrier. Managed CLI fallback, pre-Active turn, arbitrary forwarding, reconnect guessing, bytes after loss, cleanup via normal post-loss FencePermit, non-atomic runtime pair, or standalone adversarial-cache-authenticity claim: STOP. |
...
| 9 | Document guarantee tiers, Linux-only managed controller/fd-exec availability, Darwin closed unavailability plus portable supervisor, stable hook runtime, `CooperativeWorkerV1`, cancellation, current ProtocolTree unavailability, cgroup evidence, GC, and recovery; remove the stale non-authoritative implementation-branch plan copy so the final range contains zero `docs/plans/**`; add exact-SHA Darwin matrix, exact historical-baseline-plus-future-union final-range verification, disposable diff-class negatives, and full gates without binary/release changes. | `plugins/session-relay/AGENTS.md`, `plugins/session-relay/skills/productivity/session-relay/SKILL.md`, `plugins/session-relay/rust/src/main.rs`, `plugins/session-relay/test/fixtures/wip-historical-baseline.json`, `plugins/session-relay/test/fixtures/wip-step-allowlist.json`, `plugins/session-relay/test/run-build-matrix.mjs` (new), `plugins/session-relay/test/final-scope.mjs` (new), `.github/workflows/build-binaries.yml` | 1b-8 | planned | A15-A18 pass. `final-scope` first verifies the immutable `12cf2ea..STEP3B_HEAD` manifest and exact owner commits, then derives the final exact path+mode set from historical `present` entries plus the closed future per-step union while requiring every `must_be_absent_at_tip` entry absent. It prints the canonical actual-range manifest/hash and rejects old/new/add/delete/rename/copy/type/mode/submodule/status deviations plus every docs/binary/version class. No path may be accepted merely because it appeared historically. Main plan remains source of truth; implementation range has no plan/docs delta. Darwin runs portable behavior and reports authoritative managed path unavailable. |
...
| A18 | The exact clean implementation tip matches the immutable historical baseline plus closed future path+mode union and contains no binary/release/docs mutation. | `test "$(git branch --show-current)" = codex/primitives-collab && test "$(git rev-parse HEAD)" = "$IMPL_TIP" && test -z "$(git status --porcelain)" && git diff --check 12cf2ea.."$IMPL_TIP" && node plugins/session-relay/test/final-scope.mjs --verify-range --base 12cf2ea --tip "$IMPL_TIP" --historical plugins/session-relay/test/fixtures/wip-historical-baseline.json --allowlist plugins/session-relay/test/fixtures/wip-step-allowlist.json --emit-manifest /tmp/relay-final-scope-manifest.json && node plugins/session-relay/test/final-scope.mjs --negative-matrix --base 12cf2ea --tip "$IMPL_TIP" --historical plugins/session-relay/test/fixtures/wip-historical-baseline.json --allowlist plugins/session-relay/test/fixtures/wip-step-allowlist.json` | Exit 0 with exact reviewed HEAD/clean status and `FINAL_SCOPE PASS files=<N> manifest_sha256=<hex> historical_sha256=<hex>`. Helper first proves exact `12cf2ea..STEP3B_HEAD` entries/owner commits, excludes `must_be_absent_at_tip`, then derives the exact final path+mode set from historical-present plus future step rows. A future edit of a historical-present path is accepted only with its exact `historical_entry_sha256` and compatible final mode; the union deduplicates that authenticated extension. It parses raw diff/status metadata for old/new path, add/delete/rename/copy, regular/symlink/gitlink/type change, executable-bit/other mode change, submodule, staged/unstaged/untracked, within-step duplicate, conflicting/unbound overlap, historical rewrite, and unowned-path cases. Every unexpected class fails independently; no wildcard/regex or “seen historically” allowance exists. The emitted canonical actual-range manifest is outside the repo and its hash is independently recorded. Net `docs/plans/**`, bin, manifest/version, marketplace, tag/release surface is empty. |
...
- A18 uses a wildcard/regex or “seen historically” allowance, fails to bind exact `12cf2ea..STEP3B_HEAD` path/status/old+new-mode/oids to named owner commits, lets a future row rewrite/broadly absorb a historical fact, fails to require `must_be_absent_at_tip`, omits old/new paths or mode/type/gitlink/status classes, accepts a path/mode outside the historical-present plus exact future union, or fails to emit/recheck both canonical manifest hashes.
...
2. **Environment & commands:** present — path-specific plan/blob, preserved-WIP bridge, immutable historical manifest plus future step allowlist, cross-filesystem handoff, four targets, shell-first stable runtime, isolated real-runtime setup, measured deadline, foreground live runner custodian with six literal frame rows, closed env sources, direct native-gate spawn/PID/CLOEXEC, exact two-phase protocol/reap, and owner-provisioned delegated Linux runner prerequisite are explicit. Current host ineligibility is recorded, not silently skipped.
3. **Interface & data contracts:** present — exact-tuple one-use ManagedBirthPermit, non-claiming SessionStart plus controller-only claim, durable app-server controller, exact controller OperationKinds, post-loss one-use cleanup permit with no release authority, closed integrity-only delegated-runner receipt plus non-copyable inherited-fd job capability, stable cancellation/custody versions, nonce/control epoch, executable-bound cgroup receipts, attempt-bound proof, post-start fd-pinned immutable runtime generations, deterministic schema, doctor/install schemas, tombstone GC, and operator APIs are explicit.
4. **Executable acceptance:** present — A0/A0b-bootstrap/A0b–A18 plus A1b/A1c/A1d/A3a–A3d/A6b/A6c are command + expected evidence, including literal RunGate/env rows, direct credential-gate PID equality, inherited-fd/CLOEXEC and wrapper/borrower negatives, capability-bound TRACEEXEC/delegation receipt, OPEN-before-authority and COMMIT-before-advance, copy-all-bytes/no-fd and sequence negatives, exact child reap, live runner revalidation, post-loss cleanup authority/replay/no-release, ordinary hook pruning plus honest standalone-authenticity boundary and docks-kit verified-native path, per-step nonzero inventory birth, controller A9/A10 rerun, post-installer source-fd and schema determinism races, live cache upgrade, later-process controller wake, Darwin unavailable rows, and exact historical-plus-future actual-range manifests/hashes.
...
Adversarial cold-read result: Draft-21 preserves every accepted Draft-20 repair and closes its credential-PID launch gap. Standalone `/bin/sh` bootstrap still owns ordinary-prune durability and observed corruption checks only; deliberate same-UID pre-exec replace-all is explicitly out of cooperative scope, post-start native installer pinning is exact, and docks-kit retains an optional externally verified-native path. Runner continuity uses one live custodian, six literal closed frames, a direct-spawn native credential gate, one intentional fd inheritance restored CLOEXEC before OPEN, exact spawn/OPEN/COMMIT/wait/result PID equality, OPEN before any Node/authoritative byte, and COMMIT before sequence advance. Non-exec wrapper/borrowed-fd, copy/replay/crash/poison, and environment/argv injection negatives are explicit. LossCleanupPermit, ManagedBirthPermit, controller variants/inventory ownership, exact reap/live delegation, followed proc magic-link TRACEEXEC, fd-pinned installer/schema, historical-plus-future A18 union, same-version tuple, and Linux/Darwin split remain intact. No cold executor must guess the hook or runner threat boundary. Implementation remains paused for fresh immutable dual re-review; Step 4 and A5/A5b/A6/A7 remain blocked until the owner supplies the live capability-bound runner. Remote push/dispatch and packaged producer/release gates remain owner approvals.
...
Score: **98/100 (Draft-21 author candidate; fresh immutable dual re-review pending)** · trajectory **96 Draft-18 author → 90 acceptance NOT READY → 96 Draft-19 author → 92 acceptance NOT READY → 97 Draft-20 author → 95 acceptance NOT READY → 98 literal-frame/direct-gate repair** · stopped: **candidate frozen for review; implementation paused; real Linux gates require one owner-provisioned live custodian through A1c/A1d/A5/A5b/A6/A7**.
