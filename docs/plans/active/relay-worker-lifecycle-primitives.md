...
| Cgroup tree (`ConfinedCgroupCooperative`, both runtimes) | Under `CooperativeWorkerV1`, place the runtime in a delegated **domain** cgroup leaf with **atomic/gated initial placement** (`clone3 CLONE_INTO_CGROUP` or a pre-exec stop/GO handshake so no runtime code forks before verified leaf membership); retain the generation-bound leaf fd; assert `cgroup.type=domain` and writable `cgroup.kill`; `cgroup.freeze`→`frozen=1`→`cgroup.kill`→`populated 0`. Works for **both** Claude and Codex because cgroup membership is namespace-independent — Codex's bwrap-nested user/mount/PID/cgroup descendants stay in the leaf and are killed. The anti-escape seccomp filter (deny `unshare`/`mount`/`CLONE_NEW*`, arch/x32/procfs hardening) is a **Claude-only `FilteredCgroupHardening`** upgrade, never required for the cooperative tier. | Plain delegation, non-atomic placement (the spawn-then-move race), threaded topology (`cgroup.kill`→`EOPNOTSUPP`), manager-fd loss, or any claim against broker-assisted / explicit same-UID cgroup migration is unconfirmed even if kill succeeds and `populated 0` is observed. `FilteredCgroupHardening` adds defense-in-depth but makes **no** adversarial-containment claim (same-UID brokers stay outside it). |
...
- **Checkout/base:** `/home/vagrant/projects/docks-primitives`, branch `codex/relay-worker-lifecycle-primitives`; Draft-5 starts from Draft-4 commit `610ca11`; drift/review base remains `12cf2ea` as required by the originating plan. Run A0 before implementation.
...
|---|---|---|---|---|---|
| 1 | Codify the confirmed feasibility foundation in a committed probe harness and minimum evidence schema: existing runtime/hook/process rows, informational presence/absence of any app-server lineage-mutation barrier, the exact graceful stop/reject/flush/watermark/reap contract required for offline protocol proof, and every strong-cgroup prerequisite including arch/x32/procfs, raw `clone3→ENOSYS`, and dual-real-runtime ordinary-spawn probes under the candidate exact filter. Emit raw-record hashes and measure attach/protocol bounds. | `plugins/session-relay/test/feasibility-probe.mjs` (new), `plugins/session-relay/test/fixtures/lifecycle-capability-schema.json` (new), `docs/plans/active/relay-worker-lifecycle-primitives.md:Notes` | — | planned | A1 passes without editable verdicts. Shared protocol remains observation-only even if a new barrier is discovered; absent durable flush evidence disables ProtocolTree proof; any failed cgroup prerequisite disables ConfinedCgroup for that runtime, but a runtime that fails only the anti-escape filter (Codex/`bwrap`) records `FilteredCgroupHardening` unavailable and stays cooperative-tier eligible (see resolved `codex-cooperative-cgroup-tier`). If measured attach deadline exceeds 20s or raw evidence validation fails, STOP before Rust. |
...
| 4 | Implement stable handles plus the `ConfinedCgroupCooperative` wrapper (both runtimes): **atomic/gated placement** into a delegated **domain** leaf via `clone3 CLONE_INTO_CGROUP` or a pre-exec stop/GO handshake (no runtime code forks before verified leaf membership), retained generation-bound leaf fd, `cgroup.type=domain` + writable `cgroup.kill` assertion (fail-closed on threaded → `EOPNOTSUPP`), freeze→kill→populated-0, and fail-closed fd-loss recovery. Then the **Claude-only `FilteredCgroupHardening`** upgrade layered on top: move-before-cgroup-unshare, user/mount/PID/cgroup namespaces, one read-only cgroup2 view, detach-all/fresh proc, proc-pid-fd probes, fd closure, capability drop, arch/x32-aware seccomp with wholesale `clone3→ENOSYS` + legacy-clone namespace filtering, raw-ABI escape probes (Codex is expected filtered-unavailable and stays on the cooperative tier). Inventory every signal callsite. | `plugins/session-relay/rust/src/process_identity.rs` (new), `plugins/session-relay/rust/src/spawn.rs:337-495,662-793`, `plugins/session-relay/rust/src/main.rs:14-59`, `plugins/session-relay/rust/src/lifecycle.rs` (new), `plugins/session-relay/rust/src/store.rs`, `plugins/session-relay/test/process-signal-inventory.mjs` (new), `plugins/session-relay/test/fixtures/process-signal-inventory.json` (new), `plugins/session-relay/test/runtime-hook-abort.mjs` (new), `.github/workflows/build-binaries.yml` | 1-3 | planned | A4/A5/A5b prove stable signaling; **gated leaf placement with no pre-placement fork escaping the leaf**, `cgroup.type=domain`, and freeze→kill→populated-0 with no surviving host PID for BOTH runtimes; and — for the Claude-only hardening — raw `clone3` denial with exact ENOSYS return, legacy-clone namespace denial, x32/arch behavior, procfs authority removal, escape variants, and no thaw-through-release. Any gated placement fails, a descendant escapes the leaf, threaded topology is accepted, exact manager fd is lost, replacement receives a signal, or direct cooperative escape works: STOP/downgrade that runtime to Unconfirmed. |
...
| A5 | Tree proof requires the complete ordered cooperative-scope anti-migration setup. | `node plugins/session-relay/test/lifecycle-smoke.mjs --case containment` | Exit 0; transcript proves leaf move precedes cgroup namespace, every inherited proc/cgroup mount is detached, fresh proc hides host fd/fdinfo/root/cwd authority, exactly one read-only cgroup2 mount exists, control fds/capabilities are absent, and seccomp validates native AUDIT_ARCH before GO. The existing raw-syscall namespace-denial row remains separate and proves `clone3` creates no child and returns exactly `-1/ENOSYS`; namespace flags on legacy `clone` remain denied. x86 x32, wrong-arch, remount, userns, inherited-fd, alternate-mount, proc-pid-fd, and listed defense-in-depth attacks fail before/during fence; freeze cannot release until kill+`populated 0`, with no thaw. Proof/status prints `CooperativeWorkerV1`; broker escape remains `outside_threat_model`. Any prerequisite/setup/fd loss is Unconfirmed; every TrackedTree case remains `UnconfirmedDescendants`. |
| A5b | Cooperative cgroup child creation works in BOTH runtimes with gated placement; the anti-escape filter is a Claude-only hardening. | `RELAY_REAL_RUNTIME_TEST=1 node plugins/session-relay/test/runtime-hook-abort.mjs --case cgroup-cooperative-child-spawn --matrix` | Exit 0. **Cooperative tier (required, both runtimes):** isolated authenticated Claude and Codex are each placed in a delegated **domain** cgroup leaf via **atomic/gated placement** (`clone3 CLONE_INTO_CGROUP` or a pre-exec stop/GO handshake — no runtime code forks before verified membership); each runs its real sandbox (Codex with its normal `bwrap`), spawns an actual tool child (a textual claim fails) that creates nested user/mount/PID/cgroup namespaces where supported plus a double-forked held grandchild; the harness asserts every host-visible PID stays under the leaf, then `cgroup.freeze`→`frozen=1`→`cgroup.kill` during a fork-storm and requires `populated 0` with **no surviving host PID under the leaf**. Assert `cgroup.type=domain` and writable `cgroup.kill`; **fail closed on threaded topology** (`cgroup.kill`→`EOPNOTSUPP`). Rows print `PASS cgroup_cooperative_spawn runtime=<runtime> gated=<clone3_into_cgroup|stop_go> domain=1`. **Filtered hardening (Claude-only):** Claude additionally runs an ordinary child under the exact anti-escape filter (hash matches A5/A1), prints `PASS filtered_cgroup_hardening runtime=claude clone3_errno=ENOSYS filter_sha256=<hex>`; Codex under that filter is EXPECTED to fail (`bwrap` needs the denied `unshare`/`mount`) and prints `filtered_cgroup_hardening=unavailable runtime=codex reason=bwrap_namespace_denied`, which does NOT reduce Codex's cooperative-tier availability. The separate raw clone3 namespace-denial row still passes. If a runtime cannot achieve gated placement or a descendant escapes the leaf, that runtime is `ConfinedCgroup`-unavailable and A13 proves WorkerTree cannot be selected for it. |
...
| A9 | Every mutator has a concrete matching capability. | `node plugins/session-relay/test/reentry-inventory.mjs` | Exit 0; scans every drain, resume/inject/start/interrupt/clean, process creation/exec/signal, and pending-ack callsite. Each mutator maps to a target-free `ReentryGuard` API or one of the three sealed `FencePermit` APIs; no rationale-only `FenceControl`/`UnmanagedOnly` mutator exists; pure ReadOnly exceptions are source-verified. **First-birth process/thread creation — Codex CLI `Command::spawn` (runtime id unknown until its SessionStart hook) and app-server `thread/start` (thread id returned only by the mutation) — is inventoried as a source-verified NON-REENTRY CREATION class: it maps to no `ReentryGuard`/`FencePermit` because it creates a NEW Unmanaged target rather than re-entering an existing managed/fenced one, and its fence-safety is enforced by the Step-2 binding-epoch/duplicate/conflict path at SessionStart registration (a colliding id cannot smuggle into a fenced session). The inventory proves each birth callsite creates-new and never targets an existing session; no `BirthPermit` is minted.** source-derived N has no stale/missing rows. |
...
| A13 | Proof validation and fence capabilities are exhaustive, scope-safe, epoch-bound, and threat-model-labeled. | `cargo test --manifest-path plugins/session-relay/rust/Cargo.toml --locked fence_proof_ -- --nocapture` | Exit 0; every backend×scope×evidence row passes; no permit mints without all bound-session plus activity exclusives; `finish_fence` consumes matching permit and CASes exact epoch/version; stale permit cannot act/finish; freeze/kill without durable flush cannot build `DedicatedOfflineBoundaryProof`; cgroup WorkerTree (`ConfinedCgroupCooperative`) requires `CooperativeWorkerV1`, gated leaf placement, `cgroup.type=domain`, kill/populated-0, and that runtime's successful **cooperative** A5b spawn receipt (available to both Claude and Codex); `FilteredCgroupHardening` is an additional Claude-only hardening capability that never gates the cooperative WorkerTree and never upgrades it to an adversarial claim; a runtime that fails gated placement or leaf containment can never select `ConfinedCgroup`; FencePermit/ReentryGuard capabilities cannot cross; mismatched generation/boundary and unknown variants fail closed. |
...
- A first-birth (Codex `Command::spawn` with hook-assigned id, app-server `thread/start` with returned id) is a CREATION, not a re-entry: it has no pre-existing target to bind, so it takes no `ReentryGuard`/`FencePermit`. Do not fabricate a provisional target or mint a `BirthPermit`. Its only fence-relevant risk — a birth colliding with an existing session id — is closed by the Step-2 binding-epoch/duplicate/conflict path at SessionStart registration, and A9 inventories births as a source-verified non-reentry class.
...
- **“FEASIBILITY FIRST. Step 1 must be a research/feasibility spike; design only what the platforms actually allow.”** Draft-5 preserves confirmed facts and makes absent graceful-flush/cgroup/runtime-spawn capabilities explicit unavailability, never optimistic proof.
...
- Cooperative cgroup placement is non-atomic (any runtime code forks before the manager verifies leaf membership — the spawn-then-move race), the worker leaf is not `cgroup.type=domain` with a writable `cgroup.kill` (threaded topology → `EOPNOTSUPP`), a bwrap-nested or double-forked descendant survives under a different cgroup after `cgroup.kill`, or `populated 0` is claimed while any host-visible PID remains under the leaf.
...
- A1 can pass with fabricated booleans, A5b does not run both real CLIs on the cooperative cgroup tier with gated placement (or accepts a textual tool claim, a missing wait sentinel, or a descendant escaping the leaf), A6 does not prove isolated hooks loaded, A7 never starts real writers, or A9 uses a fixed expected count.
...
- **Resolved 2026-07-11** — `codex-cooperative-cgroup-tier`: the finding is that under the *exact anti-escape strong-cgroup filter* (which EPERM-denies `unshare`/`mount`/`CLONE_NEW*`), **Claude passes** the real child+wait spawn (`clone3`→`ENOSYS`→legacy `clone`) but **Codex fails** because it wraps every tool `exec` in `bwrap`, which needs exactly `unshare(CLONE_NEWUSER)`+`mount` (`bwrap: No permissions to create new namespace`; `spawn.codex.candidate-filter child_complete:false`, same `filter_sha256` as Claude's PASS). Codex is the primary fan-out worker, so the as-written A5b/A13 routing (`strong_cgroup=unavailable` → `ProcessOnly`) would deny the cap's strongest boundary to its main target. Owner deferred to gpt-5.6-sol's expert conclusion; it selected **Option A** and the orchestrator independently verified the kernel semantics (converging). **Decision — Option A:** add a **`ConfinedCgroupCooperative`** WorkerTree tier = delegated *domain* cgroup leaf + `cgroup.kill` + `cgroup.freeze` + `populated 0`, **without** the anti-escape seccomp filter, available to **both** runtimes under `CooperativeWorkerV1`; retain the anti-escape-filtered variant as a **Claude-only `FilteredCgroupHardening`** upgrade (extra hardening, explicitly **not** adversarial containment — same-UID brokers and writable cgroup interfaces stay outside it). Kernel basis (verified): cgroup v2 membership is orthogonal to namespaces — `CLONE_NEWUSER/NEWNS/NEWPID/NEWCGROUP` never migrate membership (migration needs an explicit `cgroup.procs` write or `clone3 CLONE_INTO_CGROUP`), upstream Codex/bwrap emit no `cgroup.procs` write, and `cgroup.kill` recursively SIGKILLs the leaf+descendants across all namespaces. **Binding must-haves folded into Steps 4/8 + A5b/A13/STOP:** (1) **atomic/gated initial placement** — the runtime executes/forks no code before the manager has moved it into the leaf and verified membership, via `clone3(CLONE_INTO_CGROUP)` or a pre-exec stop/GO handshake (closes the spawn-then-move race where a pre-placement fork escapes the leaf and survives `cgroup.kill` while `populated 0` lies); (2) assert `cgroup.type=domain` and `cgroup.kill` writable, **fail closed on threaded topology** (`cgroup.kill` → `EOPNOTSUPP` on threaded; no controller need be enabled for kill/freeze/populated); (3) real-runtime acceptance: run Codex with its normal bwrap sandbox, create nested user/mount/PID/cgroup namespaces + a double-forked held grandchild, freeze→`frozen=1`→`cgroup.kill` during a fork-storm, require `populated 0` with **no surviving host-visible PID under the leaf**; (4) test delegation permissions + common-ancestor containment; label `systemd-run`/D-Bus/`SCM_RIGHTS`/pre-existing-daemon as `outside_threat_model`; (5) re-run the ordinary-Codex cgroup-membership test against runtime upgrades to catch future migration behavior. Attribution: [codex gpt-5.6-sol xhigh] recommend A + taxonomy correction (`FilteredCgroupHardening`) + spawn-then-move + threaded-domain must-haves; [claude] independently verified cgroup-membership-vs-namespace semantics and the no-default-escape claim before accepting; Option B (`ProcessOnly` for codex) and Option C (codex without bwrap) both rejected — B discards the strongest cooperative crash/runaway boundary, C strips codex's own filesystem sandbox for a filter the threat model doesn't require.
...
- **Step-1 feasibility result (2026-07-11, orchestrator-run in the real authenticated env; harness on `codex/primitives-impl`, git-blob `02cea7b`, commit `c32aafa`):** `RELAY_REAL_RUNTIME_TEST=1 node …feasibility-probe.mjs --verify-current` → **PROBE_EXIT=0**, evidence `sha256=76728fa8…`, chain head `bfd40220…`, 49 records, hash chain re-validated intact. Verdicts: `shared_protocol=observation_only`; `worker_tree_threat_model=cooperative`; `protocol_tree=unavailable` (no durable-flush contract — expected/by-design); `protocol_lineage` deadline 20000 ms, measured **609 ms**; 12 hook rows; 20/20 attach-timing samples available (468–2360 ms) → **`managed_attach_deadline_ms=4360`, `within_20s=true`** (STOP "deadline >20s" cleared); **Claude `strong_cgroup=available`** (real child+wait under the exact filter, `clone3_errno=ENOSYS`, `filter_sha256=5aed8e75…`); **Codex `strong_cgroup=unavailable`** — real per-runtime finding: codex's `bwrap` tool-sandbox needs `unshare`/`mount`/`CLONE_NEW*` that the anti-escape filter EPERM-denies (`bwrap: No permissions to create new namespace`); under the as-written model this would route codex to a weaker tier. **A1 PASSES; no STOP tripped.** The codex result is now **resolved** as `codex-cooperative-cgroup-tier` → **Option A**: codex gets `ConfinedCgroupCooperative` (delegated domain leaf + gated placement + `cgroup.kill`/freeze/populated-0, no anti-escape filter); the filtered spawn is a Claude-only `FilteredCgroupHardening` upgrade. Darwin CI (A16) remains a separate owner-push-gated step. Raw transcripts stay in the ephemeral harness artifact dir, not pasted here.
