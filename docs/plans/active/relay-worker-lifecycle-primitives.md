...
updated: "2026-07-11T12:50:48-03:00"
...
| Cgroup tree | Under the cooperative-worker threat model, execute the ordered move-before-cgroup-unshare setup; retain the exact manager fd; isolate user/mount/PID/cgroup/proc views; close inherited authority; install architecture-correct seccomp whose wholesale `clone3` denial is exactly `SECCOMP_RET_ERRNO|ENOSYS`; prove both real runtimes can fall back to safe legacy-clone ordinary child/tool spawn; freeze only as an intermediate aid; then `cgroup.kill` and require `populated 0`. | Plain delegation, reordered/partial setup, manager-fd loss, failed ABI/procfs probes, `clone3` returning any errno other than `ENOSYS`, failed ordinary-spawn compatibility, or a claim against broker-assisted same-UID escape is unconfirmed even if kill succeeds and populated 0 is observed. Strong cgroup is unavailable for each runtime that lacks a safe fallback. |
...
6. **Adversarial and compatibility gates, then GO.** Preserve the raw-syscall namespace-denial fixture in the exact namespaces: it must prove `clone3` creates no child and returns `-1/ENOSYS`; namespace flags through legacy `clone` remain denied; x32/mismatched-arch, ancestor/sibling write, remount, new namespace, inherited-fd, alternate proc/cgroup mount, and `/proc/<pid>/{fd,fdinfo,root,cwd}` attacks still fail. Separately, run A5b with each real runtime under the exact installed filter and require ordinary shell/tool child creation, descendant completion, wait, and ordered sentinel output. Probe the direct attacks before/during fencing. Only a runtime passing the compatibility gate may receive GO with `ConfinedCgroup`; a runtime without a safe legacy-`clone` fallback records strong-cgroup unavailable. The manager re-verifies leaf membership and the receipt, then sends GO. Under the cooperative-worker threat model, freeze may establish a temporary stable view, but Fenced requires `cgroup.kill` plus `populated 0`; the cgroup is never thawed through release.
...
| A5b | Exact-filter ordinary child creation remains functional in both managed runtimes. | `RELAY_REAL_RUNTIME_TEST=1 node plugins/session-relay/test/runtime-hook-abort.mjs --case strong-cgroup-child-spawn --matrix` | Exit 0; isolated authenticated Claude and Codex each run beneath the exact installed filter whose hash matches A5/A1. For each runtime the harness requires an actual shell/tool invocation (a textual claim fails), that shell starts an ordinary descendant, waits for it, and writes ordered `child-complete` then `wait-complete` sentinel records; exit/wait statuses are zero and no process survives. Available rows print `PASS strong_cgroup_spawn runtime=<runtime> clone3_errno=ENOSYS filter_sha256=<hex>`. If either runtime has no safe legacy-clone fallback, that row instead prints `strong_cgroup=unavailable runtime=<runtime> reason=no_safe_legacy_clone_fallback`, and subsequent A5/A13 matrices prove `ConfinedCgroup`/WorkerTree cannot be selected for it. The separate raw clone3 namespace-denial row still passes. |
...
- A1 can pass with fabricated booleans, A5b does not run both real CLIs under the exact installed filter or accepts a textual tool claim/missing wait sentinel, A6 does not prove isolated hooks loaded, A7 never starts real writers, or A9 uses a fixed expected count.
...
- `codex-cooperative-cgroup-tier` (**choice**, custom allowed) — **NEW, from Step-1 real-runtime evidence; blocks the Steps 2-8 tier taxonomy.** The Step-1 probe (run by the orchestrator in the real authenticated env) proves an unanticipated fact: under the *exact anti-escape strong-cgroup filter* (which EPERM-denies `unshare`/`mount`/`CLONE_NEW*` to stop cgroup/namespace escape), **Claude passes** the real child+wait spawn (spawns tool children directly → `clone3`→`ENOSYS`→legacy `clone`, allowed), but **Codex fails**: codex wraps every tool `exec` in `bwrap`, which needs exactly `unshare(CLONE_NEWUSER)`+`mount`, so the filter kills it (`bwrap: No permissions to create new namespace`). Evidence: `spawn.codex.candidate-filter` `child_complete:false`, same `filter_sha256` as Claude's PASS. As written, A5b/A13 route codex → `strong_cgroup=unavailable` → codex workers can never select `ConfinedCgroup`/WorkerTree and fall to `ProcessOnly`. **Consequence the plan did not surface:** codex (gpt-5.6-sol) is the *primary fan-out worker*, so the cap's strongest kill-boundary — the reason these primitives exist — would not apply to the workers it most needs to control. The anti-escape filter is *not required* under the confirmed `CooperativeWorkerV1` (it defends against deliberate escape, already out of scope); the cgroup kill-boundary itself (delegated leaf + `cgroup.kill` + `populated 0` + freeze) contains codex's bwrap subtree without it.
  - **Option A (recommended):** Split the tier taxonomy — add a **`ConfinedCgroupCooperative`** tier = delegated cgroup + `cgroup.kill`/freeze/`populated 0`, *without* the namespace-denying seccomp filter — that **both** runtimes (incl. codex) select under `CooperativeWorkerV1`; keep the anti-escape-filtered tier as a Claude-only adversarial hardening upgrade. Gives every worker a real kill-boundary consistent with the confirmed threat model. Cost: A5b/A13 split into "cooperative-cgroup-kill (both runtimes)" vs "anti-escape-filtered (Claude-only)"; Steps 5-8 and the tier enum grow one variant.
  - **Option B:** Ship as-is — codex workers get only `ProcessOnly` (pidfd/PGID signals, no cgroup kill-boundary); only Claude workers get a cgroup tier. Simpler, but the primary worker gets the weakest containment and the fan-out cap under-delivers its core value for codex.
  - **Option C:** Run codex workers with `--sandbox danger-full-access` (no bwrap) so codex can spawn under the anti-escape filter like Claude. **Not recommended** — strips codex's own sandbox, worse security posture, fights the tool's defaults.
  - **Recommendation: Option A** — it is the direct consequence of the already-confirmed `CooperativeWorkerV1`: the anti-escape filter was B3 hardening the owner already scoped OUT ("adversarial isolation is a future plan"), and under V1 the cgroup kill-boundary alone is the boundary, filter or not.

## Self-review

Score: **95/100** · trajectory **93→95→95→95→95** · stopped: **plateau (K=3)**.
