...
updated: "2026-07-12T02:17:03-03:00"
...
- **Threat-model owner gate:** RESOLVED 2026-07-11 — owner selected Option 1 (`CooperativeWorkerV1`). This resolves the product boundary only; Draft-17 implementation remains paused for immutable review, and Step 4 separately requires A1c+A1d PASS.
...
- **Feasibility-first exec-stop gate:** before Step 4, A1c runs the production-shaped blocked-child/host-PID/fd-exec protocol independently for Claude and Codex. The supervisor must seize the exact reported task, observe that same tracee at `PTRACE_EVENT_EXEC`, open the procfs magic-link target while it is stopped, compare the opened file's fstat/content hash to the retained executable fd, and detach only after the target-code sentinel is still absent. Any runtime row that cannot meet this contract is `unavailable`; STOP before Step 4 rather than inventing a weaker launch.
- **Owner-provisioned positive Linux runner:** authoritative managed-controller/fd-exec and WorkerTree positives are Linux-only in this plan and require an owner-provisioned delegated cgroup-v2 runner before Step 4/A5b. The runner must set `RELAY_CGROUP_ROOT` to a writable delegated domain owned by the test uid, on a read-write cgroup2 mount, with permission to create/move into a disposable child and with `cgroup.kill`, `cgroup.events`, and `cgroup.procs` usable in that child; the kernel must support pidfd, `clone3(CLONE_INTO_CGROUP)` or the separately proven stop/GO fallback, ptrace seize/TRACEEXEC, and `execveat(AT_EMPTY_PATH)`. Provisioning may use a system service/unit with `Delegate=yes` or equivalent CI runner setup, but the tests do not attempt to grant themselves delegation. The current orchestrator sandbox has a read-only cgroup2 view and no user bus, so it is explicitly **not** a positive runner. A1d creates and removes one disposable leaf under the supplied fd-root, validates mount id/dev/inode/domain/writable control files plus a moved stopped child and `cgroup.kill`/`populated 0`, emits a canonical hash-bound preflight receipt, and fails (never skips) when the runner is absent or inadequate. No Step-4 implementation or A5b completion claim may begin without A1c+A1d PASS.
...
Controller-pump exit while the supervisor remains live, app-server stdio EOF, malformed/ambiguous response, request-map corruption, or an unsupported/unhandled/unknown server request first atomically batch-marks every matching in-flight operation `LostAuthority(CustodyLost)` and applies the closed lifecycle transition. In the same still-live supervisor/fence generation it then immediately consumes the exact physical-fence authorization, closes app-server stdin/stdout/stderr, emits no further application bytes, kills the retained cgroup/owned child, and bounded-waits for reap plus `populated 0`. Physical cleanup never upgrades or clears the logical failure: custody remains unconfirmed/failed and cannot release capacity without the independently valid same-fence WorkerTree proof required by this plan. If the supervisor/cgroup authority is already lost, the batch loss persists and cleanup remains unconfirmed; it never reconnects, swaps in a new transport, or guesses a response. A later relay process can wake an idle Active worker only through the same controller instance/control epoch and typed admitted command; a stale controller/socket/sequence emits zero app-server bytes. Tests keep the original launcher dead, then use a second relay process for read/inject/start/interrupt; separately kill only the controller pump, only the app-server child, and only the initiating caller, and exercise concurrent numeric/string request ids, cross-thread terminal substitution, every source-derived server-request policy, unknown methods, zero bytes after loss, and cleanup completion/failure inside `managed_cancel_grace_ms`.
...
```sh
/bin/sh -c 'plugin=$1; shift; root=${XDG_DATA_HOME:-${HOME:?HOME required}/.local/share}/docks/session-relay/runtime; if test -L "$plugin"; then exit 4; fi; if test ! -e "$plugin"; then exec "$root/current/relay" hook codex "$@"; fi; if test ! -d "$plugin"; then exit 4; fi; launcher=$plugin/bin/relay; if test -L "$launcher" || test ! -f "$launcher" || test ! -x "$launcher"; then exit 4; fi; "$launcher" __install-stable --plugin-root "$plugin" --json >/dev/null; rc=$?; if test "$rc" -eq 0; then exec "$root/current/relay" hook codex "$@"; fi; if test ! -e "$plugin" && test ! -L "$plugin"; then exec "$root/current/relay" hook codex "$@"; fi; exit "$rc"' relay-hook "${CLAUDE_PLUGIN_ROOT}"
/bin/sh -c 'plugin=$1; shift; root=${XDG_DATA_HOME:-${HOME:?HOME required}/.local/share}/docks/session-relay/runtime; if test -L "$plugin"; then exit 4; fi; if test ! -e "$plugin"; then exec "$root/current/relay" hook codex "$@"; fi; if test ! -d "$plugin"; then exit 4; fi; launcher=$plugin/bin/relay; if test -L "$launcher" || test ! -f "$launcher" || test ! -x "$launcher"; then exit 4; fi; "$launcher" __install-stable --plugin-root "$plugin" --json >/dev/null; rc=$?; if test "$rc" -eq 0; then exec "$root/current/relay" hook codex "$@"; fi; if test ! -e "$plugin" && test ! -L "$plugin"; then exec "$root/current/relay" hook codex "$@"; fi; exit "$rc"' relay-hook "${CLAUDE_PLUGIN_ROOT}" --event prompt
```

Shell/argv quoting is emitted literally by `codex-hooks.json`; neither cwd nor hook input is interpolated into code. Installer JSON is redirected so complete hook stdout contains exactly one hook response object. Any plugin-root symlink—including a dangling symlink detected by `test -L`—fails explicitly; only a truly absent, non-link root (`! -e && ! -L`) falls through to stable `current/relay`. When N's cached plugin root remains an exact real directory, the launcher must be a non-symlink regular executable and validate/install; any failure while the root still exists or is replaced by any link/non-directory propagates. If the real root disappears after the directory check, before launcher exec, or during pinned manifest/checksum reads, the command rechecks both `-e` and `-L` after the failure and falls through only when the whole root is now truly absent. A present non-directory/symlinked root or missing/nonexec/symlinked launcher fails. Missing/invalid `current` fails visibly; it never searches cache directories. A6b uses deterministic barriers for every prune point, dangling/non-dangling root-link cases, and parses complete stdout as exactly one event-correct hook JSON object.
...
| 1b | After preserved Step 3b is clean/committed, run the one-time A0c handoff, create the reusable WIP snapshot helper plus closed per-step allowlist, and migrate feasibility evidence to independent `cooperative_cgroup` and Claude-only `filtered_hardening`; prototype raw Codex `thread/start→controller claim candidate→turn/start` ordering (not production Rust claim), inventory experimental capability, define semantic hook-health snapshots, and prove stable-runtime generation/pointer feasibility. | `plugins/session-relay/test/feasibility-probe.mjs`, `plugins/session-relay/test/fixtures/lifecycle-capability-schema.json`, `plugins/session-relay/test/fixtures/wip-step-allowlist.json` (new), `plugins/session-relay/test/runtime-hook-abort.mjs` (new), `plugins/session-relay/test/runtime-hook-upgrade.mjs` (new), `plugins/session-relay/test/wip-snapshot.mjs` (new) | 1, 3b | planned | A0c/A0b/A1/A1b pass. Helper reproduces empty state and rejects every dirty class; source-derived fixture closes every remaining step/path/mode mapping. Raw trace proves SessionStart-before-thread-response and zero turn before a prototype claim decision without pretending JS is shipped lifecycle logic. Stable generation/pointer survives pruning/crash/races. No implementation-branch plan/doc edit is permitted. |
| 1c | Feasibility-first prototype the exact Linux blocked-child→host-PID handoff→seize/TRACEEXEC→retained-fd `execveat`→same-tracee exec-stop identity sequence independently for Claude and Codex. This is raw evidence, not the Step-4 production implementation. | `plugins/session-relay/test/feasibility-probe.mjs`, `plugins/session-relay/test/runtime-hook-abort.mjs`, `plugins/session-relay/test/fixtures/lifecycle-capability-schema.json` | 1b | planned | A1c passes for both runtime binaries: target sentinel is absent until detach; followed procfs magic-link fd matches retained fd; `O_NOFOLLOW` proves `ELOOP`; every injected mismatch kills/reaps before target code. Any mandatory row unavailable: STOP before Step 4 and report the primitive unbuildable on the proposed Linux path. |
| 1d | Validate the owner-provisioned delegated Linux runner without self-provisioning: pin `RELAY_CGROUP_ROOT`, create a disposable domain leaf, exercise stopped-child placement and `cgroup.kill`/`populated 0`, and emit a host/kernel/mount/root-bound receipt. | `plugins/session-relay/test/runtime-hook-abort.mjs`, `plugins/session-relay/test/fixtures/lifecycle-capability-schema.json` | 1b | planned | A1d passes on the named owner-provisioned runner. The current read-only-cgroup/no-user-bus sandbox is not eligible. Missing delegation/runner is a blocking prerequisite, never a skip; no Step 4 or A5b completion claim begins until resolved. |
...
| 8 | Complete the already-born per-step adversarial matrices and enforce inventory **completeness only**; preserve all existing relay tests and isolated cleanup. Verify that Steps 3b/3c/4/5/6 each added their target/case before their first acceptance gate, then auto-enumerate compile-fail fixtures and close any remaining source-derived rows. Include watchdog/supervisor failures, authority races, every loss source/reason, cancellation, controller birth/claim/command authorization, GC, exec-stop/cgroup receipts, hook/installer/schema races, protocol-attempt mixing, proof-retention crash, and missing-flush rows. | `plugins/session-relay/rust/src/appserver.rs`, `plugins/session-relay/rust/src/hook.rs`, `plugins/session-relay/rust/src/lifecycle.rs`, `plugins/session-relay/rust/src/process_identity.rs`, `plugins/session-relay/rust/src/runtime_install.rs`, `plugins/session-relay/rust/src/supervisor.rs`, `plugins/session-relay/rust/src/spawn.rs`, `plugins/session-relay/rust/src/store.rs`, `plugins/session-relay/test/rust-test-inventory.mjs`, `plugins/session-relay/test/fixtures/rust-test-inventory.json`, `plugins/session-relay/test/feasibility-probe.mjs`, `plugins/session-relay/test/supervisor-custody.mjs`, `plugins/session-relay/test/lifecycle-smoke.mjs`, `plugins/session-relay/test/process-signal-inventory.mjs`, `plugins/session-relay/test/reentry-inventory.mjs`, `plugins/session-relay/test/runtime-hook-abort.mjs`, `plugins/session-relay/test/runtime-hook-upgrade.mjs`, `plugins/session-relay/test/runtime-appserver-quiescence.mjs`, `plugins/session-relay/test/fake-app-server.mjs`, `plugins/session-relay/test/selftest.mjs` | 2-7 | planned | A1/A1b/A1c/A1d through A14 pass; fixture history/source checks prove no target was first introduced after its gate. Every target executes a source-derived nonzero required set; every compile-fail bin fails at its intended boundary; helpers clean in `finally`; external evidence establishes results. Step 8 may not retroactively make an earlier zero/missing target green. |
| 9 | Document guarantee tiers, Linux-only managed controller/fd-exec availability, Darwin closed unavailability plus portable supervisor, stable hook runtime, `CooperativeWorkerV1`, cancellation, current ProtocolTree unavailability, cgroup evidence, GC, and recovery; remove the stale non-authoritative implementation-branch plan copy so the final range contains zero `docs/plans/**`; add exact-SHA Darwin matrix, exact final-range allowlist/mode-union verification, disposable diff-class negatives, and full gates without binary/release changes. | `plugins/session-relay/AGENTS.md`, `plugins/session-relay/skills/productivity/session-relay/SKILL.md`, `plugins/session-relay/rust/src/main.rs`, `plugins/session-relay/test/run-build-matrix.mjs` (new), `plugins/session-relay/test/final-scope.mjs` (new), `.github/workflows/build-binaries.yml` | 1b-8 | planned | A15-A18 pass. `final-scope` derives the exact allowed path+mode union from the committed per-step allowlist, prints the canonical actual-range manifest/hash, and rejects old/new/add/delete/rename/copy/type/mode/submodule/status deviations plus every docs/binary/version class. Main plan remains source of truth; implementation range has no plan/docs delta. Darwin runs portable behavior and reports authoritative managed path unavailable. |
...
| A1c | The exact retained-fd/TRACEEXEC identity architecture is buildable for each mandatory Linux runtime before production work. | `RELAY_REAL_RUNTIME_TEST=1 node plugins/session-relay/test/feasibility-probe.mjs --case exec-stop-feasibility --matrix` | Exit 0 for Claude and Codex native binaries. Each raw row proves blocked final child, exact host PID handoff, ancestor `PTRACE_SEIZE|PTRACE_O_TRACEEXEC`, retained-fd `execveat`, exact event PID equal to seized tracee, target sentinel absent, followed `/proc/<pid>/exe` magic-link fd fstat/hash equal to retained fd while stopped, then detach and sentinel. `O_NOFOLLOW` yields `ELOOP`; wrong PID/event/hash/early release/timeout kills and reaps before sentinel. Any mandatory row unavailable is a hard STOP before Step 4. |
| A1d | The named owner-provisioned Linux runner supplies real writable cgroup-v2 delegation. | `RELAY_REAL_RUNTIME_TEST=1 node plugins/session-relay/test/runtime-hook-abort.mjs --case delegated-runner-preflight` | Exit 0 only with nonempty `RELAY_CGROUP_ROOT` on a read-write cgroup2 mount. It pins the root dirfd/mount/dev/inode, validates same-uid writable domain delegation, creates a unique disposable child leaf, validates empty subtree plus writable `cgroup.procs|cgroup.kill|cgroup.events`, moves a stopped child, kills it, observes `populated 0`, removes the leaf, and emits a host-boot/kernel/root-bound receipt hash. Absent/read-only delegation or cleanup failure is FAIL/BLOCK, never skip. |
...
| A5 | Linux tree proof requires same-tracee fd-bound exec-stop placement and same-fence kill evidence. | `RELAY_REAL_RUNTIME_TEST=1 node plugins/session-relay/test/lifecycle-smoke.mjs --case containment` | Exit 0 only after A1c+A1d. Production `launch_confined` verifies private host-PID handoff, seized tracee/event PID equality, gated leaf placement, retained-fd exec, exact TRACEEXEC stop, followed procfs magic-link fd fstat/hash equality while stopped, then detach/GO. `O_NOFOLLOW` gets `ELOOP`. Path replacement executes retained original or refuses. Forged/substituted PID, early release, seize/event timeout, wrong event, proc fd/hash mismatch, exec/detach failure, scripts/symlinks, membership mismatch, and target sentinel before detach all kill/reap and fail. Same constructor mints kill receipt; stale/cache/boot/leaf changes refuse. |
| A5b | The owner-provisioned delegated Linux runner drives BOTH real runtimes through the production confined launcher. | `RELAY_REAL_RUNTIME_TEST=1 node plugins/session-relay/test/runtime-hook-abort.mjs --case cgroup-cooperative-child-spawn --matrix` | Exit 0 only on the same receipt-validated `RELAY_CGROUP_ROOT` runner from A1d. Harness calls production supervisor/`launch_confined`, captures tracee/event/fd/proc-identity/placement and same-fence kill receipts, creates real child/grandchild/storm under leaf, and reaches populated-zero. Target sentinel proves no target instruction before detach. A second worker mints a distinct receipt. Cached/fixture evidence cannot authorize GO. Missing runner is BLOCK, not skip; Claude filtered hardening stays separate. |
| A6 | Real Linux managed admission and durable controller are runtime-correct and fail closed on custody loss. | `RELAY_REAL_RUNTIME_TEST=1 node plugins/session-relay/test/runtime-hook-abort.mjs --matrix` | Exit 0. Claude barriers pass. Codex pending creation atomically mints one sealed `ManagedBirthPermit`; only its exact Attaching tuple launches once, and replay/wrong tuple emits zero bytes. SessionStart remains non-claiming; controller alone claims Active; transcript has zero `turn/start` before durable Active. Foreign/refusal rows preserve foreign state. After caller exit a second invocation uses one typed result. Every unsupported/unknown/custody-loss row atomically records loss, closes all stdio, emits no further bytes, immediately same-fence kills when custody is live, and bounded-reaps/observes populated-zero while logical failure remains unconfirmed. No CLI fallback/bypass. |
| A6a | Managed Codex controller, schema, and command authority are complete after Step 5. | `node plugins/session-relay/test/rust-test-inventory.mjs --case lifecycle_controller && (cd plugins/session-relay/rust && cargo test --locked --test lifecycle_controller -- --nocapture) && node plugins/session-relay/test/appserver-schema-contract.mjs --verify-current --codex "$(command -v codex)" && node plugins/session-relay/test/reentry-inventory.mjs && node plugins/session-relay/test/lifecycle-smoke.mjs --case reentry-matrix` | Exit 0 with nonzero fixture-backed IDs. Birth permit, token secrecy, pending→thread/start→claim→ControllerReady, caller death, exact retries/loss, `ControllerRead|ControllerInjectItems|ControllerStartTurn`, command authorization consumption, cross-family/replay/expiry zero-byte behavior, and loss cleanup pass. Schema verification records invoked+canonical native path, uses one retained no-follow fd for generator and server, survives retarget/replacement barriers, canonicalizes `<OUT>`, fixed modes/umask/relative sorting, and proves byte-identical candidates from two temp roots. Exact schema/version/identity matches before pending creation/Active. |
| A6b | Source-built N/N+1 payloads exercise exact hooks and fd-pinned crash-atomic monotonic selection. | `RELAY_REAL_RUNTIME_TEST=1 node plugins/session-relay/test/runtime-hook-upgrade.mjs --matrix` | Exit 0. Real Codex uses synthetic N/N+1 payloads with exact hook JSON, both manifests, checksums, and hook/payload identities. Old UserPromptSubmit and new/subagent SessionStart execute through stable current; stdout is one hook JSON. Any plugin-root symlink including dangling fails; only truly absent non-link root falls through. Dirfd/openat barriers rename/recreate every manifest/checksum/hook/payload/selected-binary component; installer either fails or copies/hashes its pinned fd bytes without mixing. Same-version no-op requires equality of version,target,binary,payload,hook; each single-field mismatch is fatal. Lock/generation/pointer crash/races pass. Implementation evidence is not packaged evidence. |
...
| A18 | The exact clean implementation tip matches the closed path+mode union and contains no binary/release/docs mutation. | `test "$(git branch --show-current)" = codex/primitives-collab && test "$(git rev-parse HEAD)" = "$IMPL_TIP" && test -z "$(git status --porcelain)" && git diff --check 12cf2ea.."$IMPL_TIP" && node plugins/session-relay/test/final-scope.mjs --verify-range --base 12cf2ea --tip "$IMPL_TIP" --allowlist plugins/session-relay/test/fixtures/wip-step-allowlist.json --emit-manifest /tmp/relay-final-scope-manifest.json && node plugins/session-relay/test/final-scope.mjs --negative-matrix --base 12cf2ea --tip "$IMPL_TIP" --allowlist plugins/session-relay/test/fixtures/wip-step-allowlist.json` | Exit 0 with exact reviewed HEAD/clean status and `FINAL_SCOPE PASS files=<N> manifest_sha256=<hex>`. Helper derives the exact allowed path+mode union from the committed per-step allowlist and parses raw diff/status metadata for old/new path, add/delete/rename/copy, regular/symlink/gitlink/type change, executable-bit/other mode change, submodule, staged/unstaged/untracked, and duplicate/overlap cases. Every unexpected class fails independently; no wildcard/regex path allowance exists. The emitted canonical actual-range manifest is outside the repo and its hash is independently recorded. Net `docs/plans/**`, bin, manifest/version, marketplace, tag/release surface is empty. |
...
- A1c cannot prove the same-seized-tracee TRACEEXEC/retained-fd/proc-magic-link identity contract for either mandatory Linux runtime, or A1d lacks an owner-provisioned writable delegated cgroup-v2 runner before Step 4/A5b. The current read-only cgroup/no-user-bus sandbox cannot be relabeled positive.
...
- Any plugin-root symlink including dangling falls through, any present non-directory/root-link ambiguity reaches stable execution, installer validation reopens a source pathname after pinning, selected payload hashing/copying use different fds, rename/recreate barriers mix source generations, or same-version no-op ignores target/payload/hook identity.
...
- Unsupported/unknown controller input or custody loss fails to atomically preserve logical loss before cleanup, emits bytes after loss, leaves stdio open, omits immediate same-fence physical kill when authority is live, or treats a successful kill/reap as logical custody confirmation.
...
- A18 uses a wildcard/regex allowance, omits old/new paths or mode/type/gitlink/status classes, accepts a path/mode outside the exact committed per-step union, or fails to emit/recheck the canonical actual-range manifest hash.
...
2. **Environment & commands:** present — path-specific plan/blob, preserved-WIP bridge, helper hashes/step manifest, cross-filesystem handoff, four targets, shell-first stable runtime, isolated real-runtime setup, measured deadline, feasibility-first exec stop, and the owner-provisioned delegated Linux runner prerequisite are explicit. Current host ineligibility is recorded, not silently skipped.
3. **Interface & data contracts:** present — exact-tuple one-use ManagedBirthPermit, non-claiming SessionStart plus controller-only claim, durable app-server controller, exact controller OperationKinds, typed loss-then-physical-cleanup, stable cancellation/custody versions, nonce/control epoch, executable-bound cgroup receipts, attempt-bound proof, fd-pinned immutable runtime generations, deterministic schema, doctor/install schemas, tombstone GC, and operator APIs are explicit.
4. **Executable acceptance:** present — A0/A0b-bootstrap/A0b–A18 plus A1b/A1c/A1d/A3a–A3d/A6b/A6c are command + expected evidence, including same-tracee TRACEEXEC/proc-magic-link identity, delegated runner, per-step nonzero inventory birth, controller A9/A10 rerun, source-fd and schema determinism races, live cache upgrade, later-process controller wake, multi-operation loss cleanup, Darwin unavailable rows, and exact actual-range manifest/hash.
...
Adversarial cold-read result: Draft-17 preserves Draft-16's verified HooksList/request-user-input/controller-wire closures and repairs both immutable Draft-16 reviews: managed birth now has representable exact authority; controller variants exist and are re-inventoried after Step 5; every Rust target is born before its gate; TRACEEXEC is feasibility-first and compares a followed procfs magic-link fd at the exact same-tracee stop; source installers/schema generation are fd-pinned and deterministic; hook symlinks fail closed; controller loss is recorded before bounded physical cleanup; Darwin is honestly unavailable for the authoritative path; A18 verifies an exact path+mode union and manifest hash; and positive cgroup evidence is blocked on a named owner-provisioned runner rather than falsely claimed on this host. Implementation remains paused for immutable dual re-review. Step 4/A5b additionally remain blocked until A1d identifies a passing runner. Remote push/dispatch and packaged producer/release gates remain owner approvals.
...
Score: **96/100 (Draft-17 author candidate; independent re-review pending)** · trajectory **95 Draft-16 author → 79 architecture / 82 acceptance NOT READY → 96 authority/feasibility/TOCTOU/platform repair** · stopped: **candidate must be frozen for fresh cross-company review; implementation remains paused; Step 4 also requires A1d runner PASS**.
