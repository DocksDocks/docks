...
updated: "2026-07-12T01:23:31-03:00"
...
The committed `appserver-server-requests.json` is generated from the exact supported Codex `app-server generate-json-schema` output and source-derived enum, and must enumerate every method once. For the currently researched contract it contains `item/commandExecution/requestApproval`, `item/fileChange/requestApproval`, `item/tool/requestUserInput`, `mcpServer/elicitation/request`, `item/permissions/requestApproval`, `item/tool/call`, `account/chatgptAuthTokens/refresh`, `attestation/generate`, `currentTime/read`, and legacy `applyPatchApproval`/`execCommandApproval`. The managed controller advertises no interactive/dynamic/auth-refresh/attestation/current-time capability. It sends each approval/permission/legacy row its schema-valid decline/cancel response, sends user-input and MCP elicitation the schema-valid canceled response, and keeps custody only after a matching `serverRequest/resolved`; dynamic tool/auth refresh/attestation/current-time or any unknown method receives the version-pinned schema-valid unsupported/error response and then atomically batch-marks in-flight operations `LostAuthority(CustodyLost)`. It never accepts a command/file/permission request, fabricates user input, supplies tokens/attestation, or hangs. A Codex schema/version whose generated set differs is `unavailable` until this fixture and policy are re-reviewed.
...
3. **Pin executable bytes, then gate placement before any runtime code (mandatory, both runtimes).** Resolve the final native executable once; reject symlinks, scripts, and dispatch launchers for the authoritative confined path. Open the regular executable `O_RDONLY|O_NOFOLLOW|O_CLOEXEC`, fstat/hash that fd, and retain it through launch. Either create the child via successful `clone3(CLONE_INTO_CGROUP)` or use the separately proven trusted wrapper blocked on a private handshake pipe and, before it executes runtime code or forks, move/verify the pinned child through the retained cgroup fd. The child executes that exact open fd with `execveat(fd, "", argv, envp, AT_EMPTY_PATH)` (or the proven equivalent fd-exec primitive), never a second pathname lookup. Before GO, compare the kernel child executable identity (`/proc/<pinned-pid>/exe` opened no-follow plus dev/ino/size/content identity) to the retained fd; mismatch kills/reaps and refuses. Bind both fd-derived and child-kernel identities into the placement receipt. Verify `cgroup.type=domain`, empty `cgroup.subtree_control`, and writable `cgroup.kill`. **(Claude-only `FilteredCgroupHardening` continues from here:)** the wrapper must already be in the leaf when it calls `unshare(CLONE_NEWCGROUP)`, so `/` inside that namespace is the worker leaf rather than an ancestor.
...
6. **Adversarial and compatibility gates, then GO.** Preserve the raw-syscall namespace-denial fixture in the exact namespaces: it must prove `clone3` creates no child and returns `-1/ENOSYS`; namespace flags through legacy `clone` remain denied; wrong-arch and x32 actions are asserted exactly; ancestor/sibling write, remount, new namespace, inherited-fd, alternate proc/cgroup mount, and `/proc/<pid>/{fd,fdinfo,root,cwd}` attacks fail in the filtered layer. A deterministic barrier replaces the executable pathname after the fd hash and before exec/GO; the pinned original bytes must execute or launch must refuse, and the receipt must never describe the replacement. Separately, run A5b's **cooperative** gate for each real runtime: gated leaf placement, exact fd-executed native runtime, the real sandbox tool child plus nested-namespace double-forked grandchild/fork storm, a host-PID-stays-under-leaf assertion, then `cgroup.kill`→`populated 0`. Optional freeze is a separate observation and not a GO/proof prerequisite. Only a runtime passing the cooperative gate may receive GO with `ConfinedCgroupCooperative`; a runtime that cannot achieve fd-bound execution, gated placement, or leaf containment records `ConfinedCgroupCooperative` unavailable. Claude additionally attempts the filtered hardening; Codex is expected filtered-unavailable and remains cooperative-tier eligible. The manager re-verifies executable identity, leaf membership, and the receipt, then sends GO.
...
Hook normalization is literal and version-pinned: parse the returned command field according to the exact supported `hooks/list` schema, preserve argument order and bytes, JCS-encode `{key,eventName,handlerType,isManaged,source,pluginId,currentHash,trustStatus,enabled,timeout,command,warnings,errors}`, and SHA-256 those bytes. No shell-semantic equivalence parser, whitespace folding, path resolution, or stderr interpretation is allowed.
...
| 5 | Implement managed first-prompt admission and durable controller. Claude uses Claiming-first SessionStart plus UserPromptSubmit. Codex supervisor generates/holds the raw token, creates the pending record, retains app-server Child/stdio/request map/source-derived server-request policy/notification pump, persists ControllerReady, and alone claims the non-claiming `thread/start` identity before first turn. Later processes use typed admitted one-request controller commands. Add two exact durable shell hook bootstraps, locked generation/pointer installer, doctor/install schemas+goldens, literal hook health, numeric timeouts/dead-man, and unmanaged compatibility. | `plugins/session-relay/rust/src/appserver.rs`, `plugins/session-relay/rust/src/hook.rs`, `plugins/session-relay/rust/src/runtime_install.rs` (new), `plugins/session-relay/rust/src/main.rs`, `plugins/session-relay/rust/src/spawn.rs`, `plugins/session-relay/rust/src/store.rs`, `plugins/session-relay/rust/src/supervisor.rs`, `plugins/session-relay/hooks/hooks.json`, `plugins/session-relay/hooks/codex-hooks.json`, `plugins/session-relay/test/fixtures/appserver-server-requests.json` (new), `plugins/session-relay/test/fixtures/runtime-doctor-schema.json` (new), `plugins/session-relay/test/fixtures/runtime-doctor-goldens/` (new), `plugins/session-relay/test/runtime-hook-abort.mjs` (new), `plugins/session-relay/test/runtime-hook-upgrade.mjs` (new), `plugins/session-relay/test/selftest.mjs` | 1b-4 | planned | A2/A6/A6b/A6c prove token secrecy, production claim ordering, later-process wake, every controller/server/caller/request-policy failure row, source-built real cached hook events after root deletion, crash-atomic locked monotonic selection, independently schema-valid golden output, Claude barrier, and health degradation. Managed CLI fallback, pre-Active turn, arbitrary JSON/target forwarding, reconnect guessing, or non-atomic runtime pair: STOP. |
...
| 8 | Complete adversarial unit/cross-process/real-runtime matrices and source-inventory enforcement; preserve all existing relay tests and isolated cleanup. Add a source-derived Rust suite manifest/nonzero-count gate and auto-enumerating compile-fail runner. Include watchdog/supervisor caller-exit/SIGHUP/crash/startup-phase failures, control-epoch/version races, every lost-authority source/reason, operation CAS losers, cancellation barriers, direct app-server claim/first-turn ordering, GC-vs-claim, per-launch cgroup receipts, optional-freeze rejection, semantic hook-health drift, stable-runtime upgrade/tamper, protocol-attempt mixing, proof-retention crash, and missing-flush rows. | `plugins/session-relay/rust/src/appserver.rs`, `plugins/session-relay/rust/src/hook.rs`, `plugins/session-relay/rust/src/lifecycle.rs`, `plugins/session-relay/rust/src/process_identity.rs`, `plugins/session-relay/rust/src/runtime_install.rs`, `plugins/session-relay/rust/src/supervisor.rs`, `plugins/session-relay/rust/src/spawn.rs`, `plugins/session-relay/rust/src/store.rs`, `plugins/session-relay/test/rust-test-inventory.mjs` (new), `plugins/session-relay/test/feasibility-probe.mjs`, `plugins/session-relay/test/supervisor-custody.mjs`, `plugins/session-relay/test/lifecycle-smoke.mjs`, `plugins/session-relay/test/process-signal-inventory.mjs`, `plugins/session-relay/test/reentry-inventory.mjs`, `plugins/session-relay/test/runtime-hook-abort.mjs`, `plugins/session-relay/test/runtime-hook-upgrade.mjs`, `plugins/session-relay/test/runtime-appserver-quiescence.mjs`, `plugins/session-relay/test/fake-app-server.mjs`, `plugins/session-relay/test/selftest.mjs` | 2-7 | planned | A1/A1b through A14 pass; every named Rust target executes a source-derived nonzero required set; every compile-fail bin is enumerated and fails at the intended authority boundary; helpers clean children/stores in `finally`; externally observable sentinels and kernel/protocol evidence, not elapsed time alone, establish results. Cooperative-scope labels and stable-runtime provenance are machine-checked. |
| 9 | Document guarantee tiers, relay-owned Codex app-server admission, stable hook runtime and diagnostic health, `CooperativeWorkerV1`, exact cancellation custody, current `ProtocolTree` unavailability, per-launch cgroup evidence, GC, and operator recovery; add an exact-SHA native matrix for both macOS architectures, a disposable-checkout final-scope negative matrix, and full gates without binary/release changes. | `plugins/session-relay/AGENTS.md`, `plugins/session-relay/skills/productivity/session-relay/SKILL.md`, `plugins/session-relay/rust/src/main.rs`, `plugins/session-relay/test/run-build-matrix.mjs` (new), `plugins/session-relay/test/final-scope.mjs` (new), `.github/workflows/build-binaries.yml` | 1b-8 | planned | A15-A18 pass. Darwin natively exercises getpeereid/session/PTY/resize/signal/EOF/watchdog behavior; docs preserve evidence limits; final-scope independently rejects staged/unstaged/untracked forbidden classes; native dispatch refuses an unpushed/mismatched SHA. |
...
| A2 | Pending identity binding, controller claim, unmanaged cancellation, and multi-operation lost-authority transitions are atomic, durable, typed, and exact. | `node plugins/session-relay/test/rust-test-inventory.mjs --case lifecycle_managed && (cd plugins/session-relay/rust && cargo test --locked --test lifecycle_managed -- --nocapture)` | Exit 0 with nonzero source-derived required IDs. Durable waiter/controller retries share one bind; Codex SessionStart never claims. Expired/foreign-bound/pre-publication/controller-loss rows preserve foreign binding, consume only exact pending authority, and fence only the owned child. Every typed reason runs with 2+ operations from all source states; one batch preserves reasons and terminal/stale rows are audit-only. |
...
| A3b | Detached watchdog/supervisor own each transition-capable child from birth and preserve attach/wake I/O. | `node plugins/session-relay/test/rust-test-inventory.mjs --case lifecycle_supervisor && (cd plugins/session-relay/rust && cargo test --locked --test lifecycle_supervisor -- --nocapture) && node plugins/session-relay/test/supervisor-custody.mjs --matrix && node plugins/session-relay/test/reentry-inventory.mjs --compile-fail` | Exit 0 with nonzero required IDs. Source-derived closed startup phases inject before/after failure. Rows prove both bootstrap pipes, zero bootstrap fds, SID/custody, numeric bind/heartbeat deadlines, CLI Control EOF cancellation, managed takeover before caller exit, token absence from argv/env/store/log, caller death before/after takeover, exact ControllerReady persistence, kill/wait/reap, compatibility, and zero-byte stale events. |
...
| A5 | Tree proof requires fd-bound production launch placement and same-fence kill evidence; filtered hardening remains separate. | `node plugins/session-relay/test/lifecycle-smoke.mjs --case containment` | Exit 0; exact supervisor `launch_confined` records worker/generation/supervisor/control epoch, host boot/kernel/leaf, retained executable fd identity, child kernel executable identity, SHA256, target, runtime, and sandbox before GO. A hash→pathname-replacement barrier proves the pinned original executes or launch refuses. Scripts/symlinks/dispatch launchers refuse. Same constructor mints kill receipt; boot/leaf/receipt/cache/stale-supervisor changes refuse. Pre-kill cannot promote. |
...
| A6b | Source-built N/N+1 payloads exercise the actual committed Codex hook definitions through real runtime events and crash-atomic monotonic selection. | `RELAY_REAL_RUNTIME_TEST=1 node plugins/session-relay/test/runtime-hook-upgrade.mjs --matrix` | Exit 0. In an isolated authenticated real Codex config, build the implementation tip's target binary from source, construct synthetic N/N+1 plugin payloads with exact committed hook JSON plus generated manifests/checksums, establish real hook discovery/trust, capture N commands in a live thread, install N+1, and delete N root. Trigger old live UserPromptSubmit and new/subagent SessionStart as real runtime events; sentinels prove the correct event names and stable current. Present-root missing/nonexec/symlink launcher fails; whole absent/dangling removed root falls back. Lock/generation/pointer crash/race rows pass. This is implementation evidence, not shipped-package evidence; direct command/helper invocation cannot satisfy it. |
...
Adversarial cold-read result: Draft-15 repairs every reproduced Draft-14 blocker: exact nonzero Rust targets, authenticated Step-3b→1b bridge, supervisor-local Codex token custody, distinct durable controller wire/results, complete pre-binding refusal graph, two event-correct shell bootstraps, source-built versus packaged upgrade evidence, Step-6 controller dependency, enumerated doctor contract, sealed terminal refs/RPC ids/server-request policy, fd-bound executable provenance, portable pointer fsync, and executable final-scope negatives. No owner decision remains open; immutable independent re-review is pending. Implementation stays paused; remote push/dispatch and packaged producer/release gates remain separate owner approvals.
...
Score: **95/100 (Draft-15 author candidate; independent re-review pending)** · trajectory **94 Draft-14 author → 86 architecture / 78 acceptance NOT READY → 95 authority/gate/provenance repair** · stopped: **candidate will be frozen for fresh review; implementation remains paused**.
