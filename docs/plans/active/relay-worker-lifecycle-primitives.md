...
- Codex CLI has no pre-minted session id. Managed state therefore keeps a pre-launch `worker_id` and pending token-hash index, but the authoritative Codex claimant is the relay-owned app-server controller: it obtains `runtime_session_id` from `thread/start`, atomically binds that exact identity, and only then may send `turn/start`. A racing SessionStart hook is an exact duplicate/defense-in-depth path, never the sole barrier; token replay to another identity is refused.
...
- **Plan source:** the orchestrator and every worker read `/home/vagrant/projects/docks/docs/plans/active/relay-worker-lifecycle-primitives.md` from the main worktree at dispatch time; the older plan copy on the implementation branch is non-authoritative and need not be cherry-picked. The dispatch message records `PLAN_COMMIT=$(git -C /home/vagrant/projects/docks log -1 --format=%H -- docs/plans/active/relay-worker-lifecycle-primitives.md)` plus `PLAN_BLOB=$(git -C /home/vagrant/projects/docks hash-object docs/plans/active/relay-worker-lifecycle-primitives.md)`, and the worker repeats the reusable A0b check before every step. Unrelated main-worktree commits do not invalidate a plan snapshot; a changed path-specific plan commit or blob does.
- **Implementation checkout/base:** the registered checkout is `/tmp/docks-primitives-collab` on branch `codex/primitives-collab`, based on `701cea7e671bc40ee23d69abf79ff102e0eecb20`, and currently contains preserved in-flight Step-3b WIP. Every redispatch records `WIP_PATCH_SHA256` over `git diff --binary HEAD` plus `WIP_UNTRACKED_SHA256` over a sorted path/mode/content-hash manifest generated by committed `test/wip-snapshot.mjs`; the helper verifies every dirty/untracked path against the step allowlist and rejects any unexpected path, mode, or byte. Clean checkouts use its canonical empty hashes. A new worker independently verifies both hashes before reading or editing the WIP. `/home/vagrant/projects` and `/tmp` are different filesystems, so `git worktree move` is not a valid relocation command here. A0 records the one-time clean bootstrap. Before final handoff, first commit and verify all intended work, require a clean status, record branch and full HEAD, run non-forced `git worktree remove /tmp/docks-primitives-collab`, then `git worktree add /home/vagrant/projects/docks-primitives-collab codex/primitives-collab`, and verify the same branch/full HEAD. Never remove a dirty checkout. Drift/review base remains `12cf2ea`; completion commands run in the implementation worktree against its recorded tip, never concurrent `main` HEAD.
...
- **Confirmed-source rule:** Step 1 preserves the original evidence; Step 1b migrates every stale verdict and adds app-server direct-claim, stable-runtime upgrade, hook-health, and experimental-capability evidence. Neither may substitute hand-written booleans or re-argue the docs.
- **Local commands:**
  ```bash
...
The launcher generates a 256-bit random raw token and persists only SHA-256. Claude passes `RELAY_MANAGED_WORKER_ID`, `RELAY_MANAGED_GENERATION`, and `RELAY_MANAGED_ATTACH_TOKEN` to the owned CLI child. Codex passes the raw token only through the private relay-owned app-server controller to `claim_managed_appserver`; it is never placed in a general CLI environment. The raw token is never written to registry, logs, diagnostics, or argv. Before claim it authorizes one identity binding; after claim it is only an idempotency key for that same worker/generation/session/tool/canonical-cwd/controller tuple and can never bind another tuple.
...
For Codex the controller first starts a dedicated app-server, calls `thread/start`, persists the returned thread id/server fingerprint/controller instance as the candidate `CodexManagedBoundary`, and invokes `claim_managed_appserver`; it MUST NOT emit `turn/start` before the exact claim commits Active. A racing SessionStart with the same token and runtime id joins the same claim and may only confirm the exact candidate boundary; it cannot publish a conflicting identity. Claim failure or deadline emits no `turn/start`, exact-transitions to Fencing/FencingUnconfirmed under the lost-authority table below, and asks the retained supervisor to stop/reap the dedicated app-server and its confined tree. Managed Codex wake/resume always re-enters through this exact controller/boundary; it never falls back to `codex exec`, TUI resume, or a third-party app-server client.
...
| Input | Result |
|---|---|
| Concurrent duplicate with same claimed token + same runtime id/generation/tool/cwd and, for Codex, same server/controller boundary | Join the one durable claim and return its exact committed result without version/state churn: Active only if the original finalized Active, otherwise the same structured stop/refusal. |
...
|---|---|
| Unmanaged/pre-bind operation | `Unmanaged→UnmanagedCanceling` plus operation `LostAuthority(reason)`; a waiting claim then follows the cancel-deadline row. |
| `Claiming` / worker `Attaching` | Atomically bind the exact worker as Managed and apply exceptional `Attaching→FencingUnconfirmed(reason)`; consume the pending token; never Active. |
| `Active` | `Active→Fencing` with a new fence epoch/version, then exact `Fencing→FencingUnconfirmed(reason)`; direct `Active→FencingUnconfirmed` is forbidden. |
| `Fencing` for the matching fence epoch/version | `Fencing→FencingUnconfirmed(reason)`. |
| Any stale binding/operation/handoff/claim/fence version | Append stale audit only; zero state, signal, RPC, or output bytes. |

The table applies to every enumerated `LostAuthorityReason`; reason-specific code may not invent another transition. Closed unmanaged transitions are `Unmanaged@N→UnmanagedCanceling@N`; exact reap with a durable waiting claim `UnmanagedCanceling@N→Claiming@N+1`; exact reap/reconcile without one `→Unmanaged@N+1`; exact cancel deadline with a durable waiter `→Managed@N+1` paired with exceptional `Attaching→FencingUnconfirmed(CancelDeadline)`; risk-accepted `abandon-session→Unmanaged@N+1` with `NOT QUIESCENCE-PROVEN` receipt. No other transitions exist. Only releasable managed records and terminal unmanaged-operation tombstones with their required receipt may age out. Binding GC alone may exact-CAS plain `Unmanaged→GcDeleting→removed`; `UnmanagedCanceling` is never GC-eligible. Failed pre-delete lock acquisition restores untouched Unmanaged; once deletion starts `GcDeleting` is crash-resumable. `Claiming` and `Managed` never age independently.
...
pub enum SupervisorEvent {
    ...
    InputAccepted { operation_id: String, operation_version: String, sequence: String, byte_count: u32 },
    PtyResized { operation_id: String, operation_version: String, sequence: String, terminal_group: String, rows: u16, cols: u16 },
...
The supervisor is the hidden detached `relay __lifecycle-supervisor` process. The separately detached watchdog starts first and spawns/owns the supervisor `Child`; both use real new sessions, no parent-death signal, closed/independent stdio, and no inherited caller authority fds. On pinned Rust 1.85, `CommandExt::process_group(0)` is insufficient and stable `CommandExt::setsid` is unavailable; use the minimal async-signal-safe `pre_exec` wrapper around `libc::setsid()` and propagate failure from spawn. Acceptance proves watchdog SID=watchdog PID and supervisor SID=supervisor PID, both differ from the caller, and caller exit or caller-process-group SIGHUP leaves watchdog custody live.

The supervisor owns a mode-0600 Unix socket and validates same-UID peer credentials (`SO_PEERCRED` on Linux, `getpeereid` on Darwin), exact socket dev/inode, supervisor instance/version, stable `control_epoch`, mutable operation/version, and current Claim/Fence authority from the store; caller-supplied command fields are selectors, never authority. The launcher creates a random 256-bit control nonce, persists only `control_nonce_sha256`, and transfers the raw nonce over a private inherited pipe/already-authenticated bootstrap channel—never argv, environment, log, event, or store. The first frame is mandatory `SupervisorHello`: `Health` receives `SupervisorHealthPong` echoing its one-use challenge nonce, which is liveness-only and never command authority; exactly one matching `Control { control_epoch, control_nonce }` stream binds, receives `ControlBound` without echoing the secret, and only that stream's EOF can publish cancellation. Each later command independently validates its mutable `expected_operation_version` while retaining the same `control_epoch`. A second, stale, or role-confused control stream is refused.

Startup phases are strict. Global `Ready` means only listener/socket identity, watchdog record, acceptors, and global bounded queues are durably live. Per-operation `ControlBound` means the authenticated control reader and all per-operation queues exist. Only then may the supervisor spawn the child; after child creation every required pipe/PTY proxy thread must start before `Started` or any output byte is emitted. Any failure at listener, acceptor, global queue, control reader, per-operation queue, child spawn, or post-child proxy phase kills/waits/reaps any created child and applies `LostAuthority(ProxyStartupFailed)` through the closed transition table. `Input` carries exact bytes (max 64 KiB/frame), `InputEof` closes only child stdin, `ResizePty` is bounded to 1..4096 rows/cols and the named PTY group, and `ForwardSignal` accepts only SIGINT/SIGTERM/SIGHUP. Output frames preserve exact per-stream bytes and carry the control epoch, current operation version, and monotonic sequence; PTY endpoints preserve `isatty`, controlling terminal, foreground group, Ctrl-C, and resize behavior. Each direction has a 1 MiB bounded queue. A slow connected caller applies child-visible pipe/PTY backpressure as before, while cancellation/control polling runs independently at ≤100 ms.
...
pub struct HookHealthSnapshot {
    ...
    enabled: bool,
    trust_status: String,
    semantic_expected_command: bool,
}
```
...
Codex hook commands execute through a stable runtime at `${XDG_DATA_HOME:-$HOME/.local/share}/docks/session-relay/runtime/relay`, with adjacent `runtime.json { schema, plugin_version, target, binary_sha256, installed_at }`. A current versioned launcher invokes hidden `__install-stable --plugin-root <resolved-root>` before executing the stable binary. The installer reads the plugin manifest version and `SHA256SUMS` from that exact root—never Cargo's package version—then lstat-checks same-owner non-symlink directories/files, requires a mode-0700 runtime directory, verifies the target binary, copies to a same-directory temporary file, fsyncs, chmods 0755, atomically renames, and fsyncs the directory/record. Semver replacement is monotonic: higher replaces after validation; same version+digest is a no-op; lower is a no-op; same version+different digest is fatal. An old long-lived session whose N-versioned launcher was pruned may skip only the absent old installer path and execute the already-installed stable N/N+1 binary; any validation failure observed by a present new launcher fails that new hook invocation. Core standalone plugin behavior bootstraps this contract itself; docks-kit may proactively install/verify the same contract but is not required for safety.
...
| 1b | Migrate feasibility evidence to independent `cooperative_cgroup` and Claude-only `filtered_hardening` verdicts with optional freeze; prove relay-owned Codex app-server `thread/start→direct claim→turn/start`, inventory experimental capability, define semantic hook-health snapshots, and prove the standalone stable-runtime N→N+1 installer contract. The isolated harness may keep its labeled test-only hook-trust bypass; production may not. | `plugins/session-relay/test/feasibility-probe.mjs`, `plugins/session-relay/test/fixtures/lifecycle-capability-schema.json`, `plugins/session-relay/test/runtime-hook-abort.mjs` (new), `plugins/session-relay/test/runtime-hook-upgrade.mjs` (new), `plugins/session-relay/test/wip-snapshot.mjs` (new), `docs/plans/active/relay-worker-lifecycle-primitives.md:Notes` | 1 | planned | A0b/A1/A1b pass. Cooperative evidence is diagnostic for later per-launch construction; direct app-server claim sends zero first-turn bytes before Active; hook health never claims atomic launch authority; stable runtime survives version-cache pruning and rejects downgrade/tamper. |
...
| 3b | Add the detached watchdog-first supervisor and versioned operation substrate before turn cancellation: watchdog and supervisor each form independent sessions; watchdog owns/reaps supervisor; stable control epoch plus private nonce bind one Control stream; health challenges are non-authority; global Ready, per-operation ControlBound, child spawn, and post-child proxy startup are strictly ordered. Preserve Closed/pipe/PTY stdin, EOF, output, resize, Ctrl-C/signal, exit, and backpressure. Add durable waiting claims, closed lost-authority state mapping, `UnmanagedCanceling`, exact disconnect-vs-Claiming transfer, closed commands, private child permit, operation/handoff versions, custody transitions, sealed reap/session-operation receipts, tombstones, and internal recovery state. No pidfd/cgroup or user-facing recovery CLI yet. | `plugins/session-relay/rust/Cargo.toml`, `plugins/session-relay/rust/Cargo.lock`, `plugins/session-relay/rust/src/supervisor.rs` (new), `plugins/session-relay/rust/src/main.rs`, `plugins/session-relay/rust/src/lib.rs`, `plugins/session-relay/rust/src/lifecycle.rs`, `plugins/session-relay/rust/src/spawn.rs`, `plugins/session-relay/rust/src/store.rs`, `plugins/session-relay/rust/src/hook.rs`, `plugins/session-relay/rust/tests/lifecycle_managed.rs`, `plugins/session-relay/rust/tests/lifecycle_supervisor.rs` (new), `plugins/session-relay/test/supervisor-custody.mjs` (new), `plugins/session-relay/test/reentry-inventory.mjs`, `plugins/session-relay/test/fixtures/reentry-inventory.json`, `plugins/session-relay/test/fixtures/lifecycle-capability-bypass/Cargo.toml`, `plugins/session-relay/test/fixtures/lifecycle-capability-bypass/Cargo.lock`, `plugins/session-relay/test/selftest.mjs`, `plugins/session-relay/test/fixtures/lifecycle-capability-bypass/src/bin/child-cancel-reentry.rs` (new), `plugins/session-relay/test/fixtures/lifecycle-capability-bypass/src/bin/fabricated-owned-proof.rs` (new) | 3a | in-flight | A2/A3b prove every source-state×lost-reason row, caller exit/SIGHUP survival, all startup failure points, stable control vs mutable operation version, durable waiter crash/retry, interactive/noninteractive compatibility, disconnect/flood, exact reap, stale zero-byte behavior, and no Child/PID reconstruction. If any pre-claim attach lacks exact cancellation, watchdog detachment fails, or bidirectional supervisor behavior cannot be preserved, STOP as unbuildable. |
...
| 4 | Extend the Step-3b supervisor with pidfd and `ConfinedCgroupCooperative`: gated placement into a fresh delegated domain leaf; retained generation-bound fd; setup-only placement receipt; post-kill same-fence termination receipt; exact domain/empty-subtree/kill checks; `cgroup.kill`→`populated 0`; fail-closed fd loss. Classify `CLONE_INTO_CGROUP` `EACCES|EBUSY|EOPNOTSUPP|ENOSYS` plus narrowly validated unsupported-feature `EINVAL`; use stop/GO only when independently proven. Add Claude-only best-effort filtered hardening and inventory every signal callsite. | `plugins/session-relay/rust/src/process_identity.rs` (new), `plugins/session-relay/rust/src/supervisor.rs`, `plugins/session-relay/rust/src/spawn.rs`, `plugins/session-relay/rust/src/main.rs`, `plugins/session-relay/rust/src/lifecycle.rs`, `plugins/session-relay/rust/src/store.rs`, `plugins/session-relay/test/process-signal-inventory.mjs` (new), `plugins/session-relay/test/fixtures/process-signal-inventory.json` (new), `plugins/session-relay/test/runtime-hook-abort.mjs` (new), `plugins/session-relay/test/fixtures/lifecycle-capability-bypass/Cargo.toml`, `plugins/session-relay/test/fixtures/lifecycle-capability-bypass/Cargo.lock`, `plugins/session-relay/test/fixtures/lifecycle-capability-bypass/src/bin/fabricated-pidfd-proof.rs` (new), `plugins/session-relay/test/fixtures/lifecycle-capability-bypass/src/bin/tampered-cgroup-proof.rs` (new), `plugins/session-relay/test/reentry-inventory.mjs`, `.github/workflows/build-binaries.yml` | 1b-3d | planned | A4/A5/A5b prove stable signaling, gated membership, typed fallback, phase-correct kill/populated-0 with no surviving in-model PID for both runtimes, and independently labeled Claude-only filtered hardening. A pre-kill boundary can never become WorkerTree proof. |
| 5 | Implement managed first-prompt admission: Claude Claiming-first SessionStart plus required UserPromptSubmit barrier; Codex relay-owned app-server `thread/start→direct lifecycle claim→first turn/start` with no managed CLI fallback. Add stable monotonic hook runtime installation, semantic `HookHealthSnapshot`, explicit `timeout: 30`, measured dead-man, exact duplicate controller/hook claims, and ordinary unmanaged compatibility. | `plugins/session-relay/rust/src/appserver.rs`, `plugins/session-relay/rust/src/hook.rs`, `plugins/session-relay/rust/src/runtime_install.rs` (new), `plugins/session-relay/rust/src/main.rs`, `plugins/session-relay/rust/src/spawn.rs`, `plugins/session-relay/rust/src/store.rs`, `plugins/session-relay/hooks/hooks.json`, `plugins/session-relay/hooks/codex-hooks.json`, `plugins/session-relay/test/runtime-hook-abort.mjs` (new), `plugins/session-relay/test/runtime-hook-upgrade.mjs` (new), `plugins/session-relay/test/selftest.mjs` | 1b-4 | planned | A2/A6/A6b prove two-phase claims, zero Codex turn bytes before Active, Claude barrier behavior, exact hook-health degradation, and N→N+1 cache-prune survival/downgrade resistance. Any managed Codex CLI fallback, first-turn sentinel before Active, implicit timeout, pre-Claiming hook work, unstable versioned hook exec, or fabricated atomic trust claim: STOP. |
| 6 | Consume Step-3c exact-turn handoffs through post-drain `FencePermit`; initialize with experimental API capability; implement finite recursive lineage/turn pagination and exact background-terminal list/terminate; mint one sealed `ProtocolProofAttempt` and require its binding on every nested receipt/evidence hash before `FenceEvidence` construction. The real current Codex adapter must always return `MissingDurableFlushContract`; positive graceful reject/flush/watermark/reap/offline remains fake/future-only; shared scans stay observation-only; physical fallback is cgroup kill. | `plugins/session-relay/rust/src/appserver.rs`, `plugins/session-relay/rust/src/spawn.rs`, `plugins/session-relay/rust/src/lifecycle.rs`, `plugins/session-relay/test/fake-app-server.mjs`, `plugins/session-relay/test/runtime-appserver-quiescence.mjs` (new), `plugins/session-relay/test/lifecycle-smoke.mjs`, `plugins/session-relay/test/fixtures/lifecycle-capability-bypass/Cargo.toml`, `plugins/session-relay/test/fixtures/lifecycle-capability-bypass/Cargo.lock`, `plugins/session-relay/test/fixtures/lifecycle-capability-bypass/src/bin/fabricated-protocol-proof.rs` (new), `plugins/session-relay/test/reentry-inventory.mjs`, `plugins/session-relay/test/fixtures/reentry-inventory.json` | 3c-4 | planned | A7/A8/A13 prove typed experimental-method failure, real runtime ProtocolIncomplete, private attempt-bound fake proof construction, N/N+1 nested-receipt mix rejection, finite pagination, exact terminal termination, later-fence replay rejection, late/continuous writer rejection, and physical fallback. |
...
| 8 | Complete adversarial unit/cross-process/real-runtime matrices and source-inventory enforcement; preserve all existing relay tests and isolated cleanup. Include watchdog/supervisor caller-exit/SIGHUP/crash/startup-phase failures, control-epoch/version races, every lost-authority source/reason, operation CAS losers, cancellation barriers, direct app-server claim/first-turn ordering, GC-vs-claim, per-launch cgroup receipts, optional-freeze rejection, semantic hook-health drift, stable-runtime upgrade/tamper, protocol-attempt mixing, proof-retention crash, and missing-flush rows. | `plugins/session-relay/rust/src/appserver.rs`, `plugins/session-relay/rust/src/hook.rs`, `plugins/session-relay/rust/src/lifecycle.rs`, `plugins/session-relay/rust/src/process_identity.rs`, `plugins/session-relay/rust/src/runtime_install.rs`, `plugins/session-relay/rust/src/supervisor.rs`, `plugins/session-relay/rust/src/spawn.rs`, `plugins/session-relay/rust/src/store.rs`, `plugins/session-relay/test/feasibility-probe.mjs`, `plugins/session-relay/test/supervisor-custody.mjs`, `plugins/session-relay/test/lifecycle-smoke.mjs`, `plugins/session-relay/test/process-signal-inventory.mjs`, `plugins/session-relay/test/reentry-inventory.mjs`, `plugins/session-relay/test/runtime-hook-abort.mjs`, `plugins/session-relay/test/runtime-hook-upgrade.mjs`, `plugins/session-relay/test/runtime-appserver-quiescence.mjs`, `plugins/session-relay/test/fake-app-server.mjs`, `plugins/session-relay/test/selftest.mjs` | 2-7 | planned | A1/A1b through A14 pass; helpers clean children/stores in `finally`; externally observable sentinels and kernel/protocol evidence, not elapsed time alone, establish results. Cooperative-scope labels and stable-runtime provenance are machine-checked. |
| 9 | Document guarantee tiers, relay-owned Codex app-server admission, stable hook runtime and diagnostic health, `CooperativeWorkerV1`, exact cancellation custody, current `ProtocolTree` unavailability, per-launch cgroup evidence, GC, and operator recovery; add an exact-SHA native matrix for both macOS architectures and full gates without binary/release changes. | `plugins/session-relay/AGENTS.md`, `plugins/session-relay/skills/productivity/session-relay/SKILL.md`, `plugins/session-relay/rust/src/main.rs`, `plugins/session-relay/test/run-build-matrix.mjs` (new), `.github/workflows/build-binaries.yml` | 1b-8 | planned | A15-A18 pass. Darwin natively exercises getpeereid/session/PTY/resize/signal/EOF/watchdog behavior; docs preserve evidence limits; native dispatch refuses an unpushed/mismatched SHA. |
...
| A0b | Bind every dispatch to the plan snapshot, implementation tip, and exact WIP bytes. | `test -n "$PLAN_COMMIT" && test -n "$PLAN_BLOB" && test -n "$IMPL_HEAD" && test -n "$WIP_PATCH_SHA256" && test -n "$WIP_UNTRACKED_SHA256" && test "$(git -C /home/vagrant/projects/docks log -1 --format=%H -- docs/plans/active/relay-worker-lifecycle-primitives.md)" = "$PLAN_COMMIT" && test "$(git -C /home/vagrant/projects/docks show "$PLAN_COMMIT:docs/plans/active/relay-worker-lifecycle-primitives.md" | git hash-object --stdin)" = "$PLAN_BLOB" && test "$(git -C /home/vagrant/projects/docks hash-object docs/plans/active/relay-worker-lifecycle-primitives.md)" = "$PLAN_BLOB" && test "$(git rev-parse HEAD)" = "$IMPL_HEAD" && node plugins/session-relay/test/wip-snapshot.mjs --verify --patch-sha256 "$WIP_PATCH_SHA256" --untracked-sha256 "$WIP_UNTRACKED_SHA256" --allow-step 3b` | Exit 0 before each worker step. The helper hashes `git diff --binary HEAD` and a sorted path/mode/content-hash manifest for allowlisted untracked files; any unexpected path/mode/byte fails. Canonical empty hashes represent a clean checkout. A new worker independently reviews the verified WIP. |
...
| A1b | Codex direct managed admission, hook health, stable runtime, and experimental capability match their real APIs. | `RELAY_REAL_RUNTIME_TEST=1 node plugins/session-relay/test/feasibility-probe.mjs --case codex-managed-boundary` | Exit 0; dedicated app-server `thread/start` returns the exact id, direct claim binds it before any `turn/start`, and claim failure emits zero turn/model bytes. `hooks/list` produces semantic `HookHealthSnapshot` rows including version/source/hash/command/timeout/enabled/trust; drift degrades health but is not launch authority; production argv contains no bypass. Initialize with `experimentalApi=true` makes required methods available or records typed unavailability. Stable installer metadata comes from plugin manifest+SHA256SUMS, not Cargo version. |
| A2 | Pending identity binding, direct app-server claim, unmanaged cancellation, and lost-authority transitions are atomic, durable, epoch-serialized, and exact. | `(cd plugins/session-relay/rust && cargo test --locked managed_attach_ -- --nocapture)` | Exit 0; durable waiter and direct controller claims share one two-phase bind; exact hook/controller duplicates join and conflicting identity/server/controller tuples refuse. Cancel-first/reap/deadline rows are exact. Every enumerated lost-authority reason executes from unmanaged/pre-bind, Claiming/Attaching, Active, Fencing, and stale sources: Active always passes through a new Fencing epoch before Unconfirmed; stale actors audit only. Stable cancellation epoch survives custody-version increments. |
...
| A3b | Detached watchdog/supervisor own each transition-capable child from birth and preserve attach/wake I/O. | `(cd plugins/session-relay/rust && cargo test --locked lifecycle_supervisor_ -- --nocapture) && node plugins/session-relay/test/supervisor-custody.mjs --matrix && node plugins/session-relay/test/reentry-inventory.mjs --compile-fail` | Exit 0. Rows cover watchdog-first independent SID/no-PDEATHSIG/closed stdio/no authority fds; caller exit and caller-group SIGHUP survival; health challenge vs private control nonce; duplicate/stale control epoch and operation version; Ready→ControlBound→spawn→proxy→Started order; failure injection at every phase with kill/wait/reap; missing-custody fail closed only where custody is asserted; exact pipe/PTY/backpressure/signals/disconnect/flood behavior; supervisor/watchdog death; and zero-byte stale events. Compile-fail authority bins remain red. |
| A3c | Exact-turn and supervisor-owned-child cancellation close the guard-drain cycle within fixed bounds. | `(cd plugins/session-relay/rust && cargo test --locked lifecycle_turn_cancellation_ -- --nocapture) && node plugins/session-relay/test/lifecycle-smoke.mjs --case cancellation-custody && node plugins/session-relay/test/reentry-inventory.mjs --compile-fail` | Exit 0. Send/response/terminal/child-ignore barriers and every lost-authority source/reason/crash row pass. Each custody/handoff CAS checks exact versions; delayed losers audit only. Cancellation interrupts exactly the persisted turn and accepts only matching terminal. Every wait ≤100 ms; one 5s deadline ends terminal/reap/handoff or applies the closed state-specific Unconfirmed path. Guard releases while supervisor retains child authority. |
...
| A4 | No check-then-kill path can signal a recycled identity, and exit proof requires terminal handle evidence. | `node plugins/session-relay/test/process-signal-inventory.mjs && (cd plugins/session-relay/rust && cargo test --locked stable_signal_ -- --nocapture)` | Both exit 0; inventory rejects ObservationOnly/start-check+kill; deterministic PID substitution receives zero signal; real pidfd-open-before-validation targets only the pinned task; `PidFdExitProof` cannot construct before whole-process POLLIN or owned `waitid(P_PIDFD)` evidence; live unreaped-Child signaling affects only its child; Darwin/no-pidfd returns Unconfirmed. |
| A5 | Tree proof requires fresh per-launch placement and same-fence kill evidence; filtered hardening remains separate. | `node plugins/session-relay/test/lifecycle-smoke.mjs --case containment` | Exit 0; each production boundary records actual worker/generation/supervisor/control epoch/runtime/host boot/kernel/leaf/gated membership. A pre-kill boundary cannot promote. Only the matching supervisor/fence constructs a kill receipt bound to that placement after exact kill/populated-zero/no survivor. Cached CI capability evidence cannot authorize GO/proof; host/runtime changes invalidate diagnostics. |
| A5b | CI/release regression proves cooperative cgroup child creation in BOTH runtimes; it never authorizes a production launch. | `RELAY_REAL_RUNTIME_TEST=1 node plugins/session-relay/test/runtime-hook-abort.mjs --case cgroup-cooperative-child-spawn --matrix` | Exit 0. The harness records host boot/kernel/mount/delegation/runtime context. Isolated Claude and Codex are gated into a fresh domain leaf, create real tool child+nested grandchild+fork storm, keep every in-model host PID under the leaf, then kill/populated-zero. Rows print runtime/gated/domain evidence and `authoritative_for_production=false`. Production still constructs fresh A5 placement/kill receipts. Claude filtered hardening is separate; Codex's expected bwrap filter denial does not reduce cooperative availability. |
| A6 | Real managed first-prompt admission is runtime-correct. | `RELAY_REAL_RUNTIME_TEST=1 node plugins/session-relay/test/runtime-hook-abort.mjs --matrix` | Exit 0; Claude exact hook/timeout rows prove UserPromptSubmit blocks non-Active and SessionStart remains version-pinned empirical. Codex relay-owned app-server proves `thread/start→claim Active→turn/start`; fail/deadline/conflicting controller emits zero turn/model bytes and fences/reaps. Codex hooks record health/re-entry behavior only. No managed Codex CLI/TUI fallback or production bypass exists. |
| A6b | Codex hook execution survives plugin cache refresh without downgrade or tamper. | `RELAY_REAL_RUNTIME_TEST=1 node plugins/session-relay/test/runtime-hook-upgrade.mjs --matrix` | Exit 0; install N stable runtime, start an N session, validate/install N+1, prune N cache, then old-session UserPromptSubmit and new/subagent SessionStart both execute through the stable binary. Higher semver replaces atomically; same version+digest no-op; lower cannot downgrade; same-version/different-digest, symlink, owner/mode, truncated copy, manifest/SHA mismatch fail. Crash injection preserves either valid old or valid new runtime+record. |
| A7 | Real app-server fencing distinguishes exact cancellation, shared observation, current ProtocolTree unavailability, and physical kill. | `RELAY_REAL_RUNTIME_TEST=1 node plugins/session-relay/test/runtime-appserver-quiescence.mjs --matrix` | Exit 0; initialize advertises `experimentalApi=true` before descendant/turn/item/terminal methods and records runtime/method availability; unsupported methods fail typed. Exact interrupt reaches matching terminal; each terminal is paged/terminated by exact process id. Shared rows remain observation; current dedicated rows report MissingDurableFlushContract. Physical cgroup kill produces only WorkerTree. |
...
| A13 | Proof validation and capability families are exhaustive, sealed, scope-safe, epoch/attempt-bound, phase-correct, and threat-model-labeled. | `(cd plugins/session-relay/rust && cargo test --locked fence_proof_ -- --nocapture) && node plugins/session-relay/test/reentry-inventory.mjs --compile-fail` | Both exit 0; backend×scope×evidence rows pass. Process/protocol/boundary/kill/fence fields are private. Compile-fail bins reject fabrication/mutation; runtime rows reject later-fence replay, nested attempt mixing, cached cgroup capability authorization, pre-kill promotion, serialized tamper, stale supervisor/control epoch, and host/runtime placement mismatch. Current Codex cannot build offline boundary. |
...
| A18 | The implementation-worktree diff is scoped and contains no binary/release mutation. | `test "$(git branch --show-current)" = codex/primitives-collab && git diff --check 12cf2ea.."$IMPL_TIP" && test -z "$(git diff --name-only 12cf2ea.."$IMPL_TIP" | rg -v '^(plugins/session-relay/(rust/src/.*|rust/tests/.*|rust/Cargo\.(toml|lock)|hooks/.*|test/.*|AGENTS\.md|skills/productivity/session-relay/SKILL\.md)|\.github/workflows/build-binaries\.yml|docs/plans/active/relay-worker-lifecycle-primitives\.md)$')"` | Run in the implementation worktree with `IMPL_TIP` set to its reviewed full SHA. Exit 0 and empty unexpected paths. Concurrent main-plan commits are outside this range. No binaries/manifests/marketplace/version/release changes. |
...
2. **Environment & commands:** present — path-specific plan/blob, implementation tip, binary+untracked WIP hashes, cross-filesystem handoff, versions, four targets, stable hook runtime, isolated real-runtime setup, measured deadline, and exact gates are specified.
3. **Interface & data contracts:** present — direct Codex app-server claim, durable waiter, closed lost-authority graph, stable cancellation epoch plus mutable custody version, control epoch/private nonce, watchdog ownership, per-launch cgroup receipts, attempt-bound protocol proof, stable runtime, tombstone GC, and operator APIs are explicit.
4. **Executable acceptance:** present — A0/A0b–A18 plus A1b/A3a–A3d/A6b are command + expected evidence, including live upgrade, semantic hook health, detached watchdog/control/startup races, state×reason loss, attempt replay, per-launch cgroup phase, native Darwin, and source-derived behavior ids.
...
Adversarial cold-read result: Draft-13 addresses every Draft-12 blocker with a closed lost-authority graph, independently detached watchdog, stable control authority, strict startup phases, attempt-bound nested protocol receipts, per-launch cgroup evidence, exact WIP hashes, relay-owned Codex app-server admission, semantic hook health, and upgrade-stable hook execution. No owner decision remains open; immutable independent review is still pending. Implementation stays paused; remote push/dispatch remains a separate owner gate.
...
Score: **93/100 (Draft-13 author candidate; independent re-review pending)** · trajectory **97 Draft-11 claimed READY → 74 extended red-team NOT READY → 86 Draft-12 → 88/88 Draft-12 NOT READY → 93 app-server/durability/authority remodel** · stopped: **candidate will be frozen for fresh review; implementation remains paused**.
