...
updated: "2026-07-12T05:19:13-03:00"
...
- **Exact A0c-step1b-bootstrap command (run once, before any Step-1b edit):** only after Step 3b is committed and the orchestrator has independently recorded its full tip, dispatch `PLAN_COMMIT`, `PLAN_BLOB`, and `STEP3B_HEAD`; from `/tmp/docks-primitives-collab`, run:
  ...
  git diff --check HEAD
  ```
  This one-time bridge authenticates the clean post-3b tip before `wip-snapshot.mjs` exists. Step 1b must commit the helper, `test/fixtures/wip-historical-baseline.json`, and `test/fixtures/wip-step-allowlist.json`. The historical fixture is recursively closed at every object and records exactly `{schema:1,base,tip,entries,manifest_sha256}` for `12cf2ea..STEP3B_HEAD`. Each UTF-8 byte-sorted unique entry is exactly `{path,status,old_mode,new_mode,old_oid,new_oid,owners,final_disposition,entry_sha256}`; `owners` is a nonempty UTF-8 byte-sorted array of unique `{step,commits}` objects, and each `commits` array is byte-sorted, unique, full 40-hex SHAs that both touch the path and belong to that named step. Multi-owner paths are mandatory rather than collapsed to one owner. Because the reproduced baseline has exactly 35 additions/modifications, `status` is exactly `A|M`; any rename/copy/delete/type status is a mismatch, `A` requires zero old mode/oid, and modes/oids are canonical Git octal/40-hex. `final_disposition` is exactly `present|must_be_absent_at_final_tip`; only the stale implementation-branch `docs/plans/active/relay-worker-lifecycle-primitives.md` row is `must_be_absent_at_final_tip`, because it may exist at the historical tip but Step 9 must delete it once before the final tip. `entry_sha256` is lowercase SHA-256 of RFC-8785 JCS for that closed entry with only `entry_sha256` omitted. `manifest_sha256` is lowercase SHA-256 of RFC-8785 JCS for the entire closed top-level object with only `manifest_sha256` omitted, including every entry digest. Unknown/missing/duplicate key, unsorted array, noncanonical scalar, wrong digest, unowned commit, owner-step mismatch, or a 34/36-row range fails.

  The separate future allowlist is recursively closed as `{schema:1,historical_manifest_sha256,steps:[{step,mutations:[{status,path,final_mode,historical_entry_sha256?}]}]}`. `steps` contains every remaining step exactly once in execution order; `mutations` contains only committed mutations, byte-sorted by path, with `status=A|M|D`, exact filename, `final_mode="100644"|"100755"|null` (`null` only for `D`), and the immutable historical entry hash exactly when the path overlaps a historical entry. Gate-only/read-only mentions are empty mutation arrays. In particular Steps 8 and 9 mentioning either WIP fixture only to verify it does not authorize an `M`; Step 9 alone has one authenticated `D` row for the stale plan with its historical hash. Directory/glob rows are forbidden. The runtime-doctor golden mutations are the thirteen exact filenames listed in frontmatter and Step 5, never a directory allowance. Conflicting status/mode, a future `must_be_absent_at_final_tip` add/modify, unbound historical overlap, duplicate path, missing/extra/duplicate step, read-only mention represented as mutation, or any path not in the exact step table fails. A0c is never reused after Step 1b.

- **Exact A0d post-step range contract:** set `STEP_BASE` to the clean committed tip recorded immediately before a step, and `STEP_HEAD` to its clean committed tip. Set `STEP_RECEIPT_DIR` once to a harness-created mode-0700 sentinel-bound absolute directory outside the repo; losing it is a STOP. After every step—including the first Step-1b commit—run `node plugins/session-relay/test/wip-snapshot.mjs --verify-step-range --step "$STEP_ID" --base "$STEP_BASE" --head "$STEP_HEAD" --historical plugins/session-relay/test/fixtures/wip-historical-baseline.json --allowlist plugins/session-relay/test/fixtures/wip-step-allowlist.json --receipt-dir "$STEP_RECEIPT_DIR"`. The helper requires current clean `HEAD==STEP_HEAD`, ancestry, an exact raw `STEP_BASE..STEP_HEAD` A/M/D set equal to that step's mutation rows, canonical modes/oids/status, historical hashes on overlaps, and zero staged/unstaged/untracked residue. It writes one immutable canonical JCS receipt keyed by step, refuses overwrite with different bytes, and prints `STEP_RANGE PASS step=<id> base=<sha> head=<sha> entries=<n> manifest_sha256=<hex>`. Gate-only Steps 1c/1d use `STEP_BASE==STEP_HEAD` and an empty mutation array. If Step 1b needs a repair commit, rerun A0d over its original base through the new head and replace the prior receipt only through the helper's exact same-step supersession rule that requires the old head be an ancestor and preserves its hash chain. A0b binds pre-edit/WIP bytes; A0d independently rejects unowned committed-clean changes after the edit.
- **Runner live job capability:** Node 24 exposes no public `AF_UNIX SOCK_SEQPACKET` socketpair or `SCM_CREDENTIALS` API, so JavaScript is never the capability owner. Step 1b adds the dedicated host-native Rust autobin `plugins/session-relay/rust/src/bin/runner_job_custodian.rs`, built without new dependencies by `RUNNER_BUILD_DIR="$(mktemp -d /tmp/relay-runner-custodian.XXXXXX)"; chmod 700 "$RUNNER_BUILD_DIR"; (cd plugins/session-relay/rust && CARGO_TARGET_DIR="$RUNNER_BUILD_DIR" cargo build --locked --bin runner_job_custodian)`. The orchestrator verifies the resulting regular owner-executable `$RUNNER_BUILD_DIR/debug/runner_job_custodian` dev/inode/size/SHA-256 and starts that native binary directly in `--driver` mode. `runner-job-custodian.mjs` validates the six literal JCS frames, closed UI, and returned canonical results only; it may not create/retain/pass a capability socket, inspect ancillary credentials, launch the native driver, or spawn a gate. The native driver alone creates and retains exactly one `AF_UNIX|SOCK_SEQPACKET|SOCK_CLOEXEC` socketpair, enables credential receipt, forks/execs the same pinned binary in long-lived `--custodian` mode with only the custodian endpoint, retains the private gate endpoint, creates the public random `capability_id`, and directly spawns the same pinned binary in `--gate` mode for each row. The native binary refuses non-Linux with typed unavailability; no Node fallback exists.

  On the owner-provisioned Linux job, set nonempty `RELAY_RUNNER_ID` to the owner-recorded diagnostic job/runner identity, `RELAY_RUNNER_RECEIPT` to one new harness-owned absolute temporary JSON path, and `RELAY_CGROUP_ROOT` to the validated delegated root; none is authority. Before A1c, start one foreground `RELAY_RUNNER_JOB=1 "$RUNNER_BUILD_DIR/debug/runner_job_custodian" --driver --node-wrapper plugins/session-relay/test/runner-job-custodian.mjs --runner-id "$RELAY_RUNNER_ID" --runner-receipt "$RELAY_RUNNER_RECEIPT" --cgroup-root "$RELAY_CGROUP_ROOT"` process and retain its exact native-driver/custodian identities through A7. The native driver owns protocol state and accepts only wrapper-validated literal recursively closed JCS frames in fixed A1c→A1d→A5→A5b→A6→A7 order. `node_script`, `args`, and sorted `env` must byte-equal the named row; there is no caller-supplied executable, cwd, raw environment, shell text, assignment prefix, or extra field. The native driver resolves each `env` source only from its validated start inputs; `literal_one` is exactly `"1"`. The Node-child base-name allowlist is literally `HOME|LANG|LC_ALL|PATH|TMPDIR`; authentication contributes either no environment name when the validated credential-artifact mechanism is used or only the exact runtime-specific secret name sealed by Step 1's auth preflight into the live tuple. The six frame rows add only their listed names, and `RELAY_RUNNER_CAP_FD` exists only in the native gate environment. Duplicate names, an auth name not sealed in that tuple, or any other ambient name fails before native spawn—there is never bulk inheritance.

  For every positive row the native driver directly spawns the pinned Rust autobin in `--gate` mode as the credential-bearing gate; neither the wrapper nor driver spawns `/bin/sh`, a launcher script, or Node as the credential principal. Immediately before that one spawn the driver duplicates the retained gate endpoint to one fixed child fd, clears `FD_CLOEXEC` only on that duplicate so the native gate inherits it, and injects only its decimal number as `RELAY_RUNNER_CAP_FD` into the native gate's environment. The native gate's first fd action is `fcntl(F_SETFD, FD_CLOEXEC)` on that duplicate; failure poisons and exits. Only after immediate restoration may it validate the fd as the expected connected `SOCK_SEQPACKET`, close every other non-allowlisted fd, and send OPEN itself. The native custodian requires kernel-supplied `SCM_CREDENTIALS` on both OPEN and COMMIT; no JSON PID substitutes. Only after OPEN acknowledgement does the gate spawn the pinned absolute Node with the exact frame script/args/env; the capability fd is CLOEXEC and absent from Node plus every runtime/helper descendant. The gate exact-waits the Node child, hashes its result evidence, sends COMMIT itself, closes its duplicate after COMMIT acknowledgement, emits no application output except its closed result pipe, and exits. Thus the native-driver spawn PID is the credential-bearing native gate for its full lifetime; Node is a non-capability payload child.

  `node plugins/session-relay/test/runner-job-custodian.mjs --matrix --native-driver "$RUNNER_BUILD_DIR/debug/runner_job_custodian"` is mandatory before the live driver. It validates native positives plus copy/no-fd, descriptor leak, shell assignment/argv injection, non-exec wrapper, sibling borrower, duplicate/stale/out-of-order/concurrent OPEN, authoritative-before-OPEN, missing/wrong/duplicate COMMIT, result/evidence mismatch, credential PID/uid/gid mismatch, EOF/crash/poison, exact reap, and cleanup. The native driver's `finally` path performs the one closed shutdown transition, exact-waits driver-owned custodian/gate/payload PIDs, verifies no child/fd remains, and removes only its sentinel-bound `RUNNER_BUILD_DIR`; the Node wrapper only validates that returned cleanup result. During live A1c→A7 the native driver must not remove the build directory or receipt early. Failure/poison retains the receipt and a redacted diagnostic manifest but still reaps processes; a fresh run uses a fresh directory/receipt/capability and never resumes a poisoned sequence.
  ...
  ```
- **Runner capability protocol:** every gate has two mandatory phases on one reserved sequence. `RUNNER_BOOTSTRAP_INPUT_SHA256` is exactly 64 ASCII zeroes and means “A1c has no prior receipt”; it is a closed sentinel, never the hash of caller bytes. **OPEN, before any authoritative probe/child/RPC/fence byte:** `GateOpen {selectors}` → `GateOpenChallenge {selectors,challenge:[32 random bytes]}` → `GateOpenResponse {selectors,challenge_sha256}` → `GateOpenAck {selectors,open_ack_sha256}`, where closed `selectors={schema:1,capability_id,sequence,gate,gate_pid,live_tuple_sha256,input_receipt_sha256}`. The helper enables Linux `SO_PASSCRED` and requires both request packets' `SCM_CREDENTIALS {pid,uid,gid}` to carry the expected same uid and `pid == gate_pid`; the driver independently records and exact-waits that spawned pid. Only a valid OPEN reserves `Idle(sequence)→OpenReserved(selectors,open_ack_sha256)` and authorizes that gate to begin work. **COMMIT, after producing evidence but before success/sequence advance:** `GateCommit {selectors,open_ack_sha256,result_receipt_sha256,evidence_sha256}` → `GateCommitChallenge {same fields,challenge:[32 random bytes]}` → `GateCommitResponse {same fields,challenge_sha256}` → `GateCommitAck {same fields,commit_ack_sha256}`. COMMIT request/response repeat the same credential check; the custodian exact-matches the reserved selectors/open acknowledgement, hashes the known receipt path for A1c/A1d or requires the unchanged sealed input hash for A5/A5b/A6/A7, and binds the gate's independently validator-backed `evidence_sha256` into both acknowledgement and running chains. It then atomically advances sequences 1–5 to `Idle(sequence+1)` or sequence 6 to terminal `Complete`. No `GateResult` or later gate exists before that transition. If the exact-waiting driver observes gate exit/crash before COMMIT, it closes its retained gate endpoint, forcing helper EOF and terminal poison rather than leaving `OpenReserved` reusable.

  The driver records the native gate PID returned by direct spawn and supplies it as the custodian's sole expected PID. Every positive sequence requires exact equality `driver_spawn_pid == OPEN SCM_CREDENTIALS.pid == selectors.gate_pid == COMMIT SCM_CREDENTIALS.pid == exact_wait_pid == GateResult.pid`; uid/gid also equal the driver. The driver exact-waits that same PID and rejects a result before wait status is known. A test-only negative deliberately gives the duplicate to a non-`exec` `/bin/sh` wrapper that forks the native gate, and another lends it to a sibling borrower. In both cases the sender credential PID differs from the recorded direct-spawn PID: OPEN rejects, the capability becomes `Poisoned`, no Node payload is spawned, zero authoritative bytes exist, and COMMIT/later gates are impossible. An `exec`-preserving trampoline is unnecessary and forbidden in schema v1.

  A1c OPEN uses the bootstrap sentinel, freshly revalidated live tuple, and no receipt file; its COMMIT binds the newly written closed partial-receipt hash plus exec-stop evidence hash. A1d OPEN uses that exact partial hash; its COMMIT binds the newly sealed final receipt hash plus delegation/kill/reap/removal evidence hash. A5/A5b/A6/A7 OPEN all use the same final sealed receipt hash, while each COMMIT keeps `result_receipt_sha256` equal to that sealed hash and binds its own new gate evidence hash. A copied receipt/environment with no connected inherited fd, or a copied gate started outside the custodian parent, fails OPEN before probing. EOF/crash/timeout/wrong credentials or selectors, duplicate/stale/out-of-order/concurrent OPEN, any authoritative byte before OPEN acknowledgement, missing/wrong/duplicate COMMIT, result-receipt/evidence mismatch, or an endpoint that reuses/skips a sequence exact-transitions the capability to terminal `Poisoned`; it can never advance or run a later gate. Recovery archives the failed evidence, chooses a new receipt path, creates a new custodian/capability, and reruns A1c+A1d. After A7 COMMIT or failure the driver and custodian close both original endpoints, exact-wait/kill any gate child and the helper, clean partial artifacts/disposable leaves, and record terminal capability evidence. This proves live same-orchestrator-job continuity under `CooperativeWorkerV1`; it is not cryptographic remote attestation against a malicious same-UID process that already controls the job.
- **Runner receipt lifetime:** `runner_receipt_ttl_ms = 7_200_000` from A1c OPEN's one `CLOCK_BOOTTIME` start; no phase or retry resets it. ISO timestamps are diagnostic; `{started_boottime_ns,expires_boottime_ns,host_boot_id}` are authoritative for expiry. A1c COMMIT writes the recursively closed `PartialDelegatedRunnerReceipt {schema:1,phase:"exec-stop",runner_id,capability_id,host_boot_id,kernel_release,uid,observed_at,expires_at,started_boottime_ns,expires_boottime_ns,yama_ptrace_scope,ptrace_policy_sha256,exec_stop_results,evidence_sha256,receipt_sha256}` and binds its hash. A1d OPEN exact-validates that file/hash, then A1d COMMIT writes the final `DelegatedRunnerReceipt` with `partial_receipt_sha256` plus delegation/child-cleanup fields and atomically seals its hash. A5/A5b/A6/A7 must finish before expiry or a new live custodian with a new receipt path reruns A1c+A1d in the current job. Boot change, expiry, different runtime bytes, missing/changed partial receipt, dead/poisoned capability, or live-tuple mismatch fails. Both receipts are integrity evidence only—not a signature, attestation, authentication token, or later-run authority—and are never accepted without successful OPEN plus field/delegation revalidation and successful COMMIT.
...
- **Threat-model owner gate:** RESOLVED 2026-07-11 — owner selected Option 1 (`CooperativeWorkerV1`). This resolves the product boundary only; Draft-21 implementation remains paused for fresh immutable dual review, and Step 4 separately requires capability-bound A1c+A1d PASS.
...
The table applies to every enumerated `LostAuthorityReason`; reason-specific code may not invent strings or another transition. One supervisor/watchdog/controller failure is processed as one atomic batch across every matching operation, so later current reports see an already-fail-closed row rather than an undefined intermediate state. When a row's closed transaction ends in `FencingUnconfirmed` and still has exact live supervisor plus retained child/cgroup custody, that transaction also creates one durable `LossCleanupRecord::Pending` and returns one sealed `LossCleanupPermit` bound to `{worker_id,generation,fence_epoch,post_loss_fencing_version,supervisor_instance_id,control_epoch,loss_batch_sha256,cleanup_handle_slot}`. This permit is the sole exception to the ordinary “current Fencing only” action rule: it is explicitly valid only against the exact post-loss `FencingUnconfirmed` version written by the same transaction. A retained-custody `Attaching→Fencing` row uses the ordinary fence path instead; no live matching custody means no permit and no guessed cleanup.

`perform_loss_cleanup` atomically consumes `Pending@cleanup_version→Running@cleanup_version+1` only after re-resolving the exact post-loss worker/version, supervisor/control epoch, loss batch, and private composite cleanup-handle slot; replay, stale version/epoch, a different loss batch, or another handle emits zero close/signal/RPC bytes. It then performs one indivisible-authority sequence—close all app-server stdio from that slot, emit no further application bytes, kill its retained cgroup or owned child, exact-wait/reap the child, and observe `populated 0` where applicable—under the one monotonic `managed_cancel_grace_ms` deadline. It exact-CASes only that cleanup record to `Completed|Failed`. Reconcile/release/abandon seeing `Pending|Running` refuses without epoch/state change, so cleanup cannot race a new fence. After the deadline, only the watchdog may exact-CAS stale `Running→Failed(SupervisorLost|deadline)` after a failed exact supervisor health/identity check; it emits no signal and cannot remint the permit. Deadline, incomplete reap, lost fd, or supervisor crash therefore remains failed/unconfirmed. The worker stays `FencingUnconfirmed` before, during, and after cleanup. `LossCleanupOutcome` is diagnostic evidence only: it cannot construct `FenceEvidence`, `FenceProofRecord`, `ReleaseReceipt`, `TerminalRetained`, or free a capacity slot even when close/kill/reap/populated-zero all succeed. Only after cleanup is terminal may a separately authorized later reconcile with a new fence epoch or explicit audited abandonment change lifecycle.
...
pub enum LossCleanupState {
    ...
    Failed { reason: String, evidence_sha256: String },
}
...
pub struct RunnerGateCommitAck {
    selectors: RunnerGateCommitSelectors,
    commit_ack_sha256: String, // final JCS chain link; only this advances sequence
}
...
| 1b | From clean `STEP3B_HEAD`, run A0c, create the WIP/range helper, recursively closed immutable historical-owner fixture, and closed mutation-only future allowlist; migrate feasibility evidence to independent `cooperative_cgroup` and Claude-only `filtered_hardening`; prototype raw Codex `thread/start→decision→harmless turn/start` ordering, inventory experimental capability, define hook-health snapshots, prove stable-runtime generation/pointer feasibility, and build the native runner-job driver/custodian/gate with a validation-only Node wrapper. | `plugins/session-relay/rust/src/bin/runner_job_custodian.rs` (new), `plugins/session-relay/test/feasibility-probe.mjs`, `plugins/session-relay/test/fixtures/lifecycle-capability-schema.json`, `plugins/session-relay/test/fixtures/wip-historical-baseline.json` (new), `plugins/session-relay/test/fixtures/wip-step-allowlist.json` (new), `plugins/session-relay/test/runtime-hook-abort.mjs` (new), `plugins/session-relay/test/runtime-hook-upgrade.mjs` (new), `plugins/session-relay/test/runner-job-custodian.mjs` (new), `plugins/session-relay/test/wip-snapshot.mjs` (new) | 1, 3b | planned | A0c then `wip-snapshot.mjs --negative-matrix`, native-driver `runner-job-custodian.mjs --matrix`, `runtime-hook-upgrade.mjs --case stable-generation-pointer --matrix`, A1/A1b, and A0d pass. The historical fixture binds all 35 rows to byte-sorted multi-owner commit arrays and JCS hashes; only the stale plan is `must_be_absent_at_final_tip`. Future rows are exact A/M/D filename mutations; verification-only Step-8/9 fixture mentions are empty. The A1b transcript contains zero pre-decision turn bytes and one harmless post-decision `turn/start` request plus matching successful response. Native positives exact-compare all six frames and PID/credential equality; the closed negative matrix poisons every copy/fd/PID/CLOEXEC/OPEN/COMMIT/sequence/wrapper/cleanup violation. First commit creates the helper/fixtures; A0d authenticates that exact committed range before any further dispatch. No implementation-branch plan/doc edit is permitted. |
| 1c | Through sequence 1 of the still-live owner-runner custodian, feasibility-first prototype the exact blocked-child→host-PID handoff→seize/TRACEEXEC→retained-fd `execveat`→same-tracee exec-stop identity sequence independently for Claude and Codex and begin the closed runner receipt. This is raw evidence, not the Step-4 production implementation. | `plugins/session-relay/test/runner-job-custodian.mjs`, `plugins/session-relay/test/feasibility-probe.mjs`, `plugins/session-relay/test/runtime-hook-abort.mjs`, `plugins/session-relay/test/fixtures/lifecycle-capability-schema.json` | 1b | planned | The exact A1c frame causes the driver to spawn the native credential gate directly; only that PID inherits the capability fd and its first fd action restores CLOEXEC before validation/OPEN. Sequence 1 proves spawn/OPEN/COMMIT/wait/result PID equality, binds the live tuple and exact 64-zero bootstrap input before Node/probe bytes, then COMMIT binds the new partial receipt and exec-stop evidence hashes. The partial unexpired `RELAY_RUNNER_RECEIPT` proves target sentinel absent until detach, followed procfs magic-link fd matching retained fd, `O_NOFOLLOW→ELOOP`, and injected-mismatch kill/reap before target code. A1d must immediately OPEN sequence 2 against that partial hash on the same live capability. Any mandatory row unavailable: poison/STOP before Step 4 and report the primitive unbuildable on the proposed Linux path. |
| 1d | Through sequence 2 of the same live custodian, validate the owner-provisioned delegated Linux runner without self-provisioning: pin `RELAY_CGROUP_ROOT`, create a disposable domain leaf, exercise stopped-child placement, `cgroup.kill`, exact wait/reap, `populated 0`, and leaf removal, then seal the closed integrity-only receipt. | `plugins/session-relay/test/runner-job-custodian.mjs`, `plugins/session-relay/test/runtime-hook-abort.mjs`, `plugins/session-relay/test/fixtures/lifecycle-capability-schema.json` | 1b, 1c | planned | The exact A1d frame preserves direct native-gate spawn/OPEN/COMMIT/wait/result PID equality. OPEN binds its live tuple and A1c partial-receipt hash before Node/delegation/probe-child bytes. It exact-waits/reaps the killed probe child before populated-zero/removal and records every result; COMMIT then binds the new sealed final receipt and delegation/kill/reap/removal evidence hashes before sequence advance. Copied receipt/environment without the inherited fd, wrapper/borrower PID mismatch, duplicate/stale OPEN, authoritative-before-OPEN, missing/wrong COMMIT, timeout, EOF, zombie/unreaped child, receipt expiry/mismatch, or cleanup failure poisons and closes the capability. The current read-only-cgroup/no-user-bus sandbox is not eligible. Missing delegation/runner is never a skip; no Step 4 or A5/A5b/A6/A7 completion begins until resolved. |
...
| 3a | Retain the delivered capability-bound admission and publish-first fencing foundation, limited to its verified surface: target-free lower APIs, closed `ChildLaunchSpec`, no lifecycle-sensitive `exec`, exact guard target/kind/binding re-resolution, and the existing four compile boundaries. Do not call cancellation or inventories complete yet. | `plugins/session-relay/rust/src/lifecycle.rs`, `plugins/session-relay/rust/src/store.rs`, `plugins/session-relay/rust/src/appserver.rs`, `plugins/session-relay/rust/src/bus.rs`, `plugins/session-relay/rust/src/channel.rs`, `plugins/session-relay/rust/src/cli.rs`, `plugins/session-relay/rust/src/hook.rs`, `plugins/session-relay/rust/src/watch.rs`, `plugins/session-relay/rust/src/spawn.rs`, `plugins/session-relay/rust/tests/lifecycle_admission.rs`, `plugins/session-relay/test/fixtures/lifecycle-capability-bypass/Cargo.toml`, `plugins/session-relay/test/fixtures/lifecycle-capability-bypass/Cargo.lock`, `plugins/session-relay/test/fixtures/lifecycle-capability-bypass/src/bin/guardless.rs`, `plugins/session-relay/test/fixtures/lifecycle-capability-bypass/src/bin/wrong-target.rs`, `plugins/session-relay/test/fixtures/lifecycle-capability-bypass/src/bin/fence-reentry.rs`, `plugins/session-relay/test/fixtures/lifecycle-capability-bypass/src/bin/reentry-fence.rs` | 2 | done | Checkpoint `701cea7e671bc40ee23d69abf79ff102e0eecb20` independently passed A3a's narrowed existing guard/compile suite. The false-green cancellation/inventory claims are explicitly deferred to 3b–3d. |
| 3b | Repair and commit the detached watchdog-first supervisor and versioned operation substrate after A0b-bootstrap. | Exact paths are frozen in the historical fixture from commits `06d2324e981eed576d1c8cd3d796a5258f6fd159` and `2a864e9b6f966384e4c4ed0e4b3d563b348a3830`. | 3a | done | A2/A3b, three consecutive 122-check selftests, full Rust, clippy/fmt, and plugin CI passed; clean `STEP3B_HEAD=2a864e9b6f966384e4c4ed0e4b3d563b348a3830`. The caller-death Control repair terminalizes abandoned pre-child startup instead of false `SupervisorLost`. |
| 3c | Add exact turn/child cancellation on the 3b substrate: persist `TurnStartSentUnknown` before send and exact returned `turn.id` before pump; add `TurnCancellationPermit`, exact terminal observation, versioned `CancellationHandoff`, nonblocking 100 ms polling, fixed 5s guard resolution/handoff, and apply every authority loss only through the source-state×reason table. A child handoff remains owned by the detached supervisor. Extend the closed Rust target fixture with the Step-3c target/case before A3c runs. | `plugins/session-relay/rust/src/lifecycle.rs`, `plugins/session-relay/rust/src/appserver.rs`, `plugins/session-relay/rust/src/supervisor.rs`, `plugins/session-relay/rust/src/spawn.rs`, `plugins/session-relay/rust/src/store.rs`, `plugins/session-relay/rust/tests/lifecycle_turn_cancellation.rs` (new), `plugins/session-relay/test/fake-app-server.mjs`, `plugins/session-relay/test/lifecycle-smoke.mjs`, `plugins/session-relay/test/rust-test-inventory.mjs`, `plugins/session-relay/test/fixtures/rust-test-inventory.json`, `plugins/session-relay/test/fixtures/lifecycle-capability-bypass/Cargo.toml`, `plugins/session-relay/test/fixtures/lifecycle-capability-bypass/Cargo.lock`, `plugins/session-relay/test/fixtures/lifecycle-capability-bypass/src/bin/cancel-reentry.rs` (new), `plugins/session-relay/test/reentry-inventory.mjs` | 3b | planned | The Step-3c fixture extension lands before the gate; A3c passes deterministic send/response/terminal, child-ignore-kill, every source/reason/crash row, and operation-version winner/loser races. Any direct Active→FencingUnconfirmed, latest-turn guess, stale mutation, >100 ms individual block, guard held after 5s, dropped child authority, unresolved operation disappearance, or zero/missing target: STOP. |
| 3d | Close the pre-controller source-derived admission surface: make wake/watch mutation-guiding status guard-aware, map every generic outbound mutation primitive/caller available before Step 5 to an executed unique behavior test, invoke every then-existing non-controller `OperationKind` wrapper, and replace all placeholder/name-only counts. Include first-birth collision and wrong/stale/mid-block-fence rows. Step 5 owns `ControllerRead|ControllerInjectItems|ControllerStartTurn` and the post-Step-5 A9/A10 rerun. | `plugins/session-relay/rust/src/appserver.rs`, `plugins/session-relay/rust/src/cli.rs`, `plugins/session-relay/rust/src/watch.rs`, `plugins/session-relay/rust/tests/lifecycle_admission.rs`, `plugins/session-relay/test/reentry-inventory.mjs`, `plugins/session-relay/test/fixtures/reentry-inventory.json`, `plugins/session-relay/test/lifecycle-smoke.mjs`, `plugins/session-relay/test/selftest.mjs` | 3c | planned | A3d passes its explicitly pre-controller matrix with no placeholder behavior ids; its A9/A10 invocation is scoped to variants then present. Any mutation-guiding RPC precedes admission, a stale response guides a later mutation, or an inventory row lacks an executed production-wrapper test: STOP. |
| 4 | **Only after capability-bound A1c+A1d PASS seals the owner-provisioned Linux runner receipt while its custodian remains live**, extend the Step-3b supervisor with Linux pidfd and `ConfinedCgroupCooperative`: fd-pin/hash/fd-exec the exact native runtime; same-tracee TRACEEXEC/proc-magic-link identity; gated placement into a fresh delegated domain leaf; retained generation-bound fd; setup-only placement receipt; post-kill same-fence termination receipt; exact domain/empty-subtree/kill checks; `cgroup.kill`→exact reap→`populated 0`; fail-closed fd loss. Classify `CLONE_INTO_CGROUP` errors exactly; use stop/GO only when independently proven. Add Claude-only best-effort filtered hardening, Darwin closed `UnavailablePlatform`, inventory every signal callsite, and extend the Rust target fixture before A4. | `plugins/session-relay/rust/src/process_identity.rs` (new), `plugins/session-relay/rust/tests/process_identity.rs` (new), `plugins/session-relay/rust/src/supervisor.rs`, `plugins/session-relay/rust/src/spawn.rs`, `plugins/session-relay/rust/src/main.rs`, `plugins/session-relay/rust/src/lifecycle.rs`, `plugins/session-relay/rust/src/store.rs`, `plugins/session-relay/test/runner-job-custodian.mjs`, `plugins/session-relay/test/rust-test-inventory.mjs`, `plugins/session-relay/test/fixtures/rust-test-inventory.json`, `plugins/session-relay/test/process-signal-inventory.mjs` (new), `plugins/session-relay/test/fixtures/process-signal-inventory.json` (new), `plugins/session-relay/test/runtime-hook-abort.mjs` (new), `plugins/session-relay/test/fixtures/lifecycle-capability-bypass/Cargo.toml`, `plugins/session-relay/test/fixtures/lifecycle-capability-bypass/Cargo.lock`, `plugins/session-relay/test/fixtures/lifecycle-capability-bypass/src/bin/fabricated-pidfd-proof.rs` (new), `plugins/session-relay/test/fixtures/lifecycle-capability-bypass/src/bin/tampered-cgroup-proof.rs` (new), `plugins/session-relay/test/reentry-inventory.mjs`, `.github/workflows/build-binaries.yml` | 1c, 1d, 3d | planned | Fixture extension precedes A4. A4/A5/A5b use the literal gate rows, preserve direct native-gate spawn/OPEN/COMMIT/wait/result PID equality, consume live sequences 3–4, and revalidate unexpired runner/boot/kernel/uid/mount/root/runtime receipt fields; they then prove stable signaling, retained-fd/path-race closure, same-seized-tracee exec-stop identity, gated membership, typed fallback, phase-correct kill/reap/populated-0 with no surviving in-model PID for both runtimes, and independently labeled Claude-only filtered hardening. A receipt hash/copy or wrapper/borrowed credential fd never authorizes GO; a pre-kill boundary can never become WorkerTree proof. Missing/dead/mismatched custodian/A1c/runner/A1d: BLOCK/STOP before this step. |
| 5 | Implement managed first-prompt admission, durable controller, post-loss cleanup authority, stable hook runtime/installer, deterministic schema, and exact doctor/install contracts; extend the Rust fixture before A6/A6a and rerun complete A9/A10. | `plugins/session-relay/rust/src/appserver.rs`, `plugins/session-relay/rust/src/hook.rs`, `plugins/session-relay/rust/src/lifecycle.rs`, `plugins/session-relay/rust/src/runtime_install.rs` (new), `plugins/session-relay/rust/src/main.rs`, `plugins/session-relay/rust/src/spawn.rs`, `plugins/session-relay/rust/src/store.rs`, `plugins/session-relay/rust/src/supervisor.rs`, `plugins/session-relay/rust/tests/lifecycle_admission.rs`, `plugins/session-relay/rust/tests/lifecycle_managed.rs`, `plugins/session-relay/rust/tests/lifecycle_controller.rs` (new), `plugins/session-relay/hooks/hooks.json`, `plugins/session-relay/hooks/codex-hooks.json`, `plugins/session-relay/test/appserver-schema-contract.mjs` (new), `plugins/session-relay/test/fixtures/appserver-server-requests.json` (new), `plugins/session-relay/test/rust-test-inventory.mjs`, `plugins/session-relay/test/fixtures/rust-test-inventory.json`, `plugins/session-relay/test/reentry-inventory.mjs`, `plugins/session-relay/test/fixtures/reentry-inventory.json`, `plugins/session-relay/test/lifecycle-smoke.mjs`, `plugins/session-relay/test/fixtures/runtime-doctor-schema.json` (new), `plugins/session-relay/test/fixtures/runtime-doctor-goldens/doctor-ready.json` (new), `plugins/session-relay/test/fixtures/runtime-doctor-goldens/doctor-degraded.json` (new), `plugins/session-relay/test/fixtures/runtime-doctor-goldens/doctor-unavailable.json` (new), `plugins/session-relay/test/fixtures/runtime-doctor-goldens/install-changed.json` (new), `plugins/session-relay/test/fixtures/runtime-doctor-goldens/install-current.json` (new), `plugins/session-relay/test/fixtures/runtime-doctor-goldens/install-lower-no-op.json` (new), `plugins/session-relay/test/fixtures/runtime-doctor-goldens/command-inability.json` (new), `plugins/session-relay/test/fixtures/runtime-doctor-goldens/usage-error.json` (new), `plugins/session-relay/test/fixtures/runtime-doctor-goldens/schema-error.json` (new), `plugins/session-relay/test/fixtures/runtime-doctor-goldens/validation-error.json` (new), `plugins/session-relay/test/fixtures/runtime-doctor-goldens/tamper-error.json` (new), `plugins/session-relay/test/fixtures/runtime-doctor-goldens/io-error.json` (new), `plugins/session-relay/test/fixtures/runtime-doctor-goldens/lock-error.json` (new), `plugins/session-relay/test/runtime-hook-abort.mjs` (new), `plugins/session-relay/test/runtime-hook-upgrade.mjs` (new), `plugins/session-relay/test/selftest.mjs` | 1b-4 | planned | A6/A6a/A6b/A6c plus post-Step-5 A9/A10 prove every Step-5 authority contract already specified in Interfaces. The thirteen golden filenames are the closed set; no directory allowance exists. Managed CLI fallback, pre-Active turn, arbitrary forwarding, reconnect guessing, bytes after loss, cleanup through normal post-loss FencePermit, non-atomic runtime pair, or standalone adversarial-cache-authenticity claim: STOP. A0d must exact-match this row's mutation set. |
| 6 | Consume Step-5 controller and Step-3c exact-turn handoffs through post-drain `FencePermit`; initialize with experimental API capability; implement finite recursive lineage/turn pagination and exact sealed-ref background-terminal list/terminate; mint one sealed `ProtocolProofAttempt` and require its binding on every nested receipt/evidence hash before `FenceEvidence` construction. The real current Codex adapter must always return `MissingDurableFlushContract`; positive graceful reject/flush/watermark/reap/offline remains fake/future-only; shared scans stay observation-only; physical fallback is cgroup kill. Extend the Rust target fixture before A13. | `plugins/session-relay/rust/src/appserver.rs`, `plugins/session-relay/rust/src/spawn.rs`, `plugins/session-relay/rust/src/lifecycle.rs`, `plugins/session-relay/rust/tests/lifecycle_proof.rs` (new), `plugins/session-relay/test/rust-test-inventory.mjs`, `plugins/session-relay/test/fixtures/rust-test-inventory.json`, `plugins/session-relay/test/fake-app-server.mjs`, `plugins/session-relay/test/runtime-appserver-quiescence.mjs` (new), `plugins/session-relay/test/lifecycle-smoke.mjs`, `plugins/session-relay/test/fixtures/lifecycle-capability-bypass/Cargo.toml`, `plugins/session-relay/test/fixtures/lifecycle-capability-bypass/Cargo.lock`, `plugins/session-relay/test/fixtures/lifecycle-capability-bypass/src/bin/fabricated-protocol-proof.rs` (new), `plugins/session-relay/test/reentry-inventory.mjs`, `plugins/session-relay/test/fixtures/reentry-inventory.json` | 3c-5 | planned | Fixture extension precedes A13; A7/A8/A13 prove typed experimental-method failure, real runtime ProtocolIncomplete, private attempt-bound fake proof construction, N/N+1 nested-receipt mix rejection, finite pagination, exact terminal termination, later-fence replay rejection, late/continuous writer rejection, and physical fallback. |
...
| 8 | Complete the already-born per-step adversarial matrices and enforce inventory **completeness only**; preserve all existing relay tests and isolated cleanup. Verify that Steps 3b/3c/4/5/6 each added their target/case before their first acceptance gate, verify the immutable historical range manifest against exact owner commits, then auto-enumerate compile-fail fixtures and close any remaining source-derived rows. Include watchdog/supervisor failures, authority races, every loss source/reason, cancellation, post-loss cleanup permits, controller birth/claim/command authorization, GC, exec-stop/cgroup receipts, live runner-capability literal frames/PID/CLOEXEC/OPEN/COMMIT/sequence/receipt/reap/revalidation, honest hook boundary/installer/schema races, protocol-attempt mixing, proof-retention crash, and missing-flush rows. | `plugins/session-relay/rust/src/appserver.rs`, `plugins/session-relay/rust/src/hook.rs`, `plugins/session-relay/rust/src/lifecycle.rs`, `plugins/session-relay/rust/src/process_identity.rs`, `plugins/session-relay/rust/src/runtime_install.rs`, `plugins/session-relay/rust/src/supervisor.rs`, `plugins/session-relay/rust/src/spawn.rs`, `plugins/session-relay/rust/src/store.rs`, `plugins/session-relay/test/rust-test-inventory.mjs`, `plugins/session-relay/test/fixtures/rust-test-inventory.json`, `plugins/session-relay/test/fixtures/wip-historical-baseline.json`, `plugins/session-relay/test/fixtures/wip-step-allowlist.json`, `plugins/session-relay/test/runner-job-custodian.mjs`, `plugins/session-relay/test/feasibility-probe.mjs`, `plugins/session-relay/test/supervisor-custody.mjs`, `plugins/session-relay/test/lifecycle-smoke.mjs`, `plugins/session-relay/test/process-signal-inventory.mjs`, `plugins/session-relay/test/reentry-inventory.mjs`, `plugins/session-relay/test/runtime-hook-abort.mjs`, `plugins/session-relay/test/runtime-hook-upgrade.mjs`, `plugins/session-relay/test/runtime-appserver-quiescence.mjs`, `plugins/session-relay/test/fake-app-server.mjs`, `plugins/session-relay/test/selftest.mjs` | 2-7 | planned | A1/A1b/A1c/A1d through A14 pass; fixture history/source checks prove no target was first introduced after its gate and every `12cf2ea..STEP3B_HEAD` path/mode/status/oid belongs to named exact commits. Every target executes a source-derived nonzero required set; every compile-fail bin fails at its intended boundary; custodian positives bind exact frame and PID equality; negatives cover shell/assignment/argv injection, non-exec wrapper, borrowed fd, copy-all-bytes/no-fd, descriptor leak, duplicate/stale/out-of-order/concurrent OPEN, authoritative-before-OPEN, missing/wrong/duplicate COMMIT, result/evidence mismatch, EOF/crash/poison and exact cleanup; helpers clean in `finally`; external evidence establishes results. Step 8 may not retroactively make an earlier zero/missing target green or broaden historical ownership. |
| 9 | Document the accepted guarantee tiers and recovery contract; delete the one stale implementation-branch plan; add exact-SHA Darwin and final-scope verification without binary/release changes. | `plugins/session-relay/AGENTS.md`, `plugins/session-relay/skills/productivity/session-relay/SKILL.md`, `plugins/session-relay/rust/src/main.rs`, `docs/plans/active/relay-worker-lifecycle-primitives.md` (delete stale implementation-branch copy only), `plugins/session-relay/test/fixtures/wip-historical-baseline.json` (read-only verification), `plugins/session-relay/test/fixtures/wip-step-allowlist.json` (read-only verification), `plugins/session-relay/test/run-build-matrix.mjs` (new), `plugins/session-relay/test/final-scope.mjs` (new), `.github/workflows/build-binaries.yml` | 1b-8 | planned | A15-A18 pass. `final-scope` verifies the 35-entry historical owner/JCS manifest, the ordered A0d receipt chain, and exact mutation-only future union; the sole historical `must_be_absent_at_final_tip` path has one authenticated Step-9 deletion and is absent. No path is accepted merely because it appeared historically or was mentioned read-only. Main plan remains source of truth; implementation range has zero plan/docs delta. Darwin preserves portable behavior and reports authoritative paths unavailable. |

Every planned row must finish with A0d over its exact `STEP_BASE..STEP_HEAD`; Steps 1c/1d produce authenticated empty-range receipts because they execute already-committed harnesses. In Step 8, `wip-historical-baseline.json` and `wip-step-allowlist.json` are read-only verification inputs, never mutations. In Step 9 those same fixtures remain read-only; the sole plan-path mutation is the authenticated deletion of the implementation-branch copy `docs/plans/active/relay-worker-lifecycle-primitives.md`, represented by one `D` allowlist row with `final_mode:null` and its historical entry hash. The `docs/plans/**` phrase in the task is a zero-delta assertion, not a wildcard mutation allowance. No directory or glob in prose authorizes a mutation.
...
| A0d | Authenticate every committed step range, including Step 1b immediately after its first commit. | `test -n "$STEP_ID" && test -n "$STEP_BASE" && test -n "$STEP_HEAD" && test -n "$STEP_RECEIPT_DIR" && node plugins/session-relay/test/wip-snapshot.mjs --verify-step-range --step "$STEP_ID" --base "$STEP_BASE" --head "$STEP_HEAD" --historical plugins/session-relay/test/fixtures/wip-historical-baseline.json --allowlist plugins/session-relay/test/fixtures/wip-step-allowlist.json --receipt-dir "$STEP_RECEIPT_DIR"` | Exit 0 and print the exact `STEP_RANGE PASS` line. Current clean HEAD, ancestry, A/M/D path/status/mode/oid set, historical overlap hashes, mutation-only allowlist, and canonical receipt all match. An unowned committed-clean change, read-only fixture mutation, missing/extra row, or receipt overwrite fails. |
...
| A1b | Raw Codex API ordering, hook health, stable generation switching, and experimental capability are feasible. | `RELAY_REAL_RUNTIME_TEST=1 node plugins/session-relay/test/feasibility-probe.mjs --case codex-managed-boundary && node plugins/session-relay/test/runtime-hook-upgrade.mjs --case stable-generation-pointer --matrix` | Exit 0 with raw transcript: managed SessionStart runs before the `thread/start` response and returns non-claiming success; the returned id becomes a prototype claim candidate; zero `turn/start` request bytes occur before the explicit prototype decision; immediately after that decision the harness sends exactly one harmless `turn/start` request and receives its matching successful response with the same JSON-RPC id/thread id. A zero-turn transcript is a hard failure. This remains `feasibility_only`, not production binding. Hook snapshots and the named generation/pointer crash/race matrix pass; missing auth is unavailable/STOP, never skip. Production Rust claim is A6 only. |
| A1e-custodian-matrix | Prove the Node wrapper cannot substitute for the native socket/custodian/gate authority. | `test -x "$RUNNER_BUILD_DIR/debug/runner_job_custodian" && node plugins/session-relay/test/runner-job-custodian.mjs --matrix --native-driver "$RUNNER_BUILD_DIR/debug/runner_job_custodian"` | Exit 0 with native positive PID/SCM-credential/CLOEXEC/OPEN/COMMIT/reap rows and every named negative. Source assertions prove Node contains no socketpair/ancillary-credential/driver/gate-spawn/cleanup authority. The wrapper validates a native cleanup result proving exact waits and sentinel-only removal. |
| A1c | The exact retained-fd/TRACEEXEC identity architecture is buildable for each mandatory Linux runtime on the proposed delegated runner, and sequence 1 proves live job continuity. | Submit the exact literal `A1c` `RunGate` frame from Environment to the still-live custodian; no shell or caller-built argv/env is permitted. | The driver directly spawns the native gate, which inherits only the fixed capability fd, restores CLOEXEC as its first fd action before validation/OPEN, and satisfies `spawn PID = OPEN credential PID = COMMIT credential PID = wait PID = GateResult.pid`. The gate/Node environments contain only the literal base, sealed auth, exact frame, and gate-only fd names; any ambient/duplicate/unsealed name fails before spawn. OPEN sequence 1 binds input exactly `RUNNER_BOOTSTRAP_INPUT_SHA256` and the fresh live tuple before the native gate spawns Node or either runtime probe emits a child/ptrace/exec byte. Claude and Codex rows then pass and write one two-hour unexpired closed partial receipt containing public `capability_id` but no challenge/secret/fd. COMMIT exact-binds that new partial receipt hash plus exec-stop evidence hash and only then emits GateResult/advances. Each raw row proves blocked final child, exact host PID handoff, ancestor `PTRACE_SEIZE|PTRACE_O_TRACEEXEC`, retained-fd `execveat`, exact event PID equal to seized tracee, target sentinel absent, followed `/proc/<pid>/exe` magic-link fd fstat/hash equal to retained fd while stopped, then detach and sentinel. `O_NOFOLLOW` yields `ELOOP`; wrong PID/event/hash/early release/timeout kills and reaps before sentinel. Any mandatory row unavailable poisons and is a hard STOP before Step 4. Copying every receipt/environment byte with no inherited connected fd, a non-exec shell wrapper, or a sibling-borrowed fd fails OPEN and poisons before Node/probe bytes; missing/wrong COMMIT prevents sequence 2; challenge bytes never enter either receipt. |
| A1d | The same live custodian supplies sequence 2 while the owner-provisioned Linux job validates real writable cgroup-v2 delegation and completes the receipt. | Submit the exact literal `A1d` `RunGate` frame from Environment to that same custodian. | Direct native-gate PID equality holds across spawn/OPEN/COMMIT/wait/result. OPEN sequence 2 exact-validates the A1c partial receipt/hash and binds the fresh matching capability/runner/boot/kernel/uid/runtime tuple before Node, delegation, or probe-child work. It then pins the root dirfd/mount/dev/inode, validates same-uid writable domain delegation, creates a unique disposable child leaf, validates empty subtree plus writable `cgroup.procs|cgroup.kill|cgroup.events`, moves an exact stopped child, kills it, exact-waits/reaps it with recorded status, observes `populated 0`, removes the leaf, and seals canonical JCS final `receipt_sha256` carrying `partial_receipt_sha256`. COMMIT exact-binds that final receipt and delegation/kill/reap/removal evidence hashes before GateResult/sequence 3. Hashes are integrity only, not signature/auth. Copy-all-bytes/no-fd, wrapper/borrowed-fd PID mismatch, duplicated-fd replay, stale/out-of-order/concurrent OPEN, authoritative-before-OPEN, missing/wrong/duplicate COMMIT, EOF, absent/read-only delegation, timeout, zombie/unreaped child, populated mismatch, receipt mismatch/expiry, or cleanup failure is FAIL/BLOCK, poisons the capability, and never skips. |
...
| A9 | Every mutator has a concrete matching capability and executed behavior evidence. | `node plugins/session-relay/test/reentry-inventory.mjs` | Exit 0; scans generic app-server primitives/callers, every drain/resume/inject/start/interrupt/list/terminate, process create/exec/signal, pending acknowledgement, and all Step-5 controller variants. Existing-session actions map to target-free `ReentryGuard`, exact-owned-turn cancellation, or post-drain `FencePermit`; managed first birth maps only to the sealed exact-tuple one-use `ManagedBirthPermit`. Pure reads cannot guide later mutation after stale. Source-derived N is reported; every row has a unique production behavior id; no rationale/name-only or generic “first birth” exemption passes. |
| A10 | Every production re-entry/controller surface enforces target/kind/binding/cancellation epoch at use. | `node plugins/session-relay/test/lifecycle-smoke.mjs --case reentry-matrix` | Exit 0 after Step 5; one production-wrapper row per source-derived `OperationKind`, including `ControllerRead|ControllerInjectItems|ControllerStartTurn`, covers success, wrong kind, stale binding/controller/operation version, pre-fence, and mid-block fence with no-mutation evidence. Wake/watch status is admitted and stale responses discarded. Fenced rows emit no context/RPC/process; queue/peek remain allowed. No placeholder label. |
...
| A16 | Four architectures compile; Darwin runs the portable supervisor and reports authoritative managed paths unavailable. | `node plugins/session-relay/test/run-build-matrix.mjs --ref codex/primitives-collab --sha "$(git rev-parse HEAD)"` | **Owner approval is still required immediately before push/dispatch.** Exact clean remote SHA only. Linux targets compile. Native macos-15-intel and macos-15 run the portable A3b getpeereid/session/PTY/signal/EOF/health/watchdog subset and explicit constructors/status rows proving managed controller, TRACEEXEC fd-exec, pidfd, and cgroup WorkerTree are typed `UnavailablePlatform` with no child/RPC/signal side effect. No Darwin equivalent is advertised. |
...
| A18 | The exact clean implementation tip equals the immutable historical-owner baseline plus the ordered union of authenticated per-step A0d receipts and contains no binary/release/docs mutation. | `test "$(git branch --show-current)" = codex/primitives-collab && test "$(git rev-parse HEAD)" = "$IMPL_TIP" && test -z "$(git status --porcelain)" && git diff --check 12cf2ea.."$IMPL_TIP" && node plugins/session-relay/test/final-scope.mjs --verify-range --base 12cf2ea --tip "$IMPL_TIP" --historical plugins/session-relay/test/fixtures/wip-historical-baseline.json --allowlist plugins/session-relay/test/fixtures/wip-step-allowlist.json --receipts "$STEP_RECEIPT_DIR" --step-order 1b,1c,1d,3c,3d,4,5,6,7,8,9 --emit-manifest /tmp/relay-final-scope-manifest.json && node plugins/session-relay/test/final-scope.mjs --negative-matrix --base 12cf2ea --tip "$IMPL_TIP" --historical plugins/session-relay/test/fixtures/wip-historical-baseline.json --allowlist plugins/session-relay/test/fixtures/wip-step-allowlist.json --receipts "$STEP_RECEIPT_DIR" --step-order 1b,1c,1d,3c,3d,4,5,6,7,8,9` | Exit 0 with exact HEAD/clean status and `FINAL_SCOPE PASS files=<N> manifest_sha256=<hex> historical_sha256=<hex> receipts_sha256=<hex>`. Helper re-hashes all 35 historical entries/byte-sorted owners, validates the exact contiguous ordered receipt chain and each receipt against its Git range, then compares the deduplicated A/M/D receipt union to the mutation-only allowlist and actual `12cf2ea..IMPL_TIP` raw diff. The stale plan is allowed at the historical tip but must be absent at the final tip through the sole authenticated Step-9 `D`; Step-8/9 read-only fixture mentions authorize nothing. Missing/reordered/forked/overwritten/extra receipt, unowned committed-clean mutation, old/new path or mode/type/gitlink/status deviation, wildcard, binary/version/release surface, or any net `docs/plans/**` delta fails. |
...
- First birth is not session re-entry. Managed Codex app-server birth therefore takes only the sealed one-use `ManagedBirthPermit` minted from its exact Attaching worker/generation/version/tool/cwd/birth-operation tuple; unmanaged hook-born CLI remains a separately classified creation whose collision path rejects an existing fenced id. This does **not** exempt `turn/start`: it targets an existing thread, uses `ControllerStartTurn`, and its returned exact turn id becomes durable custody before pumping.
...
- `cgroup.events populated 0` may precede parent reap. A1d and production cleanup must exact-wait/reap every owned probe/root child before accepting populated-zero and removing the leaf.
...
- One live owner-runner custodian does not create and retain the private `AF_UNIX SOCK_SEQPACKET` capability across exact A1c→A1d→A5→A5b→A6→A7 sequences; a RunGate differs from its literal closed script/args/env row, accepts shell text/assignment syntax/arbitrary executable/cwd/env, admits an ambient/duplicate/unsealed base or auth name, or launches through a wrapper rather than directly spawning the native credential gate; the fixed fd is not the only intentional capability inheritance, is not restored CLOEXEC as the native gate's first fd action before validation/OPEN, or leaks to Node/runtime/helper; `driver_spawn_pid`, OPEN/COMMIT `SCM_CREDENTIALS.pid`, selectors `gate_pid`, exact-wait PID, and `GateResult.pid` are not identical; non-exec shell-wrapper or borrowed-fd senders can OPEN/COMMIT or emit authoritative bytes instead of poisoning; A1c OPEN does not use the exact bootstrap sentinel; capability/gate/input-receipt/live-tuple selectors differ; an authoritative probe/child/RPC/fence byte precedes OPEN acknowledgement; COMMIT does not bind the exact result-receipt and evidence hashes before sequence advance; duplicate/stale/out-of-order/concurrent OPEN, missing/wrong/duplicate COMMIT, result/evidence mismatch, EOF, crash, or replay does not poison/close the capability; copied receipt/environment bytes pass OPEN without the fd; the receipt expires; its hash is treated as signature/auth/GO authority; the probe child is not exact-waited/reaped before populated-zero/removal; or live gate revalidation fails. The current read-only cgroup/no-user-bus sandbox cannot be relabeled positive.
...
- Claude/Codex authentication preflight is unavailable or not marker-exact. Current Claude `loggedIn:false` is an external blocker until corrected; it is never a skip, fixture, zero-turn pass, or reason to weaken isolated-home/auth handling.
...
- Any Step-3c/4/5/6 Rust target/case is first added after A3c/A4/A6a/A13, Step 8 retroactively legitimizes a prior zero-test pass, or `ControllerRead|ControllerInjectItems|ControllerStartTurn` are absent from the enum/matrix/post-Step-5 A9/A10 run.
...
- A1 can pass with fabricated booleans, A5b does not run both real runtimes in recorded live capability/delegation context, A6 omits Claude timeout/block or Codex pre-Active zero-turn evidence, A6b omits old+new session ordinary-prune/known-corruption/post-installer-pinning rows or relabels deliberate same-UID replace-all as standalone authenticity, A7 never starts real writers, or A9 uses a fixed count/placeholder test id.
...
- A0b-bootstrap hashes differ before first repair, the first Step-3b edit precedes that gate, Step 1b starts before clean `STEP3B_HEAD`, `wip-snapshot --negative-matrix` is incomplete, any step omits A0d after committing, A0d accepts an unowned committed-clean mutation, the external receipt chain is missing/forked/reordered, or final A18 accepts dirty/non-current HEAD.
...
- A18 uses a wildcard/regex or “seen historically” allowance, fails to bind exact `12cf2ea..STEP3B_HEAD` path/status/old+new-mode/oids to byte-sorted owners/full commits, lets a future row rewrite/broadly absorb a historical fact, fails to require `must_be_absent_at_final_tip`, omits the ordered A0d receipt union or old/new paths/mode/type/gitlink/status classes, accepts a path/mode outside the authenticated historical-plus-receipt union, or fails to emit/recheck historical/receipt/final manifest hashes.
- A1b records zero turn traffic after the prototype decision, sends any `turn/start` before it, or lacks one matching harmless post-decision request/response. The stable-generation/pointer matrix is absent or nondeterministic.
...
2. **Environment & commands:** present — path-specific plan/blob, exact clean Step-3b head, one-time bridge, recursively closed historical/future fixtures, pre-edit A0b plus post-commit A0d receipts, cross-filesystem handoff, four targets, native Rust custodian build/invocation/cleanup, six literal frames, isolated real-runtime setup/auth STOP, and owner-provisioned delegated Linux prerequisite are explicit.
3. **Interface & data contracts:** present — byte-sorted multi-owner/JCS historical entries, mutation-only A/M/D future rows, immutable per-step receipts, native driver/custodian/gate socket authority with validation-only Node wrapper, exact-tuple ManagedBirthPermit, non-claiming SessionStart/controller claim, post-loss cleanup permit, stable custody versions, executable-bound cgroup receipts, attempt-bound proof, immutable runtime generations, thirteen exact doctor goldens, tombstone GC, and operator APIs are explicit.
4. **Executable acceptance:** present — A0/A0b-bootstrap/A0b-matrix/A0c/A0d through A18 plus the native custodian and stable-pointer matrices are exact commands with expected evidence. A1b requires one harmless post-decision request and matching response, never a zero-turn pass. A18 consumes the ordered receipt chain and revalidates historical owners/JCS, read-only mentions, the one authenticated stale-plan deletion, and every final diff class.
...
Adversarial cold-read result: Draft-22 preserves every Draft-21 authority contract while removing the Step-1b guesses. The 35-row historical range is recursively closed with real multi-owner arrays and exact JCS digests; future authorization is mutation-only and filename-exact; read-only Step-8/9 mentions grant nothing; Step 9 alone authenticates the stale-plan deletion. A0b binds incoming WIP and A0d binds every committed step range into one ordered A18 receipt chain. Node 24's missing socketpair/ancillary-credential surface is handled without weakening: the pinned Rust autobin alone owns driver/custodian/gate socket authority, while Node validates closed frames/results. The named matrices are executable, cleanup is sentinel-bound, and A1b cannot pass without one post-decision harmless request/response. Current Claude authentication remains an external STOP. LossCleanupPermit, ManagedBirthPermit, controller variants, exact reap/live delegation, TRACEEXEC, installer/schema pinning, same-version tuple, docks-kit optional verified-native path, and Linux/Darwin split are unchanged. No cold executor must infer a path owner, mutation, socket authority, receipt boundary, or zero-turn exception.
...
Score: **98/100 (Draft-22 author candidate; fresh immutable X/S review pending)** · trajectory **97 Draft-20 author → 95 acceptance NOT READY → 98 Draft-21 → 98 Draft-22 executable ownership/range repair** · stopped: **bounded blocker repair; implementation paused; Claude auth and owner-provisioned Linux runner remain external gates**.
