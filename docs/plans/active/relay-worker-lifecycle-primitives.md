...
updated: "2026-07-11T23:52:32-03:00"
...
- Codex plugin hooks are non-managed and skipped until each exact definition hash is trusted. Managed launch must prove both relay hook rows through the supported app-server `hooks/list` trust view for the target cwd and refuse before spawn on drift; plugin enablement and the test-only bypass are not production proof.
...
|---|---|---|
| Hook abort | Managed Codex launch first requires exact `hooks/list` trust/hash for both relay hooks and refuses before spawn on drift. Managed Claiming publication is the first short locked transaction; SessionStart returns universal structured stop on failure where honored; UserPromptSubmit independently blocks non-Active workers; both handlers pin 30 seconds; a detached supervisor retains an unreaped child handle and a measured deadline no greater than 20 seconds. | Plugin enablement is not hook trust; production bypass is forbidden. Claude SessionStart stop is empirical, not the sole barrier. If a required hook is skipped/regresses after the accepted cooperative launch-input check, watchdog action is bounded best effort, not a hard scheduler guarantee. Root kill alone is not tree proof. |
...
- **Confirmed-source rule:** Step 1 preserves the original evidence; Step 1b migrates every stale verdict and adds hook-trust/experimental-capability evidence. Neither may substitute hand-written booleans or re-argue the docs.
- **Local commands:**
  ```bash
...
- **Codex hook trust gate:** plugin hooks are non-managed and current Codex skips a new/changed definition until its exact hash is trusted. Before any managed Codex child receives a user prompt, query the installed app-server `hooks/list` for the exact target cwd and require both session-relay `SessionStart` and `UserPromptSubmit` rows to be enabled, `trustStatus=trusted`, and `currentHash` equal to the pinned installed definitions. Re-stat/re-hash the source/config inputs immediately before spawn and fail closed on any change. Production never writes Codex's internal trust state and never uses `--dangerously-bypass-hook-trust`; an untrusted/missing/changed row refuses before child spawn with an actionable `/hooks` instruction. The feasibility harness may use the bypass only inside its isolated, allowlisted test home and must label that row `test_only_bypass=true`.
...
| Input | Result |
|---|---|
| Concurrent duplicate with same claimed token + same runtime id/generation/tool/cwd | Join the one durable claim and return its exact committed result without version/state churn: Active only if the original finalized Active, otherwise the same structured stop/refusal. |
...
The closed worker transitions are `Attaching→Active|Fencing`; exceptional `Attaching→FencingUnconfirmed` only in the exact atomic cancel-deadline bind above; `Active→Fencing`; `Fencing→Fenced|FencingUnconfirmed`; `FencingUnconfirmed→Fencing` only through reconcile; `Fenced→TerminalRetained` only through crash-resumable proof retention; `TerminalRetained→TerminalReleasable` only through explicit release; `FencingUnconfirmed→TerminalReleasable` only through audited abandonment. `Active→Fencing` and reconcile mint `fence_epoch=Some(new_uuid)` and the resulting worker `version` is the exact `fencing_version`; later states clear the live field only after copying both values into proof/audit records. Closed unmanaged transitions are `Unmanaged@N→UnmanagedCanceling@N`; exact reap with a durable waiting claim `UnmanagedCanceling@N→Claiming@N+1`; exact reap/reconcile without one `→Unmanaged@N+1`; exact cancel deadline with a durable waiter `→Managed@N+1` paired with the exceptional worker transition; risk-accepted `abandon-session→Unmanaged@N+1` with `NOT QUIESCENCE-PROVEN` receipt. No other transitions exist. Only releasable managed records and terminal unmanaged-operation tombstones with their required receipt may age out. Binding GC alone may exact-CAS plain `Unmanaged→GcDeleting→removed`; `UnmanagedCanceling` is never GC-eligible. Failed pre-delete lock acquisition restores untouched Unmanaged; once deletion starts `GcDeleting` is crash-resumable. `Claiming` and `Managed` never age independently.
...
pub enum SupervisorEvent {
    ...
    ControlBound { operation_id: String, operation_version: String, control_nonce: String },
    Output { operation_id: String, operation_version: String, sequence: String, stream: String, bytes_b64: String },
    ...
    LostAuthority { operation_id: String, operation_version: String, reason: String },
}
...
The supervisor is the hidden detached `relay __lifecycle-supervisor` process, started with a real new session and no parent-death signal. On pinned Rust 1.85, `CommandExt::process_group(0)` is insufficient and stable `CommandExt::setsid` is unavailable; use the minimal async-signal-safe `pre_exec` wrapper around `libc::setsid()` and propagate failure from spawn. Acceptance proves the supervisor SID differs from the caller and equals the supervisor PID. It owns a mode-0600 Unix socket and validates same-UID peer credentials (`SO_PEERCRED` on Linux, `getpeereid` on Darwin), exact socket dev/inode, supervisor instance/version, operation/version, and current Claim/Fence authority from the store; caller-supplied command fields are selectors, never authority. The first frame is mandatory `SupervisorHello`: `Health` receives one identity-bound Pong and its EOF is always a no-op; exactly one current `Control` stream may bind an operation/version, receives `ControlBound`, and only that stream's EOF can publish cancellation. A second, stale, or role-confused control stream is refused. No operation reaches `ChildStarting` and no child spawns before its exact `ControlBound`. `Input` carries exact bytes (max 64 KiB/frame), `InputEof` closes only child stdin, `ResizePty` is bounded to 1..4096 rows/cols and the named PTY group, and `ForwardSignal` accepts only SIGINT/SIGTERM/SIGHUP. Output frames preserve exact per-stream bytes and carry the current operation version plus monotonic sequence; PTY endpoints preserve `isatty`, controlling terminal, foreground group, Ctrl-C, and resize behavior. Each direction has a 1 MiB bounded queue. A slow connected caller applies child-visible pipe/PTY backpressure as before, while cancellation/control polling runs independently at ≤100 ms. Every required proxy/control thread spawn and queue initialization must succeed before Ready/ControlBound; failures propagate and cancel startup rather than being discarded, and producer counts increment only after successful spawn.
...
Supervisor startup/crash recovery is explicit. A separate hidden detached `relay __lifecycle-watchdog` process spawns and owns the supervisor `Child` from birth, records its own generation-bound identity, waits/reaps it, and on exit exact-CASes every matching unresolved operation to `LostAuthority` plus lifecycle `FencingUnconfirmed` within `managed_cancel_grace_ms`. Global `Starting→Ready` means the listener is bound, exact socket identity and watchdog registration are durable, heartbeat is live, and every required service thread started successfully; operation launch still waits for its own `ControlBound`. Every admission, claim, fence drain, status, and reconcile performs a ≤100 ms `Health` exchange plus supervisor/watchdog generation and heartbeat check before mutation; health EOF never changes custody. Missing supervisor/watchdog records while any nonterminal `ChildStarting|ChildOwned|ChildCancelRequested|LostAuthority` operation exists are fail-closed: exact-CAS loss where authority still matches and refuse the mutation. If the watchdog dies first, the next such check marks authority lost; it never reconstructs/adopts the supervisor or worker Child. Tests cover supervisor death with no connected caller (watchdog publishes within 5s), watchdog-then-supervisor death (first re-entry refuses/CASes), health-first/control-first connection order, stale replacement instances, and thread-start failure. `QueryOwned` is observation only. The supervisor is also the later Step-4 cgroup manager; Step 3b adds portable owned-child custody, bidirectional stdio/PTY IPC, exact cancellation, watchdog, and reap proof, while pidfd/cgroup authority remains Step 4.
...
pub struct CgroupBoundary {
    ...
    capability: CooperativeCapabilityAttestation,
...
// A5b attests the runtime/platform capability once; a production worker binds its hash rather
// than rerunning test-fixture scenarios.
pub struct CooperativeCapabilityAttestation {
    runtime: String,
...
Proof scope remains exhaustive: `ProcessProof` satisfies only `ProcessOnly`; `ProtocolTreeProof` only `ProtocolTree`; only `WorkerTreeProof` satisfies `WorkerTree`. Every proof-critical protocol field is private/sealed and bound to worker, generation, server fingerprint, supervisor, fence epoch, and fencing version; only a private adapter builder holding the exact `FencePermit` constructs the complete proof. `DedicatedOfflineBoundaryProof` is constructible only by an adapter that reports a mutation-rejecting durable flush watermark covering every accepted mutation and whose complete offline scan reaches that watermark. The current Codex adapter is hard-coded `UnavailableCurrentCodexAppServer` and cannot construct it; the positive path exists only in the fake/future-adapter contract test. Freeze evidence is deliberately absent from this type. A `TrackedTree` backend always returns `Unconfirmed`, root exit never promotes to tree proof, and unknown evidence/backend variants fail closed. `CgroupTreeProof` satisfies WorkerTree only for `CooperativeWorkerV1`; its setup-time boundary cannot be promoted without the same-fence sealed `CgroupKillReceipt`, and attempting to omit or widen that threat-model tag fails closed.

All `CgroupBoundary`, placement/capability/kill/filtered receipt, and `CgroupTreeProof` fields are private. Public methods are read-only accessors returning copies/borrows; no mutable accessor or public constructor exists. A5b produces the platform/runtime `CooperativeCapabilityAttestation`; production workers bind that attestation hash but do not rerun fixture scenarios. Only the live matching supervisor may construct the final `CgroupTreeProof`, after re-resolving its private `SupervisorFencePermit`, writing kill, observing populated-zero, validating every in-model membership row, and binding the exact fence epoch/version. Compile-fail fixtures reject construction and field mutation; runtime tests prove a stale supervisor or altered serialized observation cannot enter `FenceEvidence::WorkerTree`.
...
Before the Codex supervisor spawns a managed child, it performs the exact `hooks/list` trust gate from Environment for that cwd and immediately revalidates the returned source/hash inputs. Missing, disabled, untrusted, changed, ambiguous, or duplicate session-relay hook rows refuse before child creation and print the exact `/hooks` recovery instruction. The production path never passes `--dangerously-bypass-hook-trust`; that flag would authorize unrelated otherwise-untrusted hooks. A deterministic barrier changes one hook definition after listing but before spawn and must still produce zero child/model bytes. This is a cooperative single-user launch-input guarantee: concurrent hostile mutation of Codex configuration is outside the threat model, but every observed drift fails closed.
...
| 1b | Migrate feasibility evidence to independent `cooperative_cgroup` and Claude-only `filtered_hardening` verdicts with optional freeze; add current Codex `hooks/list` production-trust evidence and app-server experimental-capability inventory. The isolated harness may keep its explicitly labeled test-only hook-trust bypass; production may not. | `plugins/session-relay/test/feasibility-probe.mjs`, `plugins/session-relay/test/fixtures/lifecycle-capability-schema.json`, `plugins/session-relay/test/runtime-hook-abort.mjs` (new), `docs/plans/active/relay-worker-lifecycle-primitives.md:Notes` | 1 | planned | A1/A1b pass. Cooperative availability is independent of seccomp and records exact manager-root delegation; both relay hook rows are enabled/trusted/exact-hash for the target cwd or managed Codex launch is unavailable before spawn. |
...
| 3b | Add the minimal detached lifecycle supervisor/watchdog and versioned operation substrate before turn cancellation: supervisor owns every transition-capable Child from birth; uses role-authenticated Health/Control connections; preserves Closed/pipe/PTY stdin, EOF, output, resize, Ctrl-C/signal, exit, and backpressure; disconnect publishes exact stable cancellation authority and drains/reaps; watchdog detects death without a caller. Add durable waiting claims, `UnmanagedCanceling`, exact disconnect-vs-Claiming transfer, closed commands, private child permit, operation/handoff versions, custody transitions, sealed reap proofs, tombstones, and internal session-operation recovery state. No pidfd/cgroup or user-facing recovery CLI yet. | `plugins/session-relay/rust/Cargo.toml`, `plugins/session-relay/rust/Cargo.lock`, `plugins/session-relay/rust/src/supervisor.rs` (new), `plugins/session-relay/rust/src/main.rs`, `plugins/session-relay/rust/src/lib.rs`, `plugins/session-relay/rust/src/lifecycle.rs`, `plugins/session-relay/rust/src/spawn.rs`, `plugins/session-relay/rust/src/store.rs`, `plugins/session-relay/rust/src/hook.rs`, `plugins/session-relay/rust/tests/lifecycle_managed.rs`, `plugins/session-relay/rust/tests/lifecycle_supervisor.rs` (new), `plugins/session-relay/test/supervisor-custody.mjs` (new), `plugins/session-relay/test/reentry-inventory.mjs`, `plugins/session-relay/test/fixtures/reentry-inventory.json`, `plugins/session-relay/test/fixtures/lifecycle-capability-bypass/Cargo.toml`, `plugins/session-relay/test/fixtures/lifecycle-capability-bypass/Cargo.lock`, `plugins/session-relay/test/selftest.mjs`, `plugins/session-relay/test/fixtures/lifecycle-capability-bypass/src/bin/child-cancel-reentry.rs` (new), `plugins/session-relay/test/fixtures/lifecycle-capability-bypass/src/bin/fabricated-owned-proof.rs` (new) | 3a | in-flight | A2/A3b prove durable waiter crash/retry, interactive/noninteractive compatibility, role races, real new-session detach, disconnect/flood, watchdog death publication, exact reap, stale zero-byte behavior, and no Child/PID reconstruction. Re-entry inventory edits in this step are limited to registering/running its two compile-fail bins. If any pre-claim attach lacks exact cancellation or bidirectional supervisor behavior cannot be preserved, STOP as unbuildable. |
| 3c | Add exact turn/child cancellation on the 3b substrate: persist `TurnStartSentUnknown` before send and exact returned `turn.id` before pump; add `TurnCancellationPermit`, exact terminal observation, versioned `CancellationHandoff`, nonblocking 100 ms polling, and fixed 5s guard resolution/handoff. Unknown/lost authority exact-CASes `FencingUnconfirmed`; a child handoff remains owned by the detached supervisor. | `plugins/session-relay/rust/src/lifecycle.rs`, `plugins/session-relay/rust/src/appserver.rs`, `plugins/session-relay/rust/src/supervisor.rs`, `plugins/session-relay/rust/src/spawn.rs`, `plugins/session-relay/rust/src/store.rs`, `plugins/session-relay/rust/tests/lifecycle_turn_cancellation.rs` (new), `plugins/session-relay/test/fake-app-server.mjs`, `plugins/session-relay/test/lifecycle-smoke.mjs`, `plugins/session-relay/test/fixtures/lifecycle-capability-bypass/Cargo.toml`, `plugins/session-relay/test/fixtures/lifecycle-capability-bypass/Cargo.lock`, `plugins/session-relay/test/fixtures/lifecycle-capability-bypass/src/bin/cancel-reentry.rs` (new), `plugins/session-relay/test/reentry-inventory.mjs` | 3b | planned | A3c passes deterministic send/response/terminal and child-ignore-kill barriers plus operation-version winner/loser races. Any latest-turn guess, stale mutation, >100 ms individual block, guard held after 5s, dropped child authority, or unresolved operation disappearance: STOP. |
...
| 5 | Implement managed hook abort with Claiming publication first, bounded old-epoch drain, exact idempotent resume, Codex documented/Claude version-pinned SessionStart universal stop, required cross-runtime UserPromptSubmit block, explicit `timeout: 30` on both events, supported Codex `hooks/list` trust preflight, measured dead-man, and ordinary behavior compatibility. | `plugins/session-relay/rust/src/appserver.rs`, `plugins/session-relay/rust/src/hook.rs`, `plugins/session-relay/rust/src/spawn.rs`, `plugins/session-relay/rust/src/store.rs`, `plugins/session-relay/hooks/hooks.json`, `plugins/session-relay/hooks/codex-hooks.json`, `plugins/session-relay/test/runtime-hook-abort.mjs` (new), `plugins/session-relay/test/selftest.mjs` | 1b-4 | planned | A2/A6 prove two-phase claim plus isolated Claude/Codex event×timeout/trust matrix. Untrusted or drifted Codex hooks refuse before child spawn; production never uses the bypass flag. Any first-prompt sentinel, implicit timeout, pre-Claiming hook work, or untrusted-hook child spawn: STOP. |
| 6 | Consume Step-3c exact-turn handoffs through post-drain `FencePermit`; initialize with experimental API capability; implement finite recursive lineage/turn pagination and exact background-terminal list/terminate; seal and fence-bind every protocol proof field. The real current Codex adapter must always return `MissingDurableFlushContract` for ProtocolTree. Exercise the positive graceful reject/flush/watermark/reap/offline contract only through a fake/future adapter; shared scans stay observation-only; physical fallback is `cgroup.kill`→`populated 0`. | `plugins/session-relay/rust/src/appserver.rs`, `plugins/session-relay/rust/src/spawn.rs`, `plugins/session-relay/rust/src/lifecycle.rs`, `plugins/session-relay/test/fake-app-server.mjs`, `plugins/session-relay/test/runtime-appserver-quiescence.mjs` (new), `plugins/session-relay/test/lifecycle-smoke.mjs`, `plugins/session-relay/test/fixtures/lifecycle-capability-bypass/Cargo.toml`, `plugins/session-relay/test/fixtures/lifecycle-capability-bypass/Cargo.lock`, `plugins/session-relay/test/fixtures/lifecycle-capability-bypass/src/bin/fabricated-protocol-proof.rs` (new), `plugins/session-relay/test/reentry-inventory.mjs`, `plugins/session-relay/test/fixtures/reentry-inventory.json` | 3c-4 | planned | A7/A8/A13 prove typed experimental-method failure, real runtime ProtocolIncomplete, private/fence-bound fake-adapter proof construction, finite pagination, exact terminal termination, later-fence replay rejection, late/continuous writer rejection, and physical fallback. |
...
| 8 | Complete adversarial unit/cross-process/real-runtime matrices and source-inventory enforcement; preserve all existing relay tests and isolated cleanup. Include supervisor caller-exit/crash, operation CAS losers, cancellation send/response/terminal barriers, Unmanaged attach/Claiming, GC-vs-claim pause points, raw ABI/procfs containment, optional-freeze rejection, hook-trust drift, proof-retention crash, and missing-flush runtime rows. | `plugins/session-relay/rust/src/appserver.rs`, `plugins/session-relay/rust/src/hook.rs`, `plugins/session-relay/rust/src/lifecycle.rs`, `plugins/session-relay/rust/src/process_identity.rs`, `plugins/session-relay/rust/src/supervisor.rs`, `plugins/session-relay/rust/src/spawn.rs`, `plugins/session-relay/rust/src/store.rs`, `plugins/session-relay/test/feasibility-probe.mjs`, `plugins/session-relay/test/supervisor-custody.mjs`, `plugins/session-relay/test/lifecycle-smoke.mjs`, `plugins/session-relay/test/process-signal-inventory.mjs`, `plugins/session-relay/test/reentry-inventory.mjs`, `plugins/session-relay/test/runtime-hook-abort.mjs`, `plugins/session-relay/test/runtime-appserver-quiescence.mjs`, `plugins/session-relay/test/fake-app-server.mjs`, `plugins/session-relay/test/selftest.mjs` | 2-7 | planned | A1/A1b through A14 pass; helpers clean children/stores in `finally`; externally observable sentinels and kernel/protocol evidence, not elapsed time alone, establish results. Cooperative-scope test labels are machine-checked. |
| 9 | Document guarantee tiers, `CooperativeWorkerV1`, exact cancellation custody, current `ProtocolTree` unavailability, hook-trust/version/timeout contract, cgroup placement fallbacks, GC, and operator recovery; add an exact-SHA native matrix that runs the portable supervisor suite on both macOS architectures and full gates without binary/release changes. | `plugins/session-relay/AGENTS.md`, `plugins/session-relay/skills/productivity/session-relay/SKILL.md`, `plugins/session-relay/rust/src/main.rs`, `plugins/session-relay/test/run-build-matrix.mjs` (new), `.github/workflows/build-binaries.yml` | 1b-8 | planned | A15-A18 pass. Darwin natively exercises getpeereid/session/PTY/resize/signal/EOF/watchdog behavior; docs preserve all evidence limits; native dispatch refuses an unpushed/mismatched SHA. |
...
| A0b | Bind every dispatch to the path-specific plan snapshot and implementation tip. | `test -n "$PLAN_COMMIT" && test -n "$PLAN_BLOB" && test -n "$IMPL_HEAD" && test "$(git -C /home/vagrant/projects/docks log -1 --format=%H -- docs/plans/active/relay-worker-lifecycle-primitives.md)" = "$PLAN_COMMIT" && test "$(git -C /home/vagrant/projects/docks show "$PLAN_COMMIT:docs/plans/active/relay-worker-lifecycle-primitives.md" | git hash-object --stdin)" = "$PLAN_BLOB" && test "$(git -C /home/vagrant/projects/docks hash-object docs/plans/active/relay-worker-lifecycle-primitives.md)" = "$PLAN_BLOB" && test "$(git rev-parse HEAD)" = "$IMPL_HEAD"` | Exit 0 before each worker step. Unrelated main commits do not invalidate the plan. The orchestrator records expected dirty paths separately when redispatching an in-flight uncommitted step; a new worker must first review that WIP diff. |
...
| A1b | Current Codex hook trust and experimental app-server capability are production-usable. | `RELAY_REAL_RUNTIME_TEST=1 node plugins/session-relay/test/feasibility-probe.mjs --case codex-hook-trust-and-experimental-api` | Exit 0; `hooks/list` for the exact cwd reports one enabled/trusted exact-hash session-relay row for each required event; untrusted/changed/disabled/duplicate rows refuse before child spawn and print `/hooks`; production argv contains no hook-trust bypass. Initialize with `experimentalApi=true` makes the required list/filter methods available or records typed unavailability. Test-only bypass rows are isolated and explicitly labeled. |
| A2 | Pending identity binding and unmanaged cancellation are atomic, durable, epoch-serialized, and exact. | `(cd plugins/session-relay/rust && cargo test --locked managed_attach_ -- --nocapture)` | Exit 0; a claim arriving during UnmanagedCanceling durably reserves the exact pending token and waiter tuple. Crash/retry and exact duplicate join the same claim; conflicting tuple refuses. Cancel-first→reap with a waiter atomically enters Claiming next epoch and exactly one final result; without a waiter it returns Unmanaged. Deadline with a waiter consumes the pending token, binds that exact worker Managed, and applies only exceptional FencingUnconfirmed—never Active. Stable cancellation epoch survives custody-version increments; every command separately checks current operation version. Late events are audit-only. |
...
| A3b | Detached supervisor/watchdog own each transition-capable Child/supervisor from birth and preserve attach/wake I/O. | `(cd plugins/session-relay/rust && cargo test --locked lifecycle_supervisor_ -- --nocapture) && node plugins/session-relay/test/supervisor-custody.mjs --matrix && node plugins/session-relay/test/reentry-inventory.mjs --compile-fail` | Exit 0. Machine-checked rows cover health-first/control-first, health EOF no-op, duplicate/stale control refusal, no spawn before ControlBound, supervisor SID=PID and differs from caller, thread-start failure before Ready, missing-record fail-closed admission, pipe exact I/O/1MiB slow-reader backpressure, Closed stdin, PTY controlling terminal/foreground group/input/Ctrl-C/resize bounds+wrong-group rejection, SIGINT/SIGTERM/SIGHUP acceptance, unsupported-signal rejection, partial/zero-byte disconnect, flood cancellation, watchdog death, and current-version event sequencing. Stale control/output emits zero bytes. Baseline four plus `child-cancel-reentry` and `fabricated-owned-proof` fail to compile. |
| A3c | Exact-turn and supervisor-owned-child cancellation close the guard-drain cycle within fixed bounds. | `(cd plugins/session-relay/rust && cargo test --locked lifecycle_turn_cancellation_ -- --nocapture) && node plugins/session-relay/test/lifecycle-smoke.mjs --case cancellation-custody && node plugins/session-relay/test/reentry-inventory.mjs --compile-fail` | Exit 0. Deterministic barriers cover before send, after send/before response, after response, before terminal, and supervisor child ignoring kill. Every custody transition exact-CASes `operation_version`; handoff consumes exact operation+handoff versions; losing delayed events only append stale audit. Cancellation emits exactly one interrupt for the persisted turn id and accepts only matching terminal. Every wait requests ≤100 ms; one 5s deadline ends in terminal/reap, exact handoff, or FencingUnconfirmed. Guard releases by deadline while the detached supervisor retains child authority. The six A3b bins plus new `cancel-reentry` fail. |
...
| A5 | Tree proof requires phase-separated gated placement and same-fence kill evidence; filtered hardening remains separate. | `node plugins/session-relay/test/lifecycle-smoke.mjs --case containment` | Exit 0; setup-time boundary contains only derived manager-root, retained fd identity, domain/empty-subtree/writable-kill, gated membership, and capability attestation. A pre-kill boundary cannot promote. Only the matching supervisor/fence constructs `CgroupKillReceipt` after exact kill write, populated-zero, and no in-model survivor. Error routing and cooperative/filtered rows remain independent; optional freeze never upgrades proof. |
...
| A6 | Both real CLIs block managed first-prompt failures through the documented required barrier. | `RELAY_REAL_RUNTIME_TEST=1 node plugins/session-relay/test/runtime-hook-abort.mjs --matrix` | Exit 0; isolated logs prove exact hook file/hash and timeout. Codex production preflight verifies both hooks through `hooks/list`; untrusted/changed/disabled/duplicate and deterministic post-list drift all refuse before child/model bytes, and production argv has no bypass. Codex SessionStart stop and cross-runtime UserPromptSubmit block are verified; Claude SessionStart stays empirical. Timeout is fail-open and supervisor fences/reaps by 20s. |
...
| A13 | Proof validation and capability families are exhaustive, sealed, scope-safe, epoch-bound, phase-correct, and threat-model-labeled. | `(cd plugins/session-relay/rust && cargo test --locked fence_proof_ -- --nocapture) && node plugins/session-relay/test/reentry-inventory.mjs --compile-fail` | Both exit 0; backend×scope×evidence rows pass. Process, protocol, boundary, kill, and fence proof fields/constructors are private. Compile-fail bins reject fabricated/mutated pidfd, owned-child, cgroup, ProtocolTree/ThreadQuiescence/turn/terminal proofs; runtime rows reject later-fence replay, pre-kill boundary promotion, serialized tamper, and stale supervisor. Current Codex cannot build the offline boundary. |
...
- Codex plugin enablement does not imply hook trust. `hooks/list` exact trust/hash is a production launch precondition; `--dangerously-bypass-hook-trust` is test-only because it would also authorize unrelated untrusted hooks.
...
- Managed Codex launch sees missing/disabled/untrusted/changed/duplicate relay hook rows, uses the hook-trust bypass in production, or creates a child after trust-source drift.
...
2. **Environment & commands:** present — path-specific plan snapshot, implementation tip, cross-filesystem handoff, versions, four targets, hook-trust/experimental preflight, isolated real-runtime setup, measured deadline formula, and exact gates are specified.
3. **Interface & data contracts:** present — durable waiting claim, stable cancellation epoch plus mutable custody version, fence epoch/proof retention, role-authenticated supervisor control, phase-separated cgroup evidence, sealed protocol proof, tombstone GC, and operator APIs are explicit.
4. **Executable acceptance:** present — A0/A0b–A18 plus A1b/A3a–A3d are command + expected evidence, including trust drift, detached-session/control races, crash-resumable retention, proof fabrication/replay, cgroup phase, native Darwin, and source-derived behavior ids.
...
Adversarial cold-read result: Draft-12 removes the decisions the extended review found missing: waiter durability, cancellation/custody version separation, control connection ownership, session detach, cgroup evidence phase, protocol proof construction, retention finalization, hook trust, experimental initialization, tombstone aging, and worktree relocation. No owner decision remains open; immutable independent review is still pending and remote push/dispatch remains a separate execution-time approval gate.
...
Score: **86/100 (Draft-12 author candidate; independent re-review pending)** · trajectory **97 Draft-11 claimed READY → 74 extended red-team NOT READY → 86 graph/proof/trust remodel** · stopped: **candidate frozen for fresh review; implementation remains paused**.
...
- **Historical Step-1 feasibility result (2026-07-11, orchestrator-run in the real authenticated env; harness on `codex/primitives-impl`, git-blob `02cea7b`, commit `c32aafa`):** the original harness exited 0 with an intact 49-record hash chain, current protocol unavailability, 20-second lineage deadline, and `managed_attach_deadline_ms=4360`. Its `strong_cgroup`/mandatory-freeze schema and test-only Codex hook-trust bypass are now explicitly superseded by Step 1b; this evidence cannot satisfy remodeled A1/A1b until the independent cooperative/filtered verdicts and production trust rows pass.
