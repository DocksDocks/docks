...
updated: "2026-07-13T04:03:46-03:00"
...
- **Draft-42 Step-1b implementation-review repair and root generations:** independent source review at clean implementation head `22b754adcd5756f084fd61f55436971a6b9d407f` rejected Step 1b without finding the Goal unbuildable. No A107/A108/A109/A106 Step-1b event may be appended, and no later step may start, until the repairs below commit, the root migration is durable, A109 runs first, and a superseding Step-1b A106 receipt closes the original `2a864e9b6f966384e4c4ed0e4b3d563b348a3830..STEP1B_REPAIR_HEAD` range. The preserved schema-v2 inputs are: step root `/tmp/relay-step-range.yjd6ZO` with authority `/tmp/relay-root-authority.3793b40842934249e31d4ad8.json`, pin `aa48c80e67095d6d5820bee843434843dcd882f58dfd0397cc32163a434a8291`; canonical runner root `/tmp/relay-runner-receipt.LZSKK0` with authority `/tmp/relay-root-authority.17133faf3313608750b9060b.json`, pin `04258c2c7724ae1132af3ec84dd70e63afa866d46a8075f757bf061e10315655`; and byte-equivalent interim runner root `/tmp/relay-runner-receipt.WVK0ld` with authority `/tmp/relay-root-authority.c2b3a975d023bc33378540d3.json`, pin `4a96479edac09a124d0280adfdec40311a4202fd2b76e7224c3b2509a0dd832a`. The first two authorities bind helper source `1ad1e3d0c66c6e7458825b88e713ee06a6fe2052b67573d67b90b57e48f00432`; the interim authority binds `a3a173e1b030d25219ed94128c7448c81d9a154e96147f664f3c17773a0e6e0a`. The orchestrator re-challenges all three exact pins immediately before migration; unavailable, changed, or extra roots are STOP. All three remain read-only and live until the final multi-root cleanup; the already-created interim root is evidence, never an unrecorded replacement.

  Step 1b replaces whole-helper equality as the ordinary root-compatibility rule with closed `RootAuthorityProtocolV1`: schema-v3 authority records add `protocol_sha256` over a literal embedded protocol contract and retain `custodian_source_sha256` as provenance. Current validation requires the exact protocol hash, closed authority/sentinel fields, signed fresh challenge, process generation, root dirfd identity, and pin; unrelated helper edits no longer invalidate an unchanged protocol. A schema-v2 predecessor is accepted only by the one migration entrypoint and only when its exact source hash, authority pin, root identity, sentinel bytes, and live signed challenge equal the sealed list above. It cannot become active in place.

  The same phase-aware migration entrypoint creates a fresh schema-v3 successor for each purpose and writes append-only `RootAuthorityMigrationPhaseReceipt` states `Prepared→Copied→Verified→Active`. `Prepared` binds purpose, implementation commit, old/new helper hashes, protocol hash, every predecessor root/sentinel/authority/process identity and pin, the byte-sorted path/mode/size/content manifest, and the exact step-ledger or acceptance-event head. `Copied` contains byte-identical payload entries below the new dirfd while leaving all predecessors unchanged. `Verified` re-reads both sides, requires identical manifests and chain heads, and for runner receipts records the canonical predecessor plus a separate exact-byte `equivalent_predecessor` relation to the interim root; one predecessor cannot name two successors. `Active` fsyncs the closed `RootAuthorityMigrationReceipt {schema:1,purpose,ordinal,reason,implementation_commit,protocol_sha256,predecessors,successor,payload_manifest_sha256,old_chain_head,new_chain_head,previous_migration_sha256,receipt_sha256}` inside the successor and only then prints the new root/authority/pin. A crash resumes from the last exact phase or retains both sides and refuses; it never deletes, overwrites, re-imports, or silently selects a root. A migration negative matrix covers partial destination, changed predecessor, wrong source/protocol/pin, forked successor, missing/equivalent mismatch, replay, phase skip, and cleanup-before-Active. A133 and A137 validate the complete step and runner lineage, not a single unlinked root.

  The native matrix and live runner use a retained `RootHandle` rather than a `PathBuf`: open the canonical directory without following links, retain fd plus dev/inode/uid/mode, parse `.relay-root.json` as exact compact closed JSON with a valid token, and perform every create/read/fsync/rename/unlink/rmdir fd-relatively. Cleanup is explicit and fallible; `Drop` never recursively removes a re-resolved pathname. After D exits, C must bounded-signal and exact-wait every owned/adopted descendant until `waitpid(-1, WNOHANG)` reports `ECHILD`; timeout or any remaining child is a terminal refusal and cannot write `Complete`. A109 drives a deliberately surviving adopted payload through this actual completion path and proves no `Complete` attempt exists.

  A109's native negatives must call the same production transition functions as the live custodian. Hand-modeled enums, integer comparisons, static prefix arrays, or an arbitrary non-placeholder digest do not count. Each ordering, frame-hash, sequence, expiry, prefix-reconstruction, descendant, result, and cleanup row injects one condition through the real packet/attempt/root state machine, then binds a structured before/attempt/after trace proving refusal and unchanged-or-terminal state. The offline validator exact-matches those trace schemas and rejects a model-only substitute. `NativeOperationSupplierInventoryV2` covers every OS-affecting callsite, including `libc`, `rustix`, `std::fs`, `File`/`OpenOptions`, `std::process::Command`/`Child`, and Unix extension methods; each row binds API, operation class, supplier, source callsite, and pinned toolchain/dependency. The scanner rejects an unclassified standard-library call just as it rejects an unclassified `libc`/`rustix` call, and mutation fixtures inject at least `fs::remove_file` and `Command::spawn` to prove failure. Implementers may instead replace standard-library OS operations with the already-pinned libc/rustix surfaces; adding a dependency or Cargo feature remains STOP.

  Before printing or advancing a live result, the native producer writes and fsyncs the exact `GateContext` and `GateResult` under the active runner root. It also creates the closed `RunnerStateReceipt`, every partial/expired/complete `RunnerAttemptReceipt`, a named append-only bootstrap receipt manifest, a named `gate-artifact-manifest.json`, and the terminal cleanup receipt. The terminal attempt binds the six ordered gate-artifact hashes, both manifest hashes, exact binary/Cargo identity, terminal cleanup hash, and `Complete`; `Complete` is impossible before exact descendant drain and live-scratch cleanup. One auth-free producer-to-rehash integration run uses the real producer with synthetic payloads, then passes the unmodified A133 `deriveRehashInputs` path. Missing producer bytes, stdout-only GateResult, orphan manifest, or validator-only synthetic artifact is failure.

  A108 uses one fixed total cleanup deadline for the app-server child: bounded SIGTERM grace, SIGKILL escalation, bounded final close/reap, and failure if the exact child generation remains. Stable-generation selection serializes the entire current-read→candidate-validate→generation-fsync→pointer-rename→parent-fsync transaction with the existing owner/boot/process-generation checked lock and bounded stale-owner recovery. Its matrix launches genuinely overlapping higher/lower installers with deterministic barriers in both start orders plus crash-while-lock-held recovery; the highest valid generation must remain selected. A sequential pair is not a race row.

  A137 consumes a closed `RootGenerationLedger` for both `step-range` and `runner-receipt`, requires one active schema-v3 generation, every predecessor retained byte-identically, every migration phase/receipt and equivalence link valid, all authority custodians responsive, and no unlisted generation. A138 extends its external phase chain to every ledger generation: bind byte-complete root/authority/process manifests, close and exact-wait each custodian, quarantine and fd-relatively remove each active and superseded step/runner root, remove its exact authority record, and record per-generation crash-resumable cleanup before event 82. The final summary requires all roots, quarantines, authority files, and custodian generations absent; it preserves the migration receipts and cleanup evidence in the final-scope snapshot. Deleting only the current runner root, leaving the step root, or losing a predecessor before the final summary fails.

- **Exact Draft-42 root-migration gate (after the repair commit, before A109 and any further event):** the repaired helper parses the sealed predecessor list above from the exact pinned plan bytes; caller-supplied extra roots or source hashes are forbidden. It writes the descriptor through the already-open owner-only regular file and creates all successor roots/authorities internally. Run from `/tmp/docks-primitives-collab` with clean `HEAD=$STEP1B_REPAIR_HEAD`:
  ...
  ROOT_MIGRATION_DESCRIPTOR="$(mktemp /tmp/relay-root-migration.XXXXXXXX.json)"
  chmod 0600 "$ROOT_MIGRATION_DESCRIPTOR"
  node plugins/session-relay/test/wip-snapshot.mjs --migrate-root-authorities --plan-commit "$PLAN_COMMIT" --plan-blob "$PLAN_BLOB" --descriptor "$ROOT_MIGRATION_DESCRIPTOR"
  export ROOT_GENERATION_LEDGER="$(jq -er '.ledger_path' "$ROOT_MIGRATION_DESCRIPTOR")"
  export STEP_RECEIPT_DIR="$(jq -er '.active.step.root' "$ROOT_MIGRATION_DESCRIPTOR")"
  export STEP_RECEIPT_AUTHORITY="$(jq -er '.active.step.authority' "$ROOT_MIGRATION_DESCRIPTOR")"
  export STEP_RECEIPT_AUTHORITY_PIN="$(jq -er '.active.step.pin' "$ROOT_MIGRATION_DESCRIPTOR")"
  export RUNNER_RECEIPT_ROOT="$(jq -er '.active.runner.root' "$ROOT_MIGRATION_DESCRIPTOR")"
  export RUNNER_RECEIPT_AUTHORITY="$(jq -er '.active.runner.authority' "$ROOT_MIGRATION_DESCRIPTOR")"
  export RUNNER_RECEIPT_AUTHORITY_PIN="$(jq -er '.active.runner.pin' "$ROOT_MIGRATION_DESCRIPTOR")"
  node plugins/session-relay/test/wip-snapshot.mjs --validate-root-lineage --plan-commit "$PLAN_COMMIT" --plan-blob "$PLAN_BLOB" --descriptor "$ROOT_MIGRATION_DESCRIPTOR" --ledger "$ROOT_GENERATION_LEDGER"
  ```

  The descriptor is closed compact JCS `{schema:1,plan_commit,plan_blob,implementation_head,ledger_path,active:{step:{root,authority,pin},runner:{root,authority,pin}},descriptor_sha256}`; every path is canonical and matches its closed family. The final validation prints `ROOT_AUTHORITY_MIGRATION PASS step_generations=2 runner_generations=3 events=8 step_receipts=1 ledger_sha256=<hex>`. It proves the two runner predecessor payloads byte-identical, preserves the exact eight-event head and one Step-1b range receipt, and leaves every predecessor responsive. The descriptor path and ledger are retained for A133/A137/A138; removing them early is STOP.
...
- **Acceptance event accounting (Draft-29 blocker repair):** canonical acceptance IDs are inventory keys, not chronological event ordinals. Every actual gate/check occurrence appends and fsyncs one exclusive-create `AcceptanceEventReceipt {schema:1,event_ordinal,occurrence_id,phase,criterion_id,scope,command_sha256,expected_sha256,exit_code,met,result_sha256,previous_event_sha256,event_sha256}`; ordinals 1–80 live below sentinel-bound `$RUNNER_RECEIPT_ROOT/acceptance-events/`, while terminal A137/A138 ordinals 81–82 live below sentinel-bound `$FINAL_SCOPE_ROOT/acceptance-events/` so their evidence survives runner-root cleanup. `event_sha256` is JCS SHA-256 over every preceding field. `occurrence_id` is exactly `<phase>/<criterion_id>/<scope>`, unique in the closed schedule below; `scope` is `default` unless written explicitly. The first Step-1b commit imports the exact already-immutable evidence for the `historical` phase without rerunning it and binds each imported result to its original Git/evidence hash. Later events are written only after their named command returns or, for terminal A137/A138, by the command's final crash-durable sub-operation after all preceding assertions succeed. No overwrite, deletion, duplicate id, gap, fork, reordered ordinal, unlisted phase/criterion/scope, result substitution, or `met:true` with nonzero exit is valid.
  ...
  Step 9 therefore commits its allowed mutations after A134–A136, appends the Step-9 A106 range receipt/event, and only then runs A137–A138; this is the sole deliberate numeric-order break inside that phase. A109 parses this literal map, requires all and only A101–A138, the exact 11 A106 occurrences, ten A104 occurrences, scoped A127/A128/A131 occurrences, the Step-8 repeat set, and the six one-shot RunGate positions. A137 first validates events 1–80, then its final sub-operation appends event 81 and fsyncs `AcceptancePrefixSnapshot {schema:1,schedule_sha256,event_count:81,events,partial_criteria,event_chain_head,snapshot_sha256}` at exact `$FINAL_SCOPE_ROOT/acceptance-prefix-snapshot.json`; `events` contains every ordered `{occurrence_id,criterion_id,event_sha256,result_sha256}` row for ordinals 1–81 and `partial_criteria` contains the deterministically aggregated A101–A137 summaries. Only after re-reading and validating those bytes does it fsync `AcceptancePrefixReceipt {schema:1,schedule_sha256,event_count:81,event_chain_head,criteria_through:A137,prefix_snapshot_sha256,receipt_sha256}`. A138 has one literal phase-aware `final-scope.mjs --run-a138` entrypoint and an append-only `FinalAcceptancePhaseReceipt {schema:1,ordinal,phase,previous_phase_sha256,prefix_sha256,prefix_snapshot_sha256,step8_scope_result_sha256,negative_matrix_result_sha256,source_ready_matrix_result_sha256,expected_a138_result_sha256,root_generation_ledger_sha256,root_cleanup_index,root_cleanup_phase,generation_cleanup_receipts,all_roots_removed_sha256,event82_sha256,summary_sha256,receipt_sha256}` chain under `$FINAL_SCOPE_ROOT/a138-state/`. The closed global phase order is `Prepared→GenerationCleanup*→AllRootsRemoved→Event82Written→SummaryPublished→Finalized`; each byte-sorted generation has the closed subphases `CustodianBound→CustodianClosed→Quarantined→PayloadRemoved→SentinelRemoved→RootRemoved→AuthorityRemoved`. Fields not yet available are null and later receipts preserve every earlier value. Before any root or authority access the entrypoint loads the latest exact phase. With no state it performs every root-dependent positive/negative/source-ready check, validates the complete migration lineage, requires every ledger root to share the final-scope device, hashes byte-complete payload/sentinel/authority/custodian manifests for exactly two step and three runner generations, and fsyncs `Prepared`. For each generation it signs and binds shutdown of the exact authority, exact-waits its process generation, atomically quarantines the bound root, resumes fd-relative manifest-driven deletion, performs the existing durable sentinel-unlink then rmdir boundary, removes the bound authority record, and advances only after original/quarantine/authority/process absence. Unselected generations stay live and byte-identical. `AllRootsRemoved` binds all five terminal cleanup receipts and continued absence. Only then may event 82 be written; the summary includes `all_roots_removed_sha256`; `Finalized` revalidates the entire chain and absence before PASS. A failure before `AllRootsRemoved` leaves no event 82 or final summary. The crash matrix injects every global and per-generation barrier against disposable multi-generation lineages and requires byte-identical retry through the same entrypoint. `AcceptanceSummaryReceipt {schema:1,schedule_sha256,event_chain_head,criteria,criteria_sha256,root_generation_cleanup_sha256,all_roots_removed_sha256,receipt_sha256}` contains exactly 38 `AcceptanceCriterionSummary {schema:1,criterion_id,events,met,summary_sha256}` rows in A101→A138 inventory order; `events` contains that criterion's exact scheduled `{occurrence_id,event_sha256,result_sha256}` rows in event order; `met` is true only when every scheduled occurrence passed and no extra occurrence exists; `summary_sha256` hashes the preceding summary fields. The completion review and `SourceReadyHandoffReceipt.acceptance_result_sha256s` use exactly these 38 `summary_sha256` values in inventory order and require the matching `Finalized` phase receipt. They never pretend the dependency-ordered event chain itself ran in numeric order or select a “latest” repeated result.
- **Runner live job capability:** Node 24 exposes no public `AF_UNIX SOCK_SEQPACKET`/`SCM_CREDENTIALS`, so JavaScript is validation-only and never launches or owns the native driver, sockets, gates, credentials, or cleanup. Step 1b adds `rust/src/bin/runner_job_custodian.rs` without a new package. The orchestrator builds once into the sentinel-bound immutable `RUNNER_BUILD_DIR`, verifies the owner-executable regular binary, and records `RunnerBinaryIdentity {schema:1,dev,ino,size,mode,sha256,source_head,cargo_lock_sha256}`. Direct path execution is bootstrap only. On entry the binary opens and retains `/proc/self/exe`, exact-matches the external identity, restores CLOEXEC, calls `PR_SET_CHILD_SUBREAPER`, and the original foreground PID becomes long-lived custodian C. C launches driver-loop D from the retained fd with `execveat(AT_EMPTY_PATH)`; D launches every gate G from the same retained fd. C is therefore G's ancestor subreaper: D normally exact-waits G, but if D dies, C adopts, bounded-kills, and exact-waits G plus payload descendants before cleanup, then exact-waits D. No role after bootstrap reopens or executes a pathname. Retarget/unlink after bootstrap must leave descendants on the retained bytes; wrong/missing/leaked self fd or identity fails before sockets/children. Non-Linux returns typed unavailability; no Node fallback exists.
  ...
  A109 is native-rooted and auth-free and begins by validating the exact Draft-42 root migration/ledger. The native matrix uses compiled synthetic payload modes through the production transition/root/attempt functions only and must not invoke Claude, Codex, real runtime scripts, auth, or cgroups. The later Node process is an offline artifact/source/lineage validator and has no native process ancestry. It exact-validates structured refusal traces, complete OS-operation supplier coverage, and one actual synthetic producer artifact chain through the unmodified A133 input derivation. Matrix cleanup removes only its fd-bound run directory; the immutable build, live run, active receipts, and every retained predecessor remain. Live A110 (RunGate A1c)→A125 (RunGate A7) removes only live scratch after exact terminal cleanup. A137 validates final scope with cleanup of every root generation and authority explicitly deferred; contiguous A138 consumes and mutation-tests the Step-8 rehash plus migration lineage, then alone closes all five custodians and removes all five step/runner roots and authority records. Failure retains redacted evidence but always closes fds, exact-waits owned/adopted processes, and never records `Complete` while a descendant remains.
...
| 1b | From clean `STEP3B_HEAD`, run A103, create the schema-v2 WIP/history/allowlist/receipt machinery; migrate feasibility evidence to independent cooperative/filtered verdicts; prototype post-decision Codex ordering; define hook-health and stable-runtime evidence; build the retained-self-fd native C/D/G custodian with validation-only Node wrapper; repair the independent implementation-review findings; and migrate every preserved schema-v2 step/runner root into one linked schema-v3 authority lineage. | `plugins/session-relay/rust/src/bin/runner_job_custodian.rs` (new), `plugins/session-relay/test/feasibility-probe.mjs`, `plugins/session-relay/test/fixtures/lifecycle-capability-schema.json`, `plugins/session-relay/test/fixtures/wip-historical-baseline.json` (new), `plugins/session-relay/test/fixtures/wip-step-allowlist.json` (new), `plugins/session-relay/test/runtime-hook-abort.mjs` (new), `plugins/session-relay/test/runtime-hook-upgrade.mjs` (new), `plugins/session-relay/test/runner-job-custodian.mjs` (new), `plugins/session-relay/test/wip-snapshot.mjs` (new); `plugins/session-relay/rust/Cargo.toml` (read-only production), `plugins/session-relay/rust/Cargo.lock` (read-only production). | 1, 3b | planned | A103 and the existing A105 prefix remain immutable; the repair commits cleanly; the exact Draft-42 root-migration gate prints `ROOT_AUTHORITY_MIGRATION PASS step_generations=2 runner_generations=3 events=8 step_receipts=1`; then native-rooted auth-free A109 runs before A107/A108, the bounded app-server cleanup and true concurrent stable-generation matrix pass, and a superseding A106 covers the original Step-1b base through the repair head. Historical fixture exact-maps 22 commits/35 entries including mixed `cdc70f0`/`701cea7`; future fixture contains literal step mutation rows and empty 1c/1d/8. The A108 transcript has zero pre-decision turn bytes and one successful post-decision harmless request/response. Native matrix proves self-fd/root-handle identity, exact adopted-descendant drain, DriverArm/GO/OPEN/COMMIT/reap-ack ordering, real transition/prefix negatives, closed OS-operation supplier coverage, bounded channels, and every credential/fd/PID/crash/cleanup negative with no auth/runtime/cgroup access. Its synthetic producer emits the exact state/attempt/context/GateResult/bootstrap/gate-manifest/cleanup chain consumed by unmodified A133. First commit creates helper/fixtures and A106 append-receipts it before any later step. No implementation-branch plan/doc edit or dependency drift is permitted. |
...
| A108 | A1b | Raw Codex API ordering, controller-first activation, post-Active hook health, stable generation switching, and experimental capability are feasible. | `RELAY_REAL_RUNTIME_TEST=1 node plugins/session-relay/test/feasibility-probe.mjs --case codex-managed-boundary && node plugins/session-relay/test/runtime-hook-upgrade.mjs --case stable-generation-pointer --matrix` | Exit 0 on installed Codex `0.144.1` with a raw transcript and closed hashes. `hooks/list` has exactly the enabled/trusted SessionStart and UserPromptSubmit rows. A successful idle `thread/start` response and exactly one matching `thread/started` form the prototype closed `CodexThreadBirthBarrier` in either arrival order; its request/response/notification and JCS barrier hashes verify. Missing/duplicate/conflicting/non-idle halves or any changed hash fail. Zero `turn/start` request bytes occur before that join and an explicit prototype Active decision. Immediately after the decision the harness sends exactly one harmless `turn/start`; its response id, returned turn id, and matching `turn/started` bind one thread/turn. On that exact tuple the synchronous sequence is SessionStart started/completed then UserPromptSubmit started/completed, both successful with no stop/error entry, before the first observable user/model/tool item or agent/reasoning delta; any item before the complete chain fails. The matching turn completes successfully. A zero-turn transcript, a pre-Active turn, or a claim that either hook participates in birth is a hard failure. This remains `feasibility_only`, not production binding and makes no internal scheduling claim. App-server shutdown uses one fixed total deadline with bounded SIGTERM grace, SIGKILL escalation, final close/reap, and exact child-generation absence. Hook snapshots and the stable-generation matrix pass only with overlapping higher/lower installer processes in both deterministic start orders plus crash-while-lock-held recovery under the owner/boot/process-generation checked lock; the highest valid generation remains selected. A sequential pair is rejected as a race substitute. Missing auth is unavailable/STOP, never skip. Production Rust claim is A121 (RunGate A6) only. |
| A109 | A1e-custodian-matrix | Prove linked root generations, the native C/D/G protocol, ordered fresh-capability reconstruction, executed negatives, closed native-operation/dependency provenance, the producer-to-rehash chain, and immutable-rehash rejection without auth/runtime/cgroup access. | `test -n "$PLAN_COMMIT" && test -n "$PLAN_BLOB" && test -n "$ROOT_MIGRATION_DESCRIPTOR" && test -n "$ROOT_GENERATION_LEDGER" && node plugins/session-relay/test/wip-snapshot.mjs --validate-root-lineage --plan-commit "$PLAN_COMMIT" --plan-blob "$PLAN_BLOB" --descriptor "$ROOT_MIGRATION_DESCRIPTOR" --ledger "$ROOT_GENERATION_LEDGER" && test -x "$RUNNER_BIN" && "$RUNNER_BIN" --matrix-driver --state-root "$RUNNER_MATRIX_STATE_DIR" --expected-self-sha256 "$RUNNER_BIN_SHA256" --emit "$RUNNER_MATRIX_RESULT" && node plugins/session-relay/test/runner-job-custodian.mjs --validate-matrix "$RUNNER_MATRIX_RESULT" --native-source plugins/session-relay/rust/src/bin/runner_job_custodian.rs && node plugins/session-relay/test/runner-job-custodian.mjs --supplier-negative-matrix --native-source plugins/session-relay/rust/src/bin/runner_job_custodian.rs && node plugins/session-relay/test/runner-job-custodian.mjs --producer-to-rehash-integration --matrix-result "$RUNNER_MATRIX_RESULT" --root-lineage "$ROOT_GENERATION_LEDGER" && node plugins/session-relay/test/runner-job-custodian.mjs --rehash-negative-matrix --matrix-result "$RUNNER_MATRIX_RESULT" --native-source plugins/session-relay/rust/src/bin/runner_job_custodian.rs && node plugins/session-relay/test/runner-job-custodian.mjs --validate-acceptance-map /home/vagrant/projects/docks/docs/plans/active/relay-worker-lifecycle-primitives.md --plan-blob "$PLAN_BLOB" --canonical-range A101:A138 --event-schedule acceptance-event-schedule-v1 --run-gates A110:A1c,A111:A1d,A119:A5,A120:A5b,A121:A6,A125:A7` | Exit 0 first with `ROOT_AUTHORITY_MIGRATION PASS step_generations=2 runner_generations=3 events=8 step_receipts=1`, then native `RUNNER_MATRIX PASS`, `RUNNER_MATRIX_VALIDATION PASS`, `NATIVE_OPERATION_SUPPLIER_MATRIX PASS`, `PRODUCER_REHASH_INTEGRATION PASS`, `LIVE_EVIDENCE_REHASH_NEGATIVE_MATRIX PASS`, `ACCEPTANCE_MAP PASS A101..A138`, and `ACCEPTANCE_EVENT_SCHEDULE PASS events=82`. Every ordering/frame-hash/sequence/expiry/prefix/arm/GO/OPEN/COMMIT/reap/adoption/descendant/self-fd/root-handle/result/framing/cleanup negative executes through the production transition/root/attempt functions and binds a structured before/attempt/after trace; hand-modeled enum/integer/array comparisons or arbitrary evidence digests fail validation. The adopted-descendant row leaves a payload alive past the former deadline and proves bounded termination, exact wait to `ECHILD`, and absence of any `Complete` receipt until cleanup succeeds. The supplier inventory classifies every OS-affecting libc/rustix/standard-library callsite and the mutation matrix proves injected `fs::remove_file` and `Command::spawn` fail; Cargo/dependency bytes remain exact. The producer integration uses the actual auth-free synthetic producer to fsync the RunnerStateReceipt, contexts, six GateResults, partial/expired/complete attempts, named bootstrap/gate manifests, and terminal cleanup, then passes the unmodified A133 input derivation. The static validator exact-parses the literal phase/occurrence map, unique ids/scopes, complete criterion coverage, and six exact RunGate mappings; missing/extra/reordered/duplicate occurrence, numeric-order substitution, latest-result aggregation, or extra gate fails. Expiry after A119, A120, and A121 creates a new attempt that replays its exact prefix through the native machine before the outstanding gate; every skip fails before payload bytes and attempts remain append-only. Offline mutations cover every attempt/artifact/acknowledgement/result/binary/Cargo/manifest/cleanup/prior-A106/event/root-lineage link. Node has no ancestry over C/D/G and no socket/gate/credential/root-cleanup authority. Disposable matrix scratch is removed fd-relatively; active and predecessor receipt roots remain. |
...
| A133 | A14r | Step 8 cryptographically revalidates completed one-shot evidence without replaying it. | `test -n "$PLAN_BLOB" && STEP8_HEAD="$(git rev-parse HEAD)" && test -n "$STEP8_HEAD" && test -n "$RUNNER_STATE_RECEIPT" && test -n "$RUNNER_RECEIPT_ROOT" && test -n "$STEP_RECEIPT_DIR" && test -n "$ROOT_GENERATION_LEDGER" && test -n "$ROOT_MIGRATION_DESCRIPTOR" && test "$STEP8_REHASH_RECEIPT" = "$RUNNER_RECEIPT_ROOT/step-8-live-evidence-rehash.json" && node plugins/session-relay/test/runner-job-custodian.mjs --rehash-live-evidence --plan-blob "$PLAN_BLOB" --step8-head "$STEP8_HEAD" --runner-state "$RUNNER_STATE_RECEIPT" --runner-receipt-root "$RUNNER_RECEIPT_ROOT" --step-receipts "$STEP_RECEIPT_DIR" --root-lineage "$ROOT_GENERATION_LEDGER" --root-migration-descriptor "$ROOT_MIGRATION_DESCRIPTOR" --through-step 7 --emit "$STEP8_REHASH_RECEIPT" && node plugins/session-relay/test/runner-job-custodian.mjs --verify-rehash-receipt "$STEP8_REHASH_RECEIPT" --plan-blob "$PLAN_BLOB" --step8-head "$STEP8_HEAD" --runner-state "$RUNNER_STATE_RECEIPT" --runner-receipt-root "$RUNNER_RECEIPT_ROOT" --step-receipts "$STEP_RECEIPT_DIR" --root-lineage "$ROOT_GENERATION_LEDGER" --root-migration-descriptor "$ROOT_MIGRATION_DESCRIPTOR" --through-step 7` | Exit 0 with `LIVE_EVIDENCE_REHASH PASS receipt_sha256=<hex>` then `LIVE_EVIDENCE_REHASH_VERIFY PASS receipt_sha256=<same-hex>`. Receipt is the exact closed `LiveEvidenceRehashReceipt`; it binds the clean Step-8 head, complete step/runner root-migration lineage, active plus retained predecessor identities, immutable attempt/artifact/bootstrap/prior-A106 chains, and terminal cleanup, emits no live/runtime/process byte, and retry returns the same receipt. Any missing/changed/reordered/forked input or migration, unlinked/silently replaced root, lost predecessor, non-Complete terminal, wrong gate order/count, path outside a ledger root, existing-byte mismatch, capability/runtime/auth/cgroup/process access, or liveness claim fails. Step 8 then writes its empty-range A106 receipt with base=head=`STEP8_HEAD`; final scope derives that head from the Step-8 A106 receipt rather than trusting a later environment variable. |
...
| A137 | A18 | The clean implementation tip equals the ordered historical tree plus active append-only step and acceptance-event receipts and contains no binary/release/docs mutation; all step/runner root-generation and authority cleanup remains deferred until contiguous A138. | `test "$(git branch --show-current)" = codex/primitives-collab && test "$(git rev-parse HEAD)" = "$IMPL_TIP" && test -z "$(git status --porcelain)" && git diff --check 12cf2ea.."$IMPL_TIP" && node plugins/session-relay/test/final-scope.mjs --verify-range --base 12cf2ea --tip "$IMPL_TIP" --historical plugins/session-relay/test/fixtures/wip-historical-baseline.json --allowlist plugins/session-relay/test/fixtures/wip-step-allowlist.json --receipts "$STEP_RECEIPT_DIR" --runner-state "$RUNNER_STATE_RECEIPT" --runner-receipt-root "$RUNNER_RECEIPT_ROOT" --root-lineage "$ROOT_GENERATION_LEDGER" --root-migration-descriptor "$ROOT_MIGRATION_DESCRIPTOR" --acceptance-events "$RUNNER_RECEIPT_ROOT/acceptance-events" --event-schedule acceptance-event-schedule-v1 --defer-root-generation-cleanup --step-order 1b,3c,3d,1c,1d,4,5,6,7,8,9 --root "$FINAL_SCOPE_ROOT" --emit-manifest "$FINAL_SCOPE_ROOT/final-scope-manifest.json" && node plugins/session-relay/test/final-scope.mjs --negative-matrix --base 12cf2ea --tip "$IMPL_TIP" --historical plugins/session-relay/test/fixtures/wip-historical-baseline.json --allowlist plugins/session-relay/test/fixtures/wip-step-allowlist.json --receipts "$STEP_RECEIPT_DIR" --runner-state "$RUNNER_STATE_RECEIPT" --runner-receipt-root "$RUNNER_RECEIPT_ROOT" --root-lineage "$ROOT_GENERATION_LEDGER" --root-migration-descriptor "$ROOT_MIGRATION_DESCRIPTOR" --acceptance-events "$RUNNER_RECEIPT_ROOT/acceptance-events" --event-schedule acceptance-event-schedule-v1 --defer-root-generation-cleanup --step-order 1b,3c,3d,1c,1d,4,5,6,7,8,9 --root "$FINAL_SCOPE_ROOT" && node plugins/session-relay/test/final-scope.mjs --seal-acceptance-prefix --acceptance-events "$RUNNER_RECEIPT_ROOT/acceptance-events" --terminal-events "$FINAL_SCOPE_ROOT/acceptance-events" --event-schedule acceptance-event-schedule-v1 --criterion A137 --emit-snapshot "$FINAL_SCOPE_ROOT/acceptance-prefix-snapshot.json" --emit "$FINAL_SCOPE_ROOT/acceptance-prefix.json"` | Exit 0 with `FINAL_SCOPE PASS ... root_generation_cleanup=deferred`, then `ACCEPTANCE_PREFIX_SNAPSHOT PASS events=81 snapshot_sha256=<hex>` and `ACCEPTANCE_PREFIX PASS events=81 through=A137 receipt_sha256=<hex>`. Before the final sub-operation, helper verifies 22 commits/35 entries, the full step ledger/active chain, exact events 1–80, append-only runner-attempt chain, Git ranges, Cargo blobs/supplier map, runner identity/no live PID/fd, matrix/live cleanup, and the closed step/runner `RootGenerationLedger`: exactly two step generations and three runner generations, one active schema-v3 generation per purpose, byte-identical retained predecessors, live exact authority custodians, and complete unique migration/equivalence links. Every ledger root and authority remains retained for A138. The final sub-operation appends terminal A137 event 81 only after every positive and negative check passes, fsyncs and re-reads the byte-complete ordered event/partial-summary snapshot, then seals a prefix that binds its digest. The final runner attempt carries ordered A110→A111→A119→A120→A121→A125 RunGate evidence. Missing/extra/reordered/duplicated/forked event, wrong occurrence scope, latest-result selection, prefix-snapshot mutation/omission, union/dedup shortcut, extra/reordered/forked step/runner receipt, missing/forked migration, unlisted or lost generation, predecessor mutation, dead/substituted authority custodian, tree/Cargo mismatch, wildcard, read-only mutation, cleanup escape, premature cleanup of any ledger root or authority, binary/version/release surface, or live process/fd fails. |
| A138 | A18a | Through one phase-aware entrypoint, bind the immutable Step-8 rehash and A137 snapshot+prefix to the same runner/step/event chains and complete step/runner root-generation lineage, then crash-durably close every authority and remove every active/superseded root before emitting terminal A138 and the final 38-row summary. | `node plugins/session-relay/test/final-scope.mjs --a138-crash-matrix --acceptance-prefix-snapshot "$FINAL_SCOPE_ROOT/acceptance-prefix-snapshot.json" --acceptance-prefix "$FINAL_SCOPE_ROOT/acceptance-prefix.json" --root "$FINAL_SCOPE_ROOT/a138-crash-matrix" --root-lineage "$ROOT_GENERATION_LEDGER" --root-migration-descriptor "$ROOT_MIGRATION_DESCRIPTOR" --step-order 1b,3c,3d,1c,1d,4,5,6,7,8,9 && node plugins/session-relay/test/final-scope.mjs --run-a138 --tip "$IMPL_TIP" --plan-blob "$PLAN_BLOB" --runner-state "$RUNNER_STATE_RECEIPT" --runner-receipt-root "$RUNNER_RECEIPT_ROOT" --root-lineage "$ROOT_GENERATION_LEDGER" --root-migration-descriptor "$ROOT_MIGRATION_DESCRIPTOR" --receipts "$STEP_RECEIPT_DIR" --acceptance-events "$RUNNER_RECEIPT_ROOT/acceptance-events" --acceptance-prefix-snapshot "$FINAL_SCOPE_ROOT/acceptance-prefix-snapshot.json" --acceptance-prefix "$FINAL_SCOPE_ROOT/acceptance-prefix.json" --step8-rehash "$STEP8_REHASH_RECEIPT" --terminal-events "$FINAL_SCOPE_ROOT/acceptance-events" --state-dir "$FINAL_SCOPE_ROOT/a138-state" --quarantine-root "$FINAL_SCOPE_ROOT/root-quarantines" --emit-acceptance-summary "$FINAL_SCOPE_ROOT/acceptance-summary.json" --step-order 1b,3c,3d,1c,1d,4,5,6,7,8,9` | Exit 0 first with `A138_CRASH_MATRIX PASS barriers=after-ledger-prepared,after-custodian-close,after-generation-quarantine,during-generation-cleanup,after-generation-cleanup,before-sentinel-unlink,after-sentinel-unlink,after-generation-removal,after-all-generations,after-event-82,after-summary`, then with `STEP8_REHASH_SCOPE PASS`, `STEP8_REHASH_SCOPE_NEGATIVE_MATRIX PASS`, `SOURCE_READY_DERIVATION_MATRIX PASS`, `ROOT_GENERATION_CLEANUP PASS purposes=2 generations=5 roots_removed=5 authorities_removed=5 custodians_closed=5 cleanup_sha256=<hex>`, `ACCEPTANCE_SUMMARY PASS events=82 criteria=38 receipt_sha256=<hex>`, and `A138_FINALIZED PASS`; an exact retry after any barrier prints the same final hashes. `--run-a138` loads the latest append-only phase receipt before any root or authority access. `no-state` revalidates A133/A137 plus the complete RootGenerationLedger and writes `Prepared` with byte-complete root, sentinel, authority-record, custodian-generation, payload, migration, and final-scope manifests for exactly two step and three runner generations. It then advances one byte-sorted generation at a time through `CustodianBound→CustodianClosed→Quarantined→PayloadRemoved→SentinelRemoved→RootRemoved→AuthorityRemoved`; each state fsyncs externally under `a138-state/`, binds the previous state, and accepts only its exact post-crash predecessor/successor filesystem and process state. `CustodianBound` issues the signed shutdown to the exact recorded authority; `CustodianClosed` requires that PID/start generation absent before fixed-path quarantine; cleanup is fd-relative and manifest-driven; sentinel unlink and rmdir retain the existing two-phase identity checks; `RootRemoved` requires original and quarantine absent; `AuthorityRemoved` removes only the bound authority record after preserving its bytes/hash. No generation begins before its predecessor is terminal, while roots not yet selected remain live and byte-identical. Only `AllRootsRemoved`, with all five roots/quarantines/authority files/custodian generations absent and every per-generation cleanup receipt durable, may write event 82; the final-scope snapshot retains migration and cleanup evidence after the source roots are gone. `Event82Written` writes the all-roots-removal-bound summary; `SummaryPublished` writes `Finalized`; `Finalized` revalidates the phase chain and continued absence. The crash matrix constructs matching disposable multi-generation lineages, interrupts every named phase for each generation, and retries the same entrypoint. Extra/missing/forked generation, early predecessor loss, changed migration/equivalence link, custodian substitution or survival, in-quarantine progress, unbound sentinel-less directory, event82/summary while any root/authority/process remains, removal without durable cleanup, phase skip/fork, original+quarantine coexistence, cross-device rename, manifest drift, deleted-root access after `AllRootsRemoved`, second gate/verifier, missing cleanup hash, or differing retry hash fails. |
...
- The committed Codex hook command begins with a versioned plugin executable, old cached command is not executed verbatim after deleting its whole plugin root, stable install lacks the exclusive owner/boot/process-generation checked lock around the complete selection transaction, the matrix substitutes sequential calls for overlapping higher/lower installers in both start orders, stale-lock recovery is unbounded or owner-blind, concurrent lower install can overwrite higher, or any crash selects mismatched binary/record bytes.
...
2. **Environment & commands:** present — path-specific plan/blob, exact clean Step-3b head, one-time bridge, recursively closed historical/future fixtures, the three live schema-v2 predecessor triples, exact schema-v3 migration command/descriptor, pre-edit A104 plus superseding post-commit A106 receipts, cross-filesystem handoff, four targets, native Rust custodian build/invocation/cleanup, six literal frames, isolated real-runtime setup/auth STOP, and owner-provisioned delegated Linux prerequisite are explicit.
3. **Interface & data contracts:** present — byte-sorted multi-owner/JCS historical entries, mutation-only A/M/D future rows, immutable per-step and runner-attempt receipts, RootAuthorityProtocolV1 plus crash-durable step/runner migration and generation ledgers, retained fd-relative root handles and multi-root final cleanup, sole crash-durable lifecycle authority plus five explicit live-authority collections, one keyed pre-binding pending-retirement collection, and three terminal collections with repairable legacy projection, immutable Unreserved-or-Charged birth provenance with a bidirectional zero-or-one charge invariant, exact-tuple ManagedBirthPermit, joined `CodexThreadBirthBarrier`, controller-only Active claim, post-Active first-turn hook chain, fixed-deadline app-server and post-loss cleanup, structurally separate bound and pre-binding terminal receipts with exact capacity release and permanent receipt-referencing fences, stable custody versions, executable-bound cgroup receipts, attempt-bound proof, immutable runtime generations, fourteen exact doctor goldens, tombstone GC, and operator APIs are explicit.
4. **Executable acceptance:** present — canonical A101–A138 are machine-parsed in order and each carries an exact command plus expected evidence. A108 requires a joined thread-birth barrier, an explicit prototype Active decision, one harmless first turn whose exact successful hook chain precedes every observable user/model/tool item, bounded child cleanup, and a genuinely concurrent stable-generation matrix; a zero-turn, pre-Active-turn, unbounded close, or sequential race substitute is forbidden. A109 first validates the closed root migration, then exercises native transition/root/attempt negatives, complete OS-operation provenance, and producer-to-rehash integration. Step 8 reruns only its closed repeatable set and offline-rehashes the immutable completed RunGate chain plus migration lineage; A137 validates final scope with every root/authority retained, and A138 validates the rehash/source-ready projector matrices before crash-durably closing and removing all five root generations.
...
Score: **99/100 (Draft-41 immutable birth-capacity provenance closure)** · trajectory **99 Draft-40 author → 88 X NOT READY on one reservation-provenance gap → 99 bidirectional discriminator/charge repair** · execution eligibility is controlled solely by the current schema-v1 `Review-receipt:`.
