...
updated: "2026-07-12T03:48:40-03:00"
...
- **Runner live job capability:** on the owner-provisioned Linux job, set nonempty `RELAY_RUNNER_ID` to the owner-recorded diagnostic job/runner identity and `RELAY_RUNNER_RECEIPT` to one new harness-owned absolute temporary JSON path; neither value is authority. Before A1c, start one foreground `RELAY_RUNNER_JOB=1 node plugins/session-relay/test/runner-job-custodian.mjs --serve --runner-id "$RELAY_RUNNER_ID" --runner-receipt "$RELAY_RUNNER_RECEIPT"` process and retain that exact process/session through A7. The driver compiles/hashes its committed host-native helper, creates exactly one `AF_UNIX|SOCK_SEQPACKET|SOCK_CLOEXEC` socketpair, spawns the helper with only the custodian endpoint, retains the private gate endpoint, and generates a public random `capability_id`; both processes must stay live. Its stdin accepts only recursively closed JCS `RunGate {schema:1,gate,argv}` frames in the fixed A1c→A1d→A5→A5b→A6→A7 order and its stdout emits exactly one closed `GateResult {schema:1,gate,sequence,pid,status,input_receipt_sha256,result_receipt_sha256,open_ack_sha256,commit_ack_sha256,evidence_sha256}` only after that frame's COMMIT succeeds; unknown fields/gates/argv shapes fail and close the session. The driver remains the parent that launches each exact gate invocation serially in the same orchestrator job. For each child it duplicates the retained gate endpoint, clears `CLOEXEC` only on that duplicate, and passes only its numeric fd in `RELAY_RUNNER_CAP_FD`; the driver keeps the original for the next exact gate. The gate immediately restores `CLOEXEC` on its duplicate, never passes it to a runtime/helper child, and closes it only after COMMIT acknowledgement, so Claude, Codex, shell/tool children, logs, argv, environment snapshots, artifacts, and the public receipt never receive a reusable secret or descriptor.
...
- **Threat-model owner gate:** RESOLVED 2026-07-11 — owner selected Option 1 (`CooperativeWorkerV1`). This resolves the product boundary only; Draft-20 implementation remains paused for fresh immutable dual review, and Step 4 separately requires capability-bound A1c+A1d PASS.
...
| A1c | The exact retained-fd/TRACEEXEC identity architecture is buildable for each mandatory Linux runtime on the proposed delegated runner, and sequence 1 proves live job continuity. | Submit the live custodian an exact `RunGate` frame for `A1c` whose argv executes `test -n "$RELAY_RUNNER_ID" && test -n "$RELAY_RUNNER_RECEIPT" && RELAY_REAL_RUNTIME_TEST=1 node plugins/session-relay/test/feasibility-probe.mjs --case exec-stop-feasibility --matrix --runner-id "$RELAY_RUNNER_ID" --runner-receipt "$RELAY_RUNNER_RECEIPT" --receipt-phase exec-stop`; the custodian injects only `RELAY_RUNNER_CAP_FD`. | OPEN sequence 1 exits its credential challenge with input receipt exactly `RUNNER_BOOTSTRAP_INPUT_SHA256`, the custodian-spawned pid, and fresh live tuple before either runtime probe emits a child/ptrace/exec byte. Claude and Codex rows then pass and write one two-hour unexpired closed partial receipt containing public `capability_id` but no challenge/secret/fd. COMMIT exact-binds that new partial receipt hash plus exec-stop evidence hash and only then emits GateResult/advances. Each raw row proves blocked final child, exact host PID handoff, ancestor `PTRACE_SEIZE|PTRACE_O_TRACEEXEC`, retained-fd `execveat`, exact event PID equal to seized tracee, target sentinel absent, followed `/proc/<pid>/exe` magic-link fd fstat/hash equal to retained fd while stopped, then detach and sentinel. `O_NOFOLLOW` yields `ELOOP`; wrong PID/event/hash/early release/timeout kills and reaps before sentinel. Any mandatory row unavailable poisons and is a hard STOP before Step 4. Copying every receipt/environment byte with no inherited connected fd fails OPEN before the probe; missing/wrong COMMIT prevents sequence 2; challenge bytes never enter either receipt. |
| A1d | The same live custodian supplies sequence 2 while the owner-provisioned Linux job validates real writable cgroup-v2 delegation and completes the receipt. | Submit the same custodian an exact `RunGate` frame for `A1d` whose argv executes `test -n "$RELAY_CGROUP_ROOT" && test -n "$RELAY_RUNNER_ID" && test -n "$RELAY_RUNNER_RECEIPT" && RELAY_REAL_RUNTIME_TEST=1 node plugins/session-relay/test/runtime-hook-abort.mjs --case delegated-runner-preflight --runner-id "$RELAY_RUNNER_ID" --runner-receipt "$RELAY_RUNNER_RECEIPT" --receipt-phase delegation`; the custodian injects only `RELAY_RUNNER_CAP_FD`. | OPEN sequence 2 exact-validates the A1c partial receipt/hash and binds the custodian-spawned pid plus fresh matching capability/runner/boot/kernel/uid/runtime tuple before delegation or probe-child work. It then pins the root dirfd/mount/dev/inode, validates same-uid writable domain delegation, creates a unique disposable child leaf, validates empty subtree plus writable `cgroup.procs|cgroup.kill|cgroup.events`, moves an exact stopped child, kills it, exact-waits/reaps it with recorded status, observes `populated 0`, removes the leaf, and seals canonical JCS final `receipt_sha256` carrying `partial_receipt_sha256`. COMMIT exact-binds that final receipt and delegation/kill/reap/removal evidence hashes before GateResult/sequence 3. Hashes are integrity only, not signature/auth. Copy-all-bytes/no-fd, duplicated-fd replay, stale/out-of-order/concurrent OPEN, authoritative-before-OPEN, missing/wrong/duplicate COMMIT, EOF, absent/read-only delegation, timeout, zombie/unreaped child, populated mismatch, receipt mismatch/expiry, or cleanup failure is FAIL/BLOCK, poisons the capability, and never skips. |
...
| A5 | Linux tree proof requires same-tracee fd-bound exec-stop placement, same-fence kill evidence, and live sequence 3. | Submit `RunGate` `A5` with argv `RELAY_REAL_RUNTIME_TEST=1 node plugins/session-relay/test/lifecycle-smoke.mjs --case containment --runner-receipt "$RELAY_RUNNER_RECEIPT"`. | OPEN sequence 3 must first acknowledge the custodian-spawned pid, final sealed receipt input hash, and freshly revalidated unexpired runner/boot/kernel/uid/mount/root/runtime tuple/current delegation. Only then may production `launch_confined` emit a child/ptrace/exec byte. It verifies private host-PID handoff, seized tracee/event PID equality, gated leaf placement, retained-fd exec, exact TRACEEXEC stop, followed procfs magic-link fd fstat/hash equality while stopped, then detach/GO. `O_NOFOLLOW` gets `ELOOP`. COMMIT keeps the same sealed receipt as result and binds the complete containment evidence hash before GateResult/sequence 4. Forged/substituted PID, early release, seize/event timeout, wrong event, proc fd/hash mismatch, exec/detach failure, scripts/symlinks, membership mismatch, target sentinel before detach, stale receipt, dead/duplicated capability, wrong/missing COMMIT, or live-field mismatch all kill/reap, poison, and fail. Receipt bytes alone never authorize OPEN/GO. |
| A5b | The owner-provisioned delegated Linux runner drives BOTH real runtimes through the production confined launcher under live sequence 4. | Submit `RunGate` `A5b` with argv `RELAY_REAL_RUNTIME_TEST=1 node plugins/session-relay/test/runtime-hook-abort.mjs --case cgroup-cooperative-child-spawn --matrix --runner-receipt "$RELAY_RUNNER_RECEIPT"`. | OPEN sequence 4 binds the custodian-spawned pid, final sealed receipt input hash, and live revalidation on the same unexpired receipt-bound `RELAY_CGROUP_ROOT` runner from A1d before either real runtime launches. Harness calls production supervisor/`launch_confined`, captures tracee/event/fd/proc-identity/placement and same-fence kill receipts, creates real child/grandchild/storm under leaf, exact-waits/reaps owned roots, and reaches populated-zero. Target sentinel proves no target instruction before detach. A second worker mints a distinct placement receipt. COMMIT keeps the sealed receipt as result and binds the full two-runtime evidence hash before GateResult/sequence 5. Cached/fixture/copied evidence cannot authorize OPEN/GO; missing/wrong COMMIT poisons. Missing/stale/mismatched runner or capability is BLOCK, not skip; Claude filtered hardening stays separate. |
| A6 | Real Linux managed admission and durable controller are runtime-correct and fail closed on custody loss under live sequence 5. | Submit `RunGate` `A6` with argv `RELAY_REAL_RUNTIME_TEST=1 node plugins/session-relay/test/runtime-hook-abort.mjs --matrix --runner-receipt "$RELAY_RUNNER_RECEIPT"`. | OPEN sequence 5 binds the custodian-spawned pid, final sealed receipt input hash, and live runner-receipt/delegation revalidation before pending creation, child, RPC, or fence bytes. Claude barriers pass. Codex pending creation atomically mints one sealed `ManagedBirthPermit`; only its exact Attaching tuple launches once, and replay/wrong tuple emits zero bytes. SessionStart remains non-claiming; controller alone claims Active; transcript has zero `turn/start` before durable Active. Foreign/refusal rows preserve foreign state. After caller exit a second invocation uses one typed result. Every unsupported/unknown/custody-loss row atomically records logical loss plus one exact `LossCleanupPermit`; after publication that permit alone closes stdio, emits no further bytes, kills live retained custody, and bounded-reaps/observes populated-zero. Replay/stale/cross-loss/no-live-custody emits zero bytes; crash cannot remint; success leaves lifecycle unconfirmed and capacity held. COMMIT keeps the sealed receipt as result and binds the complete admission/controller/loss evidence hash before GateResult/sequence 6; missing/wrong COMMIT poisons. No CLI fallback/bypass. |
...
| A7 | Real app-server fencing distinguishes exact cancellation, shared observation, current ProtocolTree unavailability, and physical kill under final live sequence 6. | Submit `RunGate` `A7` with argv `RELAY_REAL_RUNTIME_TEST=1 node plugins/session-relay/test/runtime-appserver-quiescence.mjs --matrix --runner-receipt "$RELAY_RUNNER_RECEIPT"`. | OPEN sequence 6 binds the custodian-spawned pid, final sealed receipt input hash, and live runner/delegation revalidation before any RPC/fence byte; initialize advertises `experimentalApi=true` before descendant/turn/item/terminal methods and records runtime/method availability; unsupported methods fail typed. Exact interrupt reaches matching terminal; each terminal is paged and terminated only through its sealed same-attempt reference. Shared rows remain observation; current dedicated rows report MissingDurableFlushContract. Physical cgroup kill produces only WorkerTree. COMMIT keeps the sealed receipt as result, binds the complete final evidence hash, records both terminal OPEN/COMMIT acknowledgement links, and only then closes the capability as Complete; missing/wrong COMMIT poisons instead. Any later/replayed gate fails. |
...
2. **Environment & commands:** present — path-specific plan/blob, preserved-WIP bridge, immutable historical manifest plus future step allowlist, cross-filesystem handoff, four targets, shell-first stable runtime, isolated real-runtime setup, measured deadline, foreground live runner custodian with exact two-phase six-gate protocol, exact reap, and owner-provisioned delegated Linux runner prerequisite are explicit. Current host ineligibility is recorded, not silently skipped.
...
Adversarial cold-read result: Draft-20 preserves every accepted Draft-19 repair and closes its receipt-ordering gap. Standalone `/bin/sh` bootstrap still owns ordinary-prune durability and observed corruption checks only; deliberate same-UID pre-exec replace-all is explicitly out of cooperative scope, post-start native installer pinning is exact, and docks-kit retains an optional externally verified-native path. Runner continuity is no longer inferred from copyable JSON/environment: one live custodian, inherited `SOCK_SEQPACKET` fd, OPEN before any authoritative byte, COMMIT of resulting receipt/evidence before sequence advance, strict six-gate progression, descriptor non-leakage, and copy/replay/crash/poison negatives make the order executable. LossCleanupPermit, ManagedBirthPermit, controller variants/inventory ownership, exact reap/live delegation, followed proc magic-link TRACEEXEC, fd-pinned installer/schema, historical-plus-future A18 union, same-version tuple, and Linux/Darwin split remain intact. No cold executor must guess the hook or runner threat boundary. Implementation remains paused for fresh immutable dual re-review; Step 4 and A5/A5b/A6/A7 remain blocked until the owner supplies the live capability-bound runner. Remote push/dispatch and packaged producer/release gates remain owner approvals.
...
Score: **97/100 (Draft-20 author candidate; fresh immutable dual re-review pending)** · trajectory **96 Draft-18 author → 90 acceptance NOT READY → 96 Draft-19 author → 92 acceptance NOT READY → 97 two-phase runner repair** · stopped: **candidate frozen for review; implementation paused; real Linux gates require one owner-provisioned live custodian through A1c/A1d/A5/A5b/A6/A7**.
