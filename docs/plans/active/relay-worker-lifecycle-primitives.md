...
updated: "2026-07-12T02:48:01-03:00"
...
- **Runner handoff variables:** on the owner-provisioned Linux job, set nonempty `RELAY_RUNNER_ID` to the owner-recorded immutable job/runner identity, generate `RELAY_RUNNER_SESSION_NONCE` as exactly 64 lowercase hex characters (256-bit correlation only), and set `RELAY_RUNNER_RECEIPT` to one new harness-owned absolute temporary JSON path. The receipt stores only the nonce SHA-256. `runner_receipt_ttl_ms = 7_200_000` from A1c's one `CLOCK_BOOTTIME` start; no phase or retry resets it. ISO timestamps are diagnostic; `{started_boottime_ns,expires_boottime_ns,host_boot_id}` are authoritative for expiry. A1c creates the exact host/runtime portion at that path; A1d must run next in the **same non-migrating job**, exact-check the nonce plus boot/kernel/uid/runtime identities, complete the delegation/child-cleanup portion, and atomically seal the receipt hash. A5/A5b/A6/A7 must finish before expiry or rerun A1c+A1d in their current same job. A copy, later job, boot change, expired receipt, different runtime bytes, or missing partial receipt fails. The receipt is integrity evidence only—not a signature, attestation, authentication token, or later-run authority—and is never accepted without live revalidation.
...
- **Threat-model owner gate:** RESOLVED 2026-07-11 — owner selected Option 1 (`CooperativeWorkerV1`). This resolves the product boundary only; Draft-18 implementation remains paused for fresh immutable dual review, and Step 4 separately requires same-job A1c+A1d PASS.
...
pub struct DelegatedRunnerReceipt {
    ...
    runner_id: String,
    runner_session_nonce_sha256: String,
    host_boot_id: String,
...
Shell/argv quoting is emitted literally by `codex-hooks.json`; neither cwd nor hook input is interpolated into code. Installer JSON is redirected so complete hook stdout contains exactly one hook response object. Any plugin-root symlink—including a dangling symlink detected by `test -L`—fails explicitly; only a truly absent, non-link root (`! -e && ! -L`) falls through to stable `current/relay`. When N's cached plugin root remains an exact real directory, the launcher must be a non-symlink regular executable and validate/install; any failure while the root still exists or is replaced by any link/non-directory propagates. If the real root disappears after the directory check, the invalid-launcher branch now rechecks both `-e` and `-L` before its exit; disappearance before launcher exec or during pinned manifest/checksum reads uses the same exact recheck and falls through only when the whole root is truly absent. A present non-directory/symlinked root or missing/nonexec/symlinked launcher fails. Missing/invalid `current` fails visibly; it never searches cache directories. A6b installs deterministic barriers immediately after the successful real-directory check and before launcher validation, before launcher exec, and during every pinned source read; it covers dangling/non-dangling root-link cases and parses complete stdout as exactly one event-correct hook JSON object.

The runtime root is same-owner mode 0700 and contains `install.lock` (same-owner regular mode 0600), immutable `generations/<plugin_version>-<binary_sha256>/{relay,runtime.json}`, a staging directory, and an intentional relative `current` symlink to one complete generation. The installer first opens the exact supplied plugin root as a same-owner `O_PATH|O_DIRECTORY|O_NOFOLLOW|O_CLOEXEC` dirfd, then opens exact `.codex-plugin/plugin.json`, `.claude-plugin/plugin.json`, `hooks/codex-hooks.json`, `hooks/hooks.json`, `bin/SHA256SUMS`, `bin/relay`, the selected `bin/<target>`, and every canonical payload-manifest member relative to pinned parent dirfds with per-component no-follow checks. It retains all manifest/checksum/hook-definition/payload-member/selected-binary fds through validation and copy; the selected payload is hashed and copied from the same open fd, and `plugin_payload_sha256`/`hook_definitions_sha256` are computed only from those retained fds. Rename/recreate/symlink barriers at every source component must either fail validation or preserve the originally opened bytes—never mix manifests, checksum, hook definitions, dispatcher, or payload across generations.

The pinned manifests must match `name=session-relay` and identical strict-semver `version`; `bin/SHA256SUMS` has a unique lowercase SHA-256 plus basename for exactly the closed target mapping `Linux x86_64→relay-x86_64-unknown-linux-musl`, `Linux aarch64→relay-aarch64-unknown-linux-musl`, `Darwin x86_64→relay-x86_64-apple-darwin`, `Darwin arm64→relay-aarch64-apple-darwin`; selected payload is a regular executable matching its digest. `bin/relay` is only the pinned validated dispatcher used to invoke the installer; stable generation copies the selected native binary, never that shell launcher. Never use Cargo package version. The installer locks `install.lock` across current-state read, source validation, generation staging, every fsync, and pointer commit. It lstat-validates every runtime parent/generation object; hostile symlinks are rejected, while managed `current` is allowed only when its target is one same-root immutable generation with matching regular 0755 binary and closed record. Exact portable commit sequence: write+fsync binary and record in staging; fsync staging generation directory; rename staging into `generations/`; fsync `generations/`; create temporary relative symlink in runtime root; fsync runtime root; rename it over `current`; fsync root again. Never fsync the symlink inode. Crash selects complete old or new pair; partial staging is never selected and is cleaned under next lock. Keep current+previous; delete older unselected only after pointer durability. Higher semver replaces; lower no-ops. A same-version no-op is permitted only when the complete canonical identity tuple `{plugin_version,target,binary_sha256,plugin_payload_sha256,hook_definitions_sha256}` equals the selected record; a mismatch in **any** field is fatal and cannot repair/replace/no-op. Lock serialization prevents downgrade. Core standalone hooks bootstrap this contract; docks-kit may proactively invoke the same interface but is not required for safety.
...
| 1b | After preserved Step 3b is clean/committed, run the one-time A0c handoff, create the reusable WIP snapshot helper, immutable exact historical-range manifest, and separate closed future per-step allowlist; migrate feasibility evidence to independent `cooperative_cgroup` and Claude-only `filtered_hardening`; prototype raw Codex `thread/start→controller claim candidate→turn/start` ordering (not production Rust claim), inventory experimental capability, define semantic hook-health snapshots, and prove stable-runtime generation/pointer feasibility. | `plugins/session-relay/test/feasibility-probe.mjs`, `plugins/session-relay/test/fixtures/lifecycle-capability-schema.json`, `plugins/session-relay/test/fixtures/wip-historical-baseline.json` (new), `plugins/session-relay/test/fixtures/wip-step-allowlist.json` (new), `plugins/session-relay/test/runtime-hook-abort.mjs` (new), `plugins/session-relay/test/runtime-hook-upgrade.mjs` (new), `plugins/session-relay/test/wip-snapshot.mjs` (new) | 1, 3b | planned | A0c/A0b/A1/A1b pass. Helper reproduces empty state and rejects every dirty class. Historical fixture exactly binds `12cf2ea..STEP3B_HEAD` path/status/modes/oids/owning commits and marks only the stale implementation plan `must_be_absent_at_tip`; future fixture closes every remaining step/path/mode mapping without absorbing baseline facts. Raw trace proves SessionStart-before-thread-response and zero turn before a prototype claim decision without pretending JS is shipped lifecycle logic. Stable generation/pointer survives pruning/crash/races. No implementation-branch plan/doc edit is permitted. |
| 1c | On the named owner-provisioned Linux job, feasibility-first prototype the exact blocked-child→host-PID handoff→seize/TRACEEXEC→retained-fd `execveat`→same-tracee exec-stop identity sequence independently for Claude and Codex and begin the closed runner receipt. This is raw evidence, not the Step-4 production implementation. | `plugins/session-relay/test/feasibility-probe.mjs`, `plugins/session-relay/test/runtime-hook-abort.mjs`, `plugins/session-relay/test/fixtures/lifecycle-capability-schema.json` | 1b | planned | A1c passes for both runtime binaries and writes the partial unexpired `RELAY_RUNNER_RECEIPT`: target sentinel is absent until detach; followed procfs magic-link fd matches retained fd; `O_NOFOLLOW` proves `ELOOP`; every injected mismatch kills/reaps before target code. A1d must immediately complete the same nonce/host receipt. Any mandatory row unavailable: STOP before Step 4 and report the primitive unbuildable on the proposed Linux path. |
| 1d | In the same non-migrating job as A1c, validate the owner-provisioned delegated Linux runner without self-provisioning: pin `RELAY_CGROUP_ROOT`, create a disposable domain leaf, exercise stopped-child placement, `cgroup.kill`, exact wait/reap, `populated 0`, and leaf removal, then seal the closed integrity-only receipt. | `plugins/session-relay/test/runtime-hook-abort.mjs`, `plugins/session-relay/test/fixtures/lifecycle-capability-schema.json` | 1b, 1c | planned | A1d passes only after matching A1c runner nonce/boot/kernel/uid/runtime bytes. It exact-waits/reaps the killed probe child before populated-zero/removal and records every result. Timeout, zombie/unreaped child, receipt expiry/mismatch, or cleanup failure blocks. The current read-only-cgroup/no-user-bus sandbox is not eligible. Missing delegation/runner is never a skip; no Step 4 or A5/A5b/A6/A7 completion begins until resolved. |
...
| 8 | Complete the already-born per-step adversarial matrices and enforce inventory **completeness only**; preserve all existing relay tests and isolated cleanup. Verify that Steps 3b/3c/4/5/6 each added their target/case before their first acceptance gate, verify the immutable historical range manifest against exact owner commits, then auto-enumerate compile-fail fixtures and close any remaining source-derived rows. Include watchdog/supervisor failures, authority races, every loss source/reason, cancellation, post-loss cleanup permits, controller birth/claim/command authorization, GC, exec-stop/cgroup receipts, runner receipt/reap/live-revalidation, hook/installer/schema races, protocol-attempt mixing, proof-retention crash, and missing-flush rows. | `plugins/session-relay/rust/src/appserver.rs`, `plugins/session-relay/rust/src/hook.rs`, `plugins/session-relay/rust/src/lifecycle.rs`, `plugins/session-relay/rust/src/process_identity.rs`, `plugins/session-relay/rust/src/runtime_install.rs`, `plugins/session-relay/rust/src/supervisor.rs`, `plugins/session-relay/rust/src/spawn.rs`, `plugins/session-relay/rust/src/store.rs`, `plugins/session-relay/test/rust-test-inventory.mjs`, `plugins/session-relay/test/fixtures/rust-test-inventory.json`, `plugins/session-relay/test/fixtures/wip-historical-baseline.json`, `plugins/session-relay/test/fixtures/wip-step-allowlist.json`, `plugins/session-relay/test/feasibility-probe.mjs`, `plugins/session-relay/test/supervisor-custody.mjs`, `plugins/session-relay/test/lifecycle-smoke.mjs`, `plugins/session-relay/test/process-signal-inventory.mjs`, `plugins/session-relay/test/reentry-inventory.mjs`, `plugins/session-relay/test/runtime-hook-abort.mjs`, `plugins/session-relay/test/runtime-hook-upgrade.mjs`, `plugins/session-relay/test/runtime-appserver-quiescence.mjs`, `plugins/session-relay/test/fake-app-server.mjs`, `plugins/session-relay/test/selftest.mjs` | 2-7 | planned | A1/A1b/A1c/A1d through A14 pass; fixture history/source checks prove no target was first introduced after its gate and every `12cf2ea..STEP3B_HEAD` path/mode/status/oid belongs to named exact commits. Every target executes a source-derived nonzero required set; every compile-fail bin fails at its intended boundary; helpers clean in `finally`; external evidence establishes results. Step 8 may not retroactively make an earlier zero/missing target green or broaden historical ownership. |
...
| A1c | The exact retained-fd/TRACEEXEC identity architecture is buildable for each mandatory Linux runtime on the proposed delegated runner. | `test -n "$RELAY_RUNNER_ID" && [[ "$RELAY_RUNNER_SESSION_NONCE" =~ ^[0-9a-f]{64}$ ]] && test -n "$RELAY_RUNNER_RECEIPT" && RELAY_REAL_RUNTIME_TEST=1 node plugins/session-relay/test/feasibility-probe.mjs --case exec-stop-feasibility --matrix --runner-id "$RELAY_RUNNER_ID" --runner-receipt "$RELAY_RUNNER_RECEIPT" --receipt-phase exec-stop` | Exit 0 for Claude and Codex native binaries and writes one two-hour unexpired partial closed receipt with only the nonce hash. Each raw row proves blocked final child, exact host PID handoff, ancestor `PTRACE_SEIZE|PTRACE_O_TRACEEXEC`, retained-fd `execveat`, exact event PID equal to seized tracee, target sentinel absent, followed `/proc/<pid>/exe` magic-link fd fstat/hash equal to retained fd while stopped, then detach and sentinel. `O_NOFOLLOW` yields `ELOOP`; wrong PID/event/hash/early release/timeout kills and reaps before sentinel. Any mandatory row unavailable is a hard STOP before Step 4; moving the receipt to another job/host is invalid. |
| A1d | The same named owner-provisioned Linux job supplies real writable cgroup-v2 delegation and completes the receipt. | `test -n "$RELAY_CGROUP_ROOT" && test -n "$RELAY_RUNNER_ID" && [[ "$RELAY_RUNNER_SESSION_NONCE" =~ ^[0-9a-f]{64}$ ]] && test -n "$RELAY_RUNNER_RECEIPT" && RELAY_REAL_RUNTIME_TEST=1 node plugins/session-relay/test/runtime-hook-abort.mjs --case delegated-runner-preflight --runner-id "$RELAY_RUNNER_ID" --runner-receipt "$RELAY_RUNNER_RECEIPT" --receipt-phase delegation` | Exit 0 only when the A1c nonce/runner/boot/kernel/uid/runtime identities match and the unexpired receipt is completed. It pins the root dirfd/mount/dev/inode, validates same-uid writable domain delegation, creates a unique disposable child leaf, validates empty subtree plus writable `cgroup.procs|cgroup.kill|cgroup.events`, moves an exact stopped child, kills it, exact-waits/reaps it with recorded status, then observes `populated 0`, removes the leaf, and seals canonical JCS `receipt_sha256`. Hash is integrity only, not signature/auth. Absent/read-only delegation, timeout, zombie/unreaped child, populated mismatch, receipt mismatch/expiry, or cleanup failure is FAIL/BLOCK, never skip. |
...
| A5 | Linux tree proof requires same-tracee fd-bound exec-stop placement and same-fence kill evidence. | `RELAY_REAL_RUNTIME_TEST=1 node plugins/session-relay/test/lifecycle-smoke.mjs --case containment --runner-receipt "$RELAY_RUNNER_RECEIPT"` | Exit 0 only after live revalidation of every unexpired A1c+A1d runner/boot/kernel/uid/mount/root/runtime field and current delegation. Production `launch_confined` verifies private host-PID handoff, seized tracee/event PID equality, gated leaf placement, retained-fd exec, exact TRACEEXEC stop, followed procfs magic-link fd fstat/hash equality while stopped, then detach/GO. `O_NOFOLLOW` gets `ELOOP`. Path replacement executes retained original or refuses. Forged/substituted PID, early release, seize/event timeout, wrong event, proc fd/hash mismatch, exec/detach failure, scripts/symlinks, membership mismatch, target sentinel before detach, stale receipt, or live-field mismatch all kill/reap and fail. Same constructor mints kill receipt; a receipt hash alone never authorizes GO. |
| A5b | The owner-provisioned delegated Linux runner drives BOTH real runtimes through the production confined launcher. | `RELAY_REAL_RUNTIME_TEST=1 node plugins/session-relay/test/runtime-hook-abort.mjs --case cgroup-cooperative-child-spawn --matrix --runner-receipt "$RELAY_RUNNER_RECEIPT"` | Exit 0 only after live revalidation on the same unexpired receipt-bound `RELAY_CGROUP_ROOT` runner from A1d. Harness calls production supervisor/`launch_confined`, captures tracee/event/fd/proc-identity/placement and same-fence kill receipts, creates real child/grandchild/storm under leaf, exact-waits/reaps owned roots, and reaches populated-zero. Target sentinel proves no target instruction before detach. A second worker mints a distinct placement receipt. Cached/fixture evidence cannot authorize GO. Missing/stale/mismatched runner is BLOCK, not skip; Claude filtered hardening stays separate. |
| A6 | Real Linux managed admission and durable controller are runtime-correct and fail closed on custody loss. | `RELAY_REAL_RUNTIME_TEST=1 node plugins/session-relay/test/runtime-hook-abort.mjs --matrix --runner-receipt "$RELAY_RUNNER_RECEIPT"` | Exit 0 only after live runner-receipt/delegation revalidation. Claude barriers pass. Codex pending creation atomically mints one sealed `ManagedBirthPermit`; only its exact Attaching tuple launches once, and replay/wrong tuple emits zero bytes. SessionStart remains non-claiming; controller alone claims Active; transcript has zero `turn/start` before durable Active. Foreign/refusal rows preserve foreign state. After caller exit a second invocation uses one typed result. Every unsupported/unknown/custody-loss row atomically records logical loss plus one exact `LossCleanupPermit`; after publication that permit alone closes stdio, emits no further bytes, kills live retained custody, and bounded-reaps/observes populated-zero. Replay/stale/cross-loss/no-live-custody emits zero bytes; crash cannot remint; success leaves lifecycle unconfirmed and capacity held. No CLI fallback/bypass. |
...
| A6b | Source-built N/N+1 payloads exercise exact hooks and fd-pinned crash-atomic monotonic selection. | `RELAY_REAL_RUNTIME_TEST=1 node plugins/session-relay/test/runtime-hook-upgrade.mjs --matrix` | Exit 0. Real Codex uses synthetic N/N+1 payloads with exact hook JSON, both manifests, checksums, and hook/payload identities. Old UserPromptSubmit and new/subagent SessionStart execute through stable current; stdout is one hook JSON with exact event suffix. Any plugin-root symlink including dangling fails; only truly absent non-link root falls through. A deterministic barrier deletes the real root after its successful directory check but before launcher validation and must fall through stable; a present invalid launcher/root link/non-directory must exit 4. Dirfd/openat barriers rename/recreate every manifest/checksum/hook/payload/selected-binary component; installer either fails or copies/hashes its pinned fd bytes without mixing. Same-version no-op requires equality of version,target,binary,payload,hook; each single-field mismatch is fatal. Lock/generation/pointer crash/races pass. Implementation evidence is not packaged evidence. |
...
- A1c and A1d do not run consecutively in one owner-named non-migrating Linux job, their closed runner receipt nonce/boot/kernel/uid/mount/root/runtime fields differ or expire, the receipt hash is treated as signature/auth/GO authority, the probe child is not exact-waited/reaped before populated-zero/removal, or live A5/A5b/A6/A7 revalidation fails. The current read-only cgroup/no-user-bus sandbox cannot be relabeled positive.
...
- Any plugin-root symlink including dangling falls through; disappearance after a successful real-directory check but before launcher validation fails to recheck `! -e && ! -L`; any present invalid launcher/non-directory/root-link ambiguity reaches stable execution; installer validation reopens a source pathname after pinning; selected payload hashing/copying use different fds; rename/recreate barriers mix source generations; or same-version no-op ignores target/payload/hook identity.
...
2. **Environment & commands:** present — path-specific plan/blob, preserved-WIP bridge, immutable historical manifest plus future step allowlist, cross-filesystem handoff, four targets, shell-first stable runtime, isolated real-runtime setup, measured deadline, same-job A1c/A1d receipt, exact reap, and owner-provisioned delegated Linux runner prerequisite are explicit. Current host ineligibility is recorded, not silently skipped.
3. **Interface & data contracts:** present — exact-tuple one-use ManagedBirthPermit, non-claiming SessionStart plus controller-only claim, durable app-server controller, exact controller OperationKinds, post-loss one-use cleanup permit with no release authority, closed integrity-only delegated-runner receipt, stable cancellation/custody versions, nonce/control epoch, executable-bound cgroup receipts, attempt-bound proof, fd-pinned immutable runtime generations, deterministic schema, doctor/install schemas, tombstone GC, and operator APIs are explicit.
4. **Executable acceptance:** present — A0/A0b-bootstrap/A0b–A18 plus A1b/A1c/A1d/A3a–A3d/A6b/A6c are command + expected evidence, including same-job TRACEEXEC/delegation receipt, exact child reap, live runner revalidation, post-loss cleanup authority/replay/no-release, after-directory-check hook pruning, per-step nonzero inventory birth, controller A9/A10 rerun, source-fd and schema determinism races, live cache upgrade, later-process controller wake, Darwin unavailable rows, and exact historical-plus-future actual-range manifests/hashes.
...
Adversarial cold-read result: Draft-18 preserves the owner-approved functional goal while stating its already-existing Linux-authoritative/Darwin-unavailable boundary in the headline. It closes Draft-17's remaining contradictions: durable controller loss atomically returns one post-loss cleanup permit without creating proof/release authority; literal hooks recheck the root in the invalid-launcher prune window; A1c+A1d produce one recursively closed same-job integrity-only receipt with exact child reap and later live revalidation; and A18 composes an immutable owned historical baseline with a separate future step union. Draft-17's ManagedBirthPermit, controller variants/inventory ownership, followed proc magic-link TRACEEXEC, fd-pinned installer/schema, same-version tuple, and typed platform split remain intact. Implementation remains paused for fresh immutable dual re-review. Step 4 and A5/A5b/A6/A7 additionally remain blocked until the owner supplies a passing same-job A1c+A1d runner. Remote push/dispatch and packaged producer/release gates remain owner approvals.
...
Score: **96/100 (Draft-18 author candidate; fresh immutable dual re-review pending)** · trajectory **96 Draft-17 author → 86 architecture NOT READY → 96 post-loss-authority/runner/hook/scope repair** · stopped: **candidate frozen for review; implementation paused; real Linux gates require one owner-provisioned same-job A1c+A1d receipt**.
