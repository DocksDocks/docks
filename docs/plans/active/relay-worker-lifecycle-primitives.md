...
pub enum ManagedAppServerCommand {
    ReadThread { operation_id: String, expected_operation_version: String },
    InjectItems { operation_id: String, expected_operation_version: String, items: ValidatedAppServerInput },
    StartTurn { operation_id: String, expected_operation_version: String, prompt: ValidatedAppServerInput },
    InterruptOwnedTurn { operation_id: String, expected_operation_version: String },
    ListDescendants { operation_id: String, expected_operation_version: String, cursor: Option<String> },
    ListOwnedTerminals { operation_id: String, expected_operation_version: String, cursor: Option<String> },
    TerminateOwnedTerminal { operation_id: String, expected_operation_version: String, process_id: String },
}
...
The committed Codex hook command is the exact durable shell bootstrap below; its first executable is `/bin/sh`, never the versioned relay:

```sh
/bin/sh -c 'plugin=$1; root=${XDG_DATA_HOME:-$HOME/.local/share}/docks/session-relay/runtime; if test -x "$plugin/bin/relay"; then "$plugin/bin/relay" __install-stable --plugin-root "$plugin" --json || exit $?; fi; exec "$root/current/relay" hook codex' relay-hook "${CLAUDE_PLUGIN_ROOT}"
```

Shell/argv quoting is emitted literally by `codex-hooks.json`; neither cwd nor hook input is interpolated into code. When N's cached plugin root still exists it may validate/install; after pruning, the already-cached command skips only that absent executable and unconditionally execs stable `current/relay`. Missing/invalid `current` fails visibly; it never searches cache directories.

The runtime root is same-owner mode 0700 and contains `install.lock` (same-owner regular mode 0600), immutable `generations/<plugin_version>-<binary_sha256>/{relay,runtime.json}`, a staging directory, and an intentional relative `current` symlink to one complete generation. The versioned installer reads plugin manifest version and `SHA256SUMS` from the exact supplied root—never Cargo package version—then locks `install.lock` across current-state read, source validation, generation staging, every fsync, and pointer commit. It lstat-validates every parent/source/generation object; hostile symlinks are rejected, while the managed `current` relative symlink is allowed only when its target is one same-root immutable generation with matching regular 0755 binary and closed record. It writes+fsyncs the entire new generation and runtime record, fsyncs the generations directory, creates/fsyncs a temporary relative pointer, atomically renames only that pointer to `current`, and fsyncs root. Thus a crash selects either the complete old or complete new pair; partial staging is never selected and is cleaned under the next lock. Keep current and previous generations; delete older unselected generations only under lock after pointer durability. Higher semver replaces; same version+digest is no-op; lower is no-op; same version+different digest is fatal. Lock serialization makes N+2 win over later N+1 rather than allowing a downgrade. Core standalone hooks bootstrap this contract; docks-kit may proactively invoke the same installer/doctor but is not required for safety.
...
Every object is recursively closed in the committed JSON schema; version/digest/generation/path/reason formats, capability keys, and the exit taxonomy (`0=result`, `2=usage/schema`, `3=unavailable`, `4=validation/tamper`, `5=I/O/lock`) are enumerated there. Nonzero JSON mode emits exactly one schema-valid `RuntimeToolError` on stdout; stderr is human diagnostic only. `plugin_payload_sha256` is a canonical sorted path/mode/content manifest over the installed plugin payload excluding the mutable stable runtime root; `hook_definitions_sha256` is the canonical normalized shipped SessionStart/UserPromptSubmit definition set. They drive diagnostics/restart delta only, never lifecycle proof.
...
| 1b | After preserved Step 3b is clean/committed, create the reusable WIP snapshot helper and migrate feasibility evidence to independent `cooperative_cgroup` and Claude-only `filtered_hardening`; prototype raw Codex `thread/start→controller claim candidate→turn/start` ordering (not production Rust claim), inventory experimental capability, define semantic hook-health snapshots, and prove stable-runtime generation/pointer feasibility. | `plugins/session-relay/test/feasibility-probe.mjs`, `plugins/session-relay/test/fixtures/lifecycle-capability-schema.json`, `plugins/session-relay/test/runtime-hook-abort.mjs` (new), `plugins/session-relay/test/runtime-hook-upgrade.mjs` (new), `plugins/session-relay/test/wip-snapshot.mjs` (new), `docs/plans/active/relay-worker-lifecycle-primitives.md:Notes` | 1, 3b | planned | A0b/A1/A1b pass. Helper reproduces empty state and rejects every dirty class. Raw trace proves SessionStart-before-thread-response and zero turn before a prototype claim decision without pretending JS is shipped lifecycle logic. Stable generation/pointer survives pruning/crash/races. |
...
| 5 | Implement managed first-prompt admission and durable controller. Claude uses Claiming-first SessionStart plus UserPromptSubmit. Codex supervisor retains app-server Child/stdio/request map/notification+approval pump; managed SessionStart is non-claiming and returns before `thread/start`; controller alone performs exact post-response claim before first turn. Later processes use typed admitted controller commands. Add exact durable shell hook bootstrap, locked generation/pointer installer, doctor/install JSON schemas, semantic hook health, explicit timeouts/dead-man, and unmanaged compatibility. | `plugins/session-relay/rust/src/appserver.rs`, `plugins/session-relay/rust/src/hook.rs`, `plugins/session-relay/rust/src/runtime_install.rs` (new), `plugins/session-relay/rust/src/main.rs`, `plugins/session-relay/rust/src/spawn.rs`, `plugins/session-relay/rust/src/store.rs`, `plugins/session-relay/rust/src/supervisor.rs`, `plugins/session-relay/hooks/hooks.json`, `plugins/session-relay/hooks/codex-hooks.json`, `plugins/session-relay/test/fixtures/runtime-doctor-schema.json` (new), `plugins/session-relay/test/runtime-hook-abort.mjs` (new), `plugins/session-relay/test/runtime-hook-upgrade.mjs` (new), `plugins/session-relay/test/selftest.mjs` | 1b-4 | planned | A2/A6/A6b/A6c prove production claim ordering, later-process wake, controller/server/caller failure rows, actual cached hook command after root deletion, crash-atomic locked monotonic selection, schema-valid doctor/install output, Claude barrier, and health degradation. Managed CLI fallback, pre-Active turn, arbitrary JSON forwarding, reconnect guessing, or non-atomic runtime pair: STOP. |
...
| A2 | Pending identity binding, controller claim, unmanaged cancellation, and multi-operation lost-authority transitions are atomic, durable, typed, and exact. | `(cd plugins/session-relay/rust && cargo test --locked managed_attach_ -- --nocapture)` | Exit 0; durable waiter/controller retries share one bind; Codex SessionStart never claims. Conflicting identity/server/controller refuses. Every typed reason runs with 2+ operations from Unmanaged, UnmanagedCanceling, Claiming/Attaching, Active, Fencing, FencingUnconfirmed, terminal, and stale sources with crash/reload between loss and claim. One batch preserves each original reason; only live deadline adds CancelDeadline; terminal/stale audit only. |
...
| A3b | Detached watchdog/supervisor own each transition-capable child from birth and preserve attach/wake I/O. | `(cd plugins/session-relay/rust && cargo test --locked lifecycle_supervisor_ -- --nocapture) && node plugins/session-relay/test/supervisor-custody.mjs --matrix && node plugins/session-relay/test/reentry-inventory.mjs --compile-fail` | Exit 0. Source-derived closed startup phases each inject before/after failure. Rows prove explicit caller→watchdog and watchdog→supervisor pipes, caller death at every boundary, SID/no-PDEATHSIG/closed stdio, Linux `/proc/<pid>/fd` and Darwin `proc_pidinfo(PROC_PIDLISTFDS)` show zero bootstrap fds at Ready, health vs control nonce, stale epoch/version, exact Ready→ControlBound→spawn→proxy→Started, kill/wait/reap, compatibility, and zero-byte stale events. |
...
| A5 | Tree proof requires fresh production-launch placement and same-fence kill evidence; filtered hardening remains separate. | `node plugins/session-relay/test/lifecycle-smoke.mjs --case containment` | Exit 0; the exact supervisor `launch_confined` path records worker/generation/supervisor/control epoch, host boot/kernel/leaf, resolved executable path+dev+ino+SHA256+target triple+runtime/sandbox versions before GO. Same production constructor mints matching kill receipt. Changed bytes at same path/version, boot change, reused leaf, missing receipt, cached fixture capability, and stale supervisor all refuse. Pre-kill cannot promote. |
| A5b | CI/release regression drives BOTH real runtimes through the exact production confined launcher; captured receipts never authorize another launch. | `RELAY_REAL_RUNTIME_TEST=1 node plugins/session-relay/test/runtime-hook-abort.mjs --case cgroup-cooperative-child-spawn --matrix` | Exit 0. Harness calls production supervisor/`launch_confined`, captures that worker's private placement+same-fence kill receipts and full binary provenance, creates real child/grandchild/storm under leaf, then populated-zero. A second worker must mint a distinct receipt. Rows state `authoritative_for_production=false`; fixture-only/cached path cannot reach GO. Claude filtered hardening remains separate. |
| A6 | Real production managed admission and durable controller are runtime-correct. | `RELAY_REAL_RUNTIME_TEST=1 node plugins/session-relay/test/runtime-hook-abort.mjs --matrix` | Exit 0. Claude barrier rows pass. Codex SessionStart-before-response/skipped/timeout rows all return without claim; Rust controller persists returned id and alone claims Active before first turn. Claim fail/deadline/conflict emits zero turn/model bytes. After original caller exits, a second relay invocation reads/injects/starts/interrupts through same controller. Controller-only/server-only/supervisor-only death, concurrent requests, approval/server-request handling, ambiguous response, and stale replacement follow typed loss/zero-byte rules. No CLI fallback/bypass. |
| A6b | The actual committed Codex hook command survives cache refresh with crash-atomic monotonic selection. | `RELAY_REAL_RUNTIME_TEST=1 node plugins/session-relay/test/runtime-hook-upgrade.mjs --matrix` | Exit 0; capture N's resolved command verbatim in one live Codex process, install N+1, delete N's entire plugin root, then invoke old UserPromptSubmit and new/subagent SessionStart through their real cached commands. They exec stable current. Install lock/generation/pointer tests cover concurrent N+2 vs N+1, same version conflict, hostile vs managed symlink, every fsync/rename crash, partial-stage recovery, absent old root, and retain current+previous. Direct stable/helper invocation cannot satisfy the row. |
| A6c | Installer and doctor expose the exact closed consumer contract without promoting diagnostics to proof. | `node plugins/session-relay/test/runtime-hook-upgrade.mjs --case doctor-schema` | Exit 0; `__install-stable --json` and `doctor --json --capabilities` validate against committed `runtime-doctor-schema.json`, reject extra/malformed fields, and report selected/previous generation, changed, version/target/binary digest, plugin payload+normalized hook digests/health, capabilities, and typed reason codes. Human output is a projection; stdout/stderr text cannot classify state. |
...
| A18 | The exact clean implementation tip is scoped and contains no binary/release mutation. | `test "$(git branch --show-current)" = codex/primitives-collab && test "$(git rev-parse HEAD)" = "$IMPL_TIP" && test -z "$(git status --porcelain)" && git diff --check 12cf2ea.."$IMPL_TIP" && test -z "$(git diff --name-only 12cf2ea.."$IMPL_TIP" | rg -v '^(plugins/session-relay/(rust/src/.*|rust/tests/.*|rust/Cargo\.(toml|lock)|hooks/.*|test/.*|AGENTS\.md|skills/productivity/session-relay/SKILL\.md)|\.github/workflows/build-binaries\.yml|docs/plans/active/relay-worker-lifecycle-primitives\.md)$')"` | Exit 0 with exact reviewed HEAD and clean status. Negative fixture plants staged, unstaged, and untracked forbidden binary/manifest files; each must fail before range checks. Concurrent main-plan commits are outside range. No binaries/manifests/marketplace/version/release changes. |
...
Adversarial cold-read result: Draft-14 closes the executable gaps found in Draft-13: helper-free WIP bootstrap, shell-first hook command, locked immutable generations with one atomic pointer, non-claiming Codex SessionStart, durable supervisor-owned app-server controller, explicit nonce pipes, typed multi-operation loss batches, production-path binary provenance, doctor schemas, and clean exact-tip completion. No owner decision remains open; immutable independent review is still pending. Implementation stays paused; remote push/dispatch remains a separate owner gate.
...
Score: **94/100 (Draft-14 author candidate; independent re-review pending)** · trajectory **93 Draft-13 author → 84 architecture / 82 acceptance NOT READY → 94 bootstrap/controller/atomic-runtime repair** · stopped: **candidate will be frozen for fresh review; implementation remains paused**.
