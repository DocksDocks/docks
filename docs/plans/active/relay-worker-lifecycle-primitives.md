...
updated: "2026-07-12T03:14:31-03:00"
...
- **Runner live job capability:** on the owner-provisioned Linux job, set nonempty `RELAY_RUNNER_ID` to the owner-recorded diagnostic job/runner identity and `RELAY_RUNNER_RECEIPT` to one new harness-owned absolute temporary JSON path; neither value is authority. Before A1c, start one foreground `RELAY_RUNNER_JOB=1 node plugins/session-relay/test/runner-job-custodian.mjs --serve --runner-id "$RELAY_RUNNER_ID" --runner-receipt "$RELAY_RUNNER_RECEIPT"` process and retain that exact process/session through A7. The driver compiles/hashes its committed host-native helper, creates exactly one `AF_UNIX|SOCK_SEQPACKET|SOCK_CLOEXEC` socketpair, spawns the helper with only the custodian endpoint, retains the private gate endpoint, and generates a public random `capability_id`; both processes must stay live. Its stdin accepts only recursively closed JCS `RunGate {schema:1,gate,argv}` frames in the fixed A1c→A1d→A5→A5b→A6→A7 order and its stdout emits exactly one closed `GateResult {schema:1,gate,sequence,pid,status,ack_sha256,evidence_sha256}` per frame; unknown fields/gates/argv shapes fail and close the session. The driver remains the parent that launches each exact gate invocation serially in the same orchestrator job. For each child it duplicates the retained gate endpoint, clears `CLOEXEC` only on that duplicate, and passes only its numeric fd in `RELAY_RUNNER_CAP_FD`; the driver keeps the original for the next exact gate. The gate harness keeps its duplicate `CLOEXEC` for every runtime/helper child and closes it immediately after its acknowledgement, so Claude, Codex, shell/tool children, logs, argv, environment snapshots, artifacts, and the public receipt never receive a reusable secret or descriptor.
- **Runner capability protocol:** the closed JCS packet sequence is `GateHello {schema:1,capability_id,sequence,gate,gate_pid,receipt_sha256,live_tuple_sha256}` → `GateChallenge {same selectors,challenge:[32 random bytes]}` → `GateResponse {same selectors,challenge_sha256}` → `GateAck {same selectors,ack_sha256}`. The helper enables Linux `SO_PASSCRED` and requires each `GateHello`/`GateResponse` packet's `SCM_CREDENTIALS {pid,uid,gid}` to carry the expected same uid and `pid == gate_pid`; the driver independently records and exact-waits that spawned pid. The custodian knows the only expected gate and next sequence and binds `live_tuple_sha256` to the gate's freshly revalidated boot/kernel/uid/mount/root/runtime tuple. Sequence is exactly monotonic with gate order; the receipt binds only public `capability_id`, while each gate's raw evidence binds its acknowledgement hash and the custodian's running acknowledgement chain. A1c binds its partial receipt hash; A1d first seals and then binds the final receipt hash; A5/A5b/A6/A7 bind that same sealed hash before any authoritative child/RPC/fence byte. A copied receipt plus every environment byte but no inherited connected fd fails before probing. EOF, custodian/gate crash, wrong credentials/pid/gate/capability/hash/tuple, duplicate/stale/out-of-order packet, concurrent gate, or a duplicated endpoint that reuses/skips a sequence irreversibly closes the capability and blocks all later gates; recovery archives the failed evidence, chooses a new receipt path, creates a new custodian/capability, and reruns A1c+A1d. After A7 or failure the driver and custodian close both original endpoints, exact-wait/kill any gate child and the helper, clean partial artifacts/disposable leaves, and record terminal capability evidence. This proves live same-orchestrator-job continuity under `CooperativeWorkerV1`; it is not cryptographic remote attestation against a malicious same-UID process that already controls the job.
- **Runner receipt lifetime:** `runner_receipt_ttl_ms = 7_200_000` from A1c's one `CLOCK_BOOTTIME` start; no phase or retry resets it. ISO timestamps are diagnostic; `{started_boottime_ns,expires_boottime_ns,host_boot_id}` are authoritative for expiry. A1c creates the exact host/runtime portion, A1d completes delegation/child-cleanup and atomically seals the receipt hash, and A5/A5b/A6/A7 must finish before expiry or a new live custodian with a new receipt path reruns A1c+A1d in the current job. Boot change, expiry, different runtime bytes, missing partial receipt, dead capability, or live-tuple mismatch fails. The receipt is integrity evidence only—not a signature, attestation, authentication token, or later-run authority—and is never accepted without the live capability handshake plus field/delegation revalidation.
...
- **Threat-model owner gate:** RESOLVED 2026-07-11 — owner selected Option 1 (`CooperativeWorkerV1`). This resolves the product boundary only; Draft-19 implementation remains paused for fresh immutable dual review, and Step 4 separately requires capability-bound A1c+A1d PASS.
- **Probe evidence contract:** each capability row contains `runtime`, `runtime_version`, `platform`, `argv`, `started_at`, `finished_at`, `exit_status`, base64 or artifact-path `stdout`/`stderr`, artifact SHA-256, parser rule, and derived verdict. The harness rejects a verdict without matching raw evidence, rejects unknown/missing schema fields, verifies its own committed git-blob hash, and emits the raw-record hash chain. There is no editable pass/fail fixture. A delegated-runner receipt additionally uses the recursively closed shape below and JCS SHA-256 over every field except `receipt_sha256`; a matching digest detects accidental byte change only and never proves who produced the receipt.
...
pub struct RunnerGateSelectors {
    schema: u8, // exactly 1
    capability_id: String,
    sequence: u64, // exactly 1..=6 in enum order
    gate: RunnerCapabilityGate,
    gate_pid: u32,
    receipt_sha256: String, // partial for A1c; final sealed hash thereafter
    live_tuple_sha256: String, // fresh boot/kernel/uid/mount/root/runtime tuple
}

pub struct RunnerGateChallenge {
    selectors: RunnerGateSelectors,
    challenge: [u8; 32], // fd-only, one-use, never persisted raw
}

pub struct RunnerGateResponse {
    selectors: RunnerGateSelectors,
    challenge_sha256: String,
}

pub struct RunnerGateAck {
    selectors: RunnerGateSelectors,
    ack_sha256: String, // JCS hash-chain link recorded in gate evidence
}
...
| A1c | The exact retained-fd/TRACEEXEC identity architecture is buildable for each mandatory Linux runtime on the proposed delegated runner, and sequence 1 proves live job continuity. | Submit the live custodian an exact `RunGate` frame for `A1c` whose argv executes `test -n "$RELAY_RUNNER_ID" && test -n "$RELAY_RUNNER_RECEIPT" && RELAY_REAL_RUNTIME_TEST=1 node plugins/session-relay/test/feasibility-probe.mjs --case exec-stop-feasibility --matrix --runner-id "$RELAY_RUNNER_ID" --runner-receipt "$RELAY_RUNNER_RECEIPT" --receipt-phase exec-stop`; the custodian injects only `RELAY_RUNNER_CAP_FD`. | GateResult sequence 1 exits 0 for Claude and Codex native binaries, acknowledges the custodian-spawned pid/partial-receipt hash/live tuple, and writes one two-hour unexpired partial closed receipt containing public `capability_id` but no challenge/secret/fd. Each raw row proves blocked final child, exact host PID handoff, ancestor `PTRACE_SEIZE|PTRACE_O_TRACEEXEC`, retained-fd `execveat`, exact event PID equal to seized tracee, target sentinel absent, followed `/proc/<pid>/exe` magic-link fd fstat/hash equal to retained fd while stopped, then detach and sentinel. `O_NOFOLLOW` yields `ELOOP`; wrong PID/event/hash/early release/timeout kills and reaps before sentinel. Any mandatory row unavailable is a hard STOP before Step 4. Copying the receipt and every environment byte into a non-descendant process with no inherited connected fd fails before the probe; challenge bytes never enter the receipt. |
| A1d | The same live custodian supplies sequence 2 while the owner-provisioned Linux job validates real writable cgroup-v2 delegation and completes the receipt. | Submit the same custodian an exact `RunGate` frame for `A1d` whose argv executes `test -n "$RELAY_CGROUP_ROOT" && test -n "$RELAY_RUNNER_ID" && test -n "$RELAY_RUNNER_RECEIPT" && RELAY_REAL_RUNTIME_TEST=1 node plugins/session-relay/test/runtime-hook-abort.mjs --case delegated-runner-preflight --runner-id "$RELAY_RUNNER_ID" --runner-receipt "$RELAY_RUNNER_RECEIPT" --receipt-phase delegation`; the custodian injects only `RELAY_RUNNER_CAP_FD`. | GateResult sequence 2 exits 0 only after the final sealed receipt hash and A1c capability/runner/boot/kernel/uid/runtime identities match. It pins the root dirfd/mount/dev/inode, validates same-uid writable domain delegation, creates a unique disposable child leaf, validates empty subtree plus writable `cgroup.procs|cgroup.kill|cgroup.events`, moves an exact stopped child, kills it, exact-waits/reaps it with recorded status, then observes `populated 0`, removes the leaf, seals canonical JCS `receipt_sha256`, and receives its exact acknowledgement. Hash is integrity only, not signature/auth. Copy-all-bytes/no-fd, duplicated fd replay, stale/out-of-order/concurrent sequence, EOF, absent/read-only delegation, timeout, zombie/unreaped child, populated mismatch, receipt mismatch/expiry, or cleanup failure is FAIL/BLOCK and closes the capability, never skip. |
...
| A5 | Linux tree proof requires same-tracee fd-bound exec-stop placement, same-fence kill evidence, and live sequence 3. | Submit `RunGate` `A5` with argv `RELAY_REAL_RUNTIME_TEST=1 node plugins/session-relay/test/lifecycle-smoke.mjs --case containment --runner-receipt "$RELAY_RUNNER_RECEIPT"`. | GateResult sequence 3 exits 0 only after the inherited-fd challenge acknowledges the custodian-spawned pid, sealed receipt hash, and freshly revalidated unexpired runner/boot/kernel/uid/mount/root/runtime tuple/current delegation. Production `launch_confined` verifies private host-PID handoff, seized tracee/event PID equality, gated leaf placement, retained-fd exec, exact TRACEEXEC stop, followed procfs magic-link fd fstat/hash equality while stopped, then detach/GO. `O_NOFOLLOW` gets `ELOOP`. Path replacement executes retained original or refuses. Forged/substituted PID, early release, seize/event timeout, wrong event, proc fd/hash mismatch, exec/detach failure, scripts/symlinks, membership mismatch, target sentinel before detach, stale receipt, dead/duplicated capability, or live-field mismatch all kill/reap and fail. Receipt bytes alone never authorize GO. |
| A5b | The owner-provisioned delegated Linux runner drives BOTH real runtimes through the production confined launcher under live sequence 4. | Submit `RunGate` `A5b` with argv `RELAY_REAL_RUNTIME_TEST=1 node plugins/session-relay/test/runtime-hook-abort.mjs --case cgroup-cooperative-child-spawn --matrix --runner-receipt "$RELAY_RUNNER_RECEIPT"`. | GateResult sequence 4 exits 0 only after capability acknowledgement and live revalidation on the same unexpired receipt-bound `RELAY_CGROUP_ROOT` runner from A1d. Harness calls production supervisor/`launch_confined`, captures tracee/event/fd/proc-identity/placement and same-fence kill receipts, creates real child/grandchild/storm under leaf, exact-waits/reaps owned roots, and reaches populated-zero. Target sentinel proves no target instruction before detach. A second worker mints a distinct placement receipt. Cached/fixture/copied evidence cannot authorize GO. Missing/stale/mismatched runner or capability is BLOCK, not skip; Claude filtered hardening stays separate. |
| A6 | Real Linux managed admission and durable controller are runtime-correct and fail closed on custody loss under live sequence 5. | Submit `RunGate` `A6` with argv `RELAY_REAL_RUNTIME_TEST=1 node plugins/session-relay/test/runtime-hook-abort.mjs --matrix --runner-receipt "$RELAY_RUNNER_RECEIPT"`. | GateResult sequence 5 exits 0 only after capability acknowledgement plus live runner-receipt/delegation revalidation. Claude barriers pass. Codex pending creation atomically mints one sealed `ManagedBirthPermit`; only its exact Attaching tuple launches once, and replay/wrong tuple emits zero bytes. SessionStart remains non-claiming; controller alone claims Active; transcript has zero `turn/start` before durable Active. Foreign/refusal rows preserve foreign state. After caller exit a second invocation uses one typed result. Every unsupported/unknown/custody-loss row atomically records logical loss plus one exact `LossCleanupPermit`; after publication that permit alone closes stdio, emits no further bytes, kills live retained custody, and bounded-reaps/observes populated-zero. Replay/stale/cross-loss/no-live-custody emits zero bytes; crash cannot remint; success leaves lifecycle unconfirmed and capacity held. No CLI fallback/bypass. |
...
| A7 | Real app-server fencing distinguishes exact cancellation, shared observation, current ProtocolTree unavailability, and physical kill under final live sequence 6. | Submit `RunGate` `A7` with argv `RELAY_REAL_RUNTIME_TEST=1 node plugins/session-relay/test/runtime-appserver-quiescence.mjs --matrix --runner-receipt "$RELAY_RUNNER_RECEIPT"`. | GateResult sequence 6 exits 0 only after capability acknowledgement plus live runner/delegation revalidation; initialize advertises `experimentalApi=true` before descendant/turn/item/terminal methods and records runtime/method availability; unsupported methods fail typed. Exact interrupt reaches matching terminal; each terminal is paged and terminated only through its sealed same-attempt reference. Shared rows remain observation; current dedicated rows report MissingDurableFlushContract. Physical cgroup kill produces only WorkerTree. Custodian then closes the capability and records the terminal six-link acknowledgement chain; any later/replayed gate fails. |
...
- One live owner-runner custodian does not create and retain the private `AF_UNIX SOCK_SEQPACKET` capability across exact A1c→A1d→A5→A5b→A6→A7 sequences; a gate lacks its inherited fd or leaks it to a runtime/helper; capability/gate/pid/receipt-hash/live-tuple selectors differ; a duplicate/stale/out-of-order/concurrent packet, EOF, crash, or replay does not close the capability; copied receipt/environment bytes pass without the fd; the receipt expires; its hash is treated as signature/auth/GO authority; the probe child is not exact-waited/reaped before populated-zero/removal; or live gate revalidation fails. The current read-only cgroup/no-user-bus sandbox cannot be relabeled positive.
...
4. **Executable acceptance:** present — A0/A0b-bootstrap/A0b–A18 plus A1b/A1c/A1d/A3a–A3d/A6b/A6c are command + expected evidence, including capability-bound TRACEEXEC/delegation receipt, copy-all-bytes/no-fd and sequence negatives, exact child reap, live runner revalidation, post-loss cleanup authority/replay/no-release, ordinary hook pruning plus honest standalone-authenticity boundary and docks-kit verified-native path, per-step nonzero inventory birth, controller A9/A10 rerun, post-installer source-fd and schema determinism races, live cache upgrade, later-process controller wake, Darwin unavailable rows, and exact historical-plus-future actual-range manifests/hashes.
...
Adversarial cold-read result: Draft-19 preserves every accepted Draft-18 repair while removing its two false guarantees. Standalone `/bin/sh` bootstrap now owns ordinary-prune durability and observed corruption checks only; deliberate same-UID pre-exec replace-all is explicitly out of cooperative scope, post-start native installer pinning is exact, and docks-kit retains an optional externally verified-native path. Runner continuity is no longer inferred from copyable JSON/environment: one live custodian, inherited `SOCK_SEQPACKET` fd, closed challenge/ack selectors, strict six-gate sequence, descriptor non-leakage, and copy/replay/crash negatives make it executable. LossCleanupPermit, ManagedBirthPermit, controller variants/inventory ownership, exact reap/live delegation, followed proc magic-link TRACEEXEC, fd-pinned installer/schema, historical-plus-future A18 union, same-version tuple, and Linux/Darwin split remain intact. No cold executor must guess the hook or runner threat boundary. Implementation remains paused for fresh immutable dual re-review; Step 4 and A5/A5b/A6/A7 remain blocked until the owner supplies the live capability-bound runner. Remote push/dispatch and packaged producer/release gates remain owner approvals.
...
Score: **96/100 (Draft-19 author candidate; fresh immutable dual re-review pending)** · trajectory **96 Draft-18 author → 90 acceptance NOT READY → 96 honest-hook/live-job-capability repair** · stopped: **candidate frozen for review; implementation paused; real Linux gates require one owner-provisioned live custodian through A1c/A1d/A5/A5b/A6/A7**.
