...
updated: "2026-07-12T16:13:35-03:00"
...
| A138 | A18a | Through one pending-aware entrypoint, bind the immutable Step-8 rehash and A137 snapshot+prefix to the same runner/step/event chains, then crash-durably clean the exact runner root and emit terminal A138 plus the final 38-row summary. | `node plugins/session-relay/test/final-scope.mjs --a138-crash-matrix --acceptance-prefix-snapshot "$FINAL_SCOPE_ROOT/acceptance-prefix-snapshot.json" --acceptance-prefix "$FINAL_SCOPE_ROOT/acceptance-prefix.json" --root "$FINAL_SCOPE_ROOT/a138-crash-matrix" --step-order 1b,3c,3d,1c,1d,4,5,6,7,8,9 && node plugins/session-relay/test/final-scope.mjs --run-a138 --tip "$IMPL_TIP" --plan-blob "$PLAN_BLOB" --runner-state "$RUNNER_STATE_RECEIPT" --runner-receipt-root "$RUNNER_RECEIPT_ROOT" --receipts "$STEP_RECEIPT_DIR" --acceptance-events "$RUNNER_RECEIPT_ROOT/acceptance-events" --acceptance-prefix-snapshot "$FINAL_SCOPE_ROOT/acceptance-prefix-snapshot.json" --acceptance-prefix "$FINAL_SCOPE_ROOT/acceptance-prefix.json" --step8-rehash "$STEP8_REHASH_RECEIPT" --terminal-events "$FINAL_SCOPE_ROOT/acceptance-events" --pending "$FINAL_SCOPE_ROOT/final-acceptance-pending.json" --emit-acceptance-summary "$FINAL_SCOPE_ROOT/acceptance-summary.json" --step-order 1b,3c,3d,1c,1d,4,5,6,7,8,9` | Exit 0 first with `A138_CRASH_MATRIX PASS barriers=after-cleanup,after-event-82`, then on a fresh production branch with `STEP8_REHASH_SCOPE PASS`, `STEP8_REHASH_SCOPE_NEGATIVE_MATRIX PASS`, `SOURCE_READY_DERIVATION_MATRIX PASS`, `RUNNER_RECEIPT_CLEANUP PASS cleanup_sha256=<hex>`, and `ACCEPTANCE_SUMMARY PASS events=82 criteria=38 receipt_sha256=<hex>`; an exact retry after either injected production-equivalent barrier instead prints `A138_RESUME PASS` plus the same event/summary hashes. `--run-a138` checks the exact pending file before any runner-root path, `test -f`, open, or verifier. With no pending record it validates the live attempt chain, six-gate order, terminal cleanup, bootstrap/prior receipt manifests, full step ledger, events 1–81, snapshot+prefix, Step-8 rehash, substitution negatives, and source-ready matrix, then fsyncs pending with every result digest before cleanup. With matching pending it skips all deleted-root work and resumes using only snapshot, prefix, pending, terminal-event state, and summary path. The crash matrix uses helper-created disposable roots and invokes this same entrypoint after deletion and after event 82, proving byte-identical recovery without reopening a root, rerunning a gate, or cleaning twice. Wrong/missing pending, pending plus mismatched surviving bytes, any resume-branch access to a runner path, snapshot/prefix substitution, wrong terminal event, or summary emitted before cleanup evidence fails. |
...
- **A137/A138:** final scope validates the rehash receipt against the same immutable complete A110→A111→A119→A120→A121→A125 RunGate attempt chain, full step ledger, empty Step-8 A106 receipt, and events 1–80. A137 appends event 81 and seals a byte-complete surviving snapshot plus digest-bound prefix only after every check. A138's one pending-aware entrypoint checks pending before any runner-root access; its fresh branch validates and fsyncs all result digests, while its resume branch uses only surviving state. The synthetic barriers prove retry through that exact entrypoint after cleanup and after event 82 before summary. No deleted path blocks the resume branch, no deleted event byte is reconstructed from a hash, no completed capability is replayed, and no repeated criterion is collapsed to its latest result.
...
Score: **98/100 (Draft-31 final candidate; current schema-v1 review pending)** · trajectory **98 Draft-30 author → 87 X formal NOT READY on unreachable retry → 98 Draft-31 pending-aware A138 entrypoint** · stopped: **formal blocker repair only; implementation resumes immediately on eligible READY; owner-provisioned Linux runner remains the later canonical A111 gate**.
