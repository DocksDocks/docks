...
export LEGACY_POLICY_RED_RECEIPT
...
export SOURCE_PROOF_SHA256 PUBLICATION_SHA256 PUBLIC_REQUEST_SHA256 PUBLIC_RELEASE_SHA256 PROMOTION_SHA256 FINALIZATION_SHA256 PUBLICATION_IMPLEMENTATION_COMMIT EXPECTED_ORIGIN_MAIN
```
...
This two-fence contract makes the already-reviewed schema/Docks-release tail immutable while permitting only this amendment's four-path test-first archive lookup and binder correction plus manager-owned plan records. Reject a missing or extra path, rename, stale/non-ancestor current head, dirty tree, mismatched constant, changed finished-plan bytes, changed source/evidence/shipped relationship, or proof whose `promoted_commit` differs from current HEAD. Plan-only lifecycle commits after the one final full gate are allowed only while all three amended implementation/test blobs are byte-identical to the gated implementation tree.
...
After this plan passes fresh changed-input review, first commit only a failing contract in `plugins/session-relay/test/release-evidence-contract.mjs` and capture `$LEGACY_POLICY_RED_RECEIPT`. The frozen focused test must consume the actual immutable archived completion receipt and state, assert every exact pin above, reject an independent mutation of each pin plus missing/duplicate settled state, prove ordinary current `fallback:"none"` validation is byte-for-byte unchanged, and prove A10 reaches no companion-policy validator. Only after that intended red may production edit `scripts/lib/session-relay-release-preparation.mjs`; the test then stays frozen. Run the focused evidence, detached companion, publication, promotion, distribution, and content-hash checks, one new final full Docks gate, and independent final code review. Reseal and ancestry-preserving-push a new final `PUBLICATION_IMPLEMENTATION_COMMIT`, then retry A10-A11.

Historical A8/A9 evidence remains valid for implementation commit `c27955df150bcdde6fe651f6e10f3fbc6feb9c37`, but it is superseded for the new final implementation tree and cannot be reused as its gate, review, seal, or push evidence. The exact four authorized-base paths and exact 35 shipped-to-promoted paths remain unchanged.
...
4. Run the exact focused test command and the full public gate, prove the implementation diff is only the three production files (`package.json`, `SoT/toolchain.json`, `cli/src/generated/sotPayload.ts`) plus the child plan, and completion-review before any tag.
...
| 4 | Obtain one fresh changed-input schema-6 review; then update the companion contract and fence fixtures in a clean test-only commit, capture the second intended red, extend the binder to the exact 35-path/four-path closure, run focused/companion/hash checks and one final Docks full gate, seal the exact amended implementation commit, independently review it through the normal branch/PR path, and push without force so Docks `origin/main` equals it. | `plugins/session-relay/test/companion-distribution-contract.mjs`; `plugins/session-relay/test/release-evidence-contract.mjs`; `scripts/lib/session-relay-release-preparation.mjs`; this plan for manager-owned review/status/evidence | 3 | done | Fresh schema-6 draft review passes before implementation; A1b then fails only because production still enforces the prior 34-path/three-path closure; A2-A9 pass after the binder update; implementation/test blobs do not change after A8; `PUBLICATION_IMPLEMENTATION_COMMIT` is clean, descends from both `cdca867...` and `25592c6...`, has exactly the four-path diff from the authorized base and exact 35-path diff from the shipped archive, and local/remote `origin/main` both equal it. Any review rejection, wrong red failure, extra path, force requirement, or main drift is STOP. |
| 4a | After this compatibility amendment passes fresh changed-input schema-6 draft review, freeze one test-only exact-artifact contract and capture the intended red before any new production edit. | `plugins/session-relay/test/release-evidence-contract.mjs`; `$LEGACY_POLICY_RED_RECEIPT`; this plan for manager-owned review/status/evidence | 4 | planned | A9a succeeds only by capturing the focused test's intended nonzero result against the actual immutable archived completion receipt/state. The frozen test asserts every path/candidate/receipt/policy/reviewed-head/input/series/state pin, rejects each independent pin mutation plus absent/duplicate state, proves current fallback-none validation is unchanged, and proves A10 reaches no companion-policy validator. Setup, parse, timeout, unrelated failure, or an already-passing test is STOP. |
| 4b | With the test frozen, add only the binder-local exact-artifact compatibility branch, run all new focused/final evidence, independently review the final code, reseal the new implementation commit, and push without force before retrying proof binding. | `scripts/lib/session-relay-release-preparation.mjs`; this plan for manager-owned evidence only | 4a | planned | A9b-A9d pass; no production path except the preparation binder changes, current schema-6 fallback-none validation remains global and unchanged, the exact 35-path/four-path fences still hold, the new final full gate and independent review bind the same implementation tree, and `PUBLICATION_IMPLEMENTATION_COMMIT == origin/main` at a new descendant of `c27955df150bcdde6fe651f6e10f3fbc6feb9c37`. The test blob stays frozen after A9a. Any unpinned legacy acceptance, need to edit `review-policy.mjs`, review non-pass, extra path, force requirement, or identity drift is STOP. |
| 5 | Bind the completed recertification proof with corrected tooling and validate canonical bytes/hash/semantics offline. | `$SOURCE_PROOF` only; no tracked file | 4b | planned | A10-A11 pass; proof derives the passed completion evidence commit, retains source/tag `3fb9211...` and shipped archive, records `promoted_commit == PUBLICATION_IMPLEMENTATION_COMMIT == origin/main`, and binds the exact public companion. No remote mutation occurs. |
...
| 8 | Execute the public child plan: bump the exact public package version, update only production Session Relay pins/payload, preserve frozen tests, run focused/full gates, completion-review, create the exact tag once, verify the one workflow/six-asset stable release/npm result, and archive the child. | `/home/vagrant/projects/public/package.json`; `/home/vagrant/projects/public/SoT/toolchain.json`; `/home/vagrant/projects/public/cli/src/generated/sotPayload.ts`; child plan lifecycle/archive path | 7 | planned | A15-A18 pass in the public repo; tag `cli-v0.10.2` points to the completion-reviewed descendant of `6c07f9...`; exactly one successful release workflow and closed six-asset Release verify; npm result is canonical; child plan archives with passed completion receipt and exact handoff values. |
...
| 12 | Record immutable receipt/evidence summaries, run Docks completion review, and archive this plan. | This plan only for manager-owned evidence, review, status, and dated active-to-finished lifecycle move | 11 | planned | Completion inventory A1-A24 plus A9a-A9d is complete, child plan is already finished, no implementation/test blob changed after A9c, completion review passes, and plan-manager archives this plan under the unique date-prefixed finished path. Non-pass is STOP; no tag/release retry follows. |

## Acceptance criteria

Run in order only after the owning step starts. A1 and A1b preserve the two prior test-first histories; A2-A9 preserve the focused/final evidence that was valid for `c27955df150bcdde6fe651f6e10f3fbc6feb9c37`. That prior evidence is now superseded, not rewritten: after this exact compatibility amendment passes fresh changed-input review, A9a captures one new exact-artifact red, A9b runs the amended focused checks, A9c runs one new final full gate, and A9d binds the independently reviewed and pushed final implementation. Commands that mutate external release state occur only in their authorized steps, never during plan amendment or draft review.
...
| A9a | `test -z "$(git status --porcelain=v1 --untracked-files=all)" && LEGACY_POLICY_RED_COMMIT="$(git rev-parse HEAD)" && node scripts/capture-tdd-red.mjs --repo "$PWD" --repository-id DocksDocks/docks --pre-production-commit "$LEGACY_POLICY_RED_COMMIT" --test plugins/session-relay/test/release-evidence-contract.mjs --receipt-out "$LEGACY_POLICY_RED_RECEIPT" -- node plugins/session-relay/test/release-evidence-contract.mjs` | Exit 0 only from an exact clean test-only commit over the intended nonzero binder-contract failure. The frozen test uses the real archived completion receipt/state, asserts every exact compatibility pin, rejects each pin mutation and missing/duplicate state, proves ordinary current fallback-none validation is unchanged, and proves A10 invokes no companion-policy validator. |
| A9b | `node plugins/session-relay/test/release-evidence-contract.mjs && node plugins/session-relay/test/companion-distribution-contract.mjs --public-remote https://github.com/DocksDocks/public.git --public-ref refs/heads/preflight/session-relay-cli-0.13.0-6c07f9bc02ef --public-commit 6c07f9bc02ef7a0a26b8ffb539c16c42a87a3172 --detached-clone && node plugins/session-relay/test/release-publication-contract.mjs && node plugins/session-relay/test/release-promotion-contract.mjs && node plugins/session-relay/test/distribution-contract.mjs && node scripts/skills/content-hash.mjs --check-only plugins/session-relay/skills` | Exit 0 with the A9a test blob unchanged; exact-artifact binder compatibility is green, companion/publication/promotion/distribution/hash contracts remain green, current fallback-none behavior is unchanged, and no other legacy policy artifact is accepted. |
| A9c | `node scripts/ci.mjs` | Exit 0 exactly once on the new final implementation tree after A9b; this supersedes the otherwise-valid A8 result for `c27955df150bcdde6fe651f6e10f3fbc6feb9c37`, and no implementation/test blob changes afterward. |
...
| A10 | `node scripts/release.mjs --bind-completion --plugin session-relay 0.13.0 --finished-plan docs/plans/finished/2026-07-23-session-relay-linux-workspace-recertification.md --embedded-candidate-sha256 75e5bf5386a203cd81e3930ca2309ceed4e1a665d995848a29eb73a0fa5cb395 --receipt-out "$SOURCE_PROOF"` | Exit 0 and prints the canonical source-proof digest only when the exact immutable completion receipt raw SHA `d929ab...`, historical policy SHA `bb95e1...`, reviewed head `762f2a...`, input `527539...`, series `cd8ec18d...`, and unique settled state SHA `064e08...` all validate through the binder-local exception; exact source/evidence/shipped/promoted ancestry, unchanged path fences, clean HEAD, candidate, public tuple, and `origin/main` identity also bind. No companion receipt or unpinned legacy policy validator is reached. |
| A11 | `test "$SOURCE_PROOF_SHA256" = "$(sha256sum "$SOURCE_PROOF" | cut -d' ' -f1)" && node --input-type=module -e 'import fs from "node:fs"; import assert from "node:assert/strict"; import {validateSourcePreparationProof} from "./scripts/lib/session-relay-release-preparation.mjs"; const bytes=fs.readFileSync(process.env.SOURCE_PROOF); assert.equal(bytes.at(-1),125); const value=JSON.parse(bytes); validateSourcePreparationProof(value); assert.equal(value.source_commit,"3fb9211f3309977f24853a10714d4b7a82b38c8f"); assert.equal(value.tag_commit,value.source_commit); assert.equal(value.promoted_commit,process.env.PUBLICATION_IMPLEMENTATION_COMMIT); assert.equal(value.public_reviewed_commit,"6c07f9bc02ef7a0a26b8ffb539c16c42a87a3172");'` | Exit 0; the captured digest equals the raw receipt hash, the closed proof validates offline with no trailing newline, and distinct shipped/promoted semantics remain producer-bound. |
...
| A16 | `cd /home/vagrant/projects/public && git diff --exit-code 6c07f9bc02ef7a0a26b8ffb539c16c42a87a3172 -- cli/test/unit/sessionRelayCli.test.ts cli/test/unit/pluginRefresh.test.ts && test "$(node -p 'require("./package.json").version')" = 0.10.2` | Exit 0; frozen tests are byte-identical to the companion and the package version is exactly `0.10.2`, proving the authorized bump landed. Structural child-plan acceptance proves only the exact `package.json` version value, Session Relay production fields, and generated payload changed outside the child plan. |
...
| A20 | `node scripts/release.mjs --promote-reviewed --plugin session-relay 0.13.0 --source-proof "$SOURCE_PROOF" --source-proof-sha256 "$SOURCE_PROOF_SHA256" --publication "$PUBLICATION_RECEIPT" --publication-sha256 "$PUBLICATION_SHA256" --public-release "$PUBLIC_RELEASE_RECEIPT" --public-release-sha256 "$PUBLIC_RELEASE_SHA256" --docks-kit-release cli-v0.10.2 --expected-origin-main "$EXPECTED_ORIGIN_MAIN" --receipt-out "$PROMOTION_RECEIPT"` | Exit 0 under the initial or one exact legal recovery branch; terminal receipt records success, expected/observed/pushed Docks main is the publication implementation commit, and exact-source/live smokes pass. |
...
- The active publication plan is one amendment path and remains part of both exact path fences. Manager-owned review/status/evidence commits may advance its blob without changing implementation bytes; rerun the final full gate if any of the three amended implementation/test blobs changes.
...
- Any implementation/test change after the final full gate, current-main base ancestry failure, exact 35-path or four-path fence mismatch, immutable tuple change, preexisting identity conflict, wrong/nonunique workflow or asset set, noncanonical receipt, failed focused/full/live gate, or review non-pass is STOP.
...
- STOP if the exact compatibility amendment's fresh changed-input schema-6 draft review does not pass before A9a, or if implementation begins while this plan remains blocked.
- STOP if A9a does not freeze a test-only intended failure against the actual archived completion receipt/state, if that test changes after capture, or if A9b-A9d do not bind the same new final implementation tree.
...
- STOP if either red test blob changes after its capture, if authorized base `25592c6...` is not an ancestor of the final implementation commit, or if the shipped-to-promoted and base-to-promoted changed-path sets differ from their exact 35-path and four-path contracts.
...
- **File manifest:** the six exact Docks changed paths, three exact public production paths (`package.json`, `SoT/toolchain.json`, `cli/src/generated/sotPayload.ts`), exact child plan, and manager-owned lifecycle/archive path rules are enumerated.
...
- `/home/vagrant/projects/public/package.json`, `SoT/toolchain.json`, `cli/src/generated/sotPayload.ts`, and `.github/workflows/release-cli.yml` — observed public `0.10.1` package identity to be bumped exactly to `0.10.2`, current `0.12.0` production pin, generated payload, tag-triggered build/release/npm behavior, and six assets.
...
Review-orchestration-state: {"apply_state":"none","current_input_sha256":"13c5b9658788f6693c43cf49bbe27df4adb1c9845d018f2979d228c267d67654","initial_input_sha256":"13c5b9658788f6693c43cf49bbe27df4adb1c9845d018f2979d228c267d67654","lifecycle_intent":"none","orchestration_attempt":1,"phase":"draft","plan_path":"docs/plans/active/session-relay-linux-workspace-publication.md","request_ids":["d90f982a-ab39-4cb8-a49a-a53287b9deab"],"retry_authorization":null,"round_index":1,"schema":2,"series_id":"2956fdd9-da0a-40cf-9fe1-528ea1efa7aa","series_sha256":null,"state_sha256":"5a91afb5519d6bf6fa7453d15ac2bd1ad6ca3797826df2337c885399f8c5f4da","status":"active","stop_reason":null,"terminal_evidence_sha256":null,"terminated_from_state":null,"terminated_from_state_sha256":null,"transitioned_from_state_sha256":null}
Review-orchestration-prepared-request: {"lifecycle_intent":"none","orchestration_series_id":"2956fdd9-da0a-40cf-9fe1-528ea1efa7aa","orchestration_state_sha256":"5a91afb5519d6bf6fa7453d15ac2bd1ad6ca3797826df2337c885399f8c5f4da","phase":"draft","plan_path":"docs/plans/active/session-relay-linux-workspace-publication.md","prepared_at":"2026-07-23T21:52:05-03:00","request":{"acceptance_inventory_sha256":null,"author":{"company":"openai","effort":"high","model":"gpt-5.6-sol","tool":"codex"},"bundle_sha256":"77a389038516d099fd75cddf57d29cab9b3f7a6f8f1633bffa6fe0c90241ad07","diff_sha256":null,"execution_base_commit":null,"input_sha256":"13c5b9658788f6693c43cf49bbe27df4adb1c9845d018f2979d228c267d67654","lifecycle_intent":"none","orchestration_series_id":"2956fdd9-da0a-40cf-9fe1-528ea1efa7aa","orchestration_state_sha256":"5a91afb5519d6bf6fa7453d15ac2bd1ad6ca3797826df2337c885399f8c5f4da","phase":"draft","planned_at_commit":null,"policy":{"candidates":[{"company":"openai","effort":"high","model":"gpt-5.6-sol","service_tier":"default","tool":"codex"}],"fallback":"none","max_rounds":2,"provenance":{"candidates":"runtime_global","fallback":"skill_default","max_rounds":"skill_default","role":"skill_default"},"role":"primary","schema":6},"policy_sha256":"4317663e576334beacd0f174120385311e3f9d36f86101ee888437eab438800f","previous_input_sha256":null,"repair_targets_sha256":null,"request_id":"d90f982a-ab39-4cb8-a49a-a53287b9deab","review_mode":"full","reviewed_commit_or_head":"8c4b5ca1c3c33737edfabed9bed3b01bc98df2b5","round_index":1,"schema":6},"request_ids":["d90f982a-ab39-4cb8-a49a-a53287b9deab"],"request_sha256":"fd202206fd9e8efbfdcc2a68fa5a139fbf0788e4d7592358bc8cb315f1a89210","schema":1,"type":"ReviewPreparedRequestV1"}
Review-orchestration-dispatch-commitment: {"argv":["exec","-C","/tmp/docks-plan-review-run/d90f982a-ab39-4cb8-a49a-a53287b9deab-primary","--skip-git-repo-check","--ephemeral","--ignore-user-config","--add-dir","/tmp/docks-plan-review/d90f982a-ab39-4cb8-a49a-a53287b9deab-primary","-s","read-only","-m","gpt-5.6-sol","-c","model_reasoning_effort=high","-c","service_tier=\"default\"","--output-schema","/tmp/docks-plan-review/d90f982a-ab39-4cb8-a49a-a53287b9deab-primary/reviewer-output.primary.v6.schema.json","--","You are the single primary plan reviewer. Read only the sealed bundle and return typed evidence. Sealed bundle: /tmp/docks-plan-review/d90f982a-ab39-4cb8-a49a-a53287b9deab-primary. Copy the request object exactly into ReviewerOutput.request.\nEvaluate exactly these criteria: standalone_executability, actionability, dependency_order, evidence_reverification, goal_coverage, executable_acceptance, failure_modes, open_questions. Each criterion needs pass, non_blocking_gap, or blocking_gap plus nonempty evidence. The verdict equals the strongest status. Every gap needs a matching finding; pass has no findings.\nA blocking finding must name the exact user requirement, safety property, or execution step that would fail. Do not emit a numeric score or rubric.\nFor request.phase === \"draft\", blocking_gap is eligible only when implementation cannot safely and correctly start because of an unresolved required user decision, contradictory goal/scope/interface, unsafe or unauthorized action, impossible dependency order, missing first executable step, or absent/non-executable acceptance contract.\nCode style, optional refactors/docs, speculative performance, exhaustive implementation edge cases, exact internal symbol choices, and defects best established by running the implementation are non_blocking_gap with rejection/defer reason defer_to_implementation_verification.\nA complete simple plan may return pass; there is no finding quota and no instruction to improve until perfect.\nThis is the full round-one review.\nREQUEST_JCS_BEGIN\n{\"acceptance_inventory_sha256\":null,\"author\":{\"company\":\"openai\",\"effort\":\"high\",\"model\":\"gpt-5.6-sol\",\"tool\":\"codex\"},\"bundle_sha256\":\"77a389038516d099fd75cddf57d29cab9b3f7a6f8f1633bffa6fe0c90241ad07\",\"diff_sha256\":null,\"execution_base_commit\":null,\"input_sha256\":\"13c5b9658788f6693c43cf49bbe27df4adb1c9845d018f2979d228c267d67654\",\"lifecycle_intent\":\"none\",\"orchestration_series_id\":\"2956fdd9-da0a-40cf-9fe1-528ea1efa7aa\",\"orchestration_state_sha256\":\"5a91afb5519d6bf6fa7453d15ac2bd1ad6ca3797826df2337c885399f8c5f4da\",\"phase\":\"draft\",\"planned_at_commit\":null,\"policy\":{\"candidates\":[{\"company\":\"openai\",\"effort\":\"high\",\"model\":\"gpt-5.6-sol\",\"service_tier\":\"default\",\"tool\":\"codex\"}],\"fallback\":\"none\",\"max_rounds\":2,\"provenance\":{\"candidates\":\"runtime_global\",\"fallback\":\"skill_default\",\"max_rounds\":\"skill_default\",\"role\":\"skill_default\"},\"role\":\"primary\",\"schema\":6},\"policy_sha256\":\"4317663e576334beacd0f174120385311e3f9d36f86101ee888437eab438800f\",\"previous_input_sha256\":null,\"repair_targets_sha256\":null,\"request_id\":\"d90f982a-ab39-4cb8-a49a-a53287b9deab\",\"review_mode\":\"full\",\"reviewed_commit_or_head\":\"8c4b5ca1c3c33737edfabed9bed3b01bc98df2b5\",\"round_index\":1,\"schema\":6}\nREQUEST_JCS_END"],"argv_sha256":"b1b4e575f0f04e4798510c48c7388e0c49164ecb7ae05cb63385cc4cd69f57d0","bundle_path":"/tmp/docks-plan-review/d90f982a-ab39-4cb8-a49a-a53287b9deab-primary","bundle_sha256":"77a389038516d099fd75cddf57d29cab9b3f7a6f8f1633bffa6fe0c90241ad07","candidate":{"company":"openai","effort":"high","model":"gpt-5.6-sol","service_tier":"default","tool":"codex"},"candidate_index":0,"committed_at":"2026-07-23T21:53:24-03:00","controller_config":{"timeout_mode":"orchestrator_tool","timeout_seconds":600},"orchestration_state_sha256":"5a91afb5519d6bf6fa7453d15ac2bd1ad6ca3797826df2337c885399f8c5f4da","plan_path":"docs/plans/active/session-relay-linux-workspace-publication.md","prepared_request_sha256":"e8b4a447a97b73f0fe7f827340ba23c54c83fd2736133702fc28e4f0edbf7e8a","prior_attempts":[],"prior_attempts_sha256":"4f53cda18c2baa0c0354bb5f9a3ecbe5ed12ab4d8e11ba873c2f11161202b945","reviewer_workspace":{"cleanup_token":"5b75ca933a447a3417b73cd045eaa5978be4fe185ec55f71497f595db38da136","leg":"primary","request_id":"d90f982a-ab39-4cb8-a49a-a53287b9deab","schema":1,"workspace":"/tmp/docks-plan-review-run/d90f982a-ab39-4cb8-a49a-a53287b9deab-primary"},"reviewer_workspace_sha256":"708a3463a377443d999fe3ef3ac940e93779d6f81dfe825eea8f207e84bdce23","schema":1,"type":"ReviewDispatchCommitmentV1"}
## Review
