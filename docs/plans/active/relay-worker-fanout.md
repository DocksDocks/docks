...
**Status: NOT dispatch-ready — draft-3 required (see below).** Two cross-check rounds:
Cross-check (2026-07-10): [codex gpt-5.6-sol xhigh] draft-1 review, 62/100, ~10 findings (3 picks + 1 blocking missed fork + 4 blockers + 3 scope calls) — all accepted; [claude opus] independently verified the `last_seen`-drop (CONFIRMED vs store.rs:482) and the pre-existing `spawned_via` field. User settled cap=2 and depth=1 via picker. Draft rewritten 5→7 steps.
Cross-check (2026-07-11): [codex gpt-5.6-sol xhigh] draft-2 confirm-pass, **70/100 — NOT READY**. All 7 draft-1 blockers PASS at design-intent, but 9 fresh **implementation-contract** findings (executable-acceptance sub-score 4/12 the weakest) — all accepted; [claude opus] independently verified #1 (codex has no premint — spawn.rs), #6 (root-parent shared-checkout hazard), #7 (O_NOFOLLOW helpers are flat-file not recursive), #9 (plan stale post-0.10.0) against source. Full verdict in the fanout-reviewer bus mail (2026-07-11T04:08Z).

## Draft-3 required changes (from the confirm-pass — all must be closed before dispatch)

1. **Reservation identity vs birth (deepest).** Reserve keyed by a **reservation token** decoupled from the session UUID (which doesn't exist pre-birth on either path — codex has no premint; app-server UUID comes from `thread/start`). Pass the token into the child env/hook + app-server pump config; atomically **attach/rekey** it to the born UUID; define rollback for every failure between reserve → worktree → launch → thread/start → registration → turn/start.
2. **Trusted derivation, not caller-supplied.** Derive `parent` from the invoking session identity, `root`+`depth` from stored ancestry; persist `depth` (typed) on Entry; validate parent exists/runs; make the cap root-owned/immutable for descendants (or drop the per-invocation override). Adversarial tests for forged root/parent/depth.
3. **Lifecycle state machine + durable lease.** Define `reservation_id`, `lease_expires_at`, actor, a validated transition table (reserved→running|failed, running→completed|failed, →collected) with expected-state CAS under the store lock + expiry reaping; reconcile "lease reclaim" vs "crash→failed"; map `conflict`; define old-Entry `lifecycle=None`.
4. **Detached supervisor for both paths.** Ordinary spawn drops the child handle after birth; the app-server pump only exits. Fan-out needs a supervisor that owns the reservation through birth, records `running`, observes first-turn exit/timeout, transitions `failed` on no valid handback — without blocking the parent.
5. **Machine-readable handback/collect.** Bus mail is opaque. Add a `relay handback` path writing an atomic store record ({base_sha,head_sha,branch,status,note}) + the human note; `collect` consumes it. Specify base-SHA capture, head verification, integration command/range, stale-head checks, conflict abort, lifecycle update, idempotent retry.
6. **Root-parent isolation.** `--fanout` only changes the prompt — the orchestrating parent still integrates in its shared cwd (the shared-checkout hazard). `--fanout` must require/imply a worktree-isolated root parent (or prove exclusive checkout ownership); initialize the root Entry explicitly.
7. **Worktree/repo/GC precision.** Capture base SHA before `git worktree add … <base_sha>`; compare git **common-dir identity**, not path strings, for same-repo; branch name uses the reservation token (UUID unknown pre-birth); executable "merged" ancestor check + named branch-deletion flag; **do NOT** use the hygiene flat-file `O_NOFOLLOW` unlinkat to delete recursive worktrees — use validated `git worktree remove` with fail-closed preflight; define missing/moved repo + parent-collection-with-active-descendants.
8. **Executable acceptance.** Replace the prose coverage lists with command + expected output per criterion; `cargo test --manifest-path plugins/session-relay/rust/Cargo.toml` (NOT bare `cargo test` — no root Cargo.toml); a reproducible sandbox smoke script; `git diff --exit-code <base>..HEAD -- plugins/session-relay/bin` (expect empty/0).
9. **De-stale.** live-view shipped in **0.10.0** (main HEAD `7e4875a`; ship_commit `e51ed8f`) — rewrite the dependency/base facts (planned_at_commit is stale at `37039d58`; ~1,101 lines changed since), remove "live-view is building / release likely 0.10.0" speculation, refresh drifted anchors (Entry now store.rs:485, build_prompt spawn.rs:258).
