...
| 3 | Port the structural guards: `transform-guard`, `no-author-scripts`, `codex-facts`, `agents/guard`, `tree/guard`, `scaffold/guard-spec`, `read-floor`. `codex.sh`/`claude.sh` already call `validate-skills.mjs` — fold or keep per OQ-3. | 1 | done |
| 4 | Port the orchestrators (D-1): `ci.sh`→`ci.mjs` and `release.sh`→`release.mjs` (release is behavioral-parity, highest care — dry-run it); rewire `ci.yml` + `guard.sh` to call the `.mjs` validators. | 2,3 | done |
| 5 | **Single-source collapse (D-2):** merge the skill frontmatter guard (`validate-skills.mjs` + `codex.sh`/`claude.sh`) + the scorer (`score.sh`) + the bash mirror (`skill-guard.sh`) into ONE bundled `write-skill/scripts/skill-guard.mjs`; point the repo CI at that shipped file over `plugins/docks/skills`; delete the duplicates; bump write-skill `metadata.updated` (bundled `scripts/` aren't content-hashed). | 2,3 | done |
| 6 | Retire the ported `.sh` (delete or thin-shim per OQ-3); shrink the shellcheck glob to the bash that remains (`ci.sh`, `release.sh`, `lib/skills.sh`, idempotency test) or drop the gate if none remain. | 4,5 | done |
| 7 | Update `docs/scaffold/spec.yaml` (seeds the validators), `scripts/AGENTS.md`, `.github/AGENTS.md`. | 4 | done |
| 8 | Validate: every parity test green, `bash scripts/ci.sh` green, scaffold seed still starts green, no skill/agent score or hash changed. Commit + push. | 6,7 | done |
...
- **D-1 (OQ-1) — port everything**, including `ci.sh` → `ci.mjs` and `release.sh` → `release.mjs`. Note: a validator's "parity" is an output diff; `release.sh` has no deterministic output, so its parity is **behavioral** (still bumps versions in lockstep, tags `docks--v*`, pushes, gates the GH Release on tag-CI) — verify by dry-run, and keep a `release` npm-script entrypoint. Treat `release.mjs` as the highest-care port (touches git tags + GH releases).
  - **Resolved (2026-06-14, on the user's go-ahead): `release.sh` → `release.mjs`.** Initially deferred (a git-tag / GH-release orchestrator is untestable without an *actual* release), it's now ported with a **`--dry-run`** mode that runs the gate + version compute + manifest bump read-only and PRINTS the destructive steps. `node scripts/release.mjs --dry-run patch` verified: gate green, `0.5.9→0.5.10`, and each manifest changes **only** the `"version"` line (native `JSON.stringify` matches the committed jq formatting — no reformatting churn). The seed's `release.sh.template` → `release.mjs.template` too. **The repo now has ZERO author-side bash** — the only `.sh` left is the shipped runtime `context-tree-nudge` hook (correctly bash). The real tag/release path still wants a smoke-test on the next actual release.
