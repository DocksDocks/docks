...
- **D-1 (OQ-1) — port everything**, including `ci.sh` → `ci.mjs` and `release.sh` → `release.mjs`. Note: a validator's "parity" is an output diff; `release.sh` has no deterministic output, so its parity is **behavioral** (still bumps versions in lockstep, tags `docks--v*`, pushes, gates the GH Release on tag-CI) — verify by dry-run, and keep a `release` npm-script entrypoint. Treat `release.mjs` as the highest-care port (touches git tags + GH releases).
  - **Deviation (2026-06-14, recorded honestly): `release.sh` was kept as bash** (its CI-gate call updated to `node scripts/ci.mjs`). Porting a git-tag / GH-release orchestrator blind, unsupervised, and untestable without an *actual* release was the one move with real blast radius and no safe verification — so it's the single deferred item. `release.sh` is not run in CI (manual, post-merge), so the branch stays fully green. **Follow-up:** port `release.sh` → `release.mjs` with a real-release smoke test. (`ci.sh` → `ci.mjs` ✓; everything else ported + parity-proven.)
