...
updated: "2026-07-16T13:29:51-03:00"
started_at: null
assignee: null
...
- Exact producer preflight is required before completion review:
  ...
  - `.github/workflows/build-binaries.yml` has one closed `workflow_dispatch` input object: required `mode` choice with values `validate-only|publish-existing-tag`, required `expected_commit` matching 40 lowercase hex, and required `expected_tag` string. Validation requires `expected_tag == ""`; manual publication requires exact `session-relay--v<manifest-version>`. Tag-push publication does not consume manual inputs.
