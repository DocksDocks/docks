...
- Exact producer preflight is required before completion review:
  ...
  - Run `node scripts/release.mjs --check-prepared --plugin session-relay 0.12.0 --source-commit "$SOURCE_COMMIT" --docks-red "$REQUEST_DIR/docks-red.json" --docks-red-sha256 <sha256> --public-red "$REQUEST_DIR/public-red.json" --public-red-sha256 <sha256> --preflight "$REQUEST_DIR/preflight.json" --preflight-sha256 <sha256> --source-ci "$REQUEST_DIR/source-ci.json" --source-ci-sha256 <sha256> --receipt-out "$REQUEST_DIR/candidate.json"`. Plan-manager embeds the exact canonical red, preflight, source-CI, and candidate bytes plus hashes under Notes, runs exact `node scripts/ci.mjs` on that uncommitted plan-only tree, records the green result, and only then commits this plan as `EVIDENCE_COMMIT`.
...
Every invocation reconstructs and validates the complete first-parent chain and then reconciles it with authoritative remote refs, GitHub Release identity, exact `origin/main`, closed-set blob IDs, and any restore/reapply commits. A crash never invents a phase: reconciliation appends the single legal next entry implied by authoritative state or appends `manual_incident` and stops. In attempt 0, pre-push smoke success is journaled before pushing `main`; after a crash, observed `main == PROMOTED_COMMIT` derives `main_pushed`, observed `main == expected-origin-main` derives `prepush_passed`, and any other head is a manual incident. `--resume-promotion` is valid only for a nonterminal attempt and requires the exact transaction ref plus the same immutable identities; it rejects absent, forked, regressed, terminal, or conflicting journals. Fixture tests kill the process after transaction creation, lock creation, pre-push smoke, main push, live smoke, restore push, reapply push, and immediately before terminal receipt rename, then prove a fresh invocation either appends the one legal successor or stops.
...
Receipt-based retry exists only as `--retry-failed <receipt-path> --retry-failed-sha256 <sha256>` against the latest journal tip. It accepts only a canonical `restored_failure` receipt whose `attempt` equals the terminal attempt, whose terminal journal commit/key and chain digest equal the permanent transaction tip, whose prior-receipt binding and immutable identities match, whose exact restore commit is still remote `main`, and whose closed-set blobs remain unchanged. It rejects `success`, `manual_incident`, non-retryable or older failure receipts, a nonterminal/forked/regressed journal, mismatched attempts or keys, missing/extra commits, a restore-head mismatch, and every intervening closed-set change. Acceptance increments `attempt` exactly once, appends `(attempt + 1, 0) initialized` bound to that receipt, reuses the same lock, creates a normal fast-forward reapply commit byte-identical to the reviewed launcher-only closed set, and reruns the live smoke without republishing immutable artifacts. A second definitive failure restores compatibility and seals a new terminal receipt for that attempt; a non-transient failure leaves `0.12.0` staged and Docks compatible and requires a new reviewed `0.12.1` supersession plan. Force-push, journal/ref deletion, lock replacement, and history rewriting are forbidden.
...
```bash
node scripts/capture-tdd-red.mjs --repo "$PWD" --repository-id DocksDocks/docks --pre-production-commit "$(git rev-parse HEAD)" --test plugins/session-relay/test/distribution-contract.mjs --test plugins/session-relay/test/selftest.mjs --receipt-out "$REQUEST_DIR/docks-red.json" -- node plugins/session-relay/test/distribution-contract.mjs
```

That command must exit nonzero. The companion's independently reviewed plan owns and freezes its exact public test paths and exact failing argv; from `/home/vagrant/projects/public` it must invoke the same Docks helper at `/home/vagrant/projects/docks/scripts/capture-tdd-red.mjs` with `--repo "$PWD"`, repository ID `DocksDocks/public`, its own committed pre-production identity, and its own receipt output. Each repository emits exactly one receipt immediately after freezing its complete tests, and no production edit in either repository may precede its capture.
...
- Companion TDD-red receipt JCS/SHA-256: pending
...
- Docks TDD-red receipt JCS/SHA-256: pending
