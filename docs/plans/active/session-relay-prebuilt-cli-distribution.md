...
updated: "2026-07-16T12:49:37-03:00"
started_at: null
assignee: null
...
- Exact producer preflight is required before completion review:
  ...
  - Download all four same-run artifacts plus the generated checksum manifest; verify target file format/architecture, stable names, `session-relay 0.12.0`, and every digest after explicitly applying staged mode `0755`.
...
| Matching stable Release | Return success without mutation. |
...
release.mjs --bind-completion --plugin session-relay 0.12.0
  --candidate <candidate.json> --candidate-sha256 <sha256>
...
`plugins/session-relay/test/distribution-contract.mjs --release-fixtures <dir>` executes fixture-backed fakes for every publication-table row, prerelease finalization, promotion success, expected-main drift, lock contention, pre-push failure, restore, retry/reapply, manual incident, supersession, unknown/duplicate arguments, stale receipt hashes, and conflicting identities without network or repository mutation.
