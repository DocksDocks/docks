...
updated: "2026-07-16T13:02:09-03:00"
started_at: null
assignee: null
...
- Exact producer preflight is required before completion review:
  ...
  - Run `gh run watch "$RUN_ID" --exit-status`, then `gh run download "$RUN_ID" --dir "$TMP/artifacts"`. Run `node scripts/verify-session-relay-preflight.mjs --run-id "$RUN_ID" --expected-commit "$SOURCE_COMMIT" --artifacts "$TMP/artifacts" --receipt-out "$TMP/preflight.json"`.
  ...
  - Run `node scripts/release.mjs --check-prepared --plugin session-relay 0.12.0 --source-commit "$SOURCE_COMMIT" --preflight "$TMP/preflight.json" --preflight-sha256 <sha256> --receipt-out "$TMP/candidate.json"`. Plan-manager embeds the exact canonical preflight and candidate bytes plus hashes under Notes and commits only this plan as `EVIDENCE_COMMIT`.
...
| A7 | `node scripts/release.mjs --check-prepared --plugin session-relay 0.12.0 --source-commit <SOURCE_COMMIT> --preflight <PREFLIGHT_RECEIPT> --preflight-sha256 <PREFLIGHT_SHA256> --receipt-out <REQUEST_SCOPED_CANDIDATE>` | Exit 0; the command reruns A1–A6, binds their output/exit hashes plus manifests, marketplace, Cargo metadata, launcher-only payload, workflows, release notes, companion lifecycle evidence, and the exact successful four-target preflight into `SourcePreparationCandidateV1`; plan-manager then embeds its canonical bytes/hash and removes the request-scoped temporary file. |
