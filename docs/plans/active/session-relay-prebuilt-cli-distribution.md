...
goal: Prepare reviewed launcher-only Session Relay 0.12.0 and docks-kit 0.9.0 sources, then execute their gated post-completion release protocol.
status: planned
created: "2026-07-16T04:10:15-03:00"
updated: "2026-07-16T12:13:03-03:00"
started_at: null
assignee: null
...
Prepare Session Relay as a normal precompiled CLI: the reviewed `0.12.0` source contains only a small launcher, GitHub Actions can build the four supported Rust targets plus `SHA256SUMS`, and a companion reviewed `docks-kit` `0.9.0` source installs an exact host asset whose digest is pinned independently in its repository. After this plan passes completion review and ships, the authorized post-completion protocol publishes `0.12.0`, pins its live digests into the public repository, completes that repository's plan, releases `0.9.0`, live-smokes the installer, and only then advances Docks `main`.
...
This plan owns the Docks release producer, final launcher-only plugin payload, plugin-consumer contract, and immutable release handoff. A companion plan in `/home/vagrant/projects/public` owns the `docks-kit` installer and its source-pinned asset digests. Session Relay is the required cross-project implementation transport; the public worker must be dispatched while the current embedded binary still exists. Each repository retains its own plan lifecycle and commit history. Publication is deliberately not an implementation acceptance row: Docks completion review seals the final source first, then the post-completion protocol performs externally visible tag, release, and branch mutations.
...
- Coordinated release order:
  1. Implement, test, commit, completion-review, and ship the final launcher-only Docks source locally; remote Docks `main` remains on compatible `0.11.2`.
  2. Post-completion, push only final tag `session-relay--v0.12.0`; tag CI creates a draft five-asset Release without advancing Docks `main`.
  3. Pin the four draft-asset digests into the companion repository through authenticated API downloads, complete and ship its plan at an immutable commit, then publish `docks-kit` `0.9.0`.
  4. Make the Session Relay Release non-draft only after `cli-v0.9.0` is live, then promote the reviewed Docks lineage and run the exact released docks-kit executable against launcher-only Docks `main`. No earlier Docks branch push is permitted.
...
`scripts/release.mjs --prepare --plugin session-relay 0.12.0` applies every manifest, marketplace, Cargo, and lockfile version change, runs the full CI gate on that exact changed tree, and commits locally without pushing or tagging. A failed post-bump gate prevents the commit. Completion review seals that final launcher-only source. Only after the plan ships, `--publish-reviewed` accepts the exact reviewed implementation commit, proves the current plan carries a reusable passed completion receipt for it, pushes only `session-relay--v0.12.0`, waits for authoritative tag CI, and verifies the draft five-asset Release without advancing remote `main` or creating a duplicate Release. `--finalize-reviewed` requires live `cli-v0.9.0` and changes only that exact draft Release to non-draft. A later `--promote-reviewed` proves both live release identities, proves the Session Relay tag commit is the reviewed code ancestor of the current plan-only lifecycle lineage, downloads and verifies the exact host docks-kit release executable, pushes Docks `HEAD`, and executes the isolated end-to-end sync itself. If that post-push smoke fails, it immediately creates and pushes a normal fast-forward compatibility revert that restores the previously published plugin payload; force-push is forbidden. Other plugins retain their existing release behavior.
...
| 5 | Update registry-driven CI and release tooling for source-built local tests, local release preparation, post-completion draft/tag publication, safe release notes, a non-mutating preview, explicit finalization, and reviewed branch promotion with an integrated live smoke. | `scripts/ci.mjs`, `scripts/lib/plugins.mjs`, `scripts/lib/rust-bin.mjs`, `scripts/release.mjs`, `scripts/AGENTS.md` | 3, 4 | planned | Local CI builds/tests the explicit host binary without comparing committed executables; `--prepare` changes all version files before exact-tree CI and commits locally only; `--publish-reviewed` requires the passed completion receipt and pushes only the final tag into a draft Release; `--finalize-reviewed` requires live `cli-v0.9.0`; `--promote-reviewed` invokes the checksum-verified released docks-kit executable and emits a closed smoke receipt, with a tested fast-forward compatibility revert on failure; other plugins retain their prior semantics; the install descriptor is exactly `docks-kit sync`; and dry-run snapshots prove no mutation. |
...
| ID | Command | Expected |
|---|---|---|
| A1 | `public_commit=$(sed -n 's/^- Companion implementation commit: //p' docs/plans/active/session-relay-prebuilt-cli-distribution.md); node -e 'process.exit(/^[0-9a-f]{40}$/.test(process.argv[1]) ? 0 : 1)' "$public_commit" && tmp=$(mktemp -d) && git clone --no-local /home/vagrant/projects/public "$tmp/public" && git -C "$tmp/public" checkout --detach "$public_commit" && test "$(git -C "$tmp/public" rev-parse HEAD)" = "$public_commit" && node plugins/session-relay/test/distribution-contract.mjs --public-repo "$tmp/public" --public-commit "$public_commit"` | Exit 0; all public evidence comes from the one plan-pinned immutable commit and reports the installer schema, sync ordering, failure preservation, release URLs, Docks source contract, and both commit identities. |
...
## Post-completion release protocol

This protocol is explicitly authorized by the user but must not begin until this plan is `finished`, its completion receipt is reusable with `review_status: passed`, and `<DOCKS_REVIEWED_COMMIT>` is the exact reviewed implementation head from that receipt. Publication does not retroactively count as implementation evidence.

1. Run `node scripts/release.mjs --publish-reviewed --plugin session-relay 0.12.0 --reviewed-commit <DOCKS_REVIEWED_COMMIT>`. It may push only `session-relay--v0.12.0`, and must wait for tag CI plus the draft five-asset GitHub Release. Docks `origin/main` remains unchanged.
2. Download draft `SHA256SUMS` and all four assets through the authenticated GitHub API, require exactly the four planned target entries, independently hash every downloaded asset, and produce one closed target→digest map. No binary is committed.
3. Spawn a fresh public Session Relay worker at explicit Codex Standard tier with that exact map. The worker replaces only the fixture/pending production digests, regenerates the embedded SoT payload, runs focused and full public gates, completes its repository-local plan through fresh completion review, ships it, and hands back `<PUBLIC_RELEASE_COMMIT>`.
4. In a `git clone --no-local` detached checkout of `<PUBLIC_RELEASE_COMMIT>`, prove package version `0.9.0`, all four source-pinned digests equal the live independently hashed assets, every Claude/Codex sync path ensures Session Relay before plugin configuration, and checksum/version failure preserves the previous executable.
5. Push public `main`, create and push immutable tag `cli-v0.9.0` at the reviewed public release lineage, wait for `release-cli.yml`, and verify the non-draft GitHub Release, exact host docks-kit executable plus checksum, and `npm view docks-kit@0.9.0 version`.
6. Run `node scripts/release.mjs --finalize-reviewed --plugin session-relay 0.12.0 --reviewed-commit <DOCKS_REVIEWED_COMMIT> --docks-kit-release cli-v0.9.0`. It verifies the exact public tag/commit and released host asset before changing only the matching Session Relay draft Release to non-draft.
7. Run `node scripts/release.mjs --promote-reviewed --plugin session-relay 0.12.0 --reviewed-commit <DOCKS_REVIEWED_COMMIT> --docks-kit-release cli-v0.9.0 --receipt-out <SMOKE_RECEIPT>`. The command downloads the exact host docks-kit `0.9.0` release asset and published checksum, verifies its SHA-256 and `--version`, creates a fresh `HOME`/`XDG_CACHE_HOME` with no `session-relay`, pushes the reviewed Docks lineage, runs that absolute executable path as `docks-kit sync codex --yes`, and then invokes the installed launcher from the configured `session-relay` plugin. It requires both launcher and installed binary to report `session-relay 0.12.0`, and requires the captured command log to place CLI ensure before plugin configuration. It writes canonical JSON only after validating a closed receipt containing both tag/commit identities, all Session Relay asset hashes, docks-kit asset name/hash/version, isolation paths, installed binary hash/version, plugin manifest version, launcher canonical path/version, ordering-log hash, command exit status, and timestamps; stdout reports the receipt SHA-256. On any post-push failure it creates and pushes a normal fast-forward compatibility revert, verifies remote `main` serves the prior plugin payload, and exits nonzero.
8. Validate `<SMOKE_RECEIPT>` and its reported SHA-256 independently, re-run the live distribution contract against detached release commits, verify both remote `main` heads/tags/releases, run `git diff --check` over both implementation ranges, and require both original worktrees clean before declaring the user goal complete.
...
- **Executable acceptance:** Present; A1–A8 are commands with exact success conditions, while externally mutating release actions are isolated in the post-completion protocol.
