...
updated: "2026-07-16T14:20:19-03:00"
started_at: null
assignee: null
...
Promotion is an idempotent serialized state machine. Before creating the lock or mutating `main`, it creates the permanent append-only remote ref `refs/heads/transactions/session-relay-0.12.0` with an initial `PromotionJournalEntryV1` commit binding all reviewed identities and `expected-origin-main`. Each later journal entry has one parent, repeats those immutable identities, records the prior entry SHA, observed external state, and one monotonic phase from `initialized|locked|prepush_passed|main_pushed|live_passed|restore_pushed|reapply_pushed|terminal_success|terminal_failure|manual_incident`; updates use a compare-and-swap lease and never rewrite or delete history. It atomically creates the versioned immutable tombstone ref `refs/heads/locks/session-relay-0.12.0` at a nonce commit and records `--expected-origin-main`; the ref is never deleted or reused, and an existing mismatched lock or any pre-push drift stops before mutation. An identity-matching retry reuses the same lock through its transaction journal or failure receipt. The exact-source pre-push smoke runs the released docks-kit executable against a detached reviewed Docks checkout supplied through a release-test-only explicit source argument, proving the launcher-only plugin before `main` changes. The regular fast-forward push and live smoke occur while the lock is held.

Every promotion invocation first reconciles the journal with authoritative remote refs, GitHub Release identity, exact `origin/main`, closed-set blob IDs, and any existing restore/reapply commits. A crash after an external mutation but before the next journal commit is recovered by deriving the only valid next phase from that authoritative state and appending it before continuing. `--resume-promotion` requires the exact transaction ref plus the same source-proof, publication, docks-kit release, and expected-main identities; it rejects absent, forked, regressed, or conflicting journals. Fixture tests terminate the process after transaction creation, lock creation, pre-push smoke, main push, live smoke, restore push, reapply push, and immediately before terminal receipt rename, then prove a fresh invocation converges to the same success, restored failure, or typed manual incident without duplicate mutation.

The compatibility restore/reapply set is closed to the entire tracked `plugins/session-relay/**` tree plus the whole `.claude-plugin/marketplace.json` blob. `PromotionReceiptV1` records before, promoted, restored, and reapplied Git blob IDs for every member. Shared `.github/**` and `scripts/**` changes deliberately remain because their contract tests prove compatibility with the restored `0.11.2` payload and unchanged docks/effect-kit release semantics. A definitive post-push contract failure emits a canonical failure receipt and creates a normal fast-forward compatibility restore commit atop the current locked `main`, restoring the closed set from `expected-origin-main`; every member must still equal its promoted blob before restore. The restore must pass manifest/catalog parity and exact full CI before push. Any unexpected main movement or closed-set blob change keeps the prerelease staged, retains the lock, stops finalization, and returns a typed manual-incident result instead of clobbering concurrent work.

A retry may use either `--retry-failed <receipt-path> --retry-failed-sha256 <sha256>` after a sealed terminal failure or `--resume-promotion --transaction-ref refs/heads/transactions/session-relay-0.12.0` after an interrupted nonterminal phase. Receipt-based retry validates the closed failure receipt, exact reviewed/tag/public identities, exact compatibility-restore commit and remote head, and absence of intervening closed-set blob changes, then appends to the same transaction and creates a normal fast-forward reapply commit whose closed compatibility set is byte-identical to the reviewed launcher-only source. It reruns both smokes without republishing tags, Releases, or npm. A second definitive failure restores compatibility again and emits an updated failure receipt. A non-transient failure leaves `0.12.0` as a prerelease with the staging notice, keeps Docks on the compatible payload, and requires a new reviewed fix/release plan that supersedes it with `0.12.1`; it never finalizes a failed `0.12.0`. Force-push and history rewriting are forbidden. Distribution-contract tests cover interruption and restart at every mutation boundary, lock contention, expected-main drift, pre-push smoke failure, initial success, failure/restore, retry/reapply success, stale receipt rejection, intervening closed-set rejection, supersession, and idempotent refusal to republish immutable artifacts. Other plugins retain their existing release behavior.
...
- `PromotionJournalEntryV1`: permanent transaction ref, prior-entry commit (null only for initialization), immutable source-proof/`TAG_COMMIT`/`PROMOTED_COMMIT`/publication/docks-kit/expected-main identities, promotion-lock identity, monotonic phase, observed remote main and Release state, relevant push/restore/reapply commits and closed-set blob IDs, smoke descriptor/result hashes when available, and timestamp. The current entry commit is derived from the Git object that contains the canonical payload and is never included in that payload. A fresh resume reconstructs and validates the full first-parent chain before consulting external state.
- `PromotionReceiptV1`: source-proof and publication-receipt SHA-256, exact `TAG_COMMIT` and `PROMOTED_COMMIT` plus ancestry/non-plan equivalence proof, transaction ref and terminal journal commit, promotion-lock ref/nonce, expected and observed `origin/main`, Docks pre-push/pushed/restore/reapply commits, closed compatibility-set paths and before/promoted/restored/reapplied Git blob IDs, public reviewed/tag commits and Release ID, docks-kit asset name/ID/digest/version, Session Relay asset map, exact-source and live isolation-root digest descriptors, exact sync argv, stdout/stderr/ordering-log hashes, installed binary and launcher canonical paths/digests/version outputs, plugin manifest version, post-restore manifest-parity/full-CI hashes, exit status, outcome `success|restored_failure|manual_incident`, failure stage/reason when applicable, and start/end timestamps. Retry accepts only `restored_failure`, exact remote restore head, and no intervening closed-set blob change.
...
|---|---|---|---|---|---|
| 1 | Dispatch the separate `DocksDocks/public` installer implementation through the currently working embedded Session Relay binary under its own reviewed plan; collect a clean commit that implements the digest-pinned installer and fixture-driven tests, while its plan remains explicitly blocked only on the four not-yet-published production digests. | `/home/vagrant/projects/public/docs/plans/active/session-relay-cli-installation.md` (external companion plan; modified only by its plan-manager), Session Relay lifecycle/handback records (external state) | — | planned | The worker is launched with explicit Codex Standard tier, creates and starts its repository-local plan, freezes failing installer tests and emits canonical `TddRedReceiptV1`, implements the exact manifest/digest/atomic-replacement contract and Claude/Codex sync ordering, commits cleanly, pushes only the exact commit to its named immutable validation ref, records `blocked_reason` as awaiting `session-relay--v0.12.0` digests, hands back all lifecycle and red-receipt identities, and is collected before Docks Step 3. |
| 2 | Add the complete final Docks distribution-contract and self-test changes before any production change, capture their failure against the embedded-binary design, and freeze both blobs. | `plugins/session-relay/test/distribution-contract.mjs`, `plugins/session-relay/test/selftest.mjs` | 1 | planned | The complete tests fail for missing external resolution, version output, release-asset publication, post-bump CI ordering, explicit fresh self-test binary, cross-repo contract receipt, or obsolete committed-binary assumptions; the exact nonzero run is sealed as canonical `TddRedReceiptV1`, embedded under Notes, and Steps 3–7 treat both paths as read-only while verifying their blob identities through `SOURCE_COMMIT`. |
| 3 | Make the Rust CLI and plugin launcher expose the final stable external CLI contract, then delete every committed platform executable and the in-plugin checksum file. | `plugins/session-relay/rust/Cargo.toml`, `plugins/session-relay/rust/Cargo.lock`, `plugins/session-relay/rust/src/main.rs`, `plugins/session-relay/bin/relay`, `plugins/session-relay/bin/SHA256SUMS`, `plugins/session-relay/bin/relay-*` | 1, 2 | planned | `--version` reports the Cargo version; the frozen launcher tests prove external resolution order, recursion rejection, and actionable failure with no embedded fallback; the frozen self-test requires `SESSION_RELAY_TEST_BIN`; neither test blob changes; `git ls-files plugins/session-relay/bin` lists only the launcher. |
...
| 7 | Prepare and seal the exact release-ready source without publication: run the Session Relay `0.12.0` prepare mode, bind the immutable companion implementation ref/commit and both red receipts, commit `SOURCE_COMMIT`, execute the validation-only producer preflight, embed its canonical receipts, run full CI on that uncommitted plan-only tree, create `EVIDENCE_COMMIT`, and leave both repositories clean for completion review. | `.claude-plugin/marketplace.json`, `plugins/session-relay/.claude-plugin/plugin.json`, `plugins/session-relay/.codex-plugin/plugin.json`, `plugins/session-relay/rust/Cargo.toml`, `plugins/session-relay/rust/Cargo.lock`; frozen Docks tests and public immutable validation ref/commit (read-only) | 1–6 | planned | Every Docks version surface is `0.12.0`; the final source is launcher-only; both frozen test blobs still match their red receipt; `--check-prepared` passes; a fresh remote fetch proves the public validation ref and clean detached commit plus frozen tests; preflight head equals `SOURCE_COMMIT`; exact `node scripts/ci.mjs` passes after receipt embedding and before `EVIDENCE_COMMIT`; the diff from source to evidence commit is plan-only; canonical red/preflight/source-CI/candidate bytes and hashes are embedded under Notes; only the named immutable Docks and public validation refs were pushed, with no production branch, tag, Release, or npm publication; all steps are ready for plan-manager completion review. |
...
Cross-check (2026-07-16, round 16): [X: anthropic unavailable] authentication preflight failed; [S: openai gpt-5.6-sol high Standard] score 68 with 3 findings — accepted S1,S2,S3 / rejected none; [Codex main-context orchestrator] reproduced all three. Repairs remove the journal entry's impossible self-hash, fix `TAG_COMMIT = SOURCE_COMMIT` and `PROMOTED_COMMIT = SHIPPED_COMMIT` with ancestry plus non-plan tree equivalence, and make Step 2 the sole writer of the frozen Docks tests.
