...
updated: "2026-07-16T13:55:11-03:00"
started_at: null
assignee: null
...
- Exact producer preflight is required before completion review:
  ...
  - Set a second UTC dispatch timestamp, run `gh workflow run ci.yml --ref "preflight/session-relay-0.12.0-${SOURCE_COMMIT:0:12}"`, select exactly one later `workflow_dispatch` run whose head SHA equals `SOURCE_COMMIT`, and run `gh run watch "$SOURCE_CI_RUN_ID" --exit-status`.
  ...
  - Run `node scripts/release.mjs --check-prepared --plugin session-relay 0.12.0 --source-commit "$SOURCE_COMMIT" --preflight "$REQUEST_DIR/preflight.json" --preflight-sha256 <sha256> --source-ci "$REQUEST_DIR/source-ci.json" --source-ci-sha256 <sha256> --receipt-out "$REQUEST_DIR/candidate.json"`. Plan-manager embeds the exact canonical preflight, source-CI, and candidate bytes plus hashes under Notes, runs exact `node scripts/ci.mjs` on that uncommitted plan-only tree, records the green result, and only then commits this plan as `EVIDENCE_COMMIT`.
...
A retry requires `--retry-failed <receipt-path> --receipt-sha256 <sha256>`; it validates the closed failure receipt, exact reviewed/tag/public identities, exact compatibility-restore commit and remote head, and absence of intervening closed-set blob changes, then creates a normal fast-forward reapply commit whose closed compatibility set is byte-identical to the reviewed launcher-only source. It reruns both smokes without republishing tags, Releases, or npm. A second definitive failure restores compatibility again and emits an updated failure receipt. A non-transient failure leaves `0.12.0` as a prerelease with the staging notice, keeps Docks on the compatible payload, and requires a new reviewed fix/release plan that supersedes it with `0.12.1`; it never finalizes a failed `0.12.0`. Force-push and history rewriting are forbidden. Distribution-contract tests cover lock contention, expected-main drift, pre-push smoke failure, initial success, failure/restore, retry/reapply success, stale receipt rejection, intervening closed-set rejection, supersession, and idempotent refusal to republish immutable artifacts. Other plugins retain their existing release behavior.
...
The preserved docks/effect-kit interface retains current bump resolution, manifest/catalog mutation, exact-tree full CI before commit, commit message, branch push, immutable plugin tag, tag-CI wait, GitHub Release notes/assets, and dry-run byte-for-byte non-mutation. Session Relay positional bump syntax rejects with the new prepare command. `plugins/session-relay/test/distribution-contract.mjs --release-fixtures <dir>` executes fixture-backed fakes for every publication-table row including tag-present/no-run recovery, prerelease finalization, promotion success, expected-main drift, lock contention, pre-push failure, closed-set restore, manifest-parity/full-CI restore gate, retry/reapply, manual incident, supersession, unknown/duplicate arguments, stale receipt hashes, conflicting identities, and unchanged docks/effect-kit positional release flows and tag names without network or repository mutation.
...
| 7 | Prepare and seal the exact release-ready source without publication: run the Session Relay `0.12.0` prepare mode, bind the immutable companion implementation commit, commit `SOURCE_COMMIT`, execute the validation-only producer preflight, embed its canonical receipts, run full CI on that uncommitted plan-only tree, create `EVIDENCE_COMMIT`, and leave both repositories clean for completion review. | `.claude-plugin/marketplace.json`, `plugins/session-relay/.claude-plugin/plugin.json`, `plugins/session-relay/.codex-plugin/plugin.json`, `plugins/session-relay/rust/Cargo.toml`, `plugins/session-relay/rust/Cargo.lock`, `plugins/session-relay/test/distribution-contract.mjs`; `/home/vagrant/projects/public` companion commit (read-only) | 1–6 | planned | Every Docks version surface is `0.12.0`; the final source is launcher-only; `--check-prepared` passes; the pinned public commit is clean and passes contract tests in a detached clone; preflight head equals `SOURCE_COMMIT`; exact `node scripts/ci.mjs` passes after receipt embedding and before `EVIDENCE_COMMIT`; the diff from source to evidence commit is plan-only; canonical preflight/candidate bytes and hashes are embedded under Notes; only the named immutable Docks preflight branch was pushed, with no production branch, tag, Release, or npm publication; all steps are ready for plan-manager completion review. |
...
| A7 | `node scripts/release.mjs --verify-embedded-preparation --plugin session-relay 0.12.0 --plan docs/plans/active/session-relay-prebuilt-cli-distribution.md` | Exit 0; the command reconstructs the embedded preflight, source-CI, and candidate JCS bytes, verifies every recorded SHA-256 and identity against the plan-derived `SOURCE_COMMIT`, proves the current release-owned tree is unchanged, and emits no replacement candidate. |
