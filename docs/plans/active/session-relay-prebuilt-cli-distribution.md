...
updated: "2026-07-16T12:34:53-03:00"
started_at: null
assignee: null
...
- Exact producer preflight is required before completion review:
  - Push only `refs/heads/preflight/session-relay-0.12.0-<reviewed-head-short>` at the exact release-ready commit; do not advance `main` or create a release tag.
  - Dispatch `.github/workflows/build-binaries.yml` against that branch in validation-only mode.
  - Download all four same-run artifacts plus the generated checksum manifest; verify target file format/architecture, stable names, executable bit, `session-relay 0.12.0`, and every digest.
  - Record the immutable branch ref, commit, workflow run/database ID, workflow SHA, artifact IDs, and four digests under Notes. The later release plan may reuse identities as evidence but must rebuild from the immutable tag; preflight artifacts are never published.
...
| Matching tag, no Release | Resume the bound workflow or create the draft from artifacts produced by that exact run. |
| Matching draft, partial assets | Verify every existing asset's name and digest, upload only missing assets from the bound run, then verify the closed five-asset set. |
| Matching complete draft | Return success without replacing assets. |
| Matching finalized Release | Return success without mutation. |
...
`--finalize-reviewed` similarly treats a matching complete draft as actionable and an already finalized matching Release as success; absent/partial/conflicting state stops. Every transition binds repository ID, tag object/commit, workflow file/SHA/run ID/attempt, Release database ID, and asset database IDs/digests. Contract tests simulate interruption at every row and prove convergence or fail-closed conflict behavior.
...
- `SourcePreparationCandidateV1`: repository ID, plan path/blob SHA-256, execution-base commit, candidate implementation commit, companion commit/plan path/input SHA-256/execution-base/review-receipt SHA-256/status/blocked reason, preflight branch/ref commit, workflow file/SHA/run ID/attempt, four artifact IDs/names/digests/file identities/version outputs, checksum-manifest digest, A1–A6 result hashes, and creation timestamp. `--check-prepared --receipt-out` emits it before completion review.
- `SourcePreparationProofV1`: every candidate field plus candidate SHA-256, final plan path/blob SHA-256, exact reviewed implementation commit, embedded reusable completion-review receipt SHA-256 and `review_status: passed`, shipped plan commit, and binding timestamp. The later release plan begins with `--bind-completion --candidate <path> --candidate-sha256 <sha256> --receipt-out <path>`; it emits the proof only when the finished plan and receipt bind the unchanged candidate implementation tree.
- `SessionRelayPublicationReceiptV1`: source-proof SHA-256, repository/tag/tag commit, workflow file/SHA/run ID/attempt, Release database ID and `draft|finalized` state, five closed asset records with database ID/name/size/digest, transition performed, and timestamp. Publish/finalize modes consume the prior receipt when resuming and emit the reconciled state.
- `PromotionReceiptV1`: source-proof and publication-receipt SHA-256, Docks reviewed/tag/pre-push/pushed/revert/reapply commits, public reviewed/tag commits and Release ID, docks-kit asset name/ID/digest/version, Session Relay asset map, isolation-root digest descriptor, exact sync argv, stdout/stderr/ordering-log hashes, installed binary and launcher canonical paths/digests/version outputs, plugin manifest version, exit status, outcome `success|reverted_failure`, failure stage/reason when applicable, and start/end timestamps. Retry accepts only `reverted_failure`, exact remote revert head, and no intervening code change.
...
| 7 | Prepare and seal the exact release-ready source without publication: run the Session Relay `0.12.0` prepare mode, bind the immutable companion implementation commit, execute and record the validation-only producer preflight, run focused and broad gates, and leave both repositories clean for completion review. | `.claude-plugin/marketplace.json`, `plugins/session-relay/.claude-plugin/plugin.json`, `plugins/session-relay/.codex-plugin/plugin.json`, `plugins/session-relay/rust/Cargo.toml`, `plugins/session-relay/rust/Cargo.lock`, `plugins/session-relay/test/distribution-contract.mjs`; `/home/vagrant/projects/public` companion commit (read-only) | 1–6 | planned | Every Docks version surface is `0.12.0`; the final source is launcher-only; `--check-prepared` passes; the pinned public commit is clean and passes contract tests in a detached clone; only the named immutable Docks preflight branch was pushed, with no production branch, tag, Release, or npm publication; all steps are ready for plan-manager completion review. |
...
| A7 | `node scripts/release.mjs --check-prepared --plugin session-relay 0.12.0 --receipt-out /tmp/session-relay-source-preparation-candidate.json` | Exit 0; manifests, marketplace, Cargo metadata, launcher-only payload, workflows, release notes, companion lifecycle evidence, A1–A6 result hashes, and the exact successful four-target preflight run are bound into a validated `SourcePreparationCandidateV1`, while no tag/Release/npm/main mutation is attempted. |
...
- **Executable acceptance:** Present; A1–A8 are commands with exact success conditions for the source-preparation goal, including its validation-only preflight, while a separately reviewed release plan owns all publication, production-branch mutations, and live evidence.
