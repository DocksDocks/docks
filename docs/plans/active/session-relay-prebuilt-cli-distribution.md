...
updated: "2026-07-16T14:06:24-03:00"
started_at: null
assignee: null
...
After this plan is finished, plan-manager must create and independently review `docs/plans/active/session-relay-prebuilt-cli-release.md` before any release or production-branch mutation. Its immutable inputs are this plan's passed completion receipt, embedded red/preflight/source-CI/candidate receipts, exact `SOURCE_COMMIT`, reviewed plan-only evidence commit, and the freshly fetchable companion public validation ref/implementation commit recorded under Notes. `--bind-completion` reconstructs the embedded candidate from the finished plan, freshly fetches the companion ref, and verifies every hash; it does not depend on `/tmp` or either original worktree. The release plan owns the authorized Session Relay prerelease, authenticated digest collection, production public pins and completion review, docks-kit `0.9.0` publication, serialized Docks promotion under an expected-main lease and durable transaction journal, exact-source and live smoke evidence, Session Relay stable finalization only after success, interruption resume/retry/reapply if needed, and terminal `0.12.1` supersession if `0.12.0` cannot safely promote. The outer user goal remains incomplete until that release plan is shipped; this source-preparation plan must not claim publication success.
