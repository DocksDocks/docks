...
## Dogfood record — step 7 (2026-07-03)

A fresh-context agent ran the UPDATED audit procedures over this repo: 6 nodes + 2 shipped skills, **228 claims verified** (168 confirmed · 16 drifted · 1 broken-ref · 43 unverifiable-external · **0 `line-anchor`** — the new guard already keeps the tree clean; a manual sweep of guard-exempt `docs/` also found zero). Every volatile fact's re-derivation command was executed and re-derived correctly (floors, scorer maxima, node counts, the OKF snippet incl. negative cases). Guard verified live: 104 docs clean; negative test (planted `scripts/ci.mjs:12`) exits 1 naming file+anchor; scope corrected mid-step to match the plan contract (exclude only `docs/plans/`, not all of `docs/` — `docs/scaffold/AGENTS.md` is long-lived and now guarded).

In-scope drift fixed (files this plan already touches, or its own deliverables): context-tree's stale dogfood/example content (the `plugins/docks/agents/` node contradiction with root, the obsolete `docs/scaffold/` deferral, the removed `_assets/` mention), the unfollowable `--check-only` consumer attribution (genericized to the semantic-change predicate), the misdescribed hook-calls-refresh mechanism (hook nudges, never invokes), root AGENTS.md Node `22.x`→`24` (ci.yml is the source), `docs/plans/active/.gitkeep` added (stated invariant was false).

**Follow-ups (out of this plan's scope, recorded not fixed):** (1) `no-author-scripts.mjs` PATTERN misses the `.mjs` entry-point forms `scripts/ci.mjs`/`scripts/release.mjs` — the guard root + skills-node claim enforcement from; (2) `.github/AGENTS.md` promises Renovate/Dependabot SHA updates but neither is configured; (3) graph lint: `plugins/session-relay/` qualifies for its own context node; (4) `plugins/docks/skills/AGENTS.md` says `name`/`description` optional/recommended while kit CI requires both; (5) agents-score "mechanically needs 2 constraint blocks" — 1 block + perfect elsewhere = 14 = floor, passes; (6) ci.mjs stale "zero bash in the repo" comment.
