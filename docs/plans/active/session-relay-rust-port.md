...
updated: "2026-07-01T16:52:10-03:00"
started_at: null
assignee: null
...
- **Build all 4 (fallback, from an Apple-Silicon Mac):** `plugins/session-relay/rust/build-all.sh` — arm64-darwin native, `rustup target add x86_64-apple-darwin` for the Intel-darwin leg, and both musl legs via `cross` (Docker); emits the four + `SHA256SUMS` into `bin/`. The Linux host CANNOT run this fully (no Apple SDK → no darwin).
