...
### Step 6 EXPERIMENTAL Claude channel (2026-07-11, Claude Code 2.1.207)

Current official research-preview docs and Anthropic's fakechat source pin the contract: an stdio MCP server advertises `capabilities.experimental["claude/channel"] = {}` and sends ordered, unacknowledged `notifications/claude/channel` events with string `content` plus identifier-keyed string `meta`. The MCP server sets the automatic `<channel source="…">`; message-derived text must stay out of trusted server instructions. A plugin declares no special channel manifest field — official fakechat is ordinary plugin metadata plus `.mcp.json` — but plugin channels are gated by Anthropic's or the organization's `allowedChannelPlugins` list. The approved v1 therefore uses the official manual custom-server seam: `claude --dangerously-load-development-channels server:session-relay-channel`. The development flag bypasses only channel allowlisting, never tool permissions or `channelsEnabled`.

`channelsEnabled` defaults were added to the executable recipe rather than assumed: Pro/Max outside an organization skips policy checks; claude.ai Team/Enterprise is blocked until an Owner enables **Admin settings → Claude Code → Channels** or deploys managed `{"channelsEnabled":true}`; Console API auth permits channels unless managed settings are deployed, in which case the key is required. On Linux/WSL the file-based managed location is `/etc/claude-code/managed-settings.json` (macOS `/Library/Application Support/ClaudeCode/`, Windows `C:\Program Files\ClaudeCode\`). This is managed-only; user/project settings cannot override it.

A read-only live probe of the installed bus MCP process confirmed Claude 2.1.207 injects `CLAUDE_CODE_SESSION_ID` with the exact host session UUID plus `CLAUDE_PROJECT_DIR`. `relay channel` treats that version-sensitive environment contract as mandatory: UUID validation, exact registry lookup, `tool=claude`, and normalized directory equality; it never falls back to `id_for_dir`. The bounded registration wait defaults to five seconds and exits non-zero with a diagnostic on timeout. After MCP initialize it takes the existing kernel watcher flock with `mode=channel`; UserPromptSubmit skips its drain only while that exact held mode is live. Process death releases the flock, so hook fallback resumes without stale reclamation.

The server is one-way: no `tools`, reply tool, or `claude/channel/permission` capability. Each mailbox record becomes one event whose content is `hook::mail_block(&[message], exact_id)`, retaining the explicit UNTRUSTED DATA label and case-insensitive sentinel defuse; only validated `recipient_id` appears in metadata. Notifications are at-most-once drain-to-transport because the protocol has no acknowledgement — no processing or exactly-once claim is made.

TDD RED evidence: the prior 114-check source binary reached the first new case and failed waiting for the absent channel notifications. Green evidence: 51 Rust tests total (48 unit + 1 bus smoke + 2 lock race) and `PASS: session-relay self-test — 117 checks (binary: rust/target/x86_64-unknown-linux-musl/release/relay)`. The three new headless cases pin capability/instructions, one fenced event per seeded message, sentinel defuse, bounded fail-closed identity errors, channel-lock prompt arbitration, and crash restoration of hook delivery. No live/production store or committed binary was touched.

### Step 7 final docs + selftest sweep (2026-07-11)

The shipped skill now has one coherent `## Live view` section with both complete opt-in paths. Claude covers the manual `server:` registration, research-preview development flag, per-authentication `channelsEnabled` defaults, managed settings locations, version-sensitive exact-id binding, one-way/no-authority constraint, unacknowledged delivery, and unchanged UNTRUSTED DATA fence. Codex covers server start, `codex --remote`, the attach-before-send `relay attach worker --exec` flow, spawn/send/wake, raw-item invisibility, neutral visible turn, contention/idempotency degradations, and the explicit Unix filesystem-permission authentication boundary; it forbids a competing `codex exec resume` writer.

The fake app-server required no new Step 7 code. Its existing protocol stub handles `thread/start`, `thread/resume`, `thread/read`, `thread/inject_items`, and `turn/start` plus started/completed and elicitation events. Existing black-box cases assert the fenced canonical user/input-text item, the separate neutral visible turn and inject-before-turn ordering, double idle reads, relay-owned spawn `thread/start` fields and returned identity, bus elicitation ownership, synchronous thread/turn-start failures, watched completion, and pump timeout. Rust unit tests independently pin the visible item and neutral nudge payload constructors. `main.rs` lists and dispatches the new EXPERIMENTAL `channel` verb in the multi-call contract.

Final verification remained `PASS: session-relay self-test — 117 checks (binary: rust/target/x86_64-unknown-linux-musl/release/relay)`; `node scripts/ci.mjs --plugin session-relay` passed with Cargo fmt and clippy active, skill hash in sync, all four committed binary checksums valid, and only the expected local host-build digest warning. No `bin/` or live relay-store changes were made.
