...
>
> **Successor-6 note — preserve the immutable tag as source identity.** Successor 5 correctly
> identified the completion diff-base defect but captured its `source_base` at the accepted
> post-tag implementation. That commit cannot be an ancestor of the already-burned release tag,
> so the unchanged source-to-tag proof would remain impossible. This successor is minted with
> `source_base` equal to immutable tag commit `875661640c1c45894c66a9b2fdc437fd47307e1f`, while
> `execution_parent` will remain accepted predecessor implementation
> `a869a7f895b5f68dde3b6aeec331e23fc5decab5`. The production repair must use those two identities
> for their separate purposes: tag ancestry from `source_base`; completion review diff from
> `execution_parent`. The prerelease, child, assets, and all inherited rows remain unchanged.
>
> **Identity closure for successor 6.** A PlanRun replacement has a new `run_id`; keeping the
> completed predecessor in the current instance or either independent current-run oracle makes
> `currentPlanRun` reject the source proof before the review-base regression can run. Step 13
> therefore moves all four live pins to `1d28abf4-a843-42db-9ac4-db86dcb3e420` while preserving
> the tag source, goal, child, assets, and every frozen generation. The inventory is regenerated
> through its own mode and is allowed to remain byte-identical when its census excludes test-only
> current-run constants.
>
> **Successor-6 note — child source-base identity collapse.** The predecessor passed completion
> review, published reviewed implementation `b8ef713f979f96a07af26cc245235b3605ddabcd` to
> `origin/main`, rebound the existing staged prerelease, and emitted the child request. The live
> `--verify-public-release` boundary then rejected the canonical finished docks-kit 0.14.0 plan:
> its independently reviewed `source_base` is its implementation commit while its
> `execution_parent` is the explicit parent request base. The current parent verifier wrongly
> requires both identities to equal one `companion_base_commit`. Rows 1-18 are inherited world
> state. This successor makes the existing current child fixture reproduce the distinct identity
> tuple, removes only the false source-base equality, preserves the exact execution-parent pin and
> every plan/review/content/ancestry check, reruns the full gate, and resumes stable promotion.
>
> **Successor-7 note — identity-complete child source-base repair.** The predecessor correctly
> identified the public-child verifier defect, but its reviewed steps omitted two required
> boundaries: every current parent oracle must move to the fresh successor run, and the repair
> checkpoint must replace the previously published `b8ef713f979f96a07af26cc245235b3605ddabcd`
> implementation on `origin/main` before promotion. Rows 1-18 remain inherited world state. This
> successor refreshes its own identity first, adds the live-representative red fixture, validates
> `source_base` ancestry independently while retaining the exact request-to-`execution_parent`
> pin, passes the full gate and completion review, then rebinds and republishes this successor
> before the unchanged stable-promotion, preflight-ref, and archive boundaries.
>
> **Successor-8 note — separate release and PlanRun source identities.** The predecessor review
> found that branch publication and prerelease reconciliation were combined under one effect.
> A second preflight found the corresponding post-tag source defect: the immutable release source
> remains tag commit `875661640c1c45894c66a9b2fdc437fd47307e1f`, while every new same-path
> successor has its own later PlanRun `source_base`. The current binder collapses those identities,
> so its publication row would fail even after the child verifier repair. Rows 1-19 remain
> inherited world state. This successor refreshes its own run identity, adds a post-tag successor
> regression, uses the instance source only for source-to-tag proof and the live run source for
> source-to-implementation proof, keeps the child source ancestry repair, then separates
> explicit main fast-forward (`push`) from the early-return prerelease rebind (`probe`) before promotion.
>
> **Successor-9 note — restore closed V3 and accept canonical UTC.** The prior successor passed
> completion review, published implementation `03f79233fd0d5275b0610aa2d20aa011e6417269` to
> `origin/main`, and rebound the unchanged prerelease. Stable promotion did not run. Its live child
> preflight then exposed two bounded defects: `assertTimestamp` rejects the canonical child
> PlanRun UTC spelling `+00:00`, and the source repair added a field to the closed
> SourcePreparationProofV3 ancestry shape despite this plan excluding receipt-schema changes.
> Rows 1-28 are inherited completed world state. This successor restores V3 byte shape while
> retaining all three live Git ancestry observations, accepts only exact millisecond UTC spellings
> (`Z` or `+00:00`), re-reviews the repair, republishes main, rebinds the staged receipt, and only
> then promotes stable.
>
> **Successor-10 note — refresh the current run before compatibility proof.** Successor 9 correctly
> bounded the closed-V3 and UTC repairs, but its second draft review found that it inherited the
> predecessor current-run pins without scheduling their move to the fresh run. Its review budget
> is terminally exhausted. This successor keeps every accepted repair, adds an explicit current
> instance and two-oracle identity refresh before reproof, and uses the live PlanRun `run_id`
> instead of another hard-coded predecessor identity.
>
## Steps
...
| 13 | refresh_successor_identity | Move every current Session Relay 0.16.0 PlanRun oracle from completed predecessor `c268ecc1-cf5e-4266-9a29-a83b59e9717d` to this reviewed successor while retaining the same goal, plan path, immutable tag source, child, and asset generation. | `scripts/lib/session-relay-release-instances/0.16.0.json`; `plugins/session-relay/test/distribution-contract.mjs`; `plugins/session-relay/test/companion-distribution-contract.mjs`; `plugins/session-relay/test/fixtures/release-identity-inventory.json` | 12 | `local` | `done` | The instance current attempt and planrun attempt plus both independent current-run oracles equal successor run `1d28abf4-a843-42db-9ac4-db86dcb3e420`; `docks_source_base` remains tag commit `875661640c1c45894c66a9b2fdc437fd47307e1f`; the identity inventory is regenerated and changes only if its own census requires it. A5, distribution, and companion contracts fail on any mixed generation. |
| 14 | binder_regression | Add a synthetic PlanRun completion fixture whose tagged `source_base` and reviewed `execution_parent` are distinct, and prove the current binder rejects the otherwise valid passed review with the observed stale-diff signature. | `plugins/session-relay/test/release-evidence-contract.mjs` | 13 | `local` | `done` | The new fixture fails before the production fix because the binder hashes the tag-based source range instead of the reviewed execution range. It keeps the immutable tag ancestry valid and changes no published receipt. A setup, ancestry, or manifest failure is not the required red result. |
| 15 | binder_review_base | Reconstruct current PlanRun completion evidence from `execution_parent` while retaining `source_base` for source/tag ancestry and the implementation commit for acceptance manifests. | `scripts/lib/session-relay-release-preparation.mjs` | 14 | `local` | `done` | The current PlanRun binder validates the exact passed CompletionReviewV1 diff when `source_base != execution_parent`; retained non-PlanRun completion behavior and all ancestry checks remain unchanged. Failure: STOP rather than weakening diff, ancestry, or acceptance equality. |
| 16 | binder_reproof | Run the focused release-evidence contract and the complete authoritative repository gate on the successor bytes. | all affected paths in frontmatter | 15 | `local` | `done` | A17 and A7 pass. Reverting only the review-base fix makes A17 fail with the original stale-diff signature; restoring it returns green. |
| 17 | successor_checkpoint | Commit the successor implementation, bind live acceptance at that exact HEAD, and pass a fresh completion review over the predecessor implementation through this fix. | all affected paths in frontmatter | 16 | `local` | `done` | HEAD, implementation commit, acceptance manifest source base, and the fresh CompletionReviewV1 all bind one exact successor commit; the reviewed diff includes the regression and production repair. |
| 18 | publish_main | Bind predecessor completion and fast-forward its exact reviewed implementation to `origin/main`. | — | 17 | `push` | `done` | Exact push authority bound implementation `b8ef713f979f96a07af26cc245235b3605ddabcd`; `--bind-completion` emitted its source proof and an explicit fast-forward `git push` made remote `main` exactly that commit. No tag, release, or asset changed in this row. |
| 19 | rebind_predecessor_publication | Read back the existing staged prerelease and emit a local publication receipt bound to the predecessor source proof. | release receipts | 18 | `probe` | `done` | `--publish-reviewed --rebind-complete-publication` took the early return through `rebindCompletePublication`, read the immutable tag, successful workflow, release metadata, and downloaded assets, then emitted a local receipt. It performed no branch, tag, release, or asset mutation. |
| 20 | refresh_source_separated_identity | Move the current 0.16.0 instance and both independent parent oracles to run `3c2a2253-3999-464f-b58c-055bf60604e1`, while preserving the immutable release source, goal, child, assets, and historical instances. | `scripts/lib/session-relay-release-instances/0.16.0.json`; `plugins/session-relay/test/distribution-contract.mjs`; `plugins/session-relay/test/companion-distribution-contract.mjs` | 19 | `local` | `done` | The instance and both current-run oracles name the reviewed predecessor run; instance `docks_source_base` and tag commit remain `875661640c1c45894c66a9b2fdc437fd47307e1f`. |
| 21 | post_tag_source_regression | Add post-tag parent source fixtures and reproduce the stale release-source equality. | `plugins/session-relay/test/release-evidence-contract.mjs`; `plugins/session-relay/test/release-promotion-contract.mjs` | 20 | `local` | `done` | The fixtures separately bind immutable release source, later PlanRun source, execution parent, and implementation; unrelated PlanRun source is refused. |
| 22 | separate_parent_sources | Separate immutable release source from live PlanRun source in completion binding and every promotion proof consumer. | `scripts/lib/session-relay-release-preparation.mjs`; `scripts/lib/session-relay-release-promotion.mjs` | 21 | `local` | `done` | Preparation and both promotion consumers independently observe PlanRun-source-to-implementation ancestry; immutable source-to-tag and execution-parent-to-implementation checks remain. The successor repair below restores the accidentally changed V3 field set without removing these live checks. |
| 23 | public_source_base_regression | Reproduce the canonical child distinction between `source_base` and `execution_parent`. | `plugins/session-relay/test/release-promotion-contract.mjs` | 22 | `local` | `done` | The unmodified verifier rejected the released child shape with `current public finished PlanRunV1 lifecycle, identity, effect, or completion binding is invalid`. |
| 24 | separate_child_source_base | Remove only the false child source-base equality, retain exact request equality for `execution_parent`, and observe `source_base`-to-implementation ancestry. | `scripts/lib/session-relay-release-promotion.mjs`; `plugins/session-relay/test/release-promotion-contract.mjs` | 23 | `local` | `done` | The canonical child passes; an unrelated source base is refused; all other child bindings remain fail-closed. |
| 25 | source_separation_reproof | Run source, child, publication, identity, and full repository gates. | all affected paths in frontmatter | 24 | `local` | `done` | The focused contracts and complete `node scripts/ci.mjs` gate passed on predecessor implementation bytes. |
| 26 | source_separation_checkpoint | Commit, bind acceptance, and pass completion review on the source-separated predecessor. | all affected paths in frontmatter | 25 | `local` | `done` | CompletionReviewV1 passed implementation `03f79233fd0d5275b0610aa2d20aa011e6417269`; the later live preflight found the closed-schema and UTC defects before stable mutation. |
| 27 | publish_source_separated_main | Bind predecessor completion and fast-forward its exact reviewed implementation to `origin/main`. | — | 26 | `push` | `done` | Exact push authority authorized `main@03f79233fd0d5275b0610aa2d20aa011e6417269`; remote main became that commit. Tag, release, and assets did not change. |
| 28 | rebind_source_separated_publication | Read back the staged prerelease and emit a publication receipt bound to predecessor source proof. | release receipts | 27 | `probe` | `done` | The early-return rebind emitted receipt digest `e4bc8f2f2b284cab2bbec903c82e1cc5ee2b0759562941f3ceaef642b8f20f9f` without branch, tag, release, or asset mutation. |
| 29 | closed_v3_regression | Make the post-tag source fixture require the original closed SourcePreparationProofV3 ancestry keys while keeping a distinct `plan_run.source_base`, and reproduce the current validator rejection. | `plugins/session-relay/test/release-evidence-contract.mjs`; `plugins/session-relay/test/release-promotion-contract.mjs`; `plugins/session-relay/test/release-publication-contract.mjs` | 28 | `local` | `planned` | Before production repair, deleting `plan_source_to_implementation` from the otherwise current V3 proof fails with `current source proof ancestry has unknown or missing fields`; the test also observes the live Git ancestry call so shape restoration cannot remove proof. |
| 30 | restore_closed_v3 | Restore the exact V3 ancestry field set and emitted receipt shape; retain PlanRun-source Git checks in preparation, current promotion, and fresh-context rebind. | `scripts/lib/session-relay-release-preparation.mjs`; `scripts/lib/session-relay-release-promotion.mjs`; `plugins/session-relay/test/release-evidence-contract.mjs`; `plugins/session-relay/test/release-promotion-contract.mjs`; `plugins/session-relay/test/release-publication-contract.mjs` | 29 | `local` | `planned` | Original V3 receipts validate byte-shape compatibility; newly emitted V3 receipts have only `source_to_tag`, `tag_to_implementation`, and `implementation_to_reviewed`; all three live paths still reject unrelated `plan_run.source_base`. |
| 31 | utc_offset_regression | Change the public child fixture to the released PlanRun spelling `+00:00` and reproduce the exact live verification failure. | `plugins/session-relay/test/release-promotion-contract.mjs` | 30 | `local` | `planned` | Before production repair, the otherwise canonical child fails with `current public finished PlanRunV1 time must be an exact RFC3339 UTC timestamp`; `Z` remains a passing control. |
| 32 | accept_exact_utc_offsets | Accept exact millisecond UTC timestamps ending in `Z` or `+00:00`, while refusing nonzero offsets, missing milliseconds, invalid dates, and non-strings. | `scripts/lib/session-relay-release-promotion.mjs`; `plugins/session-relay/test/release-promotion-contract.mjs` | 31 | `local` | `planned` | The released child verifies with `+00:00`; existing `Z` fixtures pass; explicit negative fixtures reject `-03:00`, `+01:00`, absent milliseconds, impossible dates, and non-string values. Receipt schemas and stored timestamps remain unchanged. |
| 33 | refresh_compatible_identity | Move the current 0.16.0 instance and both independent current-run oracles to this plan's live `run_id`, while preserving the immutable release source, goal, tag, child, assets, and historical generations. | `scripts/lib/session-relay-release-instances/0.16.0.json`; `plugins/session-relay/test/distribution-contract.mjs`; `plugins/session-relay/test/companion-distribution-contract.mjs` | 32 | `local` | `planned` | The instance and both current-run oracles equal this PlanRun `run_id`; instance `docks_source_base` and tag commit remain `875661640c1c45894c66a9b2fdc437fd47307e1f`; the frozen identity inventory changes only if its generator proves a current literal is part of that historical census. |
| 34 | compatibility_reproof | Run A16-A20 and the complete authoritative repository gate on the closed-schema UTC repair. | all affected paths in frontmatter | 33 | `local` | `planned` | A16-A20 and A7 pass. Reverting only V3 restoration or UTC acceptance reproduces its exact failure; restoring both returns green. |
| 35 | compatibility_checkpoint | Commit the complete repair, bind live acceptance, and pass a fresh completion review over the predecessor through both compatibility fixes. | all affected paths in frontmatter | 34 | `local` | `planned` | HEAD, implementation commit, acceptance manifest, and CompletionReviewV1 bind one exact commit whose reviewed diff contains both red fixtures, narrow repairs, and the current-run identity refresh. |
| 36 | publish_compatible_main | Bind this successor completion and fast-forward the exact reviewed implementation to `origin/main`. | — | 35 | `push` | `planned` | Exact push authority matches this implementation; explicit fast-forward makes remote main exactly reviewed HEAD without changing tag, release, or assets. |
| 37 | rebind_compatible_publication | Read back the existing staged prerelease and emit a publication receipt bound to this successor source proof. | release receipts | 36 | `probe` | `planned` | Exact read-only probe authority covers the release; early-return rebind emits a local receipt and invokes no mutator. |
| 38 | promote_stable | Verify the completed child and promote the existing byte-identical prerelease through the remaining current PlanRun release modes. | release receipts | 37 | `release` | `planned` | With exact release authority, fresh public verification accepts the canonical `+00:00` child, promotion changes only prerelease state/body, finalization validates all no-clobber receipts, and the three binaries plus checksum stay byte-identical. |
| 39 | probe_refs | Re-read remote preflight refs after promotion and prove every baseline ref individually survives. | — | 38 | `probe` | `planned` | Exact read-only probe authority covers `preflight/*`; every recorded baseline ref remains at its original object. |
| 40 | archive_push | Finish, archive, commit, push, and remotely read back the terminal plan. | — | 38, 39 | `push` | `planned` | Exact push authority matches the archive checkpoint; PlanRunV1 is finished with bound acceptance and passed completion review; the archive is remotely readable and no preflight ref was deleted. |
...
| A16 | `node plugins/session-relay/test/release-instance-contract.mjs && node plugins/session-relay/test/distribution-contract.mjs && node plugins/session-relay/test/companion-distribution-contract.mjs` | Exit 0; every current 0.16.0 parent oracle binds this plan's live `run_id` and immutable tag source `875661640c1c45894c66a9b2fdc437fd47307e1f`, while the finished child, three asset digests, historical instances, and frozen identity literals remain unchanged. |
| A17 | `node plugins/session-relay/test/release-evidence-contract.mjs && node plugins/session-relay/test/release-promotion-contract.mjs` | Exit 0; immutable release source and later PlanRun source remain separate, original closed V3 ancestry keys remain exact, and preparation plus both promotion consumers independently refuse unrelated PlanRun source. |
| A18 | `node plugins/session-relay/test/release-promotion-contract.mjs` | Exit 0 with the released child shape: `source_base` equals implementation, `execution_parent` is distinct, finished UTC uses `+00:00`, and both source/parent ancestry checks pass; unrelated source and invalid timestamps are refused. |
| A19 | `node plugins/session-relay/test/release-publication-contract.mjs` | Exit 0; `--publish-reviewed --rebind-complete-publication` takes the read-only early-return path, emits a local reconciled receipt, and invokes no branch, tag, release, or asset mutator. |
| A20 | `node plugins/session-relay/test/release-evidence-contract.mjs` | Exit 0; the source-proof compatibility fixture validates with exactly the original V3 ancestry keys and a distinct live `plan_run.source_base`. Before step:restore_closed_v3, this fixture fails `current source proof ancestry has unknown or missing fields`. |
...
13. Any current 0.16.0 release instance or independent distribution oracle still names a blocked
    or completed predecessor run instead of this successor, or refreshing identity changes the
    immutable tag source, child, asset digests, historical instance, or frozen receipt bytes.

14. The current child verifier still collapses `source_base` and `execution_parent`, or the repair
    drops the exact request-to-execution-parent pin, weakens PlanRun shape/digest/review/content
    validation, or changes any published child, tag, asset, receipt, or historical byte.

15. Any current parent oracle still names predecessor `1d28abf4-a843-42db-9ac4-db86dcb3e420`,
    or stable promotion begins before this successor is completion-reviewed, rebound, and
    published exactly to `origin/main` with no tag or asset mutation.

16. The binder still equates the live successor `source_base` with the immutable release source,
    any required ancestry chain is untested, branch push and read-only release rebind share one Effect,
    or any repair weakens completion diff, tag, acceptance, review, child, or historical bindings.

17. A publication row implies that `publishReviewed()` pushes `main`, omits the explicit fast-forward,
    classifies the early-return rebind as mutation, or the downstream promotion consumer restores the
    false instance-source equals PlanRun-source invariant.

18. SourcePreparationProofV3 gains or loses any closed field, either live PlanRun-source ancestry
    observation disappears, canonical child UTC still fails, or timestamp acceptance admits a non-UTC offset.
...
Plan-attempt-history: {"authorization_source_sha256":"0889cde97525945382fbfa4f98b7f726fca77bdb38221c558412b63fb9ae6641","plan_bytes_sha256":"f54b53ab3236966a30e946076ca8dafa87132bd5e0d6208a3a7ef67c1b8a8014","replacement_run_id":"1d28abf4-a843-42db-9ac4-db86dcb3e420","run":{"acceptance":null,"blocker":{"evidence_sha256":"1b1176fa916a264c24abac1932afc935808ae9de74450b4221eddce8c49b9a52","kind":"verification_failed"},"completion_review":{"input_sha256":null,"invocations":0,"result_sha256":null,"state":"not_started"},"draft_review":{"accepted_classes":[],"input_sha256":"cea6d5b1c9228c5071be7f35f542cc4a8649eefccb7d788a9b1a982e4578d2d1","invocations":1,"result_sha256":"4bacd14008fea328980fb38358809d7cb5d81b7db7ae13a096959557e41632ca","state":"passed"},"execution_parent":"a869a7f895b5f68dde3b6aeec331e23fc5decab5","goal_id":"cef66d21-5bd3-4e07-a0e8-e393822dcfb0","implementation_commit":null,"plan_path":"docs/plans/active/session-relay-0.16.0-release.md","plan_sha256":"ae03e1e31bf457c54b2a7b4c67185d18d975cca9f12bf7d84cafb6a008e7234e","repository_id":"docks:/home/vagrant/projects/docks","requested_effects":["local","probe","push","release"],"risk":"external","run_id":"09715c97-4d55-49d9-a268-b31c7e39cf38","schema":1,"source_base":"a869a7f895b5f68dde3b6aeec331e23fc5decab5","source_sha256":"c89f014011c8612f45251247873ab29e8b18e99527917d0f94b51e104728c5eb"},"schema":1,"status":"blocked","successor_run_sha256":"01d90336529781b50bfd6856a2940c816f5bec1cbf83d4749ee4e9292c39d274"}
Plan-attempt-history: {"authorization_source_sha256":"0889cde97525945382fbfa4f98b7f726fca77bdb38221c558412b63fb9ae6641","plan_bytes_sha256":"1f81a8a10b84457de3d6c37e4e37ea4f64857c1d2ea8207886711def53a2947b","replacement_run_id":"5725e53d-7a0d-4d5b-aadd-bd2fa993ed7b","run":{"acceptance":{"source_sha256":"faf6f5dee8018b06d10a765198e71e2646bec9b0da9697f2011aededd148f407","verification_sha256":"4d8dddcfb844a16760a59649e15f67c4cfcf33876a8e0901bf9044a0d6efa5a2"},"blocker":{"evidence_sha256":"5f61e4a494d0b557ffd39bf8ada763b2de1f1d18d4c753fa2967a611ef5da596","kind":"concurrent_change"},"completion_review":{"accepted_classes":[],"input_sha256":"991b094569f53adc60160a7c183c4da89aa411bf08a20164c0f5fcfbc4f0ea41","invocations":1,"result_sha256":"571e3f94b246958acb726a0dda5e78eea76c1ce0119bfab60240986b91d90b5f","state":"passed"},"draft_review":{"accepted_classes":["v1_acceptance_output_mismatch"],"input_sha256":"1c4330e0d8eda9f3a257b1410e34a4435f1b1108c3fc26242de005474a980f79","invocations":2,"result_sha256":"e3d6efa8b80a372f6ae78083fd698a47e4c12d01eaecc12e0b5c2cf705a137a0","state":"passed"},"execution_parent":"a869a7f895b5f68dde3b6aeec331e23fc5decab5","goal_id":"cef66d21-5bd3-4e07-a0e8-e393822dcfb0","implementation_commit":"b8ef713f979f96a07af26cc245235b3605ddabcd","plan_path":"docs/plans/active/session-relay-0.16.0-release.md","plan_sha256":"c158f868965add8d1522eb72d7b7f485ae2df756e2b70955360670cc182b2cd0","repository_id":"docks:/home/vagrant/projects/docks","requested_effects":["local","probe","push","release"],"risk":"external","run_id":"1d28abf4-a843-42db-9ac4-db86dcb3e420","schema":1,"source_base":"875661640c1c45894c66a9b2fdc437fd47307e1f","source_sha256":"6ce9e198ef90247e9c01b83a1530319d81ca46cd94f763f9cb9c07b9e341728a"},"schema":1,"status":"blocked","successor_run_sha256":"744c01c3b772d86cf4dc48b166881aba6df7b2ba11139c76233a206a0bf8f71e"}
Plan-attempt-history: {"authorization_source_sha256":"0889cde97525945382fbfa4f98b7f726fca77bdb38221c558412b63fb9ae6641","plan_bytes_sha256":"40ed194640fde2b69268dbd16d60cd1426623807e5c442d2f7371dfd631f8361","replacement_run_id":"37f87dd0-7e3e-4cfb-94a7-51066eb8ed7c","run":{"acceptance":null,"blocker":{"evidence_sha256":"44915c02aac6c91d2a119f51c9a38a2cc8b9474a1cc95ed0e5f4e884d53055b1","kind":"verification_failed"},"completion_review":{"input_sha256":null,"invocations":0,"result_sha256":null,"state":"not_started"},"draft_review":{"accepted_classes":[],"input_sha256":"5b862c245399992713b6664a42dcd00e13625d50958cb3c2c367bd51d8abaf3d","invocations":1,"result_sha256":"f6a173b4a10dfec8ab79d72606e2cf98f51e6d795e14500c58814e6edffd2396","state":"passed"},"execution_parent":"e3f896f9af6427853842ded1c3d7ac10970ecf40","goal_id":"cef66d21-5bd3-4e07-a0e8-e393822dcfb0","implementation_commit":null,"plan_path":"docs/plans/active/session-relay-0.16.0-release.md","plan_sha256":"53aae343c940716ce41df13891c61a7e3f9204f118f374bc31e778c28819ed2f","repository_id":"docks:/home/vagrant/projects/docks","requested_effects":["local","probe","push","release"],"risk":"external","run_id":"5725e53d-7a0d-4d5b-aadd-bd2fa993ed7b","schema":1,"source_base":"e713805e37c57d21844250fdb4e8a26219e2d0dd","source_sha256":"2f4bfac253752bb75a294a6d04191756422fd76a59eb0165d192bba59ea4744a"},"schema":1,"status":"blocked","successor_run_sha256":"1194ef829373dbbcae92004a2262057f005a5f2cda62292ff720cacfda401316"}
Plan-attempt-history: {"authorization_source_sha256":"0889cde97525945382fbfa4f98b7f726fca77bdb38221c558412b63fb9ae6641","plan_bytes_sha256":"61b5fa6bbf88aa2ae719a6b26efd25b5c48cfdd41de3317699d18b1c3b3d85c6","replacement_run_id":"3c2a2253-3999-464f-b58c-055bf60604e1","run":{"acceptance":null,"blocker":{"evidence_sha256":"b8c5b130231035bb5d4a9c9c718a1eb926db170b155c84e5ca58408e9985d70e","kind":"review_failed"},"completion_review":{"input_sha256":null,"invocations":0,"result_sha256":null,"state":"not_started"},"draft_review":{"accepted_classes":["v1_unauthorized_effect","v1_unstable_step_reference"],"input_sha256":"0d7434a9057043c99ea7aa863367c05eafb6331c73411ece6cdb1d48369f64df","invocations":2,"result_sha256":"b8c5b130231035bb5d4a9c9c718a1eb926db170b155c84e5ca58408e9985d70e","state":"blocked"},"execution_parent":null,"goal_id":"cef66d21-5bd3-4e07-a0e8-e393822dcfb0","implementation_commit":null,"plan_path":"docs/plans/active/session-relay-0.16.0-release.md","plan_sha256":"3456a607fdea4e1bbfda1ca20f96e30c534bde6e27fb8b65c9aae6d37ba6439a","repository_id":"docks:/home/vagrant/projects/docks","requested_effects":["local","probe","push","release"],"risk":"external","run_id":"37f87dd0-7e3e-4cfb-94a7-51066eb8ed7c","schema":1,"source_base":"32c7c6be6651361baba1b351ba0cd29edbcf9d46","source_sha256":"af2e6275569758a1d0ed1d6ba18b27679ed4bc950f56b236eb92692561cade37"},"schema":1,"status":"blocked","successor_run_sha256":"57135c044cfb6cc2e8b1bbb419a284946187d82452ef80377f384e2158e2b56e"}
Plan-attempt-history: {"authorization_source_sha256":"0889cde97525945382fbfa4f98b7f726fca77bdb38221c558412b63fb9ae6641","plan_bytes_sha256":"1f59da9d38219eebd781033b415d713e3f39543ec262ddc1fce53f7802599b1b","replacement_run_id":"154424d1-70e3-4276-bd34-cb0c91ef2947","run":{"acceptance":{"source_sha256":"09a9c05707cbd3b46aeb1aaef36dd590e93c683845498eeba10d301f20ec78c3","verification_sha256":"4d56f0d29132df7340eacddca62419d1aeed7500749b9ee6d8f5f5b8576d9418"},"blocker":{"evidence_sha256":"701e1027f07cda34beda6ce4a2850235ecddc80035ff5c7b83d4239a7cf951c1","kind":"concurrent_change"},"completion_review":{"accepted_classes":[],"input_sha256":"2133ad035ab4a64e338c78c925462386ae0776662887d5afd9b1c99d8232e8a6","invocations":1,"result_sha256":"f5a9027e5857498c9e73409c5f3b45fbefa60b6dd101f92738500415bae86b8c","state":"passed"},"draft_review":{"accepted_classes":["v1_unauthorized_effect"],"input_sha256":"092baad03da1c274e1ba6819867d102d6a2cae2416130b3f79165db7ba26a864","invocations":2,"result_sha256":"0ab627430061fca79931599126c1b10f2a0b2a76f00686b47a7d40c3bac1b16c","state":"passed"},"execution_parent":"0d46ab0a7e38145b0217858258732a3606b9d791","goal_id":"cef66d21-5bd3-4e07-a0e8-e393822dcfb0","implementation_commit":"03f79233fd0d5275b0610aa2d20aa011e6417269","plan_path":"docs/plans/active/session-relay-0.16.0-release.md","plan_sha256":"4bf2f55eee348ad9b1751fe0a888031130f3c666639b8aadfecc5017f8ca4c5f","repository_id":"docks:/home/vagrant/projects/docks","requested_effects":["local","probe","push","release"],"risk":"external","run_id":"3c2a2253-3999-464f-b58c-055bf60604e1","schema":1,"source_base":"31da76812dd9dc5f037bde50b66a23d85345533e","source_sha256":"565793a305c07c49f5471b7329c413a8fa4118c922538f465fa396dba870a75f"},"schema":1,"status":"blocked","successor_run_sha256":"04489519b38464230375d06856a3d4cda6cb6c1a6889f38f8fff2d42a6fc4483"}
Plan-attempt-history: {"authorization_source_sha256":"0889cde97525945382fbfa4f98b7f726fca77bdb38221c558412b63fb9ae6641","plan_bytes_sha256":"83594ffda2e96d421a233b1651c0b0165f4e69297e4a696842660edeac7aab5f","replacement_run_id":"5a4c1c26-4084-4488-8ced-f49c85848080","run":{"acceptance":null,"blocker":{"evidence_sha256":"6719c997677d200ac8440145257319de432d73cb3737ef07d781356a93971c41","kind":"review_failed"},"completion_review":{"input_sha256":null,"invocations":0,"result_sha256":null,"state":"not_started"},"draft_review":{"accepted_classes":["v1_acceptance_command_not_runnable","v1_contract_contradiction"],"input_sha256":"04e47211de8372c8bb8869c42a13e9b1b8375d014f21e6f276ff7434860b9e18","invocations":2,"result_sha256":"6719c997677d200ac8440145257319de432d73cb3737ef07d781356a93971c41","state":"blocked"},"execution_parent":null,"goal_id":"cef66d21-5bd3-4e07-a0e8-e393822dcfb0","implementation_commit":null,"plan_path":"docs/plans/active/session-relay-0.16.0-release.md","plan_sha256":"1a9adb60bac96d854666bfdb4270453736e69b3179d4ae83777f3ea7812b1320","repository_id":"docks:/home/vagrant/projects/docks","requested_effects":["local","probe","push","release"],"risk":"external","run_id":"154424d1-70e3-4276-bd34-cb0c91ef2947","schema":1,"source_base":"9731c62aa160a8c0a3aa847d62630a901c76f518","source_sha256":"162abb6e4aedb8ec607674825e669544a885e70a30b63f3a0ff9ec84e8b2e28c"},"schema":1,"status":"blocked","successor_run_sha256":"ae7915d6ef28097c599144f618c7d7b5f9de042e9f7a73ace2c48924e22e8161"}
...
Completion-review-result: {"diff_sha256":"2a28870a3077169ce2c99b3a009f03c3742f0786365c2604fc569b20663e1859","findings":[],"implementation_commit":"b8ef713f979f96a07af26cc245235b3605ddabcd","invocation":1,"run_id":"1d28abf4-a843-42db-9ac4-db86dcb3e420","schema":1,"verdict":"pass"}


Execution-blocker: {"evidence_sha256":"5f61e4a494d0b557ffd39bf8ada763b2de1f1d18d4c753fa2967a611ef5da596","kind":"concurrent_change","reason":"post-review public child verification rejected the canonical distinct source_base and execution_parent identities"}


Completion review invocation 1:

Completion-review-result: {"diff_sha256":"82dcb5849a78e78d5a9317c05aac01568a412417c14402ff90b889da0eaa6c4e","findings":[],"implementation_commit":"03f79233fd0d5275b0610aa2d20aa011e6417269","invocation":1,"run_id":"3c2a2253-3999-464f-b58c-055bf60604e1","schema":1,"verdict":"pass"}
