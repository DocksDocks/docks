...
Archived 2026-07-28 at the repository owner's explicit instruction. This plan's
goal was delivered outside its own lifecycle, so it is kept as history rather
than as pending work.

`session-relay--v0.13.0` was tagged 2026-07-23, subject
`fix(session-relay): recertify 0.13 on current main`, under
`docs/plans/finished/2026-07-23-session-relay-linux-workspace-recertification.md`,
which reached `finished` and names this plan directly. The capability this plan
was preparing is documented as shipped in `plugins/session-relay/AGENTS.md`:
managed writing is Linux/ext4 only, with the exact frozen macOS refusal preserved
for ordinary Relay support. The tree has since moved to `0.14.0`, also tagged.

Frontmatter status, every record line, and the classification are byte-identical
before and after this move. No record was rewritten and no status was flipped to
`finished`, which would have relabelled the legacy family. Nothing here grants
