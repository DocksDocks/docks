...
updated: "2026-07-07T00:18:36-03:00"
ship_commit: "745a269c7a0af1da9b7764064a3f20d0938e193e"
...
- **2026-07-06T23:22:06-03:00 — worker completed steps 2, 3, and 5; step 4 left to orchestrator:**
  - Step 2 commit: `dc133cd feat(session-relay): report wake token usage` — `relay wake` now uses raw stdout byte pass-through, `RELAY_WAKE_CMD_CLAUDE` / `RELAY_WAKE_CMD_CODEX`, fixture-based usage parsing, and best-effort stderr summaries.
  - Step 3 commit: `86d6bc8 docs(session-relay): add token discipline guidance` — added `## Token discipline` with the five required rules and a BAD/GOOD wake pair; refreshed `metadata.content_hash`.
  - Step 5 commit: `275dd81 test(session-relay): cover wake usage output` — selftest stubs assert usage summaries, stdout byte identity, no-trailing-newline preservation, and garbage-output no-summary behavior.
  - Local preflight before plan update: `git diff --check` passed; `node --check plugins/session-relay/test/selftest.mjs` passed; `node scripts/skills/content-hash.mjs --check-only plugins/session-relay/skills` reported `unchanged productivity/session-relay`.
  - Worker shell PATH did not include `cargo` (`cargo fmt --check`, `cargo clippy -- -D warnings`, and `cargo test` initially returned exit 127), but the toolchain was available at `/home/docks/.cargo/bin/cargo`.
  - Final verification: `/home/docks/.cargo/bin/cargo fmt --check` passed; `/home/docks/.cargo/bin/cargo clippy -- -D warnings` passed; `/home/docks/.cargo/bin/cargo test` passed (36 unit tests, 1 bus smoke integration test, 2 lock-race integration tests, doctests); `node plugins/session-relay/test/selftest.mjs` passed 70 checks; `node scripts/ci.mjs --plugin session-relay` passed with the expected local host-binary digest warning.
- **2026-07-06T23:27:06-03:00 (orchestrator) — step 4 lean-boot measurement: `--lean` does NOT ship.**
  - Spec followed: median of 3 per variant, fresh scratch cwd per run, fixed order baseline→lever1→lever2. Versions: claude 2.1.202, codex-cli 0.142.5. Claude metric = `usage.input_tokens + cache_creation_input_tokens + cache_read_input_tokens`; codex metric = `usage.input_tokens`. Cheap tiers used (sonnet low / codex low) — boot context size is model-independent.
  - **Claude medians (total input):** baseline 45,632 · `--strict-mcp-config` 45,456 (−176, −0.4%) · `--setting-sources user` 45,613 (−19, ~0%). Both levers fail the ≥25% AND ≥5k gate by two orders of magnitude.
  - **Codex medians (input_tokens):** baseline 22,183 · `--ignore-user-config` 12,985 (−9,198, −41.5%) — token gate PASSED.
  - **Functional-safety probe (codex lever): FAILED.** Control: plain `codex exec` in a scratch dir → new bus roster entry appears (`relay list` diff shows the session). Lean: identical run with `--ignore-user-config` → NO roster entry (diff empty). The lever drops the user-scoped session-relay plugin hooks, so the child never registers — disqualified per STOP condition 3 ("never ship a lever that breaks the bus"). Wake-drain probe unnecessary: one failed probe already disqualifies.
  - **Verdict: no `--lean` flag on either tool.** Finding recorded as skill guidance (one paragraph after rule 5 in `## Token discipline`, hash refreshed). The Interfaces lean-mapping table stays unused by design ("what's not worth it" outcome).
