...
updated: "2026-07-20T03:08:20.377Z"
...
- **Goal met:** partial
- **Regressions:** ["Disposable\u002dcheckout full CI fails at the Session Relay self\u002dtest\u002e","The A9 release command is non\u002drepeatable after the already\u002dpublished docks\u002d\u002dv0\u002e13\u002e1 tag\u002e"]
- **CI:** {"command":"node scripts\u002fci\u002emjs \u0028executed inside A7\u0029","exit_code":1,"first_failure":"session\u002drelay self\u002dtest failed in the helper\u002downed disposable checkout","output_sha256":"a6fd628c3dcbb0334b86ef73c73e7162e7c0eae32549f3d0073683eea8d51fe7"}
- **Follow-ups:** ["Repair the completion\u002donly acceptance ladder without changing released implementation bytes\u002c then run schema\u002d6 repair round two\u002e"]
- **Filed by:** {"role":"plan-manager","receipt_author":{"company":"openai","tool":"codex","model":"gpt\u002d5\u002e6\u002dsol","effort":"high"},"reviewed_at":"2026\u002d07\u002d20T03\u003a08\u003a20\u002e377Z"}
- **Primary review:** {"accepted":[],"date":"2026-07-20","finding_count":0,"orchestrator":{"company":"openai","effort":"high","model":"gpt-5.6-sol","tool":"codex"},"rejected":[],"reproduced_ids":[],"result":"passed","selected":{"company":"openai","effort":"high","model":"gpt-5.6-sol","service_tier":"default","tool":"codex"},"verdict":"pass"}

Completion-review-receipt: {"acceptance_inventory":{"criteria":[{"command":"env DOCKS_REVIEW_POLICY_ORCHESTRATION_ORACLE=1 node scripts/tests/plan-review-policy-regressions.mjs --orchestration-oracle","expected":"Exit 0; a temporary Git repository proves candidate zero dispatches once from empty prior attempts and candidate one dispatches once only after a matching parent candidate-zero commitment plus persisted/hash-bound availability evidence; `dispatchCommittedReviewer` reads exact current plan-only HEAD, compares worktree bytes, independently verifies the committed sealed-bundle digest and safe reviewer-workspace sentinel, and calls its adapter only for matching committed argv and actual `orchestrator_tool/600`. Stale/missing/substituted commit/blob/worktree/path/bundle/workspace/sentinel/config/argv/prior-attempt/history and proposed 650 cases call it zero times; terminal and repair invariants remain closed.","id":"A1"},{"command":"node scripts/tests/plan-review-policy-regressions.mjs --self-test","expected":"Exit 0; mutations kill missing commit/read-back, stale/non-plan/multi-parent commitment commits, uncommitted worktree drift, unsafe backslash/traversal paths, request/policy/candidate/bundle/workspace/sentinel/argv/config substitution, missing/reordered/unhashed prior attempts, missing parent commitments, post-validation replacement, adapter double-call/spawn-before-gate, stale repair records, stdout self-attestation, user-byte/base64/hash drift, parent ancestry drift, terminal/receipt/series crossover, same-input CAS, finished reopen, and weakened V1 preconditions.","id":"A2"},{"command":"node scripts/tests/plan-review-policy.mjs --case surfaces","expected":"Exit 0; review schema 6, independent current V1 record families, StateV2, validation-only StateV1, and historical review schemas 1–5 remain closed; eligible V1 inputs normalize to equivalent nonterminal V2 only after existing preconditions, including unchanged stale settlement behavior.","id":"A3"},{"command":"node scripts/tests/plan-skill-phases.mjs","expected":"Exit 0; five-skill ownership and generated manager-wrapper parity require commit-before-launch and reserve abandonment authority to main-context plan-manager/current-user input.","id":"A4"},{"command":"node scripts/skills/content-hash.mjs --check-only","expected":"Exit 0; every changed skill hash matches generated content.","id":"A5"},{"command":"node scripts/ci.mjs --plugin docks","expected":"Exit 0; Docks plus repo-wide targeted release gate is green.","id":"A6"},{"command":"IMPLEMENTATION_SHA_FILE=/tmp/docks-plan-review-controller-failure-recovery-implementation.sha IMPLEMENTATION_REF=refs/docks/release/docks-0.13.1-tested && test -z \"$(git status --porcelain=v1 --untracked-files=all)\" && git diff --quiet && git diff --cached --quiet && git rev-parse HEAD > \"$IMPLEMENTATION_SHA_FILE\" && git update-ref \"$IMPLEMENTATION_REF\" \"$(cat \"$IMPLEMENTATION_SHA_FILE\")\" && node scripts/ci.mjs && test \"$(git rev-parse HEAD)\" = \"$(cat \"$IMPLEMENTATION_SHA_FILE\")\" && test \"$(git rev-parse \"$IMPLEMENTATION_REF\")\" = \"$(cat \"$IMPLEMENTATION_SHA_FILE\")\" && test -z \"$(git status --porcelain=v1 --untracked-files=all)\" && git diff --quiet && git diff --cached --quiet","expected":"Exit 0 once; full CI passes and exact clean implementation HEAD/file/ref remain identical.","id":"A7"},{"command":"IMPLEMENTATION_SHA_FILE=/tmp/docks-plan-review-controller-failure-recovery-implementation.sha IMPLEMENTATION_REF=refs/docks/release/docks-0.13.1-tested && test \"$(cat \"$IMPLEMENTATION_SHA_FILE\")\" = \"$(git rev-parse \"$IMPLEMENTATION_REF\")\" && test \"$(git rev-parse HEAD)\" = \"$(git rev-parse \"$IMPLEMENTATION_REF\")\" && test -z \"$(git status --porcelain=v1 --untracked-files=all)\" && node scripts/release.mjs --dry-run --plugin docks patch && test \"$(git rev-parse HEAD)\" = \"$(git rev-parse \"$IMPLEMENTATION_REF\")\" && test -z \"$(git status --porcelain=v1 --untracked-files=all)\"","expected":"Exit 0 without mutation and resolve `0.13.0 → 0.13.1`.","id":"A8"},{"command":"IMPLEMENTATION_COMMIT=$(git rev-parse refs/docks/release/docks-0.13.1-tested) && test \"$(git rev-parse HEAD)\" = \"$IMPLEMENTATION_COMMIT\" && node scripts/release.mjs --plugin docks patch && RELEASE_COMMIT=$(git rev-parse 'docks--v0.13.1^{commit}') && test \"$(git show -s --format='%P' \"$RELEASE_COMMIT\")\" = \"$IMPLEMENTATION_COMMIT\"","expected":"Exit 0; release commit is the direct child of the bound implementation commit and version triples are `0.13.1`.","id":"A9"},{"command":"RELEASE_TAG=docks--v0.13.1 RELEASE_COMMIT=$(git rev-parse 'docks--v0.13.1^{commit}') && test \"$RELEASE_COMMIT\" = \"$(git rev-parse HEAD)\" && test \"$(gh run list --repo DocksDocks/docks --commit \"$RELEASE_COMMIT\" --event push --json status,conclusion,headBranch,headSha --limit 20 --jq \"map(select(.headBranch == \"$RELEASE_TAG\" and .headSha == \"$RELEASE_COMMIT\" and .status == \"completed\" and .conclusion == \"success\")) | length\")\" = 1 && test \"$(gh release view \"$RELEASE_TAG\" --repo DocksDocks/docks --json isDraft --jq .isDraft)\" = false && test \"$(gh release view \"$RELEASE_TAG\" --repo DocksDocks/docks --json isPrerelease --jq .isPrerelease)\" = false","expected":"Exit 0; exactly one successful tag run and one published stable GitHub Release bind the release commit.","id":"A10"},{"command":"RELEASE_COMMIT=$(git rev-parse 'docks--v0.13.1^{commit}') && codex plugin marketplace upgrade docks --json && codex plugin add docks@docks --json && claude plugin update docks@docks --scope user && CLAUDE_HELPER=\"$HOME/.claude/plugins/cache/docks/docks/0.13.1/skills/productivity/plan-reviewer/scripts/review-policy.mjs\" CODEX_HELPER=\"$HOME/.codex/plugins/cache/docks/docks/0.13.1/skills/productivity/plan-reviewer/scripts/review-policy.mjs\" && git show \"$RELEASE_COMMIT:plugins/docks/skills/productivity/plan-reviewer/scripts/review-policy.mjs\" | cmp - \"$CLAUDE_HELPER\" && cmp \"$CLAUDE_HELPER\" \"$CODEX_HELPER\" && env DOCKS_REVIEW_POLICY_ORCHESTRATION_ORACLE=1 DOCKS_REVIEW_POLICY_HELPER=\"$CLAUDE_HELPER\" node scripts/tests/plan-review-policy-regressions.mjs --orchestration-oracle && node scripts/tests/plan-skill-phases.mjs --case installed-catalogs --version 0.13.1","expected":"Exit 0; both installed caches equal the tagged helper and pass the new oracle plus five-phase catalog contract.","id":"A11"},{"command":"RELEASE_COMMIT=$(git rev-parse 'docks--v0.13.1^{commit}') && test \"$(git rev-parse \"$RELEASE_COMMIT:docs/plans/finished/2026-07-19-session-relay-prebuilt-cli-release.md\")\" = 824bb01733482c80eb1769f216791d8957c0039c && test \"$(git show \"$RELEASE_COMMIT:docs/plans/finished/2026-07-19-session-relay-prebuilt-cli-release.md\" | sha256sum)\" = \"dac73a2ae909485cbfd11d7319cfc23d06cd6f5ff89249ec198c390db0545957  -\" && test \"$(git rev-parse \"$RELEASE_COMMIT:docs/plans/active/plan-workflow-phases-and-loop-escape.md\")\" = b9723dd96bd1ea4231c619a7348f9c9bf92cf73b && test \"$(git show \"$RELEASE_COMMIT:docs/plans/active/plan-workflow-phases-and-loop-escape.md\" | sha256sum)\" = \"164ee5727c4b80e9fbe2a6ea5e571695312fd15c4bce15fc6ecf900d2e45e4fc  -\"","expected":"Exit 0; the released helper leaves the finished Session Relay archive and completed workflow dependency exactly at baseline.","id":"A12"}],"schema":1},"acceptance_inventory_sha256":"be85a939749425192efafff07adae82feeb59cffe20eafc102605e14e9abb985","completion_verdict":"regressed","diff_sha256":"fbf628f79c93a0315ce86b8db3d5c0a1aeb86557d8b1f731c56ea5b3cac40ee2","execution_base_commit":"b8735e9aa1a3a2dff8df284e0c706860d3acc24f","outcome":"passed","phase":"completion","plan_input_sha256":"869caa73b890b720e87b4e97a0fee45c7af9f20423e207a4583460c51253e06c","planned_at_commit":"41e61f4fdd677556c31de3e89343071d7ac67172","policy":{"candidates":[{"company":"openai","effort":"high","model":"gpt-5.6-sol","service_tier":"default","tool":"codex"},{"company":"anthropic","effort":"high","model":"fable","tool":"claude"},{"company":"anthropic","effort":"xhigh","model":"opus","tool":"claude"}],"fallback":"availability_only","max_rounds":2,"provenance":{"candidates":"skill_default","fallback":"skill_default","max_rounds":"skill_default","role":"skill_default"},"role":"primary","schema":6},"policy_sha256":"bb95e1516f9fc1b6f4d8a75991d4650428428dc35d842db1710f4d64dc082a1b","primary":{"acceptance":[{"actual_sha256":"60aacfe926e4d00cd5e410d14ae67ee097dbd7edf49c452e9079476c93ca3bd6","command":"env DOCKS_REVIEW_POLICY_ORCHESTRATION_ORACLE=1 node scripts/tests/plan-review-policy-regressions.mjs --orchestration-oracle","criterion_id":"A1","exit_code":0,"expected":"Exit 0; a temporary Git repository proves candidate zero dispatches once from empty prior attempts and candidate one dispatches once only after a matching parent candidate-zero commitment plus persisted/hash-bound availability evidence; `dispatchCommittedReviewer` reads exact current plan-only HEAD, compares worktree bytes, independently verifies the committed sealed-bundle digest and safe reviewer-workspace sentinel, and calls its adapter only for matching committed argv and actual `orchestrator_tool/600`. Stale/missing/substituted commit/blob/worktree/path/bundle/workspace/sentinel/config/argv/prior-attempt/history and proposed 650 cases call it zero times; terminal and repair invariants remain closed.","met":true},{"actual_sha256":"50dd924acacb6da59a3978caf0ccc2531607bb6320d6a5b20ffcbb3c2bde6816","command":"node scripts/tests/plan-review-policy-regressions.mjs --self-test","criterion_id":"A2","exit_code":0,"expected":"Exit 0; mutations kill missing commit/read-back, stale/non-plan/multi-parent commitment commits, uncommitted worktree drift, unsafe backslash/traversal paths, request/policy/candidate/bundle/workspace/sentinel/argv/config substitution, missing/reordered/unhashed prior attempts, missing parent commitments, post-validation replacement, adapter double-call/spawn-before-gate, stale repair records, stdout self-attestation, user-byte/base64/hash drift, parent ancestry drift, terminal/receipt/series crossover, same-input CAS, finished reopen, and weakened V1 preconditions.","met":true},{"actual_sha256":"d8051e6518ba7c34dde567aa76b5b0b1153be089ab216cba68587c0792dacf74","command":"node scripts/tests/plan-review-policy.mjs --case surfaces","criterion_id":"A3","exit_code":0,"expected":"Exit 0; review schema 6, independent current V1 record families, StateV2, validation-only StateV1, and historical review schemas 1–5 remain closed; eligible V1 inputs normalize to equivalent nonterminal V2 only after existing preconditions, including unchanged stale settlement behavior.","met":true},{"actual_sha256":"41da65756895255902e88473bafd4b518cbfd388a74fe5c743b3d5f9cc35d88e","command":"node scripts/tests/plan-skill-phases.mjs","criterion_id":"A4","exit_code":0,"expected":"Exit 0; five-skill ownership and generated manager-wrapper parity require commit-before-launch and reserve abandonment authority to main-context plan-manager/current-user input.","met":true},{"actual_sha256":"2dbc8c5f7b2d6a2625f806d8b2dc243a8b98340fb8dca0ee2237cbb659e8b34b","command":"node scripts/skills/content-hash.mjs --check-only","criterion_id":"A5","exit_code":0,"expected":"Exit 0; every changed skill hash matches generated content.","met":true},{"actual_sha256":"814d487f12ba5205d3407482c346bd10fd013940067bd5dc62733dc702ea0245","command":"node scripts/ci.mjs --plugin docks","criterion_id":"A6","exit_code":0,"expected":"Exit 0; Docks plus repo-wide targeted release gate is green.","met":true},{"actual_sha256":"a6fd628c3dcbb0334b86ef73c73e7162e7c0eae32549f3d0073683eea8d51fe7","command":"IMPLEMENTATION_SHA_FILE=/tmp/docks-plan-review-controller-failure-recovery-implementation.sha IMPLEMENTATION_REF=refs/docks/release/docks-0.13.1-tested && test -z \"$(git status --porcelain=v1 --untracked-files=all)\" && git diff --quiet && git diff --cached --quiet && git rev-parse HEAD > \"$IMPLEMENTATION_SHA_FILE\" && git update-ref \"$IMPLEMENTATION_REF\" \"$(cat \"$IMPLEMENTATION_SHA_FILE\")\" && node scripts/ci.mjs && test \"$(git rev-parse HEAD)\" = \"$(cat \"$IMPLEMENTATION_SHA_FILE\")\" && test \"$(git rev-parse \"$IMPLEMENTATION_REF\")\" = \"$(cat \"$IMPLEMENTATION_SHA_FILE\")\" && test -z \"$(git status --porcelain=v1 --untracked-files=all)\" && git diff --quiet && git diff --cached --quiet","criterion_id":"A7","exit_code":1,"expected":"Exit 0 once; full CI passes and exact clean implementation HEAD/file/ref remain identical.","met":false},{"actual_sha256":"98739db6e454ea4533b73e59de199172444561b1d9bc9a0bf53f3c75db83c9b1","command":"IMPLEMENTATION_SHA_FILE=/tmp/docks-plan-review-controller-failure-recovery-implementation.sha IMPLEMENTATION_REF=refs/docks/release/docks-0.13.1-tested && test \"$(cat \"$IMPLEMENTATION_SHA_FILE\")\" = \"$(git rev-parse \"$IMPLEMENTATION_REF\")\" && test \"$(git rev-parse HEAD)\" = \"$(git rev-parse \"$IMPLEMENTATION_REF\")\" && test -z \"$(git status --porcelain=v1 --untracked-files=all)\" && node scripts/release.mjs --dry-run --plugin docks patch && test \"$(git rev-parse HEAD)\" = \"$(git rev-parse \"$IMPLEMENTATION_REF\")\" && test -z \"$(git status --porcelain=v1 --untracked-files=all)\"","criterion_id":"A8","exit_code":125,"expected":"Exit 0 without mutation and resolve `0.13.0 → 0.13.1`.","met":false},{"actual_sha256":"7dfcba6eaaff1bfd6b7fd7e0d338733e1973ca6cca9f155b91e305faee293f91","command":"IMPLEMENTATION_COMMIT=$(git rev-parse refs/docks/release/docks-0.13.1-tested) && test \"$(git rev-parse HEAD)\" = \"$IMPLEMENTATION_COMMIT\" && node scripts/release.mjs --plugin docks patch && RELEASE_COMMIT=$(git rev-parse 'docks--v0.13.1^{commit}') && test \"$(git show -s --format='%P' \"$RELEASE_COMMIT\")\" = \"$IMPLEMENTATION_COMMIT\"","criterion_id":"A9","exit_code":125,"expected":"Exit 0; release commit is the direct child of the bound implementation commit and version triples are `0.13.1`.","met":false},{"actual_sha256":"42dc90ff910cbd14b4e125b0980f892b83ad064e0b160aee62d503a687205280","command":"RELEASE_TAG=docks--v0.13.1 RELEASE_COMMIT=$(git rev-parse 'docks--v0.13.1^{commit}') && test \"$RELEASE_COMMIT\" = \"$(git rev-parse HEAD)\" && test \"$(gh run list --repo DocksDocks/docks --commit \"$RELEASE_COMMIT\" --event push --json status,conclusion,headBranch,headSha --limit 20 --jq \"map(select(.headBranch == \"$RELEASE_TAG\" and .headSha == \"$RELEASE_COMMIT\" and .status == \"completed\" and .conclusion == \"success\")) | length\")\" = 1 && test \"$(gh release view \"$RELEASE_TAG\" --repo DocksDocks/docks --json isDraft --jq .isDraft)\" = false && test \"$(gh release view \"$RELEASE_TAG\" --repo DocksDocks/docks --json isPrerelease --jq .isPrerelease)\" = false","criterion_id":"A10","exit_code":125,"expected":"Exit 0; exactly one successful tag run and one published stable GitHub Release bind the release commit.","met":false},{"actual_sha256":"7f673c403659d2acc1213b2a69fd6ff1ab22ad746251b06e6602b686ea3ce9b8","command":"RELEASE_COMMIT=$(git rev-parse 'docks--v0.13.1^{commit}') && codex plugin marketplace upgrade docks --json && codex plugin add docks@docks --json && claude plugin update docks@docks --scope user && CLAUDE_HELPER=\"$HOME/.claude/plugins/cache/docks/docks/0.13.1/skills/productivity/plan-reviewer/scripts/review-policy.mjs\" CODEX_HELPER=\"$HOME/.codex/plugins/cache/docks/docks/0.13.1/skills/productivity/plan-reviewer/scripts/review-policy.mjs\" && git show \"$RELEASE_COMMIT:plugins/docks/skills/productivity/plan-reviewer/scripts/review-policy.mjs\" | cmp - \"$CLAUDE_HELPER\" && cmp \"$CLAUDE_HELPER\" \"$CODEX_HELPER\" && env DOCKS_REVIEW_POLICY_ORCHESTRATION_ORACLE=1 DOCKS_REVIEW_POLICY_HELPER=\"$CLAUDE_HELPER\" node scripts/tests/plan-review-policy-regressions.mjs --orchestration-oracle && node scripts/tests/plan-skill-phases.mjs --case installed-catalogs --version 0.13.1","criterion_id":"A11","exit_code":125,"expected":"Exit 0; both installed caches equal the tagged helper and pass the new oracle plus five-phase catalog contract.","met":false},{"actual_sha256":"da1bfc03fbad372e1fbe842ddfe3274834a9f1508d2e7e4257efdbdddcf85e1d","command":"RELEASE_COMMIT=$(git rev-parse 'docks--v0.13.1^{commit}') && test \"$(git rev-parse \"$RELEASE_COMMIT:docs/plans/finished/2026-07-19-session-relay-prebuilt-cli-release.md\")\" = 824bb01733482c80eb1769f216791d8957c0039c && test \"$(git show \"$RELEASE_COMMIT:docs/plans/finished/2026-07-19-session-relay-prebuilt-cli-release.md\" | sha256sum)\" = \"dac73a2ae909485cbfd11d7319cfc23d06cd6f5ff89249ec198c390db0545957  -\" && test \"$(git rev-parse \"$RELEASE_COMMIT:docs/plans/active/plan-workflow-phases-and-loop-escape.md\")\" = b9723dd96bd1ea4231c619a7348f9c9bf92cf73b && test \"$(git show \"$RELEASE_COMMIT:docs/plans/active/plan-workflow-phases-and-loop-escape.md\" | sha256sum)\" = \"164ee5727c4b80e9fbe2a6ea5e571695312fd15c4bce15fc6ecf900d2e45e4fc  -\"","criterion_id":"A12","exit_code":125,"expected":"Exit 0; the released helper leaves the finished Session Relay archive and completed workflow dependency exactly at baseline.","met":false}],"ci":{"command":"node scripts/ci.mjs (executed inside A7)","exit_code":1,"first_failure":"session-relay self-test failed in the helper-owned disposable checkout","output_sha256":"a6fd628c3dcbb0334b86ef73c73e7162e7c0eae32549f3d0073683eea8d51fe7"},"findings":[{"defect":"The helper-owned completion checkout fails the mandatory full repository gate at the Session Relay self-test, so the ordered acceptance ladder cannot complete.","fix":"Make completion verification provide a checkout-safe Session Relay host/runtime path or replace A7 with immutable release evidence that passes from the disposable checkout, then rerun the full ladder.","id":"P1","locator":"A7 full repository gate","path":"plugins/session-relay/test/selftest.mjs","reproduction":{"command":"IMPLEMENTATION_SHA_FILE=/tmp/docks-plan-review-controller-failure-recovery-implementation.sha IMPLEMENTATION_REF=refs/docks/release/docks-0.13.1-tested && test -z \"$(git status --porcelain=v1 --untracked-files=all)\" && git diff --quiet && git diff --cached --quiet && git rev-parse HEAD > \"$IMPLEMENTATION_SHA_FILE\" && git update-ref \"$IMPLEMENTATION_REF\" \"$(cat \"$IMPLEMENTATION_SHA_FILE\")\" && node scripts/ci.mjs && test \"$(git rev-parse HEAD)\" = \"$(cat \"$IMPLEMENTATION_SHA_FILE\")\" && test \"$(git rev-parse \"$IMPLEMENTATION_REF\")\" = \"$(cat \"$IMPLEMENTATION_SHA_FILE\")\" && test -z \"$(git status --porcelain=v1 --untracked-files=all)\" && git diff --quiet && git diff --cached --quiet","evidence_sha256":"a6fd628c3dcbb0334b86ef73c73e7162e7c0eae32549f3d0073683eea8d51fe7","exit_code":1,"method":"command"},"severity":"high","source":"primary"},{"defect":"A9 is a mutating release command and is not repeatable after docks--v0.13.1 already exists; running it now would attempt a different patch release rather than verify 0.13.1.","fix":"Replace A9 completion evidence with a non-mutating ancestry/version/tag assertion bound to the existing release commit.","id":"P2","locator":"A9","path":"docs/plans/active/plan-review-controller-failure-recovery.md","reproduction":{"command":null,"evidence_sha256":"323d50ce098acc5a0ff6285913a91430989d4b3a0b088fb754e6ff35b11ab9f1","exit_code":null,"method":"read"},"severity":"high","source":"primary"}],"followups":["Repair the completion-only acceptance ladder without changing released implementation bytes, then run schema-6 repair round two."],"goal_met":"partial","regressions":["Disposable-checkout full CI fails at the Session Relay self-test.","The A9 release command is non-repeatable after the already-published docks--v0.13.1 tag."]},"reproduced":[],"request":{"acceptance_inventory_sha256":"be85a939749425192efafff07adae82feeb59cffe20eafc102605e14e9abb985","author":{"company":"openai","effort":"high","model":"gpt-5.6-sol","tool":"codex"},"bundle_sha256":"56afc192d26eec69a5f565cf954b6db79a31edc87be38a3ad7b1c75c0b98ac36","diff_sha256":"fbf628f79c93a0315ce86b8db3d5c0a1aeb86557d8b1f731c56ea5b3cac40ee2","execution_base_commit":"b8735e9aa1a3a2dff8df284e0c706860d3acc24f","input_sha256":"869caa73b890b720e87b4e97a0fee45c7af9f20423e207a4583460c51253e06c","lifecycle_intent":"none","orchestration_series_id":"96ffb16d-8b5f-4b33-9e86-493142c0cd31","orchestration_state_sha256":"d8f544275e59093145438ae04de544e99840cfac73ddbb374570373814baa50d","phase":"completion","planned_at_commit":"41e61f4fdd677556c31de3e89343071d7ac67172","policy":{"candidates":[{"company":"openai","effort":"high","model":"gpt-5.6-sol","service_tier":"default","tool":"codex"},{"company":"anthropic","effort":"high","model":"fable","tool":"claude"},{"company":"anthropic","effort":"xhigh","model":"opus","tool":"claude"}],"fallback":"availability_only","max_rounds":2,"provenance":{"candidates":"skill_default","fallback":"skill_default","max_rounds":"skill_default","role":"skill_default"},"role":"primary","schema":6},"policy_sha256":"bb95e1516f9fc1b6f4d8a75991d4650428428dc35d842db1710f4d64dc082a1b","previous_input_sha256":null,"repair_targets_sha256":null,"request_id":"e2babba3-91aa-42b4-9c2f-8aa70c812fc9","review_mode":"full","reviewed_commit_or_head":"ee53fd3c2eb6dff3fbe70dc618b86792c3d4834e","round_index":1,"schema":6},"reviewed_at":"2026-07-20T03:08:20.377Z","reviewed_head":"ee53fd3c2eb6dff3fbe70dc618b86792c3d4834e","reviewer":{"accepted_finding_ids":[],"raw":{"attempts":[{"candidate":{"company":"openai","effort":"high","model":"gpt-5.6-sol","service_tier":"default","tool":"codex"},"child_id":"5a867dc2-a327-408a-b2eb-7ca81bdd198e","denial_source":null,"exit_code":0,"output_started":true,"reason":"Completion reviewer output validated after the exact committed 600-second dispatch exited 0 without signal or authoritative denial.","result":"passed","schema":6,"signal":null,"started":true,"stderr_sha256":"cc2205f3a44e75ac3069dc1627b249ecabbfc4d866f50a6376002f4e934582f3","stdout_sha256":"67016243267a1c1515c1699859dffc0aaa8ca906307c15d271af53e321932010","timeout_mode":"orchestrator_tool","timeout_seconds":600}],"findings_sha256":"4f53cda18c2baa0c0354bb5f9a3ecbe5ed12ab4d8e11ba873c2f11161202b945","reason":null,"request":{"acceptance_inventory_sha256":"be85a939749425192efafff07adae82feeb59cffe20eafc102605e14e9abb985","author":{"company":"openai","effort":"high","model":"gpt-5.6-sol","tool":"codex"},"bundle_sha256":"56afc192d26eec69a5f565cf954b6db79a31edc87be38a3ad7b1c75c0b98ac36","diff_sha256":"fbf628f79c93a0315ce86b8db3d5c0a1aeb86557d8b1f731c56ea5b3cac40ee2","execution_base_commit":"b8735e9aa1a3a2dff8df284e0c706860d3acc24f","input_sha256":"869caa73b890b720e87b4e97a0fee45c7af9f20423e207a4583460c51253e06c","lifecycle_intent":"none","orchestration_series_id":"96ffb16d-8b5f-4b33-9e86-493142c0cd31","orchestration_state_sha256":"d8f544275e59093145438ae04de544e99840cfac73ddbb374570373814baa50d","phase":"completion","planned_at_commit":"41e61f4fdd677556c31de3e89343071d7ac67172","policy":{"candidates":[{"company":"openai","effort":"high","model":"gpt-5.6-sol","service_tier":"default","tool":"codex"},{"company":"anthropic","effort":"high","model":"fable","tool":"claude"},{"company":"anthropic","effort":"xhigh","model":"opus","tool":"claude"}],"fallback":"availability_only","max_rounds":2,"provenance":{"candidates":"skill_default","fallback":"skill_default","max_rounds":"skill_default","role":"skill_default"},"role":"primary","schema":6},"policy_sha256":"bb95e1516f9fc1b6f4d8a75991d4650428428dc35d842db1710f4d64dc082a1b","previous_input_sha256":null,"repair_targets_sha256":null,"request_id":"e2babba3-91aa-42b4-9c2f-8aa70c812fc9","review_mode":"full","reviewed_commit_or_head":"ee53fd3c2eb6dff3fbe70dc618b86792c3d4834e","round_index":1,"schema":6},"result":"passed","reviewer_output":{"checklist":{"actionability":{"evidence":"pass — `plan.review.md` Steps 1–6 name concrete tasks, affected files, dependencies, done conditions, and failure actions; the Interfaces section supplies exact schemas, signatures, invariants, and ownership boundaries needed to implement them.","status":"pass"},"dependency_order":{"evidence":"pass — The step table forms an explicit acyclic 1→2→3→4→5→6 sequence: tests precede implementation, documentation follows behavior, focused/full gates precede release, and release verification precedes completion handoff. The required changed-input review and unblock are explicitly placed before Step 1.","status":"pass"},"evidence_reverification":{"evidence":"pass — A1–A12 provide rerunnable verification from focused oracles through full CI and release checks. A7 binds a clean implementation HEAD to both a collision-specific SHA file and repository ref; A8–A12 subsequently recheck that identity, release ancestry, remote CI/Release state, installed cache bytes, and protected-plan hashes.","status":"pass"},"executable_acceptance":{"evidence":"pass — `acceptance-inventory.json` contains twelve ordered, nonempty criteria with exact shell commands and observable exit/output expectations. The ladder covers dispatch behavior, mutation resistance, schema compatibility, generated-contract parity, targeted/full CI, release ancestry, remote publication, installed artifacts, and protected baselines.","status":"pass"},"failure_modes":{"evidence":"pass — The STOP conditions explicitly halt on premature or non-600 dispatch, stale Git/worktree evidence, unsafe paths or workspaces, fallback-history defects, malformed abort or abandonment records, non-atomic repair, terminal-family crossover, invalid CAS, historical-contract drift, protected-plan mutation, and failed CI/release/cache checks. Known gotchas further distinguish invalid proposals, attempts, stdout, and administrative authorization.","status":"pass"},"goal_coverage":{"evidence":"pass — The plan directly covers every stated goal: durable prepared requests and dispatch commitments, pre-spawn controller validation, provenance-free user-authorized abandonment, eligible StateV1-to-StateV2 normalization with historical compatibility, and a bound Docks 0.13.1 release. Tests, implementation, documentation, release, and protected related-plan evidence are each represented in Steps 1–6 and A1–A12.","status":"pass"},"open_questions":{"evidence":"pass — `Open questions` explicitly records N/A and states the selected contract: durable request, exact-600 commitment, parent-aware terminal validation, authoritative dispatch boundary, separate abort and abandonment roles, and immutable Session Relay archive. No unresolved choice or placeholder remains.","status":"pass"},"standalone_executability":{"evidence":"pass — `plan.review.md` supplies the repository path, Node runtime, setup assumption, exact commands, affected paths, data shapes, reducer and dispatch contracts, ownership rules, protected files, limits, STOP conditions, and cold-handoff checklist. Execution does not depend on the prior conversation.","status":"pass"}},"findings":[],"request":{"acceptance_inventory_sha256":"be85a939749425192efafff07adae82feeb59cffe20eafc102605e14e9abb985","author":{"company":"openai","effort":"high","model":"gpt-5.6-sol","tool":"codex"},"bundle_sha256":"56afc192d26eec69a5f565cf954b6db79a31edc87be38a3ad7b1c75c0b98ac36","diff_sha256":"fbf628f79c93a0315ce86b8db3d5c0a1aeb86557d8b1f731c56ea5b3cac40ee2","execution_base_commit":"b8735e9aa1a3a2dff8df284e0c706860d3acc24f","input_sha256":"869caa73b890b720e87b4e97a0fee45c7af9f20423e207a4583460c51253e06c","lifecycle_intent":"none","orchestration_series_id":"96ffb16d-8b5f-4b33-9e86-493142c0cd31","orchestration_state_sha256":"d8f544275e59093145438ae04de544e99840cfac73ddbb374570373814baa50d","phase":"completion","planned_at_commit":"41e61f4fdd677556c31de3e89343071d7ac67172","policy":{"candidates":[{"company":"openai","effort":"high","model":"gpt-5.6-sol","service_tier":"default","tool":"codex"},{"company":"anthropic","effort":"high","model":"fable","tool":"claude"},{"company":"anthropic","effort":"xhigh","model":"opus","tool":"claude"}],"fallback":"availability_only","max_rounds":2,"provenance":{"candidates":"skill_default","fallback":"skill_default","max_rounds":"skill_default","role":"skill_default"},"role":"primary","schema":6},"policy_sha256":"bb95e1516f9fc1b6f4d8a75991d4650428428dc35d842db1710f4d64dc082a1b","previous_input_sha256":null,"repair_targets_sha256":null,"request_id":"e2babba3-91aa-42b4-9c2f-8aa70c812fc9","review_mode":"full","reviewed_commit_or_head":"ee53fd3c2eb6dff3fbe70dc618b86792c3d4834e","round_index":1,"schema":6},"role":"primary","schema":6,"verdict":"pass"},"role":"primary","schema":6,"selected":{"company":"openai","effort":"high","model":"gpt-5.6-sol","service_tier":"default","tool":"codex"},"waiver":null,"waiver_sha256":null},"rejected":[]},"schema":6,"series":{"current_input_sha256":"869caa73b890b720e87b4e97a0fee45c7af9f20423e207a4583460c51253e06c","initial_input_sha256":"869caa73b890b720e87b4e97a0fee45c7af9f20423e207a4583460c51253e06c","orchestration_series_id":"96ffb16d-8b5f-4b33-9e86-493142c0cd31","policy_sha256":"bb95e1516f9fc1b6f4d8a75991d4650428428dc35d842db1710f4d64dc082a1b","repairs":[],"rounds":[{"acceptance_inventory":{"criteria":[{"command":"env DOCKS_REVIEW_POLICY_ORCHESTRATION_ORACLE=1 node scripts/tests/plan-review-policy-regressions.mjs --orchestration-oracle","expected":"Exit 0; a temporary Git repository proves candidate zero dispatches once from empty prior attempts and candidate one dispatches once only after a matching parent candidate-zero commitment plus persisted/hash-bound availability evidence; `dispatchCommittedReviewer` reads exact current plan-only HEAD, compares worktree bytes, independently verifies the committed sealed-bundle digest and safe reviewer-workspace sentinel, and calls its adapter only for matching committed argv and actual `orchestrator_tool/600`. Stale/missing/substituted commit/blob/worktree/path/bundle/workspace/sentinel/config/argv/prior-attempt/history and proposed 650 cases call it zero times; terminal and repair invariants remain closed.","id":"A1"},{"command":"node scripts/tests/plan-review-policy-regressions.mjs --self-test","expected":"Exit 0; mutations kill missing commit/read-back, stale/non-plan/multi-parent commitment commits, uncommitted worktree drift, unsafe backslash/traversal paths, request/policy/candidate/bundle/workspace/sentinel/argv/config substitution, missing/reordered/unhashed prior attempts, missing parent commitments, post-validation replacement, adapter double-call/spawn-before-gate, stale repair records, stdout self-attestation, user-byte/base64/hash drift, parent ancestry drift, terminal/receipt/series crossover, same-input CAS, finished reopen, and weakened V1 preconditions.","id":"A2"},{"command":"node scripts/tests/plan-review-policy.mjs --case surfaces","expected":"Exit 0; review schema 6, independent current V1 record families, StateV2, validation-only StateV1, and historical review schemas 1–5 remain closed; eligible V1 inputs normalize to equivalent nonterminal V2 only after existing preconditions, including unchanged stale settlement behavior.","id":"A3"},{"command":"node scripts/tests/plan-skill-phases.mjs","expected":"Exit 0; five-skill ownership and generated manager-wrapper parity require commit-before-launch and reserve abandonment authority to main-context plan-manager/current-user input.","id":"A4"},{"command":"node scripts/skills/content-hash.mjs --check-only","expected":"Exit 0; every changed skill hash matches generated content.","id":"A5"},{"command":"node scripts/ci.mjs --plugin docks","expected":"Exit 0; Docks plus repo-wide targeted release gate is green.","id":"A6"},{"command":"IMPLEMENTATION_SHA_FILE=/tmp/docks-plan-review-controller-failure-recovery-implementation.sha IMPLEMENTATION_REF=refs/docks/release/docks-0.13.1-tested && test -z \"$(git status --porcelain=v1 --untracked-files=all)\" && git diff --quiet && git diff --cached --quiet && git rev-parse HEAD > \"$IMPLEMENTATION_SHA_FILE\" && git update-ref \"$IMPLEMENTATION_REF\" \"$(cat \"$IMPLEMENTATION_SHA_FILE\")\" && node scripts/ci.mjs && test \"$(git rev-parse HEAD)\" = \"$(cat \"$IMPLEMENTATION_SHA_FILE\")\" && test \"$(git rev-parse \"$IMPLEMENTATION_REF\")\" = \"$(cat \"$IMPLEMENTATION_SHA_FILE\")\" && test -z \"$(git status --porcelain=v1 --untracked-files=all)\" && git diff --quiet && git diff --cached --quiet","expected":"Exit 0 once; full CI passes and exact clean implementation HEAD/file/ref remain identical.","id":"A7"},{"command":"IMPLEMENTATION_SHA_FILE=/tmp/docks-plan-review-controller-failure-recovery-implementation.sha IMPLEMENTATION_REF=refs/docks/release/docks-0.13.1-tested && test \"$(cat \"$IMPLEMENTATION_SHA_FILE\")\" = \"$(git rev-parse \"$IMPLEMENTATION_REF\")\" && test \"$(git rev-parse HEAD)\" = \"$(git rev-parse \"$IMPLEMENTATION_REF\")\" && test -z \"$(git status --porcelain=v1 --untracked-files=all)\" && node scripts/release.mjs --dry-run --plugin docks patch && test \"$(git rev-parse HEAD)\" = \"$(git rev-parse \"$IMPLEMENTATION_REF\")\" && test -z \"$(git status --porcelain=v1 --untracked-files=all)\"","expected":"Exit 0 without mutation and resolve `0.13.0 → 0.13.1`.","id":"A8"},{"command":"IMPLEMENTATION_COMMIT=$(git rev-parse refs/docks/release/docks-0.13.1-tested) && test \"$(git rev-parse HEAD)\" = \"$IMPLEMENTATION_COMMIT\" && node scripts/release.mjs --plugin docks patch && RELEASE_COMMIT=$(git rev-parse 'docks--v0.13.1^{commit}') && test \"$(git show -s --format='%P' \"$RELEASE_COMMIT\")\" = \"$IMPLEMENTATION_COMMIT\"","expected":"Exit 0; release commit is the direct child of the bound implementation commit and version triples are `0.13.1`.","id":"A9"},{"command":"RELEASE_TAG=docks--v0.13.1 RELEASE_COMMIT=$(git rev-parse 'docks--v0.13.1^{commit}') && test \"$RELEASE_COMMIT\" = \"$(git rev-parse HEAD)\" && test \"$(gh run list --repo DocksDocks/docks --commit \"$RELEASE_COMMIT\" --event push --json status,conclusion,headBranch,headSha --limit 20 --jq \"map(select(.headBranch == \"$RELEASE_TAG\" and .headSha == \"$RELEASE_COMMIT\" and .status == \"completed\" and .conclusion == \"success\")) | length\")\" = 1 && test \"$(gh release view \"$RELEASE_TAG\" --repo DocksDocks/docks --json isDraft --jq .isDraft)\" = false && test \"$(gh release view \"$RELEASE_TAG\" --repo DocksDocks/docks --json isPrerelease --jq .isPrerelease)\" = false","expected":"Exit 0; exactly one successful tag run and one published stable GitHub Release bind the release commit.","id":"A10"},{"command":"RELEASE_COMMIT=$(git rev-parse 'docks--v0.13.1^{commit}') && codex plugin marketplace upgrade docks --json && codex plugin add docks@docks --json && claude plugin update docks@docks --scope user && CLAUDE_HELPER=\"$HOME/.claude/plugins/cache/docks/docks/0.13.1/skills/productivity/plan-reviewer/scripts/review-policy.mjs\" CODEX_HELPER=\"$HOME/.codex/plugins/cache/docks/docks/0.13.1/skills/productivity/plan-reviewer/scripts/review-policy.mjs\" && git show \"$RELEASE_COMMIT:plugins/docks/skills/productivity/plan-reviewer/scripts/review-policy.mjs\" | cmp - \"$CLAUDE_HELPER\" && cmp \"$CLAUDE_HELPER\" \"$CODEX_HELPER\" && env DOCKS_REVIEW_POLICY_ORCHESTRATION_ORACLE=1 DOCKS_REVIEW_POLICY_HELPER=\"$CLAUDE_HELPER\" node scripts/tests/plan-review-policy-regressions.mjs --orchestration-oracle && node scripts/tests/plan-skill-phases.mjs --case installed-catalogs --version 0.13.1","expected":"Exit 0; both installed caches equal the tagged helper and pass the new oracle plus five-phase catalog contract.","id":"A11"},{"command":"RELEASE_COMMIT=$(git rev-parse 'docks--v0.13.1^{commit}') && test \"$(git rev-parse \"$RELEASE_COMMIT:docs/plans/finished/2026-07-19-session-relay-prebuilt-cli-release.md\")\" = 824bb01733482c80eb1769f216791d8957c0039c && test \"$(git show \"$RELEASE_COMMIT:docs/plans/finished/2026-07-19-session-relay-prebuilt-cli-release.md\" | sha256sum)\" = \"dac73a2ae909485cbfd11d7319cfc23d06cd6f5ff89249ec198c390db0545957  -\" && test \"$(git rev-parse \"$RELEASE_COMMIT:docs/plans/active/plan-workflow-phases-and-loop-escape.md\")\" = b9723dd96bd1ea4231c619a7348f9c9bf92cf73b && test \"$(git show \"$RELEASE_COMMIT:docs/plans/active/plan-workflow-phases-and-loop-escape.md\" | sha256sum)\" = \"164ee5727c4b80e9fbe2a6ea5e571695312fd15c4bce15fc6ecf900d2e45e4fc  -\"","expected":"Exit 0; the released helper leaves the finished Session Relay archive and completed workflow dependency exactly at baseline.","id":"A12"}],"schema":1},"acceptance_inventory_sha256":"be85a939749425192efafff07adae82feeb59cffe20eafc102605e14e9abb985","completion_verdict":"regressed","diff_sha256":"fbf628f79c93a0315ce86b8db3d5c0a1aeb86557d8b1f731c56ea5b3cac40ee2","kind":"completion","outcome":"passed","plan_input_sha256":"869caa73b890b720e87b4e97a0fee45c7af9f20423e207a4583460c51253e06c","primary":{"acceptance":[{"actual_sha256":"60aacfe926e4d00cd5e410d14ae67ee097dbd7edf49c452e9079476c93ca3bd6","command":"env DOCKS_REVIEW_POLICY_ORCHESTRATION_ORACLE=1 node scripts/tests/plan-review-policy-regressions.mjs --orchestration-oracle","criterion_id":"A1","exit_code":0,"expected":"Exit 0; a temporary Git repository proves candidate zero dispatches once from empty prior attempts and candidate one dispatches once only after a matching parent candidate-zero commitment plus persisted/hash-bound availability evidence; `dispatchCommittedReviewer` reads exact current plan-only HEAD, compares worktree bytes, independently verifies the committed sealed-bundle digest and safe reviewer-workspace sentinel, and calls its adapter only for matching committed argv and actual `orchestrator_tool/600`. Stale/missing/substituted commit/blob/worktree/path/bundle/workspace/sentinel/config/argv/prior-attempt/history and proposed 650 cases call it zero times; terminal and repair invariants remain closed.","met":true},{"actual_sha256":"50dd924acacb6da59a3978caf0ccc2531607bb6320d6a5b20ffcbb3c2bde6816","command":"node scripts/tests/plan-review-policy-regressions.mjs --self-test","criterion_id":"A2","exit_code":0,"expected":"Exit 0; mutations kill missing commit/read-back, stale/non-plan/multi-parent commitment commits, uncommitted worktree drift, unsafe backslash/traversal paths, request/policy/candidate/bundle/workspace/sentinel/argv/config substitution, missing/reordered/unhashed prior attempts, missing parent commitments, post-validation replacement, adapter double-call/spawn-before-gate, stale repair records, stdout self-attestation, user-byte/base64/hash drift, parent ancestry drift, terminal/receipt/series crossover, same-input CAS, finished reopen, and weakened V1 preconditions.","met":true},{"actual_sha256":"d8051e6518ba7c34dde567aa76b5b0b1153be089ab216cba68587c0792dacf74","command":"node scripts/tests/plan-review-policy.mjs --case surfaces","criterion_id":"A3","exit_code":0,"expected":"Exit 0; review schema 6, independent current V1 record families, StateV2, validation-only StateV1, and historical review schemas 1–5 remain closed; eligible V1 inputs normalize to equivalent nonterminal V2 only after existing preconditions, including unchanged stale settlement behavior.","met":true},{"actual_sha256":"41da65756895255902e88473bafd4b518cbfd388a74fe5c743b3d5f9cc35d88e","command":"node scripts/tests/plan-skill-phases.mjs","criterion_id":"A4","exit_code":0,"expected":"Exit 0; five-skill ownership and generated manager-wrapper parity require commit-before-launch and reserve abandonment authority to main-context plan-manager/current-user input.","met":true},{"actual_sha256":"2dbc8c5f7b2d6a2625f806d8b2dc243a8b98340fb8dca0ee2237cbb659e8b34b","command":"node scripts/skills/content-hash.mjs --check-only","criterion_id":"A5","exit_code":0,"expected":"Exit 0; every changed skill hash matches generated content.","met":true},{"actual_sha256":"814d487f12ba5205d3407482c346bd10fd013940067bd5dc62733dc702ea0245","command":"node scripts/ci.mjs --plugin docks","criterion_id":"A6","exit_code":0,"expected":"Exit 0; Docks plus repo-wide targeted release gate is green.","met":true},{"actual_sha256":"a6fd628c3dcbb0334b86ef73c73e7162e7c0eae32549f3d0073683eea8d51fe7","command":"IMPLEMENTATION_SHA_FILE=/tmp/docks-plan-review-controller-failure-recovery-implementation.sha IMPLEMENTATION_REF=refs/docks/release/docks-0.13.1-tested && test -z \"$(git status --porcelain=v1 --untracked-files=all)\" && git diff --quiet && git diff --cached --quiet && git rev-parse HEAD > \"$IMPLEMENTATION_SHA_FILE\" && git update-ref \"$IMPLEMENTATION_REF\" \"$(cat \"$IMPLEMENTATION_SHA_FILE\")\" && node scripts/ci.mjs && test \"$(git rev-parse HEAD)\" = \"$(cat \"$IMPLEMENTATION_SHA_FILE\")\" && test \"$(git rev-parse \"$IMPLEMENTATION_REF\")\" = \"$(cat \"$IMPLEMENTATION_SHA_FILE\")\" && test -z \"$(git status --porcelain=v1 --untracked-files=all)\" && git diff --quiet && git diff --cached --quiet","criterion_id":"A7","exit_code":1,"expected":"Exit 0 once; full CI passes and exact clean implementation HEAD/file/ref remain identical.","met":false},{"actual_sha256":"98739db6e454ea4533b73e59de199172444561b1d9bc9a0bf53f3c75db83c9b1","command":"IMPLEMENTATION_SHA_FILE=/tmp/docks-plan-review-controller-failure-recovery-implementation.sha IMPLEMENTATION_REF=refs/docks/release/docks-0.13.1-tested && test \"$(cat \"$IMPLEMENTATION_SHA_FILE\")\" = \"$(git rev-parse \"$IMPLEMENTATION_REF\")\" && test \"$(git rev-parse HEAD)\" = \"$(git rev-parse \"$IMPLEMENTATION_REF\")\" && test -z \"$(git status --porcelain=v1 --untracked-files=all)\" && node scripts/release.mjs --dry-run --plugin docks patch && test \"$(git rev-parse HEAD)\" = \"$(git rev-parse \"$IMPLEMENTATION_REF\")\" && test -z \"$(git status --porcelain=v1 --untracked-files=all)\"","criterion_id":"A8","exit_code":125,"expected":"Exit 0 without mutation and resolve `0.13.0 → 0.13.1`.","met":false},{"actual_sha256":"7dfcba6eaaff1bfd6b7fd7e0d338733e1973ca6cca9f155b91e305faee293f91","command":"IMPLEMENTATION_COMMIT=$(git rev-parse refs/docks/release/docks-0.13.1-tested) && test \"$(git rev-parse HEAD)\" = \"$IMPLEMENTATION_COMMIT\" && node scripts/release.mjs --plugin docks patch && RELEASE_COMMIT=$(git rev-parse 'docks--v0.13.1^{commit}') && test \"$(git show -s --format='%P' \"$RELEASE_COMMIT\")\" = \"$IMPLEMENTATION_COMMIT\"","criterion_id":"A9","exit_code":125,"expected":"Exit 0; release commit is the direct child of the bound implementation commit and version triples are `0.13.1`.","met":false},{"actual_sha256":"42dc90ff910cbd14b4e125b0980f892b83ad064e0b160aee62d503a687205280","command":"RELEASE_TAG=docks--v0.13.1 RELEASE_COMMIT=$(git rev-parse 'docks--v0.13.1^{commit}') && test \"$RELEASE_COMMIT\" = \"$(git rev-parse HEAD)\" && test \"$(gh run list --repo DocksDocks/docks --commit \"$RELEASE_COMMIT\" --event push --json status,conclusion,headBranch,headSha --limit 20 --jq \"map(select(.headBranch == \"$RELEASE_TAG\" and .headSha == \"$RELEASE_COMMIT\" and .status == \"completed\" and .conclusion == \"success\")) | length\")\" = 1 && test \"$(gh release view \"$RELEASE_TAG\" --repo DocksDocks/docks --json isDraft --jq .isDraft)\" = false && test \"$(gh release view \"$RELEASE_TAG\" --repo DocksDocks/docks --json isPrerelease --jq .isPrerelease)\" = false","criterion_id":"A10","exit_code":125,"expected":"Exit 0; exactly one successful tag run and one published stable GitHub Release bind the release commit.","met":false},{"actual_sha256":"7f673c403659d2acc1213b2a69fd6ff1ab22ad746251b06e6602b686ea3ce9b8","command":"RELEASE_COMMIT=$(git rev-parse 'docks--v0.13.1^{commit}') && codex plugin marketplace upgrade docks --json && codex plugin add docks@docks --json && claude plugin update docks@docks --scope user && CLAUDE_HELPER=\"$HOME/.claude/plugins/cache/docks/docks/0.13.1/skills/productivity/plan-reviewer/scripts/review-policy.mjs\" CODEX_HELPER=\"$HOME/.codex/plugins/cache/docks/docks/0.13.1/skills/productivity/plan-reviewer/scripts/review-policy.mjs\" && git show \"$RELEASE_COMMIT:plugins/docks/skills/productivity/plan-reviewer/scripts/review-policy.mjs\" | cmp - \"$CLAUDE_HELPER\" && cmp \"$CLAUDE_HELPER\" \"$CODEX_HELPER\" && env DOCKS_REVIEW_POLICY_ORCHESTRATION_ORACLE=1 DOCKS_REVIEW_POLICY_HELPER=\"$CLAUDE_HELPER\" node scripts/tests/plan-review-policy-regressions.mjs --orchestration-oracle && node scripts/tests/plan-skill-phases.mjs --case installed-catalogs --version 0.13.1","criterion_id":"A11","exit_code":125,"expected":"Exit 0; both installed caches equal the tagged helper and pass the new oracle plus five-phase catalog contract.","met":false},{"actual_sha256":"da1bfc03fbad372e1fbe842ddfe3274834a9f1508d2e7e4257efdbdddcf85e1d","command":"RELEASE_COMMIT=$(git rev-parse 'docks--v0.13.1^{commit}') && test \"$(git rev-parse \"$RELEASE_COMMIT:docs/plans/finished/2026-07-19-session-relay-prebuilt-cli-release.md\")\" = 824bb01733482c80eb1769f216791d8957c0039c && test \"$(git show \"$RELEASE_COMMIT:docs/plans/finished/2026-07-19-session-relay-prebuilt-cli-release.md\" | sha256sum)\" = \"dac73a2ae909485cbfd11d7319cfc23d06cd6f5ff89249ec198c390db0545957  -\" && test \"$(git rev-parse \"$RELEASE_COMMIT:docs/plans/active/plan-workflow-phases-and-loop-escape.md\")\" = b9723dd96bd1ea4231c619a7348f9c9bf92cf73b && test \"$(git show \"$RELEASE_COMMIT:docs/plans/active/plan-workflow-phases-and-loop-escape.md\" | sha256sum)\" = \"164ee5727c4b80e9fbe2a6ea5e571695312fd15c4bce15fc6ecf900d2e45e4fc  -\"","criterion_id":"A12","exit_code":125,"expected":"Exit 0; the released helper leaves the finished Session Relay archive and completed workflow dependency exactly at baseline.","met":false}],"ci":{"command":"node scripts/ci.mjs (executed inside A7)","exit_code":1,"first_failure":"session-relay self-test failed in the helper-owned disposable checkout","output_sha256":"a6fd628c3dcbb0334b86ef73c73e7162e7c0eae32549f3d0073683eea8d51fe7"},"findings":[{"defect":"The helper-owned completion checkout fails the mandatory full repository gate at the Session Relay self-test, so the ordered acceptance ladder cannot complete.","fix":"Make completion verification provide a checkout-safe Session Relay host/runtime path or replace A7 with immutable release evidence that passes from the disposable checkout, then rerun the full ladder.","id":"P1","locator":"A7 full repository gate","path":"plugins/session-relay/test/selftest.mjs","reproduction":{"command":"IMPLEMENTATION_SHA_FILE=/tmp/docks-plan-review-controller-failure-recovery-implementation.sha IMPLEMENTATION_REF=refs/docks/release/docks-0.13.1-tested && test -z \"$(git status --porcelain=v1 --untracked-files=all)\" && git diff --quiet && git diff --cached --quiet && git rev-parse HEAD > \"$IMPLEMENTATION_SHA_FILE\" && git update-ref \"$IMPLEMENTATION_REF\" \"$(cat \"$IMPLEMENTATION_SHA_FILE\")\" && node scripts/ci.mjs && test \"$(git rev-parse HEAD)\" = \"$(cat \"$IMPLEMENTATION_SHA_FILE\")\" && test \"$(git rev-parse \"$IMPLEMENTATION_REF\")\" = \"$(cat \"$IMPLEMENTATION_SHA_FILE\")\" && test -z \"$(git status --porcelain=v1 --untracked-files=all)\" && git diff --quiet && git diff --cached --quiet","evidence_sha256":"a6fd628c3dcbb0334b86ef73c73e7162e7c0eae32549f3d0073683eea8d51fe7","exit_code":1,"method":"command"},"severity":"high","source":"primary"},{"defect":"A9 is a mutating release command and is not repeatable after docks--v0.13.1 already exists; running it now would attempt a different patch release rather than verify 0.13.1.","fix":"Replace A9 completion evidence with a non-mutating ancestry/version/tag assertion bound to the existing release commit.","id":"P2","locator":"A9","path":"docs/plans/active/plan-review-controller-failure-recovery.md","reproduction":{"command":null,"evidence_sha256":"323d50ce098acc5a0ff6285913a91430989d4b3a0b088fb754e6ff35b11ab9f1","exit_code":null,"method":"read"},"severity":"high","source":"primary"}],"followups":["Repair the completion-only acceptance ladder without changing released implementation bytes, then run schema-6 repair round two."],"goal_met":"partial","regressions":["Disposable-checkout full CI fails at the Session Relay self-test.","The A9 release command is non-repeatable after the already-published docks--v0.13.1 tag."]},"reproduced":[],"request":{"acceptance_inventory_sha256":"be85a939749425192efafff07adae82feeb59cffe20eafc102605e14e9abb985","author":{"company":"openai","effort":"high","model":"gpt-5.6-sol","tool":"codex"},"bundle_sha256":"56afc192d26eec69a5f565cf954b6db79a31edc87be38a3ad7b1c75c0b98ac36","diff_sha256":"fbf628f79c93a0315ce86b8db3d5c0a1aeb86557d8b1f731c56ea5b3cac40ee2","execution_base_commit":"b8735e9aa1a3a2dff8df284e0c706860d3acc24f","input_sha256":"869caa73b890b720e87b4e97a0fee45c7af9f20423e207a4583460c51253e06c","lifecycle_intent":"none","orchestration_series_id":"96ffb16d-8b5f-4b33-9e86-493142c0cd31","orchestration_state_sha256":"d8f544275e59093145438ae04de544e99840cfac73ddbb374570373814baa50d","phase":"completion","planned_at_commit":"41e61f4fdd677556c31de3e89343071d7ac67172","policy":{"candidates":[{"company":"openai","effort":"high","model":"gpt-5.6-sol","service_tier":"default","tool":"codex"},{"company":"anthropic","effort":"high","model":"fable","tool":"claude"},{"company":"anthropic","effort":"xhigh","model":"opus","tool":"claude"}],"fallback":"availability_only","max_rounds":2,"provenance":{"candidates":"skill_default","fallback":"skill_default","max_rounds":"skill_default","role":"skill_default"},"role":"primary","schema":6},"policy_sha256":"bb95e1516f9fc1b6f4d8a75991d4650428428dc35d842db1710f4d64dc082a1b","previous_input_sha256":null,"repair_targets_sha256":null,"request_id":"e2babba3-91aa-42b4-9c2f-8aa70c812fc9","review_mode":"full","reviewed_commit_or_head":"ee53fd3c2eb6dff3fbe70dc618b86792c3d4834e","round_index":1,"schema":6},"reviewer":{"accepted_finding_ids":[],"raw":{"attempts":[{"candidate":{"company":"openai","effort":"high","model":"gpt-5.6-sol","service_tier":"default","tool":"codex"},"child_id":"5a867dc2-a327-408a-b2eb-7ca81bdd198e","denial_source":null,"exit_code":0,"output_started":true,"reason":"Completion reviewer output validated after the exact committed 600-second dispatch exited 0 without signal or authoritative denial.","result":"passed","schema":6,"signal":null,"started":true,"stderr_sha256":"cc2205f3a44e75ac3069dc1627b249ecabbfc4d866f50a6376002f4e934582f3","stdout_sha256":"67016243267a1c1515c1699859dffc0aaa8ca906307c15d271af53e321932010","timeout_mode":"orchestrator_tool","timeout_seconds":600}],"findings_sha256":"4f53cda18c2baa0c0354bb5f9a3ecbe5ed12ab4d8e11ba873c2f11161202b945","reason":null,"request":{"acceptance_inventory_sha256":"be85a939749425192efafff07adae82feeb59cffe20eafc102605e14e9abb985","author":{"company":"openai","effort":"high","model":"gpt-5.6-sol","tool":"codex"},"bundle_sha256":"56afc192d26eec69a5f565cf954b6db79a31edc87be38a3ad7b1c75c0b98ac36","diff_sha256":"fbf628f79c93a0315ce86b8db3d5c0a1aeb86557d8b1f731c56ea5b3cac40ee2","execution_base_commit":"b8735e9aa1a3a2dff8df284e0c706860d3acc24f","input_sha256":"869caa73b890b720e87b4e97a0fee45c7af9f20423e207a4583460c51253e06c","lifecycle_intent":"none","orchestration_series_id":"96ffb16d-8b5f-4b33-9e86-493142c0cd31","orchestration_state_sha256":"d8f544275e59093145438ae04de544e99840cfac73ddbb374570373814baa50d","phase":"completion","planned_at_commit":"41e61f4fdd677556c31de3e89343071d7ac67172","policy":{"candidates":[{"company":"openai","effort":"high","model":"gpt-5.6-sol","service_tier":"default","tool":"codex"},{"company":"anthropic","effort":"high","model":"fable","tool":"claude"},{"company":"anthropic","effort":"xhigh","model":"opus","tool":"claude"}],"fallback":"availability_only","max_rounds":2,"provenance":{"candidates":"skill_default","fallback":"skill_default","max_rounds":"skill_default","role":"skill_default"},"role":"primary","schema":6},"policy_sha256":"bb95e1516f9fc1b6f4d8a75991d4650428428dc35d842db1710f4d64dc082a1b","previous_input_sha256":null,"repair_targets_sha256":null,"request_id":"e2babba3-91aa-42b4-9c2f-8aa70c812fc9","review_mode":"full","reviewed_commit_or_head":"ee53fd3c2eb6dff3fbe70dc618b86792c3d4834e","round_index":1,"schema":6},"result":"passed","reviewer_output":{"checklist":{"actionability":{"evidence":"pass — `plan.review.md` Steps 1–6 name concrete tasks, affected files, dependencies, done conditions, and failure actions; the Interfaces section supplies exact schemas, signatures, invariants, and ownership boundaries needed to implement them.","status":"pass"},"dependency_order":{"evidence":"pass — The step table forms an explicit acyclic 1→2→3→4→5→6 sequence: tests precede implementation, documentation follows behavior, focused/full gates precede release, and release verification precedes completion handoff. The required changed-input review and unblock are explicitly placed before Step 1.","status":"pass"},"evidence_reverification":{"evidence":"pass — A1–A12 provide rerunnable verification from focused oracles through full CI and release checks. A7 binds a clean implementation HEAD to both a collision-specific SHA file and repository ref; A8–A12 subsequently recheck that identity, release ancestry, remote CI/Release state, installed cache bytes, and protected-plan hashes.","status":"pass"},"executable_acceptance":{"evidence":"pass — `acceptance-inventory.json` contains twelve ordered, nonempty criteria with exact shell commands and observable exit/output expectations. The ladder covers dispatch behavior, mutation resistance, schema compatibility, generated-contract parity, targeted/full CI, release ancestry, remote publication, installed artifacts, and protected baselines.","status":"pass"},"failure_modes":{"evidence":"pass — The STOP conditions explicitly halt on premature or non-600 dispatch, stale Git/worktree evidence, unsafe paths or workspaces, fallback-history defects, malformed abort or abandonment records, non-atomic repair, terminal-family crossover, invalid CAS, historical-contract drift, protected-plan mutation, and failed CI/release/cache checks. Known gotchas further distinguish invalid proposals, attempts, stdout, and administrative authorization.","status":"pass"},"goal_coverage":{"evidence":"pass — The plan directly covers every stated goal: durable prepared requests and dispatch commitments, pre-spawn controller validation, provenance-free user-authorized abandonment, eligible StateV1-to-StateV2 normalization with historical compatibility, and a bound Docks 0.13.1 release. Tests, implementation, documentation, release, and protected related-plan evidence are each represented in Steps 1–6 and A1–A12.","status":"pass"},"open_questions":{"evidence":"pass — `Open questions` explicitly records N/A and states the selected contract: durable request, exact-600 commitment, parent-aware terminal validation, authoritative dispatch boundary, separate abort and abandonment roles, and immutable Session Relay archive. No unresolved choice or placeholder remains.","status":"pass"},"standalone_executability":{"evidence":"pass — `plan.review.md` supplies the repository path, Node runtime, setup assumption, exact commands, affected paths, data shapes, reducer and dispatch contracts, ownership rules, protected files, limits, STOP conditions, and cold-handoff checklist. Execution does not depend on the prior conversation.","status":"pass"}},"findings":[],"request":{"acceptance_inventory_sha256":"be85a939749425192efafff07adae82feeb59cffe20eafc102605e14e9abb985","author":{"company":"openai","effort":"high","model":"gpt-5.6-sol","tool":"codex"},"bundle_sha256":"56afc192d26eec69a5f565cf954b6db79a31edc87be38a3ad7b1c75c0b98ac36","diff_sha256":"fbf628f79c93a0315ce86b8db3d5c0a1aeb86557d8b1f731c56ea5b3cac40ee2","execution_base_commit":"b8735e9aa1a3a2dff8df284e0c706860d3acc24f","input_sha256":"869caa73b890b720e87b4e97a0fee45c7af9f20423e207a4583460c51253e06c","lifecycle_intent":"none","orchestration_series_id":"96ffb16d-8b5f-4b33-9e86-493142c0cd31","orchestration_state_sha256":"d8f544275e59093145438ae04de544e99840cfac73ddbb374570373814baa50d","phase":"completion","planned_at_commit":"41e61f4fdd677556c31de3e89343071d7ac67172","policy":{"candidates":[{"company":"openai","effort":"high","model":"gpt-5.6-sol","service_tier":"default","tool":"codex"},{"company":"anthropic","effort":"high","model":"fable","tool":"claude"},{"company":"anthropic","effort":"xhigh","model":"opus","tool":"claude"}],"fallback":"availability_only","max_rounds":2,"provenance":{"candidates":"skill_default","fallback":"skill_default","max_rounds":"skill_default","role":"skill_default"},"role":"primary","schema":6},"policy_sha256":"bb95e1516f9fc1b6f4d8a75991d4650428428dc35d842db1710f4d64dc082a1b","previous_input_sha256":null,"repair_targets_sha256":null,"request_id":"e2babba3-91aa-42b4-9c2f-8aa70c812fc9","review_mode":"full","reviewed_commit_or_head":"ee53fd3c2eb6dff3fbe70dc618b86792c3d4834e","round_index":1,"schema":6},"role":"primary","schema":6,"verdict":"pass"},"role":"primary","schema":6,"selected":{"company":"openai","effort":"high","model":"gpt-5.6-sol","service_tier":"default","tool":"codex"},"waiver":null,"waiver_sha256":null},"rejected":[]},"schema":6}],"schema":6},"settled_orchestration_state_sha256":"884375c1d9caf200c7fad09d66379e25b0d52462104018ef38e5907fe0311c7d"}
...
`legacy-quarantined` (malformed or crossed legacy family: schema-6 receipt requires settled orchestration state) and is preserved byte for byte: frontmatter
