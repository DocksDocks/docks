...
| 5 | Live smoke: run the crosscheck end-to-end on a real draft plan in this repo (offer → yes → codex leg returns findings → attributed ingest) and paste the resulting `## Self-review` excerpt into this plan's `## Notes` | done |
| 6 | Release docks minor; verify manifest lockstep (`.claude-plugin` + `.codex-plugin` + marketplace catalogs) | done |
...
**Step 5 live smoke (2026-07-06)** — the crosscheck ran end-to-end on a real plan: this one, while its own steps 1–4 were executing.

- Offer point exercised: on-demand ("cross-check <slug>" path) — the user explicitly requested "it should have a codex review in plan-review-crosscheck for sure", which is the accepted-offer branch; the picker-phrased variant shares the identical downstream leg.
- Availability gate: `command -v codex` ok + `codex login status` → `Logged in using ChatGPT`, exit 0 (verified earlier same day).
- Leg run verbatim: `timeout 600 codex exec -s read-only -m gpt-5.5 -c model_reasoning_effort=xhigh -- "<reviewer prompt + docs/plans/active/plan-review-crosscheck.md>"` → 8 findings (5 high / 3 med), one-line verdict, 98,089 tokens.
- Attributed ingest excerpt (from `## Self-review` above): `Cross-check (2026-07-06): [codex gpt-5.5 xhigh] 8 findings (5 high / 3 med) — 7 accepted, 1 partially accepted …` plus a recorded `DISAGREEMENT:` line resolved by the orchestrating agent — the full grammar exercised including the disagreement branch.
- Negative test: `env PATH=/usr/bin:/bin sh -c 'command -v codex'` → exit 127 (gate fails ⇒ silent skip path).
