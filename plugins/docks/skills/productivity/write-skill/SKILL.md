...
3. **Score check.** `bash scripts/score-skills.sh --per-file | grep <name>`. If < 14, look at the rubric and find the missing point.
4. **Structural check.** `bash scripts/guard-skills.sh`. Failures are non-negotiable — fix them.
