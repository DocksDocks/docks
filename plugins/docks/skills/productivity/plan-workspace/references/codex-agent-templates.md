...
validateReviewTerminalFamily({currentPlanBytes,parentPlanBytes}) before commit;
after commit, read the child and its single parent plan blobs from Git and
rerun it. Settle or consume through the helper exactly once. Never implement
...
11. Permit one full plus at most one changed-input repair round. Permit a
    ...
    never retry.
12. Validate terminal reducer output before commit and again on committed
    child/single-parent plan blobs. Settle normal typed evidence once, then
    consume an eligible intent once or return NeedsUserAction.
