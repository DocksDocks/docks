...
| `dead-pointer` | a backticked repo path that does not resolve from the repo root, or a "see root" / "see parent" with no path | re-locate the owner by the pointer's stated purpose (grep the symbol) and fix the path; if no owner exists, state the rule in the node or cut the pointer |
