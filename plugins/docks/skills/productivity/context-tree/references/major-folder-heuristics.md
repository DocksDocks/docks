...
`docs/plans/` · `plugins/docks/skills/` · `plugins/effect-kit/skills/` · `plugins/session-relay/` · `scripts/` · `.github/`. The set drifts as plugins land — re-derive it with `find . -name CLAUDE.md -not -path "*/node_modules/*"` (every hit except the root file is a node).
