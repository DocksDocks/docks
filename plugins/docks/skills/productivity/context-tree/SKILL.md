...
metadata:
  pattern: meta-skill
  updated: "2026-09-23"
  content_hash: "eb08c641f8f6ea24b733ef173200ce8a7ab7c0d6fd91440d499aff1fa82cfead"
---
...
3. **Inventory + propose (per-section).** Copy the root `AGENTS.md` and every existing node the run will change into the backup directory outside the repo (`references/data-preservation.md` step 1), and list every source `^#{1,3}` section. Render TWO tables: (a) node list — `folder | new? | sources | one-line summary`; (b) **relocation table — `Section | Destination | Reason`** covering EVERY root section; unclassified → `KEEP in root` (only an explicit user instruction may mark a row `DROP`). A `--dry-run` or preview-only request stops here after also printing the post-prune root preview + each node preview. For an implementation request, resolve only genuine open decisions and route the complete proposal through `plan-manager` when canonical. Then print the tables as your FINAL message and END THE TURN (the approval gate). Write nothing until the user approves.
...
5. **Phase B — prune root after preimage verification.** Re-read the root and verify it still matches the backed-up preimage, show the exact lines assigned to written nodes, and confirm those sections are present there. Then END THE TURN for the second confirmation; delete them from root only after the user confirms. Never delete a section whose content you cannot point to in an already-written node.
