...
The plans convention lives in TWO places: the `plan-*` skills (the machinery) and each consumer project's `docs/plans/AGENTS.md` (the per-project contract its agents actually read, generated from `plan-init/references/plans-agents-md-template.md`). Whenever a `plan-*` skill changes the contract — frontmatter schema, body spine, the two-folder status-as-field model, the self-review checklist, open-questions surfacing, age tokens — the SAME change must land in plan-init's template in the same commit; `plan-manager` cites the project's `docs/plans/AGENTS.md` as the per-project source of truth and offers an explicit `plan-init refresh` for a stale known contract rather than silently diverging. The schema-5 primary role, ordered GPT-first/Claude-availability-only candidates, exact provenance, checklist verdict, two-round cap, fallback stop rules, and repair eligibility must move together across this repo's `docs/plans/AGENTS.md`, `plan-manager`, `plan-review`, `plan-improver`, plan-init's template, and the executable review-policy helper. The plan system tracks only the `.md` — views (the `plan-manager` chat glance, a throwaway visual-question `.html`) render on demand and are never committed.

The independent-review contract moves as one surface too: current primary-role waivers, canonical receipts, `prepare → main dispatch → apply`, and plan-improver's accepted-reproduced-blockers-only boundary must agree across the live contract, plan-init template, all three plan skills, their wrappers, scaffold templates, and public routing prose. X/S attribution, numeric scores/rubrics, consent/zero-review policy, and five-round receipts are historical-only and must retain their exact validation results. The dependency-free helper under plan-review's bundled `scripts/` is the executable schema/canonicalization source; author tests must execute that shipped copy rather than reimplement it.
