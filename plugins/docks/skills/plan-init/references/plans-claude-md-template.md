# Embedded Template — `docs/plans/CLAUDE.md`

Verbatim content to write at `docs/plans/CLAUDE.md`. Substitute `{{ISO_DATE}}` placeholders with the current ISO 8601 timestamp at write time (run `date +"%Y-%m-%dT%H:%M:%S%:z"`).
