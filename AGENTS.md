...
├── plugins/plan-lifecycle/           GitHub-issue plan lifecycle plugin (cross-tool): three skills, shipped plan.mjs, v4 contract reference, and two read-only reviewer wrappers under agents/; self-versioned with a closed compatibility.json checked by its self-test
...
├── docs/                             PLAN.md contract pointer, optional PLAN-QUEUE.md human note, and frozen plans/finished/ pre-GitHub archive
...
The three skills are `plan-workspace` for workspace maintenance, main-context
`plan-manager` for orchestration, and internal `plan-reviewer` for plan review.
...
installed `plan-lifecycle` plugin for the complete v4 contract.
